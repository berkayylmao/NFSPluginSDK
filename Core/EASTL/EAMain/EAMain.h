...
namespace EA {
  namespace EAMain {
    ...
    inline ReportFunction GetDefaultReportFunction() {
      return static_cast<ReportFunction>([](const char8_t* pOutput) { printf("%s", pOutput); });
    }

    inline void VReport(const char8_t* pFormat, va_list arguments) { vprintf(pFormat, arguments); }
    ...
    class CommandLine {
     public:
      ...
      int FindSwitch(const char* pSwitch, bool bCaseSensitive = false, const char** pResult = NULL,
                     int nStartingIndex = 0, char delimiter = DEFAULT_DELIMITER) const {
        return -1;
      }
