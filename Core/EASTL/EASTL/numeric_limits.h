...
//
// Defined as 0 or 1.
// Indicates whether we need to define our own implementations of inf, nan, snan, denorm floating
// point constants.
//
#if !defined(EASTL_CUSTOM_FLOAT_CONSTANTS_REQUIRED)
#if (defined(EA_COMPILER_GNUC) || defined(EA_COMPILER_CLANG) && defined(__FLT_MIN__)) || \
    defined(_YVALS)  // __FLT_MIN__ detects if it's really GCC/clang and not a mimic. yvals
                     // (Dinkumware) covers VC++, and Microsoft platforms.
...
#define EASTL_LIMITS_DIGITS(T) \
  ((EASTL_LIMITS_IS_SIGNED(T) ? EASTL_LIMITS_DIGITS_S(T) : EASTL_LIMITS_DIGITS_U(T)))
...
#define EASTL_LIMITS_DIGITS10(T) \
  ((EASTL_LIMITS_IS_SIGNED(T) ? EASTL_LIMITS_DIGITS10_S(T) : EASTL_LIMITS_DIGITS10_U(T)))
...
namespace eastl {
  ...
  namespace Internal {
    ...
    struct numeric_limits_base {
      // true if the type has an explicit specialization defined in the template class; false if
      // not.
      static EA_CONSTEXPR_OR_CONST bool is_specialized = false;

      // Integer types: the number of *bits* in the representation of T.
      // Floating types: the number of digits in the mantissa of T (same as FLT_MANT_DIG,
      // DBL_MANT_DIG or LDBL_MANT_DIG).
      ...
      // The number of decimal digits required to make sure that two distinct values of the type
      ...
      // Floating types: the base of the exponent representation. Always FLT_RADIX (typically 2) for
      // float.
      ...
      // In practice the application or OS can change this at runtime via hardware-specific commands
      // or via compiler optimizations.
  ...
  struct numeric_limits<bool> {
    ...
    static EA_CONSTEXPR_OR_CONST bool traps =
        true;  // Should this be true or false? Given that it's implemented in hardware as an
               // integer type, we use true.
    static EA_CONSTEXPR_OR_CONST bool               tinyness_before   = false;
    static EA_CONSTEXPR_OR_CONST float_round_style  round_style       = round_toward_zero;
    static EA_CONSTEXPR_OR_CONST bool               has_infinity      = false;
    static EA_CONSTEXPR_OR_CONST bool               has_quiet_NaN     = false;
