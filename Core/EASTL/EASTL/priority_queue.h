...
#define EASTL_PRIORITY_QUEUE_DEFAULT_ALLOCATOR allocator_type(EASTL_PRIORITY_QUEUE_DEFAULT_NAME)
  ...
  class priority_queue {
   public:
    ...
    // typedef typename Container::allocator_type           allocator_type;  // We can't currently
    // declare this because the container may be a type that doesn't have an allocator.
   ...
   public
    ...
    container_type c;  // The C++ standard specifies that you declare a protected member variable of
                       // type Container called 'c'.
    compare_type comp;  // The C++ standard specifies that you declare a protected member variable
                        // of type Compare called 'comp'.

   public:
    priority_queue();

    // Allocator is templated here because we aren't allowed to infer the allocator_type from the
    // Container, as some containers (e.g. array) don't have allocators. For containers that don't
    // have allocator types, you could use void or char as the Allocator template type.

    template <class Allocator>
    explicit priority_queue(
        const Allocator& allocator,
        typename eastl::enable_if<eastl::uses_allocator<container_type, Allocator>::value>::type* =
            NULL)
        : c(allocator), comp() {}
    ...
    priority_queue(std::initializer_list<value_type> ilist,
                   const compare_type&               compare =
                       compare_type());  // C++11 doesn't specify that std::priority_queue has
    ...
    void pop(value_type& value);  // Extension to the C++11 Standard that allows popping a move-only
                                  // type (e.g. unique_ptr).

    void change(size_type n);  /// Moves the item at the given array index to a new location based
                               /// on its current priority.
  ...
  inline priority_queue<T, Container, Compare>::priority_queue()
      : c(),  // To consider: use c(EASTL_PRIORITY_QUEUE_DEFAULT_ALLOCATOR) here, though that would
              // add the requirement that the user supplied container support this.
        comp() {}
  ...
  inline void priority_queue<T, Container, Compare>::emplace(Args&&... args) {
    push(value_type(eastl::forward<Args>(
        args)...));  // The C++11 Standard 23.6.4/1 states that c.emplace is used, but also declares
                     // that c doesn't need to have an emplace function.
  }
#else
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Container, typename Compare>
  inline void priority_queue<T, Container, Compare>::emplace(value_type&& value) {
    push(eastl::move(value));  // The C++11 Standard 23.6.4/1 states that c.emplace is used, but
                               // also declares that c doesn't need to have an emplace function.
  }
#endif

  template <typename T, typename Container, typename Compare>
  inline void priority_queue<T, Container, Compare>::emplace(const value_type& value) {
    push(value);  // The C++11 Standard 23.6.4/1 states that c.emplace is used, but also declares
                  // that c doesn't need to have an emplace function.
  }
#endif
