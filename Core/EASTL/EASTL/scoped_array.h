...
namespace eastl {
  ...
  class scoped_array {
   ...
   public:
    ...
    explicit scoped_array(T* pArray = NULL) : mpArray(pArray) {}
    ...
    ///    if(!ptr)
    ///        assert(false);
    bool operator!() const { return (mpArray == NULL); }
...
#define UNIQUE_ARRAY_T(T, Deleter) unique_array<T, Deleter>
  ...
  template <typename T, typename D>
  inline bool operator<(const scoped_array<T, D>& scopedArray1,
                        const scoped_array<T, D>& scopedArray2) {
    return (
        scopedArray1.get() <
        scopedArray2
            .get());  // Alternatively use: std::less<T*>(scopedArray1.get(), scopedArray2.get());
  }
