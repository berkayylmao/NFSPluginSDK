...
// Due to a bug in the VS2013 compiler pre Update 3 which causes an Internal Compiler Error when
// expanding the TupleImpl parameter pack together with the defaulted is_empty argument here, we
// disable the empty base class optimization for older versions of VS2013
    ...
    class TupleLeaf {
     public:
      TupleLeaf() : mValue() {}
// Work around a compiler bug in VS2013 that causes compiler errors in certain situations when
// explicitly defaulting copy constructor
      ...
      // We shouldn't need this explicit constructor as it should be handled by the template below
      // but OSX clang is_constructible type trait incorrectly gives false for is_constructible<T&&,
      // T&&>::value
      explicit TupleLeaf(ValueType&& v) : mValue(move(v)) {}

      template <typename T,
                typename = typename enable_if<is_constructible<ValueType, T&&>::value>::type>
      explicit TupleLeaf(T&& t) : mValue(forward<T>(t)) {}
    ...
    // Specialize for when ValueType is a reference as VS2013 doesn't do the right thing with the
    // default constructor
    template <size_t I, typename ValueType, bool IsEmpty>
...
// explicitly defaulting copy constructor
    ...
    class TupleLeaf<I, ValueType, true> : private ValueType {
     public:
      // true_type / false_type constructors for case where ValueType is default constructible and
      // should be value initialized and case where it is not
      TupleLeaf(const TupleLeaf&) = default;
    ...
    using MakeTupleTypes_t = typename MakeTupleTypesImpl<
        TupleTypes<>, Tuple, 0, tuple_size<typename remove_reference<Tuple>::type>::value>::type;
    ...
    struct TupleImpl<integer_sequence<size_t, Indices...>, Ts...>
      ...
      TupleImpl(OtherTuple&& t)
          : TupleLeaf<Indices, Ts>(forward<tuple_element_t<Indices, MakeTupleTypes_t<OtherTuple>>>(
                get<Indices>(t)))... {}
    ...
    struct TupleConvertibleImpl<true, TupleTypes<FromFirst, FromRest...>,
                                TupleTypes<ToFirst, ToRest...>>
        : public integral_constant<bool, is_convertible<FromFirst, ToFirst>::value &&
    ...
    template <typename TargetFirst, typename... TargetRest, typename FromFirst,
              typename... FromRest>
    struct TupleAssignableImpl<true, TupleTypes<TargetFirst, TargetRest...>,
                               TupleTypes<FromFirst, FromRest...>>
        : public integral_constant<bool, is_assignable<TargetFirst, FromFirst>::value &&
                                             TupleAssignableImpl<true, TupleTypes<TargetRest...>,
    ...
    struct TupleAssignable<Target, From, true, true>
        : public TupleAssignableImpl<tuple_size<typename remove_reference<Target>::type>::value ==
  ...
  class tuple {
   public:
    ...
    template <
        ...
        typename = typename enable_if<sizeof...(Us) + 1 == sizeof...(Ts) &&
                                          Internal::TupleConvertible<tuple<U, Us...>, tuple>::value,
                                      bool>::type>
    explicit EA_CONSTEXPR tuple(U&& u, Us&&... us)
        : mImpl(make_index_sequence<sizeof...(Us) + 1>{}, Internal::MakeTupleTypes_t<tuple>{},
                forward<U>(u), forward<Us>(us)...) {}

    template <typename OtherTuple,
              typename enable_if<Internal::TupleConvertible<OtherTuple, tuple>::value, bool>::type =
                  false>
    tuple(OtherTuple&& t) : mImpl(forward<OtherTuple>(t)) {}
