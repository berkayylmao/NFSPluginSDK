...
#define EASTL_VECTOR_MULTIMAP_DEFAULT_ALLOCATOR allocator_type(EASTL_VECTOR_MULTIMAP_DEFAULT_NAME)
  ...
  class vector_multimap : public RandomAccessContainer {
   ...
   public:
    ...
    explicit vector_multimap(const key_compare& comp, const allocator_type& allocator =
                                                          EASTL_VECTOR_MULTIMAP_DEFAULT_ALLOCATOR);
...
#endif
    vector_multimap(std::initializer_list<value_type> ilist,
  ...
  template <typename K, typename T, typename C, typename A, typename RAC>
  inline vector_multimap<K, T, C, A, RAC>::vector_multimap(const key_compare&    comp,
  ...
  vector_multimap<K, T, C, A, RAC>::insert(const_iterator position, const value_type& value) {
    ...
    if ((position == end()) ||
        !mValueCompare(*position, value))  // If value is <= the element at position...
    {
      if ((position == begin()) ||
  ...
  template <typename K, typename T, typename C, typename A, typename RAC>
  inline typename vector_multimap<K, T, C, A, RAC>::iterator vector_multimap<K, T, C, A, RAC>::find(
      const key_type& k) {
    const eastl::pair<iterator, iterator> pairIts(equal_range(k));
