...
#include <Core/EASTL/EASTL/sort.h>
#include <stddef.h>
...
#define EASTL_SLIST_DEFAULT_ALLOCATOR allocator_type(EASTL_SLIST_DEFAULT_NAME)
  ...
  inline SListIterator<T, Pointer, Reference>::SListIterator(const SListNodeBase* pNode)
      : mpNode(static_cast<node_type*>((SListNode<T>*)const_cast<SListNodeBase*>(
            pNode)))  // All this casting is in the name of making runtime debugging much easier on
                      // the user.
  {
    // Empty
  }
  ...
  template <typename T, typename Allocator>
  inline typename SListBase<T, Allocator>::allocator_type& SListBase<T, Allocator>::get_allocator()
      EA_NOEXCEPT {
    return mAllocator;
  }
  ...
  inline slist<T, Allocator>::slist(const slist& x) : base_type(x.mAllocator) {
    DoInsertAfter((SListNodeBase*)&mNode, const_iterator((SListNodeBase*)x.mNode.mpNext),
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::begin() const
      EA_NOEXCEPT {
    return const_iterator((SListNodeBase*)mNode.mpNext);
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::cbegin() const
      EA_NOEXCEPT {
    return const_iterator((SListNodeBase*)mNode.mpNext);
  }
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::cend() const
      EA_NOEXCEPT {
    return const_iterator(NULL);
  }
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::before_begin() const
      EA_NOEXCEPT {
    return const_iterator((SListNodeBase*)&mNode);
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::cbefore_begin() const
      EA_NOEXCEPT {
    return const_iterator((SListNodeBase*)&mNode);
  }
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::previous(
      const_iterator position) const {
    return const_iterator(
        SListNodeGetPrevious((SListNodeBase*)&mNode, (SListNodeBase*)position.mpNode));
  }
  ...
  inline void slist<T, Allocator>::insert(const_iterator position, size_type n,
    ...
    // To do: get rid of DoAssignValues and put its implementation directly here.
    DoInsertValuesAfter(
        SListNodeGetPrevious((SListNodeBase*)&mNode, (SListNodeBase*)position.mpNode), n, value);
  }

  template <typename T, typename Allocator>
  template <typename InputIterator>
  inline void slist<T, Allocator>::insert(const_iterator position, InputIterator first,
                                          InputIterator last) {
    DoInsertAfter(SListNodeGetPrevious((SListNodeBase*)&mNode, (SListNodeBase*)position.mpNode),
                  first, last);
  }
  ...
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::insert_after(
    ...
    return iterator((SListNodeBase*)DoInsertAfter((SListNodeBase*)position.mpNode, ilist.begin(),
  ...
  template <typename T, typename Allocator>
  template <class... Args>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::emplace_after(
      const_iterator position, Args&&... args) {
    return iterator(
        (SListNodeBase*)DoInsertValueAfter(position.mpNode, eastl::forward<Args>(args)...));
  }
#else
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Allocator>
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::erase(const_iterator first,
                                                                           const_iterator last) {
  ...
  template <typename T, typename Allocator>
  void slist<T, Allocator>::splice(const_iterator position, this_type&& x, const_iterator first,
                                   const_iterator last) {
    return splice(
        position, x, first,
  ...
  inline void slist<T, Allocator>::splice_after(const_iterator position, this_type&& x,
                                                const_iterator i) {
    return splice_after(
        position, x, i);  // This will call splice_after(const_iterator, this_type&, const_iterator)
  }
    ...
    if (before_first != before_last)  // If there is anything to splice...
    {
      ...
      if (i == iEnd)  // If the input came from an external range...
        mSize += (size_type)eastl::distance(
            before_first,
            before_last);  // Note that we have no way of knowing how to decrementing the size from
                           // the external container, assuming it came from one.
      else {
        EASTL_FAIL_MSG(
  ...
  inline void slist<T, Allocator>::reverse() EA_NOEXCEPT {
    if (mNode.mpNext)
      mNode.mpNext =
          static_cast<node_type*>((base_node_type*)SListNodeReverse((SListNodeBase*)mNode.mpNext));
  }
  ...
  template <typename T, typename Allocator>
  template <typename InputIterator>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoInsertAfter(
      SListNodeBase* pNode, InputIterator first, InputIterator last) {
  ...
  template <typename InputIterator>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoInsertAfter(
      SListNodeBase* pNode, InputIterator first, InputIterator last, false_type) {
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoInsertValueAfter(
      SListNodeBase* pNode) {
#if EASTL_SLIST_SIZE_CACHE
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoInsertValueAfter(
      SListNodeBase* pNode, value_type&& value) {
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoInsertValueAfter(
      SListNodeBase* pNode, const value_type& value) {
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoInsertValuesAfter(
      SListNodeBase* pNode, size_type n, const value_type& value) {
