...
namespace eastl {
  ...
  struct equal_to_2 : public binary_function<T, U, bool> {
    ...
    EA_CPP14_CONSTEXPR bool operator()(
        const U& b, const T& a) const  // If you are getting a 'operator() already defined' error
  ...
  template <class Operation1, class Operation2, class Operation3>
  inline binary_compose<Operation1, Operation2, Operation3> compose2(const Operation1& op1,
                                                                     const Operation2& op2,
  ...
  template <typename Result, typename T, typename Argument>
  inline const_mem_fun1_t<Result, T, Argument> mem_fun(Result (T::*MemberFunction)(Argument)
                                                           const) {
  ...
  template <typename Result, typename T, typename Argument>
  inline const_mem_fun1_ref_t<Result, T, Argument> mem_fun_ref(Result (T::*MemberFunction)(Argument)
                                                                   const) {
...
// If wchar_t is a native type instead of simply a define to an existing type...
  ...
  struct hash<char8_t*> {
    size_t operator()(const char8_t* p) const {
      uint32_t c,
          result =
              2166136261U;  // FNV1 hash. Perhaps the best string hash. Intentionally uint32_t
...
#include <Core/EASTL/EASTL/internal/function.h>
