...
#define EASTL_HASH_MULTIMAP_DEFAULT_ALLOCATOR allocator_type(EASTL_HASH_MULTIMAP_DEFAULT_NAME)
  ...
  template <typename Key, typename T, typename Hash = eastl::hash<Key>,
            typename Predicate = eastl::equal_to<Key>, typename Allocator = EASTLAllocatorType,
            bool bCacheHashCode = false>
  class hash_map : public hashtable<Key, eastl::pair<const Key, T>, Allocator,
                                    eastl::use_first<eastl::pair<const Key, T> >, Predicate, Hash,
                                    mod_range_hashing, default_ranged_hash, prime_rehash_policy,
                                    bCacheHashCode, true, true> {
   public:
    typedef hashtable<Key, eastl::pair<const Key, T>, Allocator,
                      eastl::use_first<eastl::pair<const Key, T> >, Predicate, Hash,
                      mod_range_hashing, default_ranged_hash, prime_rehash_policy, bCacheHashCode,
                      true, true>
                                                                         base_type;
   ...
   public:
    ...
    hash_map(std::initializer_list<value_type> ilist, size_type nBucketCount = 0,
             ...
             const allocator_type& allocator = EASTL_HASH_MAP_DEFAULT_ALLOCATOR)
        : base_type(ilist.begin(), ilist.end(), nBucketCount, hashFunction, mod_range_hashing(),
    ...
    hash_map(ForwardIterator first, ForwardIterator last, size_type nBucketCount = 0,
             const Hash& hashFunction = Hash(), const Predicate& predicate = Predicate(),
             const allocator_type& allocator = EASTL_HASH_MAP_DEFAULT_ALLOCATOR)
  ...
  template <typename Key, typename T, typename Hash = eastl::hash<Key>,
            typename Predicate = eastl::equal_to<Key>, typename Allocator = EASTLAllocatorType,
            bool bCacheHashCode = false>
  class hash_multimap : public hashtable<Key, eastl::pair<const Key, T>, Allocator,
  ...
  inline bool operator==(const hash_map<Key, T, Hash, Predicate, Allocator, bCacheHashCode>& a,
    ...
    typedef typename hash_map<Key, T, Hash, Predicate, Allocator, bCacheHashCode>::const_iterator
        const_iterator;
    ...
    for (const_iterator ai = a.begin(), aiEnd = a.end(), biEnd = b.end(); ai != aiEnd; ++ai) {
      ...
      if ((bi == biEnd) ||
          !(*ai == *bi))  // We have to compare the values, because lookups are done by keys alone
  ...
  inline bool operator==(
    ...
    typedef
        typename hash_multimap<Key, T, Hash, Predicate, Allocator, bCacheHashCode>::const_iterator
                                                                             const_iterator;
         ...
         ai = aRange.second)  // For each element in a...
    {
      ...
      if (aDistance == 1) {
        if (!(*aRange.first ==
              *bRange.first))  // We have to compare the values, because lookups are done by keys
