...
//    --------------------------------------------------------------
//    T                            The value type.
//    Compare                      A function which takes two arguments and returns the lesser of
//    the two. Predicate                    A function which takes one argument returns true if the
//    argument meets some criteria. BinaryPredicate              A function which takes two
//    arguments and returns true if some criteria is met (e.g. they are equal). StrickWeakOrdering
//    A BinaryPredicate that compares two objects, returning true if the first precedes the second.
//    Like Compare but has additional requirements. Used for sorting routines. Function A function
//    which takes one argument and applies some operation to the target. Size A count or size.
//    Generator                    A function which takes no arguments and returns a value (which
//    will usually be assigned to an object). UnaryOperation               A function which takes
//    one argument and returns a value (which will usually be assigned to second object).
//    BinaryOperation              A function which takes two arguments and returns a value (which
//    will usually be assigned to a third object). InputIterator                An input iterator
//    (iterator you read from) which allows reading each element only once and only in a forward
//    direction. ForwardIterator              An input iterator which is like InputIterator except
//    it can be reset back to the beginning. BidirectionalIterator        An input iterator which is
//    like ForwardIterator except it can be read in a backward direction as well.
//    RandomAccessIterator         An input iterator which can be addressed like an array. It is a
//    superset of all other input iterators. OutputIterator               An output iterator
//    (iterator you write to) which allows writing each element only once in only in a forward
//    direction.
//
...
//      max
//      max<Compare>
//     +min_alt                                     Exists to work around the problem of conflicts
//     with min/max #defines on some systems. +min_alt<Compare> +max_alt +max_alt<Compare> +median
//     +median<Compare>
...
#include <Core/EASTL/EASTL/type_traits.h>
#include <Core/EASTL/EASTL/internal/move_help.h>
#include <Core/EASTL/EASTL/internal/copy_help.h>
#include <Core/EASTL/EASTL/internal/fill_help.h>
#include <Core/EASTL/EASTL/initializer_list.h>
#include <Core/EASTL/EASTL/iterator.h>
#include <Core/EASTL/EASTL/functional.h>
#include <Core/EASTL/EASTL/utility.h>
#include <Core/EASTL/EASTL/internal/generic_iterator.h>
#include <Core/EASTL/EASTL/random.h>
...
namespace eastl {
  ...
  ForwardIterator min_element(ForwardIterator first, ForwardIterator last) {
    if (first != last) {
      ...
      while (++first != last) {
        if (*first < *currentMin) currentMin = first;
      }
      return currentMin;
    }
    return first;
  ...
  ForwardIterator min_element(ForwardIterator first, ForwardIterator last, Compare compare) {
    if (first != last) {
      ...
      while (++first != last) {
        if (compare(*first, *currentMin)) currentMin = first;
      }
      return currentMin;
    }
    return first;
  ...
  ForwardIterator max_element(ForwardIterator first, ForwardIterator last) {
    if (first != last) {
      ...
      while (++first != last) {
        if (*currentMax < *first) currentMax = first;
      }
      return currentMax;
    }
    return first;
  ...
  ForwardIterator max_element(ForwardIterator first, ForwardIterator last, Compare compare) {
    if (first != last) {
      ...
      while (++first != last) {
        if (compare(*currentMax, *first)) currentMax = first;
      }
      return currentMax;
    }
    return first;
  ...
  ///
  template <typename T>
  inline EA_CONSTEXPR typename eastl::enable_if<eastl::is_scalar<T>::value, T>::type min(T a, T b) {
    return b < a ? b : a;
  }

  template <typename T>
  inline typename eastl::enable_if<!eastl::is_scalar<T>::value, const T&>::type min(const T& a,
                                                                                    const T& b) {
    return b < a ? b : a;
  }

#if defined(_MSC_VER) && defined(EA_PROCESSOR_POWERPC)
  inline float min(float a, float b) { return (float)__fsel(a - b, b, a); }

  inline double min(double a, double b) { return (double)__fsel(a - b, b, a); }
  ...
  ///
  template <typename T>
  inline EA_CONSTEXPR typename eastl::enable_if<eastl::is_scalar<T>::value, T>::type min_alt(T a,
                                                                                             T b) {
    return b < a ? b : a;
  }

  template <typename T>
  inline typename eastl::enable_if<!eastl::is_scalar<T>::value, const T&>::type min_alt(
      const T& a, const T& b) {
    return b < a ? b : a;
  }

#if defined(_MSC_VER) && defined(EA_PROCESSOR_POWERPC)
  inline float min_alt(float a, float b) { return (float)__fsel(a - b, b, a); }

  inline double min_alt(double a, double b) { return (double)__fsel(a - b, b, a); }
  ...
  ///
  template <typename T>
  inline EA_CONSTEXPR typename eastl::enable_if<eastl::is_scalar<T>::value, T>::type max(T a, T b) {
    return a < b ? b : a;
  }

  template <typename T>
  inline typename eastl::enable_if<!eastl::is_scalar<T>::value, const T&>::type max(const T& a,
                                                                                    const T& b) {
    return a < b ? b : a;
  }

#if defined(_MSC_VER) && defined(EA_PROCESSOR_POWERPC)
  inline float max(float a, float b) { return (float)__fsel(a - b, a, b); }

  inline double max(double a, double b) { return (double)__fsel(a - b, a, b); }
  ...
  ///
  template <typename T>
  inline EA_CONSTEXPR typename eastl::enable_if<eastl::is_scalar<T>::value, T>::type max_alt(T a,
                                                                                             T b) {
    return a < b ? b : a;
  }

  template <typename T>
  inline typename eastl::enable_if<!eastl::is_scalar<T>::value, const T&>::type max_alt(
      const T& a, const T& b) {
    return a < b ? b : a;
  }

#if defined(_MSC_VER) && defined(EA_PROCESSOR_POWERPC)
  inline float max_alt(float a, float b) { return (float)__fsel(a - b, a, b); }

  inline double max_alt(double a, double b) { return (double)__fsel(a - b, a, b); }
  ...
  ///
  template <typename ForwardIterator, typename Compare>
  eastl::pair<ForwardIterator, ForwardIterator> minmax_element(ForwardIterator first,
    ...
    if (!(first == last) && !(++first == last)) {
      ...
      while (++first != last) {
        ...
        if (++first == last) {
          ...
          if (compare(*first, *i)) {
            if (compare(*first, *result.first)) result.first = first;

            if (!compare(*i, *result.second)) result.second = i;
          } else {
            if (compare(*i, *result.first)) result.first = i;

            if (!compare(*first, *result.second)) result.second = first;
          }
        }
      }
    }
  ...
  /// T&>(a, b) otherwise. Remarks: Returns pair<const T&, const T&>(a, b) when the arguments are
  /// equivalent. Complexity: Exactly one comparison.
  ///

  // The following optimization is a problem because it changes the return value in a way that would
  // break users unless they used auto (e.g. auto result = minmax(17, 33); )
  //
  // template <typename T>
  // inline EA_CONSTEXPR typename eastl::enable_if<eastl::is_scalar<T>::value, eastl::pair<T, T>
  // >::type minmax(T a, T b)
  // {
  ...
  //
  // template <typename T>
  // inline typename eastl::enable_if<!eastl::is_scalar<T>::value, eastl::pair<const T&, const T&>
  // >::type minmax(const T& a, const T& b)
  // {
  //     return (b < a) ? eastl::make_pair(b, a) : eastl::make_pair(a, b);
  // }

  // It turns out that the following conforming definition of minmax generates a warning when used
  // with VC++ up to at least VS2012. The VS2012 version of minmax is a broken and non-conforming
  // definition, and we don't want to do that. We could do it for scalars alone, though we'd have to
  // decide if we are going to do that for all compilers, because it changes the return value from a
  // pair of references to a pair of values.
  template <typename T>
  ...
  /// all_of
  ///
  /// Returns: true if the unary predicate p returns true for all elements in the range [first,
  /// last)
  ///
  ...
  /// any_of
  ///
  /// Returns: true if the unary predicate p returns true for any of the elements in the range
  /// [first, last)
  ///
  template <typename InputIterator, typename Predicate>
  ...
  /// none_of
  ///
  /// Returns: true if the unary predicate p returns true for none of the elements in the range
  /// [first, last)
  ///
  template <typename InputIterator, typename Predicate>
  ...
  ///
  /// Returns: The first iterator i such that both i and i + 1 are in the range
  /// [first, last) for which the following corresponding conditions hold: predicate(*i, *(i + 1))
  ...
  inline ForwardIterator adjacent_find(ForwardIterator first, ForwardIterator last,
    ...
    if (first != last) {
      ...
      for (++i; i != last; ++i) {
        if (predicate(*first, *i)) return first;
        first = i;
      }
    }
    return last;
...
///
/// Example usage:
///     struct Rand{ eastl_size_t operator()(eastl_size_t n) { return (eastl_size_t)(rand() % n); }
///     }; // Note: The C rand function is poor and slow. Rand randInstance; shuffle(pArrayBegin,
///     pArrayEnd, randInstance);
///
  ...
  void shuffle(RandomAccessIterator first, RandomAccessIterator last,
    ...
    if (first != last) {
      typedef
          typename eastl::iterator_traits<RandomAccessIterator>::difference_type difference_type;
      typedef typename eastl::make_unsigned<difference_type>::type unsigned_difference_type;
      typedef typename eastl::uniform_int_distribution<unsigned_difference_type>
...
///
/// Example usage:
///     eastl_size_t Rand(eastl_size_t n) { return (eastl_size_t)(rand() % n); } // Note: The C rand
///     function is poor and slow. pointer_to_unary_function<eastl_size_t, eastl_size_t>
///     randInstance(Rand); random_shuffle(pArrayBegin, pArrayEnd, randInstance);
///
/// Example usage:
///     struct Rand{ eastl_size_t operator()(eastl_size_t n) { return (eastl_size_t)(rand() % n); }
///     }; // Note: The C rand function is poor and slow. Rand randInstance;
...
///
  ...
  ///
  /// *** Disabled until we decide if we want to get into the business of writing random number
  /// generators. ***
  ///
  ...
  /// move_n
  ///
  /// Same as move(InputIterator, InputIterator, OutputIterator) except based on count instead of
  /// iterator range.
  ///
  template <typename InputIterator, typename Size, typename OutputIterator>
  inline OutputIterator move_n_impl(InputIterator first, Size n, OutputIterator result,
                                    EASTL_ITC_NS::input_iterator_tag) {
    for (; n > 0; --n) *result++ = eastl::move(*first++);
    return result;
  }

  template <typename RandomAccessIterator, typename Size, typename OutputIterator>
  inline OutputIterator move_n_impl(RandomAccessIterator first, Size n, OutputIterator result,
  ...
  /// copy_n
  ///
  /// Same as copy(InputIterator, InputIterator, OutputIterator) except based on count instead of
  /// iterator range. Effects: Copies exactly count values from the range beginning at first to the
  /// range beginning at result, if count > 0. Does nothing otherwise. Returns: Iterator in the
  /// destination range, pointing past the last element copied if count>0 or first otherwise.
  /// Complexity: Exactly count assignments, if count > 0.
  ///
  template <typename InputIterator, typename Size, typename OutputIterator>
  inline OutputIterator copy_n_impl(InputIterator first, Size n, OutputIterator result,
                                    EASTL_ITC_NS::input_iterator_tag) {
  ...
  template <typename RandomAccessIterator, typename Size, typename OutputIterator>
  inline OutputIterator copy_n_impl(RandomAccessIterator first, Size n, OutputIterator result,
                                    EASTL_ITC_NS::random_access_iterator_tag) {
    return eastl::copy(
        first, first + n,
  ...
  inline OutputIterator copy_if(InputIterator first, InputIterator last, OutputIterator result,
                                Predicate predicate) {
    // This implementation's performance could be improved by taking a more complicated approach
    // like with the copy algorithm.
    for (; first != last; ++first) {
  ...
  // Implementation moving copying both trivial and non-trivial data via a lesser iterator than
  // random-access.
  ...
  struct move_and_copy_backward_helper {
    ...
    static BidirectionalIterator2 move_or_copy_backward(BidirectionalIterator1 first,
                                                        BidirectionalIterator1 last,
                                                        BidirectionalIterator2 resultEnd) {
      while (first != last) *--resultEnd = *--last;
      return resultEnd;  // resultEnd now points to the beginning of the destination sequence
                         // instead of the end.
    }
  };
  ...
  struct move_and_copy_backward_helper<BidirectionalIterator1Category, true, false> {
    ...
    static BidirectionalIterator2 move_or_copy_backward(BidirectionalIterator1 first,
                                                        BidirectionalIterator1 last,
                                                        BidirectionalIterator2 resultEnd) {
      while (first != last) *--resultEnd = eastl::move(*--last);
      return resultEnd;  // resultEnd now points to the beginning of the destination sequence
                         // instead of the end.
    }
  };

  // Specialization for moving non-trivial data via a random-access iterator. It's theoretically
  // faster because the compiler can see the count when its a compile-time const.
  template <>
  struct move_and_copy_backward_helper<EASTL_ITC_NS::random_access_iterator_tag, true, false> {
    ...
    static BidirectionalIterator2 move_or_copy_backward(BidirectionalIterator1 first,
                                                        ...
                                                        BidirectionalIterator2 resultEnd) {
      typedef
          typename eastl::iterator_traits<BidirectionalIterator1>::difference_type difference_type;

      for (difference_type n = (last - first); n > 0; --n) *--resultEnd = eastl::move(*--last);
      return resultEnd;  // resultEnd now points to the beginning of the destination sequence
                         // instead of the end.
    }
  };

  // Specialization for copying non-trivial data via a random-access iterator. It's theoretically
  // faster because the compiler can see the count when its a compile-time const. This
  // specialization converts the random access BidirectionalIterator1 last-first to an integral
  // type. There's simple way for us to take advantage of a random access output iterator, as the
  // range is specified by the input instead of the output, and distance(first, last) for a
  ...
  struct move_and_copy_backward_helper<EASTL_ITC_NS::random_access_iterator_tag, false, false> {
    ...
    static BidirectionalIterator2 move_or_copy_backward(BidirectionalIterator1 first,
      ...
      for (difference_type n = (last - first); n > 0; --n) *--resultEnd = *--last;
      return resultEnd;  // resultEnd now points to the beginning of the destination sequence
                         // instead of the end.
    }
  };

  // Specialization for when we can use memmove/memcpy. See the notes above for what conditions
  // allow this.
  ...
  struct move_and_copy_backward_helper<EASTL_ITC_NS::random_access_iterator_tag, isMove, true> {
    ...
    static T* move_or_copy_backward(const T* first, const T* last, T* resultEnd) {
      ...
      // We could use memcpy here if there's no range overlap, but memcpy is rarely much faster than
      // memmove.
    }
  };

  template <bool isMove, typename BidirectionalIterator1, typename BidirectionalIterator2>
  inline BidirectionalIterator2 move_and_copy_backward_chooser(BidirectionalIterator1 first,
                                                               BidirectionalIterator1 last,
                                                               BidirectionalIterator2 resultEnd) {
    ...
    return eastl::move_and_copy_backward_helper<IIC, isMove, canBeMemmoved>::move_or_copy_backward(
        first, last, resultEnd);  // Need to chose based on the input iterator tag and not the
                                  // output iterator tag, because containers accept input ranges of
                                  // iterator types different than self.
  }

  // We have a second layer of unwrap_iterator calls because the original iterator might be
  // something like move_iterator<generic_iterator<int*> > (i.e. doubly-wrapped).
  template <bool isMove, typename BidirectionalIterator1, typename BidirectionalIterator2>
  inline BidirectionalIterator2 move_and_copy_backward_unwrapper(BidirectionalIterator1 first,
                                                                 BidirectionalIterator1 last,
                                                                 BidirectionalIterator2 resultEnd) {
    return BidirectionalIterator2(eastl::move_and_copy_backward_chooser<isMove>(
        ...
        eastl::unwrap_iterator(resultEnd)));  // Have to convert to BidirectionalIterator2 because
                                              // result.base() could be a T*
  }

  /// move_backward
  ///
  /// The elements are moved in reverse order (the last element is moved first), but their relative
  /// order is preserved. After this operation the elements in the moved-from range will still
  /// contain valid values of the appropriate type, but not necessarily the same values as before
  /// the move. Returns the beginning of the result range. Note: When moving between containers, the
  /// dest range must be valid; this function doesn't resize containers. Note: If result is within
  /// [first, last), move must be used instead of move_backward.
  ///
  /// Example usage:
  ...
  ///     template <typename BidirectionalIterator1, typename BidirectionalIterator2>
  ///     BidirectionalIterator2 move_backward(BidirectionalIterator1 first, BidirectionalIterator1
  ///     last, BidirectionalIterator2 resultEnd)
  ///     {
  ///         while(last != first)
  ...
  inline BidirectionalIterator2 move_backward(BidirectionalIterator1 first,
                                              BidirectionalIterator1 last,
                                              BidirectionalIterator2 resultEnd) {
    return eastl::move_and_copy_backward_unwrapper<true>(eastl::unwrap_iterator(first),
                                                         eastl::unwrap_iterator(last), resultEnd);
  }
  ...
  inline BidirectionalIterator2 copy_backward(BidirectionalIterator1 first,
    ...
    return eastl::move_and_copy_backward_unwrapper<isMove>(eastl::unwrap_iterator(first),
                                                           eastl::unwrap_iterator(last), resultEnd);
  }
  ...
  inline typename eastl::iterator_traits<InputIterator>::difference_type count(InputIterator first,
                                                                               InputIterator last,
                                                                               const T& value) {
  ...
  inline typename eastl::iterator_traits<InputIterator>::difference_type count(
    ...
    for (; first != last; ++first) {
      if (predicate(*first, value)) ++result;
    }
    return result;
  }
  ...
  ///
  template <typename InputIterator, typename Predicate>
  inline InputIterator find_if_not(InputIterator first, InputIterator last, Predicate predicate) {
    for (; first != last; ++first) {
  ...
  ForwardIterator1 find_first_of(ForwardIterator1 first1, ForwardIterator1 last1,
    ...
    for (; first1 != last1; ++first1) {
      for (ForwardIterator2 i = first2; i != last2; ++i) {
        if (predicate(*first1, *i)) return first1;
      }
    }
    return last1;
  }
  ...
  /// find_first_not_of
  ///
  /// Searches through first range for the first element that does not belong the second input
  /// range. This is very much like the C++ string find_first_not_of function.
  ///
  /// Returns: The first iterator i in the range [first1, last1) such that for some
  /// integer j in the range [first2, last2) the following conditions hold: !(*i == *j).
  /// Returns last1 if no such iterator is found.
  ...
  ForwardIterator1 find_first_not_of(ForwardIterator1 first1, ForwardIterator1 last1,
                                     ForwardIterator2 first2, ForwardIterator2 last2) {
    for (; first1 != last1; ++first1) {
      if (eastl::find(first2, last2, *first1) == last2) break;
    }
  ...
  inline ForwardIterator1 find_first_not_of(ForwardIterator1 first1, ForwardIterator1 last1,
                                            ForwardIterator2 first2, ForwardIterator2 last2,
                                            BinaryPredicate predicate) {
    ...
    for (; first1 != last1; ++first1) {
      if (eastl::find_if(first2, last2,
                         eastl::bind1st<BinaryPredicate, value_type>(predicate, *first1)) == last2)
        break;
    }
  ...
  inline BidirectionalIterator1 find_last_of(BidirectionalIterator1 first1,
    ...
    if ((first1 != last1) && (first2 != last2)) {
      ...
      if ((it1 != first1) || (eastl::find(first2, last2, *it1) != last2)) return it1;
    }
  ...
  BidirectionalIterator1 find_last_of(BidirectionalIterator1 first1, BidirectionalIterator1 last1,
    ...
    if ((first1 != last1) && (first2 != last2)) {
      ...
      while ((--it1 != first1) && (eastl::find_if(first2, last2,
      ...
      if ((it1 != first1) ||
          (eastl::find_if(first2, last2,
                          eastl::bind1st<BinaryPredicate, value_type>(predicate, *it1)) != last2))
        return it1;
    }
  ...
  ///
  /// Effects: Assigns through every iterator i in the range [result, result + (last1 - first1))
  /// a new corresponding value equal to binaryOperation(*(first1 + (i - result), *(first2 + (i -
  /// result))).
  ///
  ...
  inline bool equal(const unsigned char* first1, const unsigned char* last1, const unsigned char*
  first2) { return (memcmp(first1, first2, (size_t)((uintptr_t)last1 - (uintptr_t)first1)) == 0); }

  inline bool equal(const signed char* first1, const signed char* last1, const signed char* first2)
          { return (memcmp(first1, first2, (size_t)((uintptr_t)last1 - (uintptr_t)first1)) == 0); }

  #ifndef EA_WCHAR_T_NON_NATIVE // EABase defines this. If you are getting a compiler error here,
  then somebody has taken away EABase or broken it. inline bool equal(const wchar_t* first1, const
  wchar_t* last1, const wchar_t* first2) { return (memcmp(first1, first2, (size_t)((uintptr_t)last1
  ...
  */
  ...
  ///
  template <typename InputIterator1, typename InputIterator2>
  bool identical(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2,
  ...
  template <typename InputIterator1, typename InputIterator2, typename BinaryPredicate>
  bool identical(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2,
                 InputIterator2 last2, BinaryPredicate predicate) {
  ...
  lexicographical_compare(const char* first1, const char* last1, const char* first2,
                          const char* last2) {
  ...
  lexicographical_compare(unsigned char* first1, unsigned char* last1, unsigned char* first2,
                          unsigned char* last2) {
    const ptrdiff_t n1(last1 - first1), n2(last2 - first2);
    const int       result = memcmp(first1, first2, (size_t)eastl::min_alt(n1, n2));
    return result ? (result < 0) : (n1 < n2);
  ...
  lexicographical_compare(signed char* first1, signed char* last1, signed char* first2,
                          signed char* last2) {
    const ptrdiff_t n1(last1 - first1), n2(last2 - first2);
    const int       result = memcmp(first1, first2, (size_t)eastl::min_alt(n1, n2));
    return result ? (result < 0) : (n1 < n2);
  ...
  // lexicographical_compare(const bool* first1, const bool* last1, const bool* first2, const bool*
  // last2)
  //{
  //    const ptrdiff_t n1(last1 - first1), n2(last2 - first2);
  //    const int result = memcmp(first1, first2, (size_t)eastl::min_alt(n1, n2));
  ...
  {
    while ((first1 != last1) &&
           (*first1 == *first2))  // && (first2 != last2) <- C++ standard mismatch function doesn't
                                  // check first2/last2.
    {
      ++first1;
      ++first2;
  ...
  inline eastl::pair<InputIterator1, InputIterator2> mismatch(
      ...
      BinaryPredicate predicate) {
    while ((first1 != last1) &&
           predicate(*first1, *first2))  // && (first2 != last2) <- C++ standard mismatch function
                                         // doesn't check first2/last2.
    {
      ++first1;
      ++first2;
    }
  ...
  ForwardIterator lower_bound(ForwardIterator first, ForwardIterator last, const T& value) {
    ...
    while (d > 0) {
      ForwardIterator i  = first;
      DifferenceType  d2 = d >> 1;  // We use '>>1' here instead of '/2' because MSVC++ for some
      ...
      if (*i < value) {
        // Disabled because std::lower_bound doesn't specify (23.3.3.3, p3) this can be done:
  ...
  ForwardIterator lower_bound(ForwardIterator first, ForwardIterator last, const T& value,
    ...
    while (d > 0) {
      ...
      if (compare(*i, value)) {
        // Disabled because std::lower_bound doesn't specify (23.3.3.1, p3) this can be done:
        // EASTL_VALIDATE_COMPARE(!compare(value, *i)); // Validate that the compare function is
        // sane.
        first = ++i;
        d -= d2 + 1;
      } else
  ...
  ForwardIterator upper_bound(ForwardIterator first, ForwardIterator last, const T& value) {
    ...
    while (len > 0) {
      ForwardIterator i = first;
      DifferenceType  len2 =
          len >> 1;  // We use '>>1' here instead of '/2' because MSVC++ for some reason generates
      ...
      if (!(value < *i))  // Note that we always express value comparisons in terms of < or ==.
      {
        ...
        // Disabled because std::upper_bound doesn't specify (23.3.3.2, p3) this can be done:
  ...
  ForwardIterator upper_bound(ForwardIterator first, ForwardIterator last, const T& value,
    ...
    while (len > 0) {
      ...
      if (!compare(value, *i)) {
        ...
        // Disabled because std::upper_bound doesn't specify (23.3.3.2, p3) this can be done:
        // EASTL_VALIDATE_COMPARE(!compare(*i, value)); // Validate that the compare function is
        // sane.
        len = len2;
      }
    }
  ...
  /// This is because both versions would have the same parameter count and there could be
  /// ambiguity.
  ///
  template <typename ForwardIterator, typename T>
  inline void replace(ForwardIterator first, ForwardIterator last, const T& old_value,
                      const T& new_value) {
    for (; first != last; ++first) {
  ...
  inline void replace_if(ForwardIterator first, ForwardIterator last, Predicate predicate,
                         const T& new_value) {
    for (; first != last; ++first) {
      if (predicate(*first)) *first = new_value;
    }
  }
  ...
  inline OutputIterator remove_copy(InputIterator first, InputIterator last, OutputIterator result,
                                    const T& value) {
    for (; first != last; ++first) {
  ...
  ///    vector<int> intArray;
  ///    ...
  ///    intArray.erase(remove(intArray.begin(), intArray.end(), 4), intArray.end()); // Erase all
  ///    elements of value 4.
  ///
  template <typename ForwardIterator, typename T>
  ...
  ///    vector<int> intArray;
  ///    ...
  ///    intArray.erase(remove(intArray.begin(), intArray.end(), bind2nd(less<int>(), (int)3)),
  ///    intArray.end()); // Erase all elements less than 3.
  ///
  ...
  inline ForwardIterator remove_if(ForwardIterator first, ForwardIterator last,
    ...
    if (first != last) {
      ForwardIterator i(first);
      return eastl::remove_copy_if<ForwardIterator, ForwardIterator, Predicate>(++i, last, first,
                                                                                predicate);
    }
    return first;
  }
  ...
  /// Complexity: Exactly 'last - first' applications of the corresponding predicate.
  ///
  /// Note: The predicate version of replace_copy is replace_copy_if and not another variation of
  /// replace_copy. This is because both versions would have the same parameter count and there
  /// could be ambiguity.
  ///
  template <typename InputIterator, typename OutputIterator, typename T>
  inline OutputIterator replace_copy(InputIterator first, InputIterator last, OutputIterator result,
  ...
  /// Complexity: Exactly 'last - first' applications of the corresponding predicate.
  ///
  /// Note: The predicate version of replace_copy_if is replace_copy and not another variation of
  /// replace_copy_if. This is because both versions would have the same parameter count and there
  /// could be ambiguity.
  ///
  ...
  inline OutputIterator replace_copy_if(InputIterator first, InputIterator last,
                                        OutputIterator result, Predicate predicate,
                                        const T& new_value) {
    for (; first != last; ++first, ++result) *result = predicate(*first) ? new_value : *first;
    return result;
  }
  ...
  inline void reverse_impl(RandomAccessIterator first, RandomAccessIterator last,
                           EASTL_ITC_NS::random_access_iterator_tag) {
    if (first != last) {
      for (; first < --last; ++first)   // With a random access iterator, we can use operator < to
                                        // more efficiently implement
        eastl::iter_swap(first, last);  // this algorithm. A generic iterator doesn't necessarily
  ...
  inline OutputIterator reverse_copy(BidirectionalIterator first, BidirectionalIterator last,
  ...
  /// search
  ///
  /// Search finds a subsequence within the range [first1, last1) that is identical to [first2,
  /// last2) when compared element-by-element. It returns an iterator pointing to the beginning of
  /// that subsequence, or else last1 if no such subsequence exists. As such, it is very much like
  ...
  /// any nonnegative integer n less than 'last2 - first2' the following corresponding condition
  /// holds:
  ...
  /// Returns: The first iterator i in the range [first1, last1 - (last2 - first2)) such that for
  /// any nonnegative integer n less than 'last2 - first2' the following corresponding condition
  /// holds: predicate(*(i + n), *(first2 + n)) != false. Returns last1 if no such iterator is
  /// found.
  ///
  /// Complexity: At most (last1 first1) * (last2 first2) applications of the corresponding
  /// predicate.
  ...
  ForwardIterator1 search(ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2,
    ...
    if (d2 != 0) {
      ...
      for (difference_type_1 d1 = eastl::distance(first1, last1); d1 >= d2; --d1) {
        if (eastl::equal<ForwardIterator1, ForwardIterator2, BinaryPredicate>(first1, i, first2,
                                                                              predicate))
          return first1;
  ...
  search_n_impl(ForwardIterator first, ForwardIterator last, Size count, const T& value,
                EASTL_ITC_NS::forward_iterator_tag) {
    if (count <= 0) return first;

    Size d1 = (Size)eastl::distance(
        first, last);  // Should d1 be of type Size, ptrdiff_t, or
                       // iterator_traits<ForwardIterator>::difference_type? The problem with using
                       // iterator_traits<ForwardIterator>::difference_type is that
    if (count >
  ...
  inline RandomAccessIterator  // Random access iterator implementation. Much faster than generic
                               // implementation.
      ...
      for (lookAhead = first + skipOffset; tailSize >= count; lookAhead += count) {
        ...
        if (*lookAhead == value) {
          ...
          for (backTrack = lookAhead - 1; *backTrack == value; --backTrack) {
            if (--remainder == 0) return (lookAhead - skipOffset);  // success
          }
          ...
          if (remainder <= tailSize) {
            ...
            while (*(++lookAhead) == value) {
              if (--remainder == 0) return (backTrack + 1);  // success
            }
  ...
  ///
  template <typename ForwardIterator, typename T>
  inline ForwardIterator binary_search_i(ForwardIterator first, ForwardIterator last,
                                         const T& value) {
    // To do: This can be made slightly faster by not using lower_bound.
    ForwardIterator i(eastl::lower_bound<ForwardIterator, T>(first, last, value));
    if ((i != last) &&
  ...
  inline ForwardIterator binary_search_i(ForwardIterator first, ForwardIterator last,
    ...
    // To do: This can be made slightly faster by not using lower_bound.
    ForwardIterator i(eastl::lower_bound<ForwardIterator, T, Compare>(first, last, value, compare));
    if ((i != last) && !compare(value, *i)) return i;
    return last;
  }
  ...
  ForwardIterator unique(ForwardIterator first, ForwardIterator last) {
    ...
    if (first != last)  // We expect that there are duplicated items, else the user wouldn't be
  ...
  ForwardIterator unique(ForwardIterator first, ForwardIterator last, BinaryPredicate predicate) {
                        ...
                        // calling this function.
    {
      ...
      for (++first; first != last; ++first) {
        if (!predicate(*dest, *first)) *++dest = *first;
      }
      return ++dest;
    }
    return last;
  ...
  ForwardIterator1 find_end_impl(ForwardIterator1 first1, ForwardIterator1 last1,
                                 ...
                                 EASTL_ITC_NS::forward_iterator_tag,
                                 EASTL_ITC_NS::forward_iterator_tag) {
    if (first2 != last2)  // We have to do this check because the search algorithm below will return
                          // first1 (and not last1) if the first2/last2 range is empty.
    {
  ...
  BidirectionalIterator1 find_end_impl(BidirectionalIterator1 first1, BidirectionalIterator1 last1,
    ...
    if (rresult.base() != first1)  // If we found something...
    {
      ...
      eastl::advance(
          result,
          -eastl::distance(first2, last2));  // We have an opportunity to optimize this, as the
  ...
  ForwardIterator1 find_end_impl(ForwardIterator1 first1, ForwardIterator1 last1,
                                 ForwardIterator2 first2, ForwardIterator2 last2,
                                 BinaryPredicate predicate, EASTL_ITC_NS::forward_iterator_tag,
                                 EASTL_ITC_NS::forward_iterator_tag) {
    if (first2 != last2)  // We have to do this check because the search algorithm below will return
                          // first1 (and not last1) if the first2/last2 range is empty.
    {
  ...
  BidirectionalIterator1 find_end_impl(BidirectionalIterator1 first1, BidirectionalIterator1 last1,
    ...
    reverse_iterator1 rresult(eastl::search<reverse_iterator1, reverse_iterator2, BinaryPredicate>(
        reverse_iterator1(last1), reverse_iterator1(first1), reverse_iterator2(last2),
        reverse_iterator2(first2), predicate));
    if (rresult.base() != first1)  // If we found something...
    {
      BidirectionalIterator1 result(rresult.base());
    ...
    if (first1 != last1) {
      ...
      for (ForwardIterator1 i = first1; i != last1; ++i) {
        if (i == eastl::find(first1, i, *i)) {
          ...
          if ((c == 0) || (c != eastl::count(i, last1, *i))) return false;
        }
      }
    }
    ...
    if (first1 != last1) {
      ...
      for (ForwardIterator1 i = first1; i != last1; ++i) {
        if (i == eastl::find(first1, i, *i, predicate)) {
          ...
          if ((c == 0) || (c != eastl::count(i, last1, *i, predicate))) return false;
        }
      }
    }
    ...
    if (first != last)  // If there is anything in the range...
    {
      ...
      if (first != --i)  // If the range has more than one item...
      {
        for (;;) {
          ...
          {
            j = last;
            while (!compare(*i, *--j))  // Find the final value that's greater than the first (it
                                        // may be equal to the second).
            {
            }
          ...
          if (i == first)  // There are no two consecutive values where the first is less than the
                           // second, meaning the range is in reverse order. The reverse ordered
                           // range is always the last permutation.
          {
  ...
  ///
  /// Note: While rotate works on ForwardIterators (e.g. slist) and BidirectionalIterators (e.g.
  /// list), you can get much better performance (O(1) instead of O(n)) with slist and list rotation
  /// by doing splice operations on those lists instead of calling this rotate function.
  ///
  ...
  ///       (with any iterator type) to avoid a lot of logic involved with algorithms like "flipping
  ///       hands" and achieve near optimal O(n) behavior. it turns out that rotate-by-one is a
  ///       common use case in practice.
  ///
  namespace Internal {
    template <typename ForwardIterator>
    ForwardIterator rotate_general_impl(ForwardIterator first, ForwardIterator middle,
                                        ForwardIterator last) {
      ...
      do {
        ...
        if (first == middle) middle = current;
      } while (current != last);
    ...
    ForwardIterator move_rotate_left_by_one(ForwardIterator first, ForwardIterator last) {
      ...
      *result = eastl::move(temp);  // iterator is a RandomAccessIterator then this move will be a
                                    // memmove for trivial types.
    ...
    BidirectionalIterator move_rotate_right_by_one(BidirectionalIterator first,
      ...
      *first = eastl::move(temp);  // iterator is a RandomAccessIterator then this move will be a
                                   // memmove for trivial types.
    ...
    struct rotate_helper {
      template <typename ForwardIterator>
      static ForwardIterator rotate_impl(ForwardIterator first, ForwardIterator middle,
                                         ForwardIterator last) {
    ...
    struct rotate_helper<EASTL_ITC_NS::bidirectional_iterator_tag, false> {
      ...
      /*
      // Simplest "flipping hands" implementation. Disabled because it's slower on average than
      rotate_general_impl. template <typename BidirectionalIterator> static BidirectionalIterator
      rotate_impl(BidirectionalIterator first, BidirectionalIterator middle, BidirectionalIterator
      last)
      {
              eastl::reverse(first, middle);
              eastl::reverse(middle, last);
              eastl::reverse(first, last);
              return first + (last - middle); // This can be slow for large ranges because operator
      + and - are O(n).
      }

      // Smarter "flipping hands" implementation, but still disabled because benchmarks are showing
      it to be slower than rotate_general_impl. template <typename BidirectionalIterator> static
      BidirectionalIterator rotate_impl(BidirectionalIterator first, BidirectionalIterator middle,
      BidirectionalIterator last)
      {
              // This is the "flipping hands" algorithm.
              eastl::reverse_impl(first,  middle, EASTL_ITC_NS::bidirectional_iterator_tag()); //
      Reverse the left side. eastl::reverse_impl(middle, last,
    ...
    struct rotate_helper<EASTL_ITC_NS::bidirectional_iterator_tag, true> {
      ...
      static BidirectionalIterator rotate_impl(BidirectionalIterator first,
        ...
        if (eastl::next(first) ==
            middle)  // If moving trivial types by a single element, memcpy is fast for that case.
          return Internal::move_rotate_left_by_one(first, last);
        if (eastl::next(middle) == last) return Internal::move_rotate_right_by_one(first, last);
        return Internal::rotate_general_impl(first, middle, last);
      }
    };
    ...
    struct rotate_helper<EASTL_ITC_NS::random_access_iterator_tag, false> {
      ...
      // In practice this implementation is about 25% faster than rotate_general_impl. We may want
      // to consider sticking with just rotate_general_impl and avoid the code generation of this
      // function.
      template <typename RandomAccessIterator>
      ...
      if (middle != last) {
        ...
        return Internal::rotate_helper < IC,
               eastl::is_trivially_move_assignable<
                   value_type>::value ||  // This is the best way of telling if we can move types
                                          // via memmove, but without a conforming C++11 compiler it
                                          // usually returns false.
                   eastl::is_pod<value_type>::
                       value ||  // This is a more conservative way of telling if we can move types
                                 // via memmove, and most compilers support it, but it doesn't have
                                 // as full of coverage as is_trivially_move_assignable.
                   eastl::is_scalar<value_type>::value >  // This is the most conservative means and
                                                          // works with all compilers, but works
                                                          // only for scalars.
