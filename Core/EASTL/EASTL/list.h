...
#define EASTL_LIST_DEFAULT_ALLOCATOR allocator_type(EASTL_LIST_DEFAULT_NAME)
  ...
  struct ListNodeBase {
    ...
    void splice(ListNodeBase* pFirst, ListNodeBase* pLast)
        EA_NOEXCEPT;  // Removes [pFirst,pLast) from the list it's in and inserts it before this in
                      // this node's list.
    void reverse()
  ...
  inline ListIterator<T, Pointer, Reference>::ListIterator(const ListNodeBase* pNode) EA_NOEXCEPT
      : mpNode(static_cast<node_type*>((ListNode<T>*)const_cast<ListNodeBase*>(
  ...
  template <typename T, typename Allocator>
  const typename ListBase<T, Allocator>::allocator_type& ListBase<T, Allocator>::get_allocator()
      const EA_NOEXCEPT {
    return mAllocator;
  }

  template <typename T, typename Allocator>
  typename ListBase<T, Allocator>::allocator_type& ListBase<T, Allocator>::get_allocator()
      EA_NOEXCEPT {
    return mAllocator;
  }
  ...
  inline typename ListBase<T, Allocator>::node_type* ListBase<T, Allocator>::DoAllocateNode() {
    node_type* pNode =
        (node_type*)allocate_memory(mAllocator, sizeof(node_type), EASTL_ALIGN_OF(T), 0);
  ...
  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::const_iterator list<T, Allocator>::cbegin() const
      EA_NOEXCEPT {
    return const_iterator((ListNodeBase*)mNode.mpNext);
  }
  ...
  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::const_reverse_iterator list<T, Allocator>::crbegin() const
      EA_NOEXCEPT {
    return const_reverse_iterator((ListNodeBase*)&mNode);
  }
  ...
  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::const_reverse_iterator list<T, Allocator>::crend() const
      EA_NOEXCEPT {
    return const_reverse_iterator((ListNodeBase*)mNode.mpNext);
  }
  ...
  template <typename T, typename Allocator>
  inline void list<T, Allocator>::insert(const_iterator position, size_type n,
                                         const value_type& value) {
  ...
  template <typename T, typename Allocator>
  template <typename InputIterator>
  inline void list<T, Allocator>::insert(const_iterator position, InputIterator first,
                                         InputIterator last) {
  ...
  typename list<T, Allocator>::reverse_iterator list<T, Allocator>::erase(
      const_reverse_iterator first, reverse_iterator last) {
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Allocator>
  inline void list<T, Allocator>::splice(const_iterator position, list<T, Allocator>&& x,
                                         const_iterator i) {
  ...
  template <typename T, typename Allocator>
  template <typename Compare>
  typename list<T, Allocator>::iterator list<T, Allocator>::DoSort(iterator i1, iterator end2,
                                                                   size_type n, Compare& compare) {
    ...
    i1 =
        DoSort(i1, end1, nMid, compare);  // Return the new beginning of the first sorted sub-range.
  ...
  template <typename T, typename Allocator>
  template <typename Integer>
  inline void list<T, Allocator>::DoInsert(ListNodeBase* pNode, Integer n, Integer value,
                                           true_type) {
  ...
  template <typename T, typename Allocator>
  template <typename InputIterator>
  inline void list<T, Allocator>::DoInsert(ListNodeBase* pNode, InputIterator first,
  ...
  inline void list<T, Allocator>::DoSwap(this_type& x) {
    ListNodeBase::swap(
        (ListNodeBase&)mNode,
        (ListNodeBase&)
            x.mNode);  // We need to implement a special swap because we can't do a shallow swap.
