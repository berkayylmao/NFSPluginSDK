...
#define EASTL_RATIO_ABS(x) ((x) < 0 ? -(x) : (x))
  ...
  struct AdditionOverFlow {
    static const bool c1 =
        (X <= 0 && 0 <= Y) || (Y < 0 && 0 < X);  // True if digits do not have the same sign.
  ...
  namespace Internal {
    ...
    template <class R1, class R2>
    struct ratio_add {
      typedef typename ct_simplify<typename ratio<
    ...
    struct ratio_multiply {
      typedef
          typename ct_simplify<typename ratio<ct_multi<R1::num, R2::num>::value,
    ...
    struct ratio_divide {
      typedef
          typename ct_simplify<typename ratio<ct_multi<R1::num, R2::den>::value,
