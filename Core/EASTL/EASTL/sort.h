...
#include <Core/EASTL/EASTL/heap.h>
    ...
    if (first != last)  // if the range is non-empty...
    {
      ...
      for (++iSorted; iSorted != last; ++iSorted) {
        ...
        for (--iCurrent; (iNext != first) && compare(temp, *iCurrent); --iNext, --iCurrent) {
          EASTL_VALIDATE_COMPARE(
              !compare(*iCurrent, temp));  // Validate that the compare function is sane.
          *iNext = *iCurrent;
        }
...
the compare function is sane. *end = *prev;                                                                                     \
  ...
  void shell_sort(RandomAccessIterator first, RandomAccessIterator last,
    ...
    if (first != last) {
      ...
      while (nSpace < nSize)
        nSpace =
            (nSpace * 3) +
            1;  // This is the Knuth 'h' sequence: 1, 4, 13, 40, 121, 364, 1093, 3280, 9841, 29524,
        ...
        for (difference_type i = 0; i < nSpace; i++) {
          ...
          for (iSorted = iInsertFirst + nSpace; iSorted < last; iSorted += nSpace) {
            ...
            for (iBack -= nSpace; (iCurrent != iInsertFirst) && compare(*iCurrent, *iBack);
              ...
              EASTL_VALIDATE_COMPARE(
                  !compare(*iBack, *iCurrent));  // Validate that the compare function is sane.
  ...
  void merge_sort_buffer(RandomAccessIterator first, RandomAccessIterator last, T* pBuffer,
    ...
    if (nCount > 1) {
      ...
      if (nMid > 1) {
        ...
        eastl::merge_sort_buffer<RandomAccessIterator, T, StrictWeakOrdering>(first, part, pBuffer,
                                                                              compare);
        eastl::merge_sort_buffer<RandomAccessIterator, T, StrictWeakOrdering>(
            part, half, pBuffer + nQ1, compare);
      ...
      if ((nCount - nMid) > 1) {
        const difference_type nQ3((difference_type)(
            ((size_t)nMid + (size_t)nCount) >> 1));  // Equivalent to (nQ3 = (nMid + nCount) / 2)
                                                     // but handles the case of integer rollover.
        RandomAccessIterator part(first + nQ3);

        eastl::merge_sort_buffer<RandomAccessIterator, T, StrictWeakOrdering>(
            half, part, pBuffer + nMid, compare);
        eastl::merge_sort_buffer<RandomAccessIterator, T, StrictWeakOrdering>(
            part, last, pBuffer + nQ3, compare);
        eastl::merge<RandomAccessIterator, RandomAccessIterator, T*, StrictWeakOrdering>(
  ...
  inline RandomAccessIterator get_partition(RandomAccessIterator first, RandomAccessIterator last,
    ...
    for (;; ++first) {
      while (*first < pivotCopy) {
        EASTL_VALIDATE_COMPARE(
            !(pivotCopy < *first));  // Validate that the compare function is sane.
        ++first;
      }
      --last;
      ...
      while (pivotCopy < *last) {
        EASTL_VALIDATE_COMPARE(
            !(*last < pivotCopy));  // Validate that the compare function is sane.
        --last;
      }
  ...
  inline RandomAccessIterator get_partition(RandomAccessIterator first, RandomAccessIterator last,
    ...
    for (;; ++first) {
      while (compare(*first, pivotCopy)) {
        EASTL_VALIDATE_COMPARE(
            !compare(pivotCopy, *first));  // Validate that the compare function is sane.
        ++first;
      }
      --last;
      ...
      while (compare(pivotCopy, *last)) {
        EASTL_VALIDATE_COMPARE(
            !compare(*last, pivotCopy));  // Validate that the compare function is sane.
        --last;
      }
  ...
  namespace Internal {
    ...
    inline void insertion_sort_simple(RandomAccessIterator first, RandomAccessIterator last) {
      for (RandomAccessIterator current = first; current != last; ++current) {
        ...
        for (--prev; value < *prev;
             --end, --prev)  // We skip checking for (prev >= first) because quick_sort (our caller)
  ...
  inline void nth_element(RandomAccessIterator first, RandomAccessIterator nth,
    ...
    while ((last - first) > 5) {
      ...
      const RandomAccessIterator midPos(
          eastl::get_partition<RandomAccessIterator, value_type>(first, last, midValue));
  ...
  inline void nth_element(RandomAccessIterator first, RandomAccessIterator nth,
    ...
    while ((last - first) > 5) {
      ...
      const RandomAccessIterator midPos(
          eastl::get_partition<RandomAccessIterator, value_type, Compare>(first, last, midValue,
                                                                          compare));
  ...
  namespace Internal {
    ...
    inline void quick_sort_impl(RandomAccessIterator first, RandomAccessIterator last,
      ...
      while (((last - first) > kQuickSortLimit) && (kRecursionCount > 0)) {
        ...
        eastl::Internal::quick_sort_impl<RandomAccessIterator, Size>(position, last,
                                                                     --kRecursionCount);
        last = position;
      }
      ...
      while (((last - first) > kQuickSortLimit) && (kRecursionCount > 0)) {
        const RandomAccessIterator position(
            eastl::get_partition<RandomAccessIterator, value_type, Compare>(
                first, last,
                eastl::median<value_type, Compare>(*first, *(first + (last - first) / 2),
                                                   *(last - 1), compare),
                compare));

        eastl::Internal::quick_sort_impl<RandomAccessIterator, Size, Compare>(
  ...
  void quick_sort(RandomAccessIterator first, RandomAccessIterator last, Compare compare) {
    ...
    if (first != last) {
      ...
      if ((last - first) > (difference_type)kQuickSortLimit) {
        eastl::insertion_sort<RandomAccessIterator, Compare>(first, first + kQuickSortLimit,
                                                             compare);
...
#define EASTL_COUNT_LEADING_ZEROES eastl_count_leading_zeroes
    ...
    //
    template <typename RandomAccessIterator, typename StrictWeakOrdering>
    intptr_t tim_sort_count_run(const RandomAccessIterator first, const intptr_t start,
      ...
      if ((size - start) > 1)  // If there is anything in the set...
      {
        ...
        if (!compare(
                *(first + start + 1),
                *(first +
                  start)))  // If (first[start + 1] >= first[start]) (If the run is increasing) ...
        {
          for (;; ++curr) {
    ...
    template <typename RandomAccessIterator, typename T, typename StrictWeakOrdering>
    void tim_sort_merge(RandomAccessIterator first, const tim_sort_run* run_stack,
                        const intptr_t stack_curr, T* pBuffer, StrictWeakOrdering compare) {
    ...
    template <typename RandomAccessIterator, typename T, typename StrictWeakOrdering>
    intptr_t tim_sort_collapse(RandomAccessIterator first, tim_sort_run* run_stack,
                               intptr_t stack_curr, T* pBuffer, const intptr_t size,
                               StrictWeakOrdering compare) {
...
#if EASTL_DEV_DEBUG
            EASTL_DEV_ASSERT((run_stack[stack_curr - 2].start + run_stack[stack_curr - 2].length) <=
                             size);
            EASTL_DEV_ASSERT((run_stack[stack_curr - 1].start + run_stack[stack_curr - 1].length) <=
                             size);
            memset(&run_stack[stack_curr], 0, sizeof(run_stack[stack_curr]));
#endif
          } else {
    ...
    //
    template <typename RandomAccessIterator, typename T, typename StrictWeakOrdering>
    bool tim_sort_add_run(tim_sort_run* run_stack, RandomAccessIterator first, T* pBuffer,
                          const intptr_t size, const intptr_t minrun, intptr_t& len, intptr_t& run,
                          intptr_t& curr, intptr_t& stack_curr, StrictWeakOrdering compare) {
      ...
      if (run >
          (size - curr))  // But if there isn't minrun data remaining, just sort what's remaining.
        run = (size - curr);

      if (run > len)  // If there is any additional data we want to sort to bring up the run length
                      // to minrun.
      {
        insertion_sort_already_started(first + curr, first + curr + run, first + curr + len,
                                       compare);
        len = run;
      }
  ...
  void tim_sort_buffer(RandomAccessIterator first, RandomAccessIterator last, T* pBuffer,
    ...
    if (size < 64)
      ...
      if (tim_sort_add_run(run_stack, first, pBuffer, size, minrun, len, run, curr, stack_curr,
                           compare))
        return;
      if (tim_sort_add_run(run_stack, first, pBuffer, size, minrun, len, run, curr, stack_curr,
                           compare))
  ...
  namespace Internal {
    ...
    void radix_sort_impl(RandomAccessIterator first, RandomAccessIterator last,
                         RandomAccessIterator buffer, ExtractKey extractKey, uint8_t) {
    ...
    template <typename RandomAccessIterator, typename ExtractKey>
    void radix_sort_impl(RandomAccessIterator first, RandomAccessIterator last,
                         RandomAccessIterator buffer, ExtractKey extractKey, uint16_t) {
      uint32_t EA_PREFIX_ALIGN(EASTL_PLATFORM_PREFERRED_ALIGNMENT)
          bucketSize[256];  // The alignment of this variable isn't required; it merely
      uint32_t EA_PREFIX_ALIGN(EASTL_PLATFORM_PREFERRED_ALIGNMENT)
          bucketPosition[256];  // allows the code below to be faster on some platforms.
    ...
    void radix_sort_impl(RandomAccessIterator first, RandomAccessIterator last,
                         RandomAccessIterator buffer, ExtractKey extractKey, IntegerType) {
      uint32_t EA_PREFIX_ALIGN(EASTL_PLATFORM_PREFERRED_ALIGNMENT)
          bucketSize[256];  // The alignment of this variable isn't required; it merely
      uint32_t EA_PREFIX_ALIGN(EASTL_PLATFORM_PREFERRED_ALIGNMENT)
          bucketPosition[256];  // allows the code below to be faster on some platforms.
  ...
  namespace Internal {
    ...
    void bubble_sort_impl(BidirectionalIterator first, BidirectionalIterator last,
                          StrictWeakOrdering compare, EASTL_ITC_NS::bidirectional_iterator_tag) {
      if (first != last) {
  ...
  inline void bubble_sort(ForwardIterator first, ForwardIterator last, StrictWeakOrdering compare) {
    ...
    eastl::Internal::bubble_sort_impl<ForwardIterator, StrictWeakOrdering>(first, last, compare,
                                                                           IC());
  }
...
#else
    eastl::merge_sort<RandomAccessIterator, EASTLAllocatorType>(first, last,
                                                                *get_default_allocator(0));
#endif
  }
...
#else
    eastl::merge_sort<RandomAccessIterator, Allocator, StrictWeakOrdering>(first, last, allocator,
                                                                           compare);
#endif
  }
