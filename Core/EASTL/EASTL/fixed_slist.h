...
#include <Core/EASTL/EASTL/slist.h>
...
#define EASTL_FIXED_SLIST_DEFAULT_ALLOCATOR overflow_allocator_type(EASTL_FIXED_SLIST_DEFAULT_NAME)
  ...
  class fixed_slist
      : public slist<
            T, fixed_node_allocator<sizeof(typename slist<T>::node_type), nodeCount,
                                    EASTL_ALIGN_OF(T), 0, bEnableOverflow, OverflowAllocator> > {
   public:
    typedef fixed_node_allocator<sizeof(typename slist<T>::node_type), nodeCount, EASTL_ALIGN_OF(T),
   ...
   public:
    fixed_slist();
    explicit fixed_slist(const overflow_allocator_type&
                             overflowAllocator);  // Only applicable if bEnableOverflow is true.
    explicit fixed_slist(size_type n);            // Currently we don't support overflowAllocator
    ...
    fixed_slist(
        ...
        const overflow_allocator_type&    overflowAllocator = EASTL_FIXED_SLIST_DEFAULT_ALLOCATOR);
...
// Deprecated:
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  template <typename InputIterator>
  fixed_slist<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_slist(InputIterator first,
                                                                             InputIterator last)
      : base_type(fixed_allocator_type(mBuffer)) {
#if EASTL_NAME_ENABLED
    mAllocator.set_name(EASTL_FIXED_SLIST_DEFAULT_NAME);
  ...
  inline bool fixed_slist<T, nodeCount, bEnableOverflow, OverflowAllocator>::full() const {
    ...
    // the nodes from the fixed buffer.
    return !mAllocator
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline bool fixed_slist<T, nodeCount, bEnableOverflow, OverflowAllocator>::has_overflowed()
      const {
  ...
  fixed_slist<T, nodeCount, bEnableOverflow, OverflowAllocator>::get_overflow_allocator() const
      EA_NOEXCEPT {
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline void fixed_slist<T, nodeCount, bEnableOverflow, OverflowAllocator>::set_overflow_allocator(
