...
namespace eastl {
  ...
  class intrusive_list_iterator {
   ...
   public:
    ...
    explicit intrusive_list_iterator(
        pointer pNode);  // Note that you can also construct an iterator from T via this, since
  ...
  class intrusive_list_base {
   public:
    typedef eastl_size_t
                      size_type;  // See config.h for the definition of this, which defaults to uint32_t.
    typedef ptrdiff_t difference_type;
   ...
   public:
    ...
    EASTL_API bool validate()
        const;  ///< Scans a list for linkage inconsistencies; O(n) time, O(1) space. Returns false
                ///< if errors are detected, such as loops or branching.
  ...
  class intrusive_list : public intrusive_list_base {
   ...
   public:
    ...
    const_reverse_iterator rbegin() const
        EA_NOEXCEPT;  ///< Returns a const_reverse_iterator pointing at the end of the list (start
    ...
    const_reverse_iterator rend() const
        EA_NOEXCEPT;  ///< Returns a const_reverse_iterator pointing at the start of the list (end
                      ///< of the reverse sequence).
    ...
    void push_front(
        value_type& x);  ///< Adds an element to the front of the list; O(1). The element is not
    ...
    iterator locate(
        value_type& x);  ///< Converts a reference to an object in the list back to an iterator, or
                         ///< returns end() if it is not part of the list. O(n)
    const_iterator locate(const value_type& x)
        const;  ///< Converts a const reference to an object in the list back to a const iterator,
  ...
  template <typename T>
  inline typename intrusive_list<T>::const_reverse_iterator intrusive_list<T>::crbegin() const
      EA_NOEXCEPT {
    return const_reverse_iterator(const_iterator(static_cast<const T*>(&mAnchor)));
  }
  ...
  template <typename T>
  inline typename intrusive_list<T>::const_reverse_iterator intrusive_list<T>::crend() const
      EA_NOEXCEPT {
    return const_reverse_iterator(const_iterator(static_cast<const T*>(mAnchor.mpNext)));
  }
  ...
  template <typename T>
  inline typename intrusive_list<T>::iterator intrusive_list<T>::insert(const_iterator pos,
                                                                        value_type&    x) {
#if EASTL_VALIDATE_INTRUSIVE_LIST && EASTL_ASSERT_ENABLED
    if (x.mpNext || x.mpPrev)
  ...
  template <typename T>
  inline typename intrusive_list<T>::iterator intrusive_list<T>::erase(const_iterator first,
                                                                       const_iterator last) {
