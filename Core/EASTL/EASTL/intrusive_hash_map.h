...
#include <Core/EASTL/EASTL/internal/intrusive_hashtable.h>
...
namespace eastl {
  ...
  ///
  /// Template parameters:
  ///     Key             The key object (key in the key/value pair). T must contain a member of
  ///     type Key named mKey. T               The type of object the map holds (a.k.a. value).
  ...
  class intrusive_hash_map
      : public intrusive_hashtable<Key, T, Hash, Equal, bucketCount, false, true> {
   public:
   ...
   public:
    ...
    // To consider: Is this feasible, given how initializer_list works by creating a temporary
    // array? Even if it is feasible, is it a good idea?
  ...
  class intrusive_hash_multimap
      : public intrusive_hashtable<Key, T, Hash, Equal, bucketCount, false, false> {
   public:
