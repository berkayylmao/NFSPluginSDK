...
#include <Core/EASTL/EASTL/deque.h>
...
#define EASTL_QUEUE_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " queue"  // Unless the user overrides something, this is "EASTL queue".
#endif
...
#define EASTL_QUEUE_DEFAULT_ALLOCATOR allocator_type(EASTL_QUEUE_DEFAULT_NAME)
  ...
  class queue {
   ...
   public:
    ...
    queue(std::initializer_list<value_type>
  ...
  inline queue<T, Container>::queue()
      : c()  // To consider: use c(EASTL_QUEUE_DEFAULT_ALLOCATOR) here, though that would add the
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Container>
  inline queue<T, Container>::queue(Container&& x) : c(eastl::move(x)) {
    // Empty
  }
#endif
  ...
  template <typename T, typename Container>
  inline typename queue<T, Container>::container_type& queue<T, Container>::get_container() {
    return c;
  }

  template <typename T, typename Container>
  inline const typename queue<T, Container>::container_type& queue<T, Container>::get_container()
      const {
    return c;
  }

  template <typename T, typename Container>
  void queue<T, Container>::swap(this_type& x)
      EA_NOEXCEPT_IF((eastl::is_nothrow_swappable<this_type::container_type>::value)) {
    using eastl::swap;
    swap(c, x.c);
  }
  ...
  template <typename T, typename Container>
  inline void swap(queue<T, Container>& a, queue<T, Container>& b) EA_NOEXCEPT_IF(
