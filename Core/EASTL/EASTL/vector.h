...
#define EASTL_VECTOR_DEFAULT_ALLOCATOR allocator_type(EASTL_VECTOR_DEFAULT_NAME)
  ...
  class vector : public VectorBase<T, Allocator> {
   ...
   public:
    ...
    typedef eastl::reverse_iterator<iterator>
        reverse_iterator;  //       change in the future is that a debugging iterator system will be
                           //       created.
   ...
   public:
    ...
    void set_capacity(
        size_type n =
            base_type::npos);  // Revises the capacity to the user-specified value. Resizes the
    ...
    iterator erase_unsorted(
        const_iterator
            position);  // Same as erase, except it doesn't preserve order, but is faster because it
   ...
   protected:
    ...
    template <typename ForwardIterator>  // Allocates a pointer of array count n and copy-constructs
                                         // it with [first,last).
                                         pointer DoRealloc(size_type n, ForwardIterator first,
    ...
    void DoInsertFromIterator(const_iterator position, BidirectionalIterator first,
...
#if EASTL_MOVE_SEMANTICS_ENABLED && \
    EASTL_VARIADIC_TEMPLATES_ENABLED  // If we can do variadic arguments...
...
#if EASTL_MOVE_SEMANTICS_ENABLED
    this_type temp =
        this_type(move_iterator<iterator>(begin()), move_iterator<iterator>(end()), mAllocator);
#else
    this_type temp(*this);
#endif
  ...
  inline void vector<T, Allocator>::emplace_back(Args&&... args) {
    if (mpEnd < mpCapacity) {
      ::new ((void*)mpEnd) value_type(
          eastl::forward<Args>(args)...);  // If value_type has a move constructor, it will use it
  ...
  inline void vector<T, Allocator>::emplace_back(value_type&& value) {
    if (mpEnd < mpCapacity) {
      ::new ((void*)mpEnd)
          value_type(eastl::move(value));  // If value_type has a move constructor, it will use it
                                           // and this operation may be faster than otherwise.
      ++mpEnd;  // Increment this after the construction above in case the construction throws an
                // exception.
    } else
  ...
  template <typename T, typename Allocator>
  inline void vector<T, Allocator>::insert(const_iterator position, size_type n,
  ...
  template <typename T, typename Allocator>
  template <typename InputIterator>
  inline void vector<T, Allocator>::insert(const_iterator position, InputIterator first,
                                           InputIterator last) {
  ...
  template <typename T, typename Allocator>
  inline typename vector<T, Allocator>::iterator vector<T, Allocator>::erase_unsorted(
      const_iterator position) {
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((position < mpBegin) || (position >= mpEnd)))
      EASTL_FAIL_MSG("vector::erase -- invalid position");
  ...
  template <typename T, typename Allocator>
  inline typename vector<T, Allocator>::reverse_iterator vector<T, Allocator>::erase(
  ...
  inline void vector<T, Allocator>::DoInit(Integer n, Integer value, true_type) {
    ...
    typedef typename eastl::remove_const<T>::type
        non_const_value_type;  // If T is a const type (e.g. const int) then we need to initialize
                               // it as if it were non-const.
  ...
  template <typename T, typename Allocator>
  template <typename Integer>
  inline void vector<T, Allocator>::DoInsert(const_iterator position, Integer n, Integer value,
                                             true_type) {
  ...
  template <typename T, typename Allocator>
  template <typename InputIterator>
  inline void vector<T, Allocator>::DoInsert(const_iterator position, InputIterator first,
...
#else
        pointer pNewEnd =
            eastl::uninitialized_move_ptr_if_noexcept(mpBegin, destPosition, pNewData);
        pNewEnd = eastl::uninitialized_copy_ptr(first, last, pNewEnd);
        pNewEnd = eastl::uninitialized_move_ptr_if_noexcept(destPosition, mpEnd, pNewEnd);
#endif
  ...
  void vector<T, Allocator>::DoInsertValue(const_iterator position, Args&&... args) {
    ...
    if (mpEnd != mpCapacity)  // If size < capacity ...
    {
      ...
      EASTL_ASSERT(
          position <
          mpEnd);  // While insert at end() is valid, our design is such that calling code should
      ...
      auto value = value_type(
          eastl::forward<Args>(args)...);  // Workaround for compiler bug in VS2013 which results in
                                           // a compiler internal crash while compiling this code.
#else
      value_type value(eastl::forward<Args>(
          args)...);  // Need to do this before the move_backward below because maybe args refers to
                      // something within the moving range.
#endif
      ::new (static_cast<void*>(mpEnd)) value_type(eastl::move(
          *(mpEnd - 1)));  // mpEnd is uninitialized memory, so we must construct into it instead of
    ...
    } else  // else (size == capacity)
    {
      ...
      try {  // To do: We are not handling exceptions properly below.  In particular we don't want
        ...
        // costructed.
        ::new ((void*)(pNewData + nPosSize)) value_type(
            eastl::forward<Args>(args)...);  // Because the old data is potentially being moved
        ...
        if (pNewEnd)
          ...
          eastl::destruct(pNewData +
                          nPosSize);  // The exception occurred during the first unintialized move,
  ...
  void vector<T, Allocator>::DoInsertValue(const_iterator position, value_type&& value) {
    ...
    if (mpEnd != mpCapacity)  // If size < capacity (and we can do this without reallocation)...
    {
      ...
      if ((pValue >= destPosition) &&
          (pValue < mpEnd))  // If value comes from within the range to be moved...
  ...
  void vector<T, Allocator>::DoInsertValue(const_iterator position, const value_type& value) {
    ...
    } else  // else (size == capacity)
    {
      ...
      try {
        ::new ((void*)(pNewData + nPosSize))
            value_type(value);  // Because the old data is being moved rather than copied, we need
                                // to move the value first,
        pNewEnd = NULL;  // Set to NULL so that in catch we can tell the exception occurred during
                         // the next call.
        pNewEnd = eastl::uninitialized_move_ptr_if_noexcept(
            mpBegin, destPosition,
...
#else
      ::new ((void*)(pNewData + nPosSize))
          value_type(value);  // Because the old data is being moved rather than copied, we need to
