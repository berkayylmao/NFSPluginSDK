...
#include <Core/EASTL/EASTL/memory.h>
...
#define DEQUE_DEFAULT_SUBARRAY_SIZE(T) \
  ((sizeof(T) <= 4)                    \
       ? 64                            \
       : ((sizeof(T) <= 8) ? 32 : ((sizeof(T) <= 16) ? 16 : ((sizeof(T) <= 32) ? 8 : 4))))
#else
#define DEQUE_DEFAULT_SUBARRAY_SIZE(T) 16
  ...
  struct DequeIterator {
   ...
   protected:
    ...
    template <typename U, typename PointerA, typename ReferenceA, typename PointerB,
              typename ReferenceB, unsigned kDequeSubarraySizeU>
   ...
   public:
    ...
    template <typename InputIterator>  // It turns out that the C++ std::deque<int, int> specifies a
                                       ...
                                       void assign(
                                           InputIterator first,
                                           InputIterator
                                               last);  // version of assign that takes (int size,
  ...
  DequeBase<T, Allocator, kDequeSubarraySize>::DoReallocSubarray(size_type nAdditionalCapacity,
                                                                 Side      allocationSide) {
    ...
    if (allocationSide == kSideFront) {
              ...
              nAdditionalCapacity))  // If we need to grow downward into a new subarray...
      {
        const difference_type nSubarrayIncrease = (difference_type)(
            ((nAdditionalCapacity - nCurrentAdditionalCapacity) + kDequeSubarraySize - 1) /
            kDequeSubarraySize);
        difference_type i;

        if (nSubarrayIncrease >
            (mItBegin.mpCurrentArrayPtr - mpPtrArray))  // If there are not enough pointers in front
                                                        // of the current (first) one...
          DoReallocPtrArray(
              (size_type)(nSubarrayIncrease - (mItBegin.mpCurrentArrayPtr - mpPtrArray)),
              kSideFront);
        ...
        if (nSubarrayIncrease >
          ...
          DoReallocPtrArray(
              (size_type)(nSubarrayIncrease -
                          (((mpPtrArray + mnPtrArraySize) - mItEnd.mpCurrentArrayPtr) - 1)),
              kSideBack);
  ...
  void DequeBase<T, Allocator, kDequeSubarraySize>::DoReallocPtrArray(size_type nAdditionalCapacity,
    ...
    if ((allocationSide == kSideBack) &&
        (nAdditionalCapacity <=
         nUnusedPtrCountAtFront))  // If we can take advantage of unused pointers at the front
                                   // without doing any reallocation...
    {
      ...
      memset(pPtrArrayBegin + nUsedPtrCount, 0,
             (size_t)(mpPtrArray + mnPtrArraySize) - (size_t)(pPtrArrayBegin + nUsedPtrCount));
               ...
               (nAdditionalCapacity <=
                nUnusedPtrCountAtBack))  // If we can take advantage of unused pointers at the back
                                         // without doing any reallocation...
    {
      if (nAdditionalCapacity <
      ...
      const size_type nNewPtrArraySize = mnPtrArraySize +
                                         eastl::max_alt(mnPtrArraySize, nAdditionalCapacity) +
  ...
  void DequeBase<T, Allocator, kDequeSubarraySize>::DoInit(size_type n) {
    ...
    const size_type kMinPtrArraySize_ =
        kMinPtrArraySize;  // GCC 4.0 blows up unless we define this constant.

    mnPtrArraySize =
        eastl::max_alt(kMinPtrArraySize_, (nNewPtrArraySize + 2));  // GCC 4.0 blows up on this.
...
// I am sorry for the mess of #ifs and indentations below.
  ...
  template <typename T, typename Pointer, typename Reference, unsigned kDequeSubarraySize>
  DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::DequeIterator(T** pCurrentArrayPtr,
                                                                          T*  pCurrent)
      : mpCurrent(pCurrent),
  ...
  DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::operator+=(difference_type n) {
    ...
    if ((size_t)subarrayPosition <
        (size_t)kDequeSubarraySize)  // If the new position is within the current subarray (i.e. >=
                                     // 0 && < kSubArraySize)...
      mpCurrent += n;
    else {
      ...
      mpCurrent =
          mpBegin + (subarrayPosition - (subarrayIndex * (difference_type)kDequeSubarraySize));
    }
    return *this;
  }
  ...
  template <typename T, typename Pointer, typename Reference, unsigned kDequeSubarraySize>
  typename DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::this_type
  DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::copy(const iterator& first,
  ...
  operator-(const DequeIterator<T, PointerA, ReferenceA, kDequeSubarraySize>& a,
    ...
    // original HP STL:
    typedef typename DequeIterator<T, PointerA, ReferenceA, kDequeSubarraySize>::difference_type
        difference_type;
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  inline deque<T, Allocator, kDequeSubarraySize>::deque(size_type             n,
                                                        const allocator_type& allocator)
  ...
  inline deque<T, Allocator, kDequeSubarraySize>::deque(InputIterator first, InputIterator last)
      : base_type(
            EASTL_DEQUE_DEFAULT_ALLOCATOR)  // Call the empty base constructor, which does nothing.
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  inline void deque<T, Allocator, kDequeSubarraySize>::assign(size_type         n,
...
// In this case we need to free the subarray and point mItEnd to the previous subarray.
  ...
  deque<T, Allocator, kDequeSubarraySize>::emplace(const_iterator position, Args&&... args) {
    if (EASTL_UNLIKELY(position.mpCurrent ==
                       mItEnd.mpCurrent))  // If we are doing the same thing as push_back...
    {
      emplace_back(eastl::forward<Args>(args)...);
      return iterator(
          mItEnd, typename iterator::Decrement());  // Unfortunately, we need to make an iterator
    ...
    auto valueSaved =
        value_type(eastl::forward<Args>(args)...);  // Workaround for compiler bug in VS2013
#else
    value_type valueSaved(eastl::forward<Args>(
        args)...);  // We need to save this because value may come from within our container. It
  ...
  void deque<T, Allocator, kDequeSubarraySize>::emplace_front(Args&&... args) {
    if (mItBegin.mpCurrent !=
        mItBegin.mpBegin)  // If we have room in the first subarray... we hope that usually this
                           // 'new' pathway gets executed, as it is slightly faster.
      ::new ((void*)--mItBegin.mpCurrent) value_type(
          eastl::forward<Args>(args)...);  // Construct in place. If args is a single arg of type
...
// To consider: Detect if value isn't coming from within this container and handle that efficiently.
...
#else
      value_type valueSaved(eastl::forward<Args>(
          args)...);  // We need to make a temporary, because args may be a value_type that comes
  ...
  void deque<T, Allocator, kDequeSubarraySize>::emplace_back(Args&&... args) {
    if ((mItEnd.mpCurrent + 1) !=
        mItEnd.mpEnd)  // If we have room in the last subarray... we hope that usually this 'new'
                       // pathway gets executed, as it is slightly faster.
...
// To consider: Detect if value isn't coming from within this container and handle that efficiently.
      ...
      try {
        ...
        ::new ((void*)mItEnd.mpCurrent)
            value_type(eastl::move(valueSaved));  // We can move valueSaved into position.
        mItEnd.SetSubarray(mItEnd.mpCurrentArrayPtr + 1);
  ...
  deque<T, Allocator, kDequeSubarraySize>::emplace(const_iterator position, value_type&& value) {
    ...
    iterator itPosition(position, typename iterator::FromConst());
    value_type valueSaved(eastl::move(
        value));  // We need to save this because value may come from within our container. It would
  ...
  void deque<T, Allocator, kDequeSubarraySize>::emplace_front(value_type&& value) {
    if (mItBegin.mpCurrent !=
      ...
      ::new ((void*)--mItBegin.mpCurrent)
          value_type(eastl::move(value));  // Move value into position.
    else {
      // To consider: Detect if value isn't coming from within this container and handle that
      // efficiently.
      value_type valueSaved(
          eastl::move(value));  // We need to make a temporary, because value may come from within
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::iterator
  deque<T, Allocator, kDequeSubarraySize>::emplace(const_iterator position,
                                                   const value_type& value) {
    if (EASTL_UNLIKELY(position.mpCurrent ==
                       mItEnd.mpCurrent))  // If we are doing the same thing as push_back...
    {
  ...
  void deque<T, Allocator, kDequeSubarraySize>::emplace_front(const value_type& value) {
    if (mItBegin.mpCurrent != mItBegin.mpBegin)  // If we have room in the first subarray...
      ::new ((void*)--mItBegin.mpCurrent)
          value_type(value);  // We hope that usually this 'new' pathway gets executed, as it is
                              // slightly faster.
    else  // Note that in this 'else' case we create a temporary, which is less desirable.
    {
      ...
      // efficiently.
      value_type valueSaved(
          value);  // We need to make a temporary, because value may come from within our container
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::iterator
  deque<T, Allocator, kDequeSubarraySize>::insert(const_iterator    position,
                                                  const value_type& value) {
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  void deque<T, Allocator, kDequeSubarraySize>::insert(const_iterator position, size_type n,
                                                       const value_type& value) {
  ...
  deque<T, Allocator, kDequeSubarraySize>::erase(const_iterator first, const_iterator last) {
                             ...
                             // erase(begin, end) because instead the user would just call clear.)
    {
      ...
      if (i < (difference_type)((size() - n) /
                                2))  // Should we move the front entries forward or the back entries
  ...
  void deque<T, Allocator, kDequeSubarraySize>::clear() {
    ...
    if (mItBegin.mpCurrentArrayPtr !=
        mItEnd.mpCurrentArrayPtr)  // If there are multiple subarrays (more often than not, this
                                   // will be so)...
    {
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  template <typename InputIterator>
  void deque<T, Allocator, kDequeSubarraySize>::DoInit(InputIterator first, InputIterator last,
                                                       false_type) {
  ...
  template <typename ForwardIterator>
  void deque<T, Allocator, kDequeSubarraySize>::DoInitFromIterator(
      ForwardIterator first, ForwardIterator last, EASTL_ITC_NS::forward_iterator_tag) {
    ...
    try {
#endif
      for (pPtrArrayCurrent = mItBegin.mpCurrentArrayPtr;
      ...
      for (iterator itCurrent(mItBegin), itEnd(pPtrArrayCurrent, *pPtrArrayCurrent);
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  void deque<T, Allocator, kDequeSubarraySize>::DoAssignValues(size_type         n,
                                                               const value_type& value) {
    const size_type nSize = size();
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  template <typename Integer>
  void deque<T, Allocator, kDequeSubarraySize>::DoInsert(const const_iterator& position, Integer n,
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  template <typename InputIterator>
  void deque<T, Allocator, kDequeSubarraySize>::DoInsert(const const_iterator& position,
  ...
  void deque<T, Allocator, kDequeSubarraySize>::DoInsertFromIterator(
      const_iterator position, const InputIterator& first, const InputIterator& last,
      EASTL_ITC_NS::forward_iterator_tag) {
    const size_type n = (size_type)eastl::distance(first, last);
    ...
    if (position.mpCurrent ==
        mItBegin.mpCurrent)  // If inserting at the beginning or into an empty container...
    {
                              ...
                              mItEnd.mpCurrent))  // If inserting at the end (i.e. appending)...
    {
      const iterator itNewEnd(DoReallocSubarray(
          n, kSideBack));  // mItEnd to itNewEnd refers to memory that isn't initialized yet; so
                           // it's not truly a valid iterator. Or at least not a dereferencable one.
      ...
      if (nInsertionIndex <
          (difference_type)(nSize / 2))  // If the insertion index is in the front half of the
                                         // deque... grow the deque at the front.
      {
        ...
        const iterator itPosition(mItBegin +
                                  nInsertionIndex);  // We need to reset this value because the
        ...
        const iterator        itPosition(mItEnd -
                                  nPushedCount);  // We need to reset this value because the
                                                  // reallocation above can invalidate iterators.
        ...
        try {
                  ...
                  n)  // If the newly inserted items will be entirely within the old area...
          {
            ...
            itUCopyEnd = eastl::move_backward(
                itPosition, itUCopyEnd, itOldEnd);  // Recycle 'itUCopyEnd' to mean something else.
            eastl::fill(itPosition, itUCopyEnd, valueSaved);
          } else  // Else the newly inserted items are going within the newly allocated area at the
                  // back.
          {
            eastl::uninitialized_fill_move(mItEnd, itPosition + (difference_type)n, valueSaved,
                                           itPosition, mItEnd);  // This can throw.
