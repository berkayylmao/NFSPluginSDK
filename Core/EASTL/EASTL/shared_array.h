...
#include <Core/EASTL/EASTL/internal/smart_ptr.h>  // Defines smart_array_deleter
...
#define EASTL_SHARED_ARRAY_DEFAULT_ALLOCATOR allocator_type(EASTL_SHARED_ARRAY_DEFAULT_NAME)
  ...
  class shared_array {
   ...
   public:
    ...
    explicit shared_array(T*               pArray    = NULL,
                          const allocator& allocator = EASTL_SHARED_ARRAY_DEFAULT_ALLOCATOR)
        : mpArray(pArray), mpRefCount(NULL), mAllocator(allocator) {
      // Allocate memory for the reference count.
      void* const pMemory = EASTLAlloc(mAllocator, sizeof(ref_count));
      if (pMemory) mpRefCount = ::new (pMemory) ref_count(1);
    }
    ...
    ///    X* pX = ptr.get();
    ///    pX->DoSomething();
    T* get() const EA_NOEXCEPT { return mpArray; }
    ...
    ///    if(!ptr)
    ///        assert(false);
    bool operator!() const EA_NOEXCEPT { return (mpArray == NULL); }

    /// get_allocator
    /// Returns the memory allocator associated with this class.
    const allocator_type& get_allocator() const EA_NOEXCEPT { return mAllocator; }
    allocator_type&       get_allocator() EA_NOEXCEPT { return mAllocator; }
  ...
  inline bool operator==(const shared_array<T, TA, TD>& sharedArray1,
    ...
    // assert((sharedArray1.get() != sharedArray2.get()) || (sharedArray1.use_count() ==
    // sharedArray2.use_count()));
