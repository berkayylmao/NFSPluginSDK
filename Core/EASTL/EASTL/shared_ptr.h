...
#include <Core/EASTL/EASTL/internal/smart_ptr.h>
#include <Core/EASTL/EASTL/internal/thread_support.h>
#include <Core/EASTL/EASTL/unique_ptr.h>
...
#define EASTL_SHARED_PTR_DEFAULT_ALLOCATOR EASTLAllocatorType(EASTL_SHARED_PTR_DEFAULT_NAME)
  ...
  void do_enable_shared_from_this(const ref_count_sp*               pRefCount,
    ...
    if (pEnableSharedFromThis)
      pEnableSharedFromThis->mWeakPtr.assign(const_cast<U*>(pValue),
                                             const_cast<ref_count_sp*>(pRefCount));
  }
  ...
  class shared_ptr {
   ...
   public:
    ...
    shared_ptr(const shared_ptr& sharedPtr) EA_NOEXCEPT : mpValue(sharedPtr.mpValue),
    ...
    shared_ptr(const shared_ptr<U>& sharedPtr,
               typename eastl::enable_if<eastl::is_convertible<U*, element_type*>::value>::type* =
                   0) EA_NOEXCEPT : mpValue(sharedPtr.mpValue),
                                    mpRefCount(sharedPtr.mpRefCount) {
      if (mpRefCount) mpRefCount->addref();
    }
...
#if EASTL_MOVE_SEMANTICS_ENABLED
    shared_ptr(shared_ptr&& sharedPtr) EA_NOEXCEPT : mpValue(sharedPtr.mpValue),
                                                     mpRefCount(sharedPtr.mpRefCount) {
...
#if EASTL_RTTI_ENABLED
      return mpRefCount ? static_cast<Deleter*>(
                              mpRefCount->get_deleter(typeid(typename remove_cv<Deleter>::type)))
                        : NULL;
#else
   ...
   protected:
    ...
    void alloc_internal(U* pValue, Allocator allocator, Deleter deleter) {
      ...
      try {
        ...
        mpRefCount =
            ::new (pMemory) ref_count_type(pValue, eastl::move(deleter), eastl::move(allocator));
        mpValue = pValue;
  ...
  template <typename T>
  inline typename shared_ptr<T>::element_type* get_pointer(const shared_ptr<T>& sharedPtr)
      EA_NOEXCEPT {
  ...
  inline bool operator<(const shared_ptr<T>& a, const shared_ptr<U>& b) EA_NOEXCEPT {
    ...
    typedef typename eastl::common_type<T*, U*>::type
        CPointer;  // We currently need to make these temporary variables, as otherwise clang
                   // complains about CPointer being int*&&&.
    ...
    return less<CPointer>()(
        pT, pU);  // It looks like common_type is making CPointer be (e.g.) int*&& instead of int*,
  ...
  template <typename T, typename U>  // Retained for support for pre-C++11 shared_ptr.
  inline typename eastl::enable_if<!eastl::is_array<T>::value && !eastl::is_array<U>::value,
                                   shared_ptr<T> >::type
  ...
  shared_ptr<T> allocate_shared(const Allocator& allocator, A1&& a1, A2&& a2, A3&& a3) {
    ...
    if (pMemory) {
      ref_count_type* pRefCount = ::new (pMemory) ref_count_type(
          allocator, eastl::forward<A1>(a1), eastl::forward<A2>(a2), eastl::forward<A3>(a3));
      allocate_shared_helper(ret, pRefCount, pRefCount->GetValue());
    }
    return ret;
  }
  ...
  shared_ptr<T> make_shared(A1&& a1, A2&& a2, A3&& a3) {
    return allocate_shared<T>(EASTL_SHARED_PTR_DEFAULT_ALLOCATOR, eastl::forward<A1>(a1),
                              eastl::forward<A2>(a2), eastl::forward<A3>(a3));
  }
  ...
  template <typename T>
  inline shared_ptr<T> atomic_exchange_explicit(shared_ptr<T>* pSharedPtrA,
...
#define EASTL_WEAK_PTR_DEFAULT_ALLOCATOR allocator_type(EASTL_WEAK_PTR_DEFAULT_NAME)
  ...
  class weak_ptr {
   ...
   public:
    ...
    /// weak_ptr
    /// Construction with self type.
    weak_ptr(const this_type& weakPtr) EA_NOEXCEPT : mpValue(weakPtr.mpValue),
...
// to here.
#include <Core/EASTL/EASTL/internal/enable_shared.h>
