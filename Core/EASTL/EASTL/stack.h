...
#define EASTL_STACK_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " stack"  // Unless the user overrides something, this is "EASTL stack".
#endif
...
#define EASTL_STACK_DEFAULT_ALLOCATOR allocator_type(EASTL_STACK_DEFAULT_NAME)
  ...
  class stack {
   public:
    ...
    // typedef typename Container::allocator_type   allocator_type;  // We can't currently declare
    // this because the container may be a type that doesn't have an allocator.
   ...
   public:
    ...
    stack(std::initializer_list<value_type>
              ilist);  // The first item in the initializer list is pushed first. C++11 doesn't
                       // specify that std::stack has initializer list support.
  ...
  inline stack<T, Container>::stack()
      : c()  // To consider: use c(EASTL_STACK_DEFAULT_ALLOCATOR) here, though that would add the
             // requirement that the user supplied container support this.
  {
    // Empty
  }
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Container>
  inline stack<T, Container>::stack(Container&& x) : c(eastl::move(x)) {
    // Empty
  }
#endif
  ...
  inline stack<T, Container>::stack(std::initializer_list<value_type> ilist)
      : c()  // We could alternatively use c(ilist) here, but that would require c to have an ilist
             // constructor.
  {
    ...
    for (typename std::initializer_list<value_type>::iterator it = ilist.begin(); it != ilist.end();
         ++it) {
  ...
  template <typename T, typename Container>
  inline typename stack<T, Container>::container_type& stack<T, Container>::get_container() {
    return c;
  }

  template <typename T, typename Container>
  inline const typename stack<T, Container>::container_type& stack<T, Container>::get_container()
      const {
    return c;
  }
  ...
  template <typename T, typename Container>
  inline void swap(stack<T, Container>& a, stack<T, Container>& b) EA_NOEXCEPT_IF(
      (eastl::is_nothrow_swappable<typename stack<T, Container>::container_type>::value)) {
    a.swap(b);
  }
