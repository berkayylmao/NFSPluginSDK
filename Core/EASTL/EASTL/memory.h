...
#include <Core/EASTL/EASTL/internal/pair_fwd_decls.h>
#include <Core/EASTL/EASTL/internal/functional_base.h>
#include <Core/EASTL/EASTL/internal/allocator_traits_fwd_decls.h>
...
#include <Core/EASTL/EASTL/numeric_limits.h>
...
EA_DISABLE_SN_WARNING(
    828);  // The EDG SN compiler has a bug in its handling of variadic template arguments and
...
#define EASTL_TEMP_DEFAULT_NAME \
  ...
  ///
  template <typename T>
  eastl::pair<T*, ptrdiff_t> get_temporary_buffer(ptrdiff_t n, size_t alignment = 1,
    ...
    return eastl::pair<T*, ptrdiff_t>(
        static_cast<T*>(EASTLAllocAligned(allocator, n * sizeof(T), alignment, alignmentOffset)),
        n);
  }
  ...
  class late_constructed {
   public:
    ...
    typedef
        typename eastl::aligned_storage<sizeof(value_type),
    ...
    void construct(A1&& a1, A2&& a2, A3&& a3) {
      if (!mpValue)
        mpValue = new (&mStorage)
            value_type(eastl::forward<A1>(a1), eastl::forward<A2>(a2), eastl::forward<A3>(a3));
    }
#endif
  ...
  namespace Internal {
    ...
    struct uninitialized_relocate_impl<true, EASTL_ITC_NS::random_access_iterator_tag> {
      ...
      static T* do_move_start(T* first, T* last, T* dest) {
        return (T*)memcpy(dest, first, (size_t)((uintptr_t)last - (uintptr_t)first)) +
               (last - first);
      }
  ...
  ///
  template <typename ForwardIterator, typename ForwardIteratorDest>
  inline ForwardIteratorDest uninitialized_relocate_start(ForwardIterator     first,
    ...
    return Internal::uninitialized_relocate_impl<bHasTrivialMove, IC>::do_move_start(first, last,
                                                                                     dest);
  }

  template <typename ForwardIterator, typename ForwardIteratorDest>
  inline ForwardIteratorDest uninitialized_relocate_commit(ForwardIterator     first,
                                                           ForwardIterator     last,
                                                           ForwardIteratorDest dest) {
    typedef typename eastl::iterator_traits<ForwardIterator>::iterator_category IC;
    typedef typename eastl::iterator_traits<ForwardIterator>::value_type        value_type_input;
    ...
    return Internal::uninitialized_relocate_impl<bHasTrivialMove, IC>::do_move_commit(first, last,
                                                                                      dest);
  }

  template <typename ForwardIterator, typename ForwardIteratorDest>
  inline ForwardIteratorDest uninitialized_relocate_abort(ForwardIterator     first,
                                                          ForwardIterator     last,
                                                          ForwardIteratorDest dest) {
    typedef typename eastl::iterator_traits<ForwardIterator>::iterator_category IC;
    typedef typename eastl::iterator_traits<ForwardIterator>::value_type        value_type_input;
    ...
    return Internal::uninitialized_relocate_impl<bHasTrivialMove, IC>::do_move_abort(first, last,
                                                                                     dest);
  }
  ...
  inline ForwardIterator uninitialized_copy_n(InputIterator first, Count n, ForwardIterator dest) {
    ...
    return Internal::uninitialized_copy_n_impl<InputIterator, Count, ForwardIterator, IC>::impl(
        first, n, dest);
  }
...
///
  ...
  namespace Internal {
    template <typename InputIterator, typename ForwardIterator>
    inline ForwardIterator uninitialized_move_impl(InputIterator first, InputIterator last,
                                                   ForwardIterator dest, true_type) {
...
// We must run a loop over every element and move-construct it at the new location.
      ...
      try {
        for (; first != last; ++first, ++currentDest)
          ::new ((void*)&*currentDest) value_type(eastl::move(
  ...
  inline Result uninitialized_move_ptr(First first, Last last, Result dest) {
    ...
    const generic_iterator<Result, void> i(Internal::uninitialized_move_impl(
        ...
        eastl::is_trivially_copy_assignable<
            value_type>()));  // is_trivially_copy_assignable identifies if copy assignment would be
                              // as valid as move assignment, which means we have the opportunity to
                              // memcpy/memmove optimization.
    return i.base();
  }
#else
  template <typename First, typename Last, typename Result>
  ...
  ///
  template <typename InputIterator, typename ForwardIterator>
  inline ForwardIterator uninitialized_move_if_noexcept(InputIterator first, InputIterator last,
                                                        ForwardIterator dest) {
#if EASTL_MOVE_SEMANTICS_ENABLED
  ...
  inline void uninitialized_fill(ForwardIterator first, ForwardIterator last, const T& value) {
    typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
    Internal::uninitialized_fill_impl(first, last, value,
                                      eastl::is_trivially_copy_assignable<value_type>());
  }
  ...
  inline void uninitialized_fill_ptr(T* first, T* last, const T& value) {
    typedef
        typename eastl::iterator_traits<eastl::generic_iterator<T*, void> >::value_type value_type;
  ...
  inline void uninitialized_fill_n(ForwardIterator first, Count n, const T& value) {
    typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
    Internal::uninitialized_fill_n_impl(first, n, value,
                                        eastl::is_trivially_copy_assignable<value_type>());
  }
  ...
  inline ForwardIterator uninitialized_fill_copy(ForwardIterator result, ForwardIterator mid,
                                                 const T& value, InputIterator first,
                                                 InputIterator last) {
  ...
  ///
  template <typename ForwardIterator, typename T, typename InputIterator>
  inline ForwardIterator uninitialized_fill_move(ForwardIterator result, ForwardIterator mid,
                                                 const T& value, InputIterator first,
                                                 InputIterator last) {
    eastl::uninitialized_fill(result, mid, value);
  ...
  inline void* align_advance(size_t alignment, size_t size, void* ptr, size_t space,
  ...
  struct uses_allocator_impl
      : public integral_constant<
            bool, eastl::is_convertible<Allocator, typename T::allocator_type>::value> {};
  ...
  struct pointer_traits<T*> {
   ...
   public:
    static pointer pointer_to(typename eastl::conditional<eastl::is_void<element_type>::value, void,
...
#include <Core/EASTL/EASTL/internal/allocator_traits.h>
