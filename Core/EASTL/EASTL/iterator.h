...
namespace eastl {
  ...
  enum iterator_status_flag {
    ...
    isf_current =
        0x02,  /// The iterator is valid and points to the same element it did when created. For
...
// we define here. These categories come directly from the C++ standard.
  ...
  inline bool operator==(const reverse_iterator<Iterator1>& a,
  ...
  inline typename reverse_iterator<Iterator1>::difference_type operator-(
      const reverse_iterator<Iterator1>& a, const reverse_iterator<Iterator2>& b) {
    return b.base() - a.base();
  }

  template <typename Iterator>
  inline reverse_iterator<Iterator> operator+(
      typename reverse_iterator<Iterator>::difference_type n, const reverse_iterator<Iterator>& a) {
...
// condition.
  ...
  template <typename Iterator>
  inline typename Internal::mminei_helper<Iterator>::ReturnType make_move_if_noexcept_iterator(
      Iterator i) {
    return Internal::mminei_helper_1<Iterator,
                                     Internal::mminei_helper<Iterator>::is_noexcept>::mminei(i);
  }
#else
  template <typename Iterator,
  ...
  template <typename InputIterator>
  inline InputIterator prev(InputIterator                                                   it,
                            typename eastl::iterator_traits<InputIterator>::difference_type n = 1) {
    eastl::advance(it, -n);
    return it;
  }
