...
#include <Core/EASTL/EASTL/list.h>
...
#define EASTL_FIXED_LIST_DEFAULT_ALLOCATOR overflow_allocator_type(EASTL_FIXED_LIST_DEFAULT_NAME)
  ...
  class fixed_list
      : public list<
            T, fixed_node_allocator<sizeof(typename list<T>::node_type), nodeCount,
                                    EASTL_ALIGN_OF(T), 0, bEnableOverflow, OverflowAllocator> > {
   public:
    typedef fixed_node_allocator<sizeof(typename list<T>::node_type), nodeCount, EASTL_ALIGN_OF(T),
   ...
   protected:
    char mBuffer[fixed_allocator_type::kBufferSize];  // kBufferSize will take into account
   ...
   public:
    fixed_list();
    explicit fixed_list(const overflow_allocator_type&
                            overflowAllocator);  // Only applicable if bEnableOverflow is true.
    explicit fixed_list(size_type n);  // Currently we don't support overflowAllocator specification
    ...
    fixed_list(
        ...
        const overflow_allocator_type&    overflowAllocator = EASTL_FIXED_LIST_DEFAULT_ALLOCATOR);
...
// Deprecated:
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline fixed_list<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_list(
      std::initializer_list<value_type> ilist, const overflow_allocator_type& overflowAllocator)
      : base_type(fixed_allocator_type(mBuffer, overflowAllocator)) {
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  template <typename InputIterator>
  fixed_list<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_list(InputIterator first,
                                                                           InputIterator last)
      : base_type(fixed_allocator_type(mBuffer)) {
#if EASTL_NAME_ENABLED
    mAllocator.set_name(EASTL_FIXED_LIST_DEFAULT_NAME);
  ...
  inline bool fixed_list<T, nodeCount, bEnableOverflow, OverflowAllocator>::full() const {
    // Note: This implementation isn't right in the case of bEnableOverflow = true because it will
    // return false for the case that  there are free nodes from the buffer but also nodes from the
    // dynamic heap. This can happen if the container exceeds the fixed size and then frees some of
    // the nodes from the fixed buffer. The only simple fix for this is to take on another member
    // variable which tracks whether this overflow has occurred at some point in the past.
    return !mAllocator
                .can_allocate();  // This is the quickest way of detecting this. has_overflowed uses
                                  // a different method because it can't use this quick method.
  }

  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline bool fixed_list<T, nodeCount, bEnableOverflow, OverflowAllocator>::has_overflowed() const {
#if EASTL_FIXED_SIZE_TRACKING_ENABLED  // If we can use this faster pathway (as size() may be
                                       // slow)...
  ...
  fixed_list<T, nodeCount, bEnableOverflow, OverflowAllocator>::get_overflow_allocator() const
      EA_NOEXCEPT {
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline void fixed_list<T, nodeCount, bEnableOverflow, OverflowAllocator>::set_overflow_allocator(
