...
/// EASTL_MACRO_SWAP
///
/// Use EASTL_MACRO_SWAP because GCC (at least v4.6-4.8) has a bug where it fails to compile
...
#define EASTL_MACRO_SWAP(Type, a, b) \
  ...
  EA_DISABLE_VC_WARNING(4625)  // disable warning: "copy constructor could not be generated because
  ...
  struct hash_node<Value, true> {
    ...
    hash_node* mpNext;
    eastl_size_t
        mnHashCode;  // See config.h for the definition of eastl_size_t, which defaults to uint32_t.
  } EASTL_MAY_ALIAS;
  ...
  struct hashtable_iterator_base {
   ...
   public:
    ...
    void increment() {
      ...
      while (mpNode == NULL) mpNode = *++mpBucket;
    }
  ...
  inline typename eastl::iterator_traits<Iterator>::difference_type distance_fw_impl(
      Iterator /*first*/, Iterator /*last*/, EASTL_ITC_NS::input_iterator_tag) {
    return 0;
  }

  template <typename Iterator>
  inline typename eastl::iterator_traits<Iterator>::difference_type distance_fw_impl(
      Iterator first, Iterator last, EASTL_ITC_NS::forward_iterator_tag) {
    return eastl::distance(first, last);
  }

  template <typename Iterator>
  inline typename eastl::iterator_traits<Iterator>::difference_type ht_distance(Iterator first,
                                                                                Iterator last) {
  ...
  struct EASTL_API prime_rehash_policy {
   ...
   public:
    ...
    eastl::pair<bool, uint32_t> GetRehashRequired(uint32_t nBucketCount, uint32_t nElementCount,
  ...
  ///
  template <typename Key, typename Value, typename ExtractKey, typename Equal, typename H1,
            typename H2, typename H, bool bCacheHashCode>
  ...
  struct hash_code_base<Key, Value, ExtractKey, Equal, H1, H2, default_ranged_hash, false> {
   ...
   protected:
    ...
    hash_code_base(const ExtractKey& ex, const Equal& eq, const H1& h1, const H2& h2,
  ...
  ///
  template <typename Key, typename Value, typename Allocator, typename ExtractKey, typename Equal,
            typename H1, typename H2, typename H, typename RehashPolicy, bool bCacheHashCode,
            bool bMutableIterators, bool bUniqueKeys>
  class hashtable
      : public rehash_base<
            RehashPolicy, hashtable<Key, Value, Allocator, ExtractKey, Equal, H1, H2, H,
   ...
   public:
    ...
    typedef hash_code_base<Key, Value, ExtractKey, Equal, H1, H2, H, bCacheHashCode>
                                                      hash_code_base_type;
    ...
    typedef typename type_select<bUniqueKeys, eastl::pair<iterator, bool>, iterator>::type
        insert_return_type;
    typedef hashtable<Key, Value, Allocator, ExtractKey, Equal, H1, H2, H, RehashPolicy,
                      bCacheHashCode, bMutableIterators, bUniqueKeys>
                                                 this_type;
    ...
    enum {
      ...
      kAllocFlagBuckets =
          eastl::kHashtableAllocFlagBuckets  // Flag to allocator which indicates that we are
   ...
   public:
    hashtable(size_type nBucketCount, const H1&, const H2&, const H&, const Equal&,
              const ExtractKey&,
              const allocator_type& allocator = EASTL_HASHTABLE_DEFAULT_ALLOCATOR);

    template <typename FowardIterator>
    hashtable(
        FowardIterator first, FowardIterator last, size_type nBucketCount, const H1&, const H2&,
        const H&, const Equal&, const ExtractKey&,
        const allocator_type& allocator =
            EASTL_HASHTABLE_DEFAULT_ALLOCATOR);  // allocator arg removed because VC7.1 fails on the
                                                 // default arg. To do: Make a second version of
                                                 // this function without a default arg.

    hashtable(const hashtable& x);

    // initializer_list ctor support is implemented in subclasses (e.g. hash_set).
    // hashtable(initializer_list<value_type>, size_type nBucketCount, const H1&, const H2&, const
    // H&,
   ...
   public:
    ...
    // of a thread-shared hash table that's accessed during a mutex lock, because the hash
    // calculation and node creation is done outside of the lock. If pNodeNew is supplied by the
    // user (i.e. non-NULL) then it must be freeable via the hash table's allocator. If the return
    // value is true then this function took over ownership of pNodeNew, else pNodeNew is still
    // owned by the caller to free or to pass to another call to insert. pNodeNew need not be
    // assigned the value by the caller, as the insert function will assign value to pNodeNew upon
    // insertion into the hash table. pNodeNew may be created by the user with the
    // allocate_uninitialized_node function, and freed by the free_uninitialized_node function.
    insert_return_type insert(hash_code_t c, node_type* pNodeNew, const value_type& value);

    // Used to allocate and free memory used by insert(const value_type& value, hash_code_t c,
    // node_type* pNodeNew).
   ...
   public:
    ...
    void clear();
    void clear(bool clearBuckets);  // If clearBuckets is true, we free the bucket memory and set
                                    // the bucket count back to the newly constructed count.
   ...
   public:
    ...
    /// It returns an iterator to the first element that matches the given hash. However, there may
    /// be multiple elements that match the given hash.
...
#if EASTL_MOVE_SEMANTICS_ENABLED
    eastl::pair<iterator, bool> DoInsertValueExtra(true_type, const key_type& k, hash_code_t c,
                                                   node_type* pNodeNew, value_type&& value);
    eastl::pair<iterator, bool> DoInsertValue(true_type, value_type&& value);
    iterator   DoInsertValueExtra(false_type, const key_type& k, hash_code_t c, node_type* pNodeNew,
                                  value_type&& value);
  ...
  template <typename K, typename V, typename A, typename EK, typename Eq, typename H1, typename H2,
            typename H, typename RP, bool bC, bool bM, bool bU>
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::hashtable(size_type nBucketCount,
                                                                   const H1& h1, const H2& h2,
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::hashtable(
      FowardIterator first, FowardIterator last, size_type nBucketCount, const H1& h1, const H2& h2,
      const H& h, const Eq& eq, const EK& ek, const allocator_type& allocator)
      : rehash_base<rehash_policy_type, hashtable>(),
        hash_code_base<key_type, value_type, extract_key_type, key_equal, h1_type, h2_type, h_type,
                       kCacheHashCode>(ek, eq, h1, h2, h),
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::hashtable(this_type&& x)
      : rehash_base<RP, hashtable>(x),
        ...
        mRehashPolicy(x.mRehashPolicy),
        mAllocator(x.mAllocator) {
    reset_lose_memory();  // We do this here the same as we do it in the default ctor because it
                          // puts the container in a proper initial empty state. This code would be
                          // cleaner if we could rely on being able to use C++11 delegating
                          // constructors and just call the default ctor here.
    swap(x);
  }
  ...
  template <typename K, typename V, typename A, typename EK, typename Eq, typename H1, typename H2,
            typename H, typename RP, bool bC, bool bM, bool bU>
  inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::set_allocator(
      const allocator_type& allocator) {
    mAllocator = allocator;
  }
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoAllocateNodeFromKey(
    ...
    node_type* const pNode =
        (node_type*)allocate_memory(mAllocator, sizeof(node_type), EASTL_ALIGN_OF(value_type), 0);
  ...
  template <typename K, typename V, typename A, typename EK, typename Eq, typename H1, typename H2,
            typename H, typename RP, bool bC, bool bM, bool bU>
  void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoFreeNodes(node_type** pNodeArray,
                                                                          size_type   n) {
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoAllocateBuckets(size_type n) {
    ...
    EASTL_CT_ASSERT(kHashtableAllocFlagBuckets ==
                    0x00400000);  // Currently we expect this to be so, because the allocator has a
                                  // copy of this enum.
  ...
  template <typename K, typename V, typename A, typename EK, typename Eq, typename H1, typename H2,
            typename H, typename RP, bool bC, bool bM, bool bU>
  inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoFreeBuckets(
      node_type** pBucketArray, size_type n) {
    // If n <= 1, then pBucketArray is from the shared gpEmptyBucketArray. We don't test
    // for pBucketArray == &gpEmptyBucketArray because one library have a different
    // gpEmptyBucketArray than another but pass a hashtable to another. So we go by the size.
    if (n > 1)
      EASTLFree(
          mAllocator, pBucketArray,
          (n + 1) * sizeof(node_type*));  // '+1' because DoAllocateBuckets allocates nBucketCount +
                                          // 1 buckets in order to have a NULL sentinel at the end.
  }
  ...
  void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::swap(this_type& x) {
    if (mAllocator == x.mAllocator)  // If allocators are equivalent...
    {
      ...
      EASTL_MACRO_SWAP(
          node_type**, mpBucketArray,
          x.mpBucketArray);  // Use EASTL_MACRO_SWAP because GCC (at least v4.6-4.8) has a bug where
                             // it fails to compile eastl::swap(mpBucketArray, x.mpBucketArray).
  ...
  inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::rehash_policy(
    ...
    if (nBuckets > mnBucketCount) DoRehash(nBuckets);
  }
  ...
  template <typename U, typename UHash, typename BinaryPredicate>
  inline typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::find_as(const U& other, UHash uhash,
                                                                 BinaryPredicate predicate) {
    const hash_code_t c = (hash_code_t)uhash(other);
    const size_type   n =
        (size_type)(c % mnBucketCount);  // This assumes we are using the mod range policy.
  ...
  template <typename K, typename V, typename A, typename EK, typename Eq, typename H1, typename H2,
            typename H, typename RP, bool bC, bool bM, bool bU>
  inline typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::find_by_hash(const key_type& k,
                                                                      hash_code_t     c) {
    const size_type n = (size_type)bucket_index(c, (uint32_t)mnBucketCount);
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoFindNode(node_type*      pNode,
    ...
    for (; pNode; pNode = pNode->mpNext) {
      if (compare(k, c, pNode)) return pNode;
    }
    return NULL;
  }
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoFindNode(node_type*  pNode,
                                                                    hash_code_t c) const {
    for (; pNode; pNode = pNode->mpNext) {
      if (pNode->mnHashCode == c) return pNode;
    }
    return NULL;
  }
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(
      true_type, Args&&... args)  // true_type means bUniqueKeys is true.
  {
    ...
    // We have a chicken-and-egg problem here. In order to know if and where to insert the value, we
    // need to get the hashtable key for the value. But we don't explicitly have a value argument,
    // we have a templated Args&&... argument. We need the value_type in order to proceed, but that
    // entails getting an instance of a value_type from the args. And it may turn out that the value
    // is already present in the hashtable and we need to cancel the insertion, despite having
    // obtained a value_type to put into the hashtable. We have mitigated this problem somewhat by
    // providing specializations of the insert function for const value_type& and value_type&&, and
    // so the only time this function should get called is when args refers to arguments to
    // construct a value_type.
    ...
    if (pNode == NULL)  // If value is not present... add it.
    {
      ...
      // To do: We have an inefficiency to deal with here. We allocated a node above but we are
      // freeing it here because it turned out it wasn't needed. But we needed to create the node in
      // order to get the hashtable key for the node. One possible resolution is to create
      // specializations: DoInsertValue(true_type, value_type&&) and DoInsertValue(true_type, const
      // value_type&) which don't need to create a node up front in order to get the hashtable key.
      // Probably most users would end up using these pathways instead of this Args... pathway.
      // While we should considering handling this to-do item, a lot of the performance limitations
      // of maps and sets in practice is with finding elements rather than adding (potentially
      // redundant) new elements.
      DoFreeNode(pNodeNew);
    }
  ...
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(
      false_type, Args&&... args)  // false_type means bUniqueKeys is false.
  {
    const eastl::pair<bool, uint32_t> bRehash = mRehashPolicy.GetRehashRequired(
        (uint32_t)mnBucketCount, (uint32_t)mnElementCount, (uint32_t)1);

    if (bRehash.first) DoRehash(bRehash.second);
  ...
  // but are for value_type&& and const value_type& arguments. It's useful for us to have the
  // functions below, even when using a fully compliant C++11 compiler that supports the above
  // functions. The reason is because the specializations below are slightly more efficient because
  // they can delay the creation of a node until it's known that it will be needed.
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValueExtra(
      true_type, const key_type& k, hash_code_t c, node_type* pNodeNew,
      value_type&& value)  // true_type means bUniqueKeys is true.
  {
    // Adds the value to the hash table if not already present.
    // If already present then the existing value is returned via an iterator/bool pair.
    size_type        n     = (size_type)bucket_index(k, c, (uint32_t)mnBucketCount);
...
// do a rehash if the allocation throws.
#if EASTL_EXCEPTIONS_ENABLED
      bool nodeAllocated;  // If exceptions are enabled then we we need to track if we allocated the
                           // node so we can free it in the catch block.
#endif

      if (pNodeNew) {
        ::new ((void*)&pNodeNew->mValue) value_type(eastl::move(
      ...
      try {
        ...
        if (nodeAllocated)  // If we allocated the node within this function, free it. Else let the
                            // caller retain ownership of it.
          DoFreeNode(pNodeNew);
        throw;
      }
#endif
  ...
  eastl::pair<typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator, bool>
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(
      true_type, value_type&& value)  // true_type means bUniqueKeys is true.
  {
  ...
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValueExtra(
      false_type, const key_type& k, hash_code_t c, node_type* pNodeNew,
      value_type&& value)  // false_type means bUniqueKeys is false.
  {
    const eastl::pair<bool, uint32_t> bRehash = mRehashPolicy.GetRehashRequired(
        (uint32_t)mnBucketCount, (uint32_t)mnElementCount, (uint32_t)1);

    if (bRehash.first)
      DoRehash(bRehash.second);  // Note: We don't need to wrap this call with try/catch because
                                 // there's nothing we would need to do in the catch.
  ...
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(
      false_type, value_type&& value)  // false_type means bUniqueKeys is false.
  {
    const key_type&   k = mExtractKey(value);
    const hash_code_t c = get_hash_code(k);
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValueExtra(
      true_type, const key_type& k, hash_code_t c, node_type* pNodeNew,
      const value_type& value)  // true_type means bUniqueKeys is true.
  {
    // Adds the value to the hash table if not already present.
    // If already present then the existing value is returned via an iterator/bool pair.
    size_type        n     = (size_type)bucket_index(k, c, (uint32_t)mnBucketCount);
...
// do a rehash if the allocation throws.
      ...
      if (pNodeNew) {
        ::new ((void*)&pNodeNew->mValue) value_type(
            value);  // It's expected that pNodeNew was allocated with allocate_uninitialized_node.
#if EASTL_EXCEPTIONS_ENABLED
        nodeAllocated = false;
#endif
  ...
  eastl::pair<typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator, bool>
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(
      true_type, const value_type& value)  // true_type means bUniqueKeys is true.
  {
    const key_type&   k = mExtractKey(value);
    const hash_code_t c = get_hash_code(k);
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValueExtra(
      false_type, const key_type& k, hash_code_t c, node_type* pNodeNew,
      const value_type& value)  // false_type means bUniqueKeys is false.
  {
    const eastl::pair<bool, uint32_t> bRehash = mRehashPolicy.GetRehashRequired(
        (uint32_t)mnBucketCount, (uint32_t)mnElementCount, (uint32_t)1);
  ...
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(
      false_type, const value_type& value)  // true_type means bUniqueKeys is true.
  {
    const key_type&   k = mExtractKey(value);
    const hash_code_t c = get_hash_code(k);
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::allocate_uninitialized_node() {
    // We don't wrap this in try/catch because users of this function are expected to do that
  ...
  template <typename K, typename V, typename A, typename EK, typename Eq, typename H1, typename H2,
            typename H, typename RP, bool bC, bool bM, bool bU>
  void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::free_uninitialized_node(
      node_type* pNode) {
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertKey(
      true_type, const key_type& key)  // true_type means bUniqueKeys is true.
  {
  ...
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertKey(
      false_type, const key_type& key)  // false_type means bUniqueKeys is false.
  {
    const eastl::pair<bool, uint32_t> bRehash = mRehashPolicy.GetRehashRequired(
        (uint32_t)mnBucketCount, (uint32_t)mnElementCount, (uint32_t)1);
  ...
  template <class... Args>
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::emplace_hint(const_iterator,
                                                                      Args&&... args) {
  ...
  template <typename K, typename V, typename A, typename EK, typename Eq, typename H1, typename H2,
            typename H, typename RP, bool bC, bool bM, bool bU>
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::emplace_hint(const_iterator position,
                                                                      value_type&& value) {
    // We currently ignore the iterator argument as a hint.
  ...
  template <class P>
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert_return_type
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert(hash_code_t c, node_type* pNodeNew,
                                                                P&& otherValue) {
    // pNodeNew->mValue is expected to be uninitialized.
    value_type value(
        eastl::forward<P>(otherValue));  // Need to use forward instead of move because P&& is a
                                         // "universal reference" instead of an rvalue reference.
    const key_type& k = mExtractKey(value);
  ...
  template <typename K, typename V, typename A, typename EK, typename Eq, typename H1, typename H2,
            typename H, typename RP, bool bC, bool bM, bool bU>
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert(const_iterator,
                                                                value_type&& value) {
    // We currently ignore the iterator argument as a hint.
    insert_return_type result =
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert(const_iterator,
                                                                const value_type& value) {
    // We ignore the first argument (hint iterator). It's not likely to be useful for hashtable
    // containers.
    insert_return_type result = DoInsertValue(has_unique_keys_type(), value);
    return result.first;  // Note by Paul Pedriana while perusing this code: This code will fail to
                          // compile when bU is false (i.e. for multiset, multimap).
  }
  ...
  template <typename K, typename V, typename A, typename EK, typename Eq, typename H1, typename H2,
            typename H, typename RP, bool bC, bool bM, bool bU>
  template <typename InputIterator>
  void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert(InputIterator first,
                                                                     InputIterator last) {
    ...
    for (; first != last; ++first) DoInsertValue(has_unique_keys_type(), *first);
  }
  ...
  template <typename K, typename V, typename A, typename EK, typename Eq, typename H1, typename H2,
            typename H, typename RP, bool bC, bool bM, bool bU>
  inline typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::erase(const_iterator first,
                                                               const_iterator last) {
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::erase(const key_type& k) {
    ...
    while (*pBucketArray && !compare(k, c, *pBucketArray)) pBucketArray = &(*pBucketArray)->mpNext;
  ...
  inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::reset_lose_memory()
    ...
    memcpy(&mpBucketArray, &p, sizeof(mpBucketArray));  // Other compilers implement strict aliasing
                                                        // and casting is thus unsafe.
#endif
  ...
  void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoRehash(size_type nNewBucketCount) {
    node_type** const pBucketArray =
        DoAllocateBuckets(nNewBucketCount);  // nNewBucketCount should always be >= 2.
  ...
  inline bool hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::validate() const {
    ...
    if (gpEmptyBucketArray[0] != NULL) return false;

    if (gpEmptyBucketArray[1] != (void*)uintptr_t(~0)) return false;

    // Verify that we have at least one bucket. Calculations can
    // trigger division by zero exceptions otherwise.
    if (mnBucketCount == 0) return false;
    ...
    for (const_iterator temp = begin(), tempEnd = end(); temp != tempEnd; ++temp) ++nElementCount;

    if (nElementCount != mnElementCount) return false;
