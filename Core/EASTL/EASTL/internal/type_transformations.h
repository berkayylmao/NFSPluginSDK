...
#define EASTL_TYPE_TRAIT_add_const_CONFORMANCE 1  // add_const is conforming.
  ...
  template <typename T, bool = eastl::is_const<T>::value || eastl::is_reference<T>::value ||
...
#define EASTL_TYPE_TRAIT_add_volatile_CONFORMANCE 1  // add_volatile is conforming.
  ...
  template <typename T, bool = eastl::is_volatile<T>::value || eastl::is_reference<T>::value ||
                               eastl::is_function<T>::value>
...
// To do: This implementation needs to be updated to support C++11 conformance (recognition of
// enums) and to support volatile-qualified types. It will probably be useful to have it fail for
// unsupported types.
...
#if (defined(CHAR_MAX) && defined(UCHAR_MAX) && \
     (CHAR_MAX ==                               \
      UCHAR_MAX))  // If char is unsigned, we convert char to signed char. However, if char is
                   // signed then make_signed returns char itself and not signed char.
  template <>
...
#ifndef EA_WCHAR_T_NON_NATIVE  // If wchar_t is a native type instead of simply a define to an
                               // existing type...
#if (defined(__WCHAR_MAX__) && \
     (__WCHAR_MAX__ == 4294967295U))  // If wchar_t is a 32 bit unsigned value...
  template <>
...
#define EASTL_TYPE_TRAIT_make_unsigned_CONFORMANCE 0  // make_unsigned is only partially conforming.
...
#if (CHAR_MIN < 0)  // If char is signed, we convert char to unsigned char. However, if char is
                    // unsigned then make_unsigned returns char itself and not unsigned char.
  template <>
  ...
  struct make_unsigned<const char> {
    typedef unsigned char type;
                               ...
                               // existing type...
#if (defined(__WCHAR_MAX__) && \
     (__WCHAR_MAX__ != 4294967295U))  // If wchar_t is a 32 bit signed value...
  template <>
...
#define EASTL_TYPE_TRAIT_add_pointer_CONFORMANCE 1
  ...
  // For example, given a multi-dimensional array type T[M][N], remove_extent<T[M][N]>::type is
  // equivalent to T[N].
  ///////////////////////////////////////////////////////////////////////
...
#define EASTL_TYPE_TRAIT_remove_extent_CONFORMANCE 1  // remove_extent is conforming.
  ...
  // For a given array type const T[N], remove_all_extents<const T[N]>::type is equivalent to const
  // T. For example, given a multi-dimensional array type T[M][N], remove_all_extents<T[M][N]>::type
  // is equivalent to T.
  ///////////////////////////////////////////////////////////////////////
...
#elif defined(EA_COMPILER_MSVC) || (defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION < 4007)) || \
    defined(EA_COMPILER_EDG)  // At some point GCC fixed their attribute(align) to support
                              // non-literals, though it's not clear what version aside from being
                              // no later than 4.7 and no earlier than 4.2.
...
#define EASTL_TYPE_TRAIT_aligned_union_CONFORMANCE \
  ...
  // To consider: Expand this to include more possible types. We may want to convert this to be a
  // recursive
  //              template instead of like below.
  template <size_t minSize, typename Type0, typename Type1 = char, typename Type2 = char,
            typename Type3 = char>
  ...
  DestType union_cast(SourceType sourceValue) {
    EASTL_CT_ASSERT(
        ...
        (EA_ALIGN_OF(DestType) ==
         EA_ALIGN_OF(SourceType)));  // To support differening alignments, we would need to use a
                                     // memcpy-based solution or find a way to make the two union
                                     // members align with each other.
    // EASTL_CT_ASSERT(is_pod<DestType>::value && is_pod<SourceType>::value);           // Disabled
    // because we don't want to restrict what the user can do, as some compiler's definitions of
    // is_pod aren't up to C++11 Standards. EASTL_CT_ASSERT(!is_pointer<DestType>::value &&
    // !is_pointer<SourceType>::value); // Disabled because it's valid to alias pointers as long as
    // you are aliasong the pointer value and not what it points to.
