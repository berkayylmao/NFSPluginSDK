...
#define EASTL_TYPE_TRAIT_is_void_CONFORMANCE 1  // is_void is conforming.
  ...
  template <typename A0, typename A1 = int, typename A2 = int, typename A3 = int, typename A4 = int,
            typename A5 = int>
  struct has_void_arg {
    static const bool value =
        eastl::is_void<A0>::value ||  // If we add more arguments then change this
        eastl::is_void<A1>::value ||  // to be a recursive template.
        eastl::is_void<A2>::value || eastl::is_void<A3>::value || eastl::is_void<A4>::value ||
...
#define EASTL_TYPE_TRAIT_is_null_pointer_CONFORMANCE 1
  ...
  struct is_null_pointer
      : public eastl::is_same<typename eastl::remove_cv<T>::type, decltype(nullptr)> {
  };  // A C++11 compiler defines nullptr, but you need a C++11 standard library to declare
      // std::nullptr_t. So it's safer to compare against decltype(nullptr) than to use
      // std::nullptr_t, because we may have a C++11 compiler but C++98 library (happens with Apple
      // frequently).
#else
#define EASTL_TYPE_TRAIT_is_null_pointer_CONFORMANCE 1
  ...
  template <typename T>
  struct is_null_pointer
      : public eastl::is_same<typename eastl::remove_cv<T>::type, std::nullptr_t> {};
#endif
...
#define EASTL_TYPE_TRAIT_is_integral_CONFORMANCE 1  // is_integral is conforming.
  ...
  struct is_integral_helper<char> : public true_type {};
                               ...
                               // existing type which is already handled above...
  template <>
...
#define EASTL_TYPE_TRAIT_is_floating_point_CONFORMANCE 1  // is_floating_point is conforming.
  ...
  struct is_floating_point
      : public eastl::is_floating_point_helper<typename eastl::remove_cv<T>::type> {};
...
#define EASTL_TYPE_TRAIT_is_fundamental_CONFORMANCE 1  // is_fundamental is conforming.
  ...
  struct is_fundamental
      : public integral_constant<bool, is_void<T>::value || is_integral<T>::value ||
