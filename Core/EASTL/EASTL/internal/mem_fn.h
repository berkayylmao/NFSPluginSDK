...
namespace eastl {
  ...
  template <class R, class C>
  struct weak_result_type<R (C::*)() volatile> : public unary_function<volatile C*, R> {};
  template <class R, class C>
  struct weak_result_type<R (C::*)() const volatile> : public unary_function<const volatile C*, R> {
  };
  ...
  template <class R, class C, class A1>
  struct weak_result_type<R (C::*)(A1) const> : public binary_function<const C*, A1, R> {};
  template <class R, class C, class A1>
  struct weak_result_type<R (C::*)(A1) volatile> : public binary_function<volatile C*, A1, R> {};
  template <class R, class C, class A1>
  ...
  struct invoke_return1 {
    typedef decltype(
        invoke_impl(eastl::declval<T>(), eastl::declval<A0>(), eastl::declval<A1>())) type;
  };

  template <class T, class A0, class A1, class A2>
  struct invoke_return2 {
    typedef decltype(invoke_impl(eastl::declval<T>(), eastl::declval<A0>(), eastl::declval<A1>(),
                                 eastl::declval<A2>())) type;
  };
#endif
...
#if EASTL_VARIADIC_TEMPLATES_ENABLED
  template <class F, class... Args>
  typename invoke_return<F, Args...>::type invoke(F&& f, Args&&... args) {
    return invoke_impl(eastl::forward<F>(f), eastl::forward<Args>(args)...);
  }
#else
  template <class F, class A0>
  typename invoke_return0<F, A0>::type invoke(F&& f, A0&& a0) {
    return invoke_impl(eastl::forward<F>(f), eastl::forward<A0>(a0));
  }

  template <class F, class A0, class A1>
  typename invoke_return1<F, A0, A1>::type invoke(F&& f, A0&& a0, A1&& a1) {
    return invoke_impl(eastl::forward<F>(f), eastl::forward<A0>(a0), eastl::forward<A1>(a1));
  }

  template <class F, class A0, class A1, class A2>
  typename invoke_return2<F, A0, A1, A2>::type invoke(F&& f, A0&& a0, A1&& a1, A2&& a2) {
    return invoke_impl(eastl::forward<F>(f), eastl::forward<A0>(a0), eastl::forward<A1>(a1),
                       eastl::forward<A2>(a2));
  }
#endif
  ...
  class mem_fn_impl
      ...
      // Due to a (seemingly random) internal compiler error on VS2013 we disable
      // eastl::unary_function and binary_function support for eastl::mem_fn as its not widely (if
      // at all) used.  If you require this support on VS2013 or below please contact us.
  ...
  template <class R, class T, class A0, class A1, class A2>
  EASTL_FORCE_INLINE mem_fn_impl<R (T::*)(A0, A1, A2) volatile> mem_fn(R (T::*pm)(A0, A1,
                                                                                  A2) volatile) {
  ...
  template <class R, class T, class A0>
  EASTL_FORCE_INLINE mem_fn_impl<R (T::*)(A0) const volatile> mem_fn(R (T::*pm)(A0)
                                                                         const volatile) {
    return mem_fn_impl<R (T::*)(A0) const volatile>(pm);
  }

  template <class R, class T, class A0, class A1>
  EASTL_FORCE_INLINE mem_fn_impl<R (T::*)(A0, A1) const volatile> mem_fn(R (T::*pm)(A0, A1)
                                                                             const volatile) {
    return mem_fn_impl<R (T::*)(A0, A1) const volatile>(pm);
  }

  template <class R, class T, class A0, class A1, class A2>
  EASTL_FORCE_INLINE mem_fn_impl<R (T::*)(A0, A1, A2) const volatile> mem_fn(R (T::*pm)(A0, A1, A2)
                                                                                 const volatile) {
