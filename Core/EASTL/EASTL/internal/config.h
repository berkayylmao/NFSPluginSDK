...
//     printf("EASTL version: %d.%d.%d", EASTL_VERSION_N / 10000 % 100, EASTL_VERSION_N / 100 % 100,
//     EASTL_VERSION_N % 100);
//
///////////////////////////////////////////////////////////////////////////////
...
// of using EABase versions prior to the addition of its EA_COMPILER_NO_STANDARD_CPP_LIBRARY
// support.
//
...
//
//    struct EASTL_API SomeClass{           // Export SomeClass and its member functions in a DLL
//    build.
...
//    allocator_type(EASTL_NAME_VAL("xxx")));
//
///////////////////////////////////////////////////////////////////////////////
...
#define EASTL_DEBUG_BREAK() \
  __debugbreak()  // This is a compiler intrinsic which will map to appropriate inlined asm for the
                  // platform.
...
#if defined(EASTL_BITSET_WORD_SIZE)  // EASTL_BITSET_WORD_SIZE is deprecated, but we temporarily
                                     // support the ability for the user to specify it. Use
                                     // EASTL_BITSET_WORD_TYPE_DEFAULT instead.
...
//
// Defined as EASTL_ASSERT or defined away. Default is EASTL_ASSERT if EASTL_VALIDATION_ENABLED is
// enabled. This is used to validate user-supplied comparison functions, particularly for sorting
// purposes.
//
///////////////////////////////////////////////////////////////////////////////
...
/* Disabled because we don't currently need it.
        ...
        #pragma warning(push, 0)
                #include <yvals.h>
        #pragma warning(pop)
        #if ((defined(_HAS_TR1) && _HAS_TR1) || _MSC_VER >= 1700)  // VS2012 (1700) and later has
built-in type traits support. #define EASTL_STD_TYPE_TRAITS_AVAILABLE 1 #include <type_traits> #else
                #define EASTL_STD_TYPE_TRAITS_AVAILABLE 0
        #endif

#elif defined(EA_COMPILER_CLANG) || (defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4003) &&
!defined(__GCCXML__)) && !defined(EA_COMPILER_NO_STANDARD_CPP_LIBRARY) #include <cstddef> // This
will define __GLIBCXX__ if using GNU's libstdc++ and _LIBCPP_VERSION if using clang's libc++.

        #if defined(EA_COMPILER_CLANG) && !defined(EA_PLATFORM_APPLE) // As of v3.0.0, Apple's clang
doesn't support type traits.
                ...
                #elif defined(__GLIBCXX__) && (__GLIBCXX__ >= 20090124) // It's not clear if this is
the oldest version that has type traits; probably it isn't. #define EASTL_STD_TYPE_TRAITS_AVAILABLE
1

                        #if defined(__GXX_EXPERIMENTAL_CXX0X__) // To do: Update this test to
include conforming C++11 implementations. #include <type_traits> #else #include <tr1/type_traits>
                        #endif
                #else
                        #define EASTL_STD_TYPE_TRAITS_AVAILABLE 0
                #endif
...
#pragma warning(pop)
     ...
     _MSC_VER >= 1700)  // VS2012 (1700) and later has built-in type traits support.
...
//         enum : size_type { npos = container_type::npos };    // Microsoft extension which results
//         in significantly smaller debug symbols.
//     #else
...
#if EASTL_NO_RVALUE_REFERENCES  // If the compiler doesn't support rvalue references or EASTL is
...
#if defined(EA_COMPILER_NO_VARIADIC_TEMPLATES)  // If the compiler doesn't support variadic
                                                // templates
...
///////////////////////////////////////////////////////////////////////////////
    ...
    (__EDG_VERSION__ >= 403)  // It may in fact be supported by 4.01 or 4.02 but we don't have
...
// Workaround for this VS 2010 compiler bug:
// https://connect.microsoft.com/VisualStudio/feedback/details/682695
...
#ifndef EASTL_SIZE_T_32BIT  // Defines whether EASTL_SIZE_T uses uint32_t/int32_t as opposed to
                            // size_t/ssize_t.
...
typedef EASTL_SSIZE_T eastl_ssize_t;  // Signed version of eastl_size_t. Concept is similar to
                                      // Posix's ssize_t.
...
#ifndef EASTLAlloc  // To consider: Instead of calling through pAllocator, just go directly to
                    // operator new, since that's what allocator does.
#define EASTLAlloc(allocator, n) (allocator).allocate(n);
#endif

#ifndef EASTLAllocFlags  // To consider: Instead of calling through pAllocator, just go directly to
                         // operator new, since that's what allocator does.
...
#ifndef EASTLAllocatorDefault EASTLAllocatorDefault returns the default
                                      // allocator instance. This is not a global
...
/// EASTL_OPENSOURCE
/// This is enabled when EASTL is building built in an "open source" mode.  Which is a mode that
/// eliminates code dependencies on other technologies that have not been released publically.
