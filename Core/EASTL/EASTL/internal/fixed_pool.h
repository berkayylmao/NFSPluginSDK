...
#define EASTL_FIXED_POOL_DEFAULT_NAME \
  ...
  class EASTL_API fixed_pool : public fixed_pool_base {
   public:
    ...
    ///
    fixed_pool(void* pMemory, size_t memorySize, size_t nodeSize, size_t alignment,
               size_t alignmentOffset = 0) {
  ...
  class fixed_node_allocator {
   public:
    typedef typename type_select<bEnableOverflow, fixed_pool_with_overflow<OverflowAllocator>,
                                 fixed_pool>::type pool_type;
    typedef fixed_node_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset,
   ...
   public:
    ...
    void copy_overflow_allocator(
        const this_type& x)  // This function exists so we can write generic code that works for
  ...
  class fixed_node_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false,
                             OverflowAllocator> {
   public:
    typedef fixed_pool pool_type;
    typedef fixed_node_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false,
                                 OverflowAllocator>
                              this_type;
    typedef OverflowAllocator overflow_allocator_type;
   ...
   public:
    ...
    void copy_overflow_allocator(
        const this_type&)  // This function exists so we can write generic code that works for
                           // allocators that do and don't have overflow allocators.
    {
      // We don't have an overflow allocator.
    }
  ...
  ///     bEnableOverflow == true. Defaults to the global heap.
  ///
  template <size_t bucketCount, size_t nodeSize, size_t nodeCount, size_t nodeAlignment,
  ...
  class fixed_hashtable_allocator {
   public:
    typedef typename type_select<bEnableOverflow, fixed_pool_with_overflow<OverflowAllocator>,
                                 fixed_pool>::type pool_type;
    typedef fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount, nodeAlignment,
                                      nodeAlignmentOffset, bEnableOverflow, OverflowAllocator>
                              this_type;
    typedef OverflowAllocator overflow_allocator_type;

    enum {
      kBucketCount =
          bucketCount + 1,  // '+1' because the hash table needs a null terminating bucket.
   ...
   public:
    ...
    void* allocate(size_t n, int flags = 0) {
      ...
      EASTL_CT_ASSERT(kAllocFlagBuckets ==
                      0x00400000);  // Currently we expect this to be so, because the hashtable has
                                    // a copy of this enum.
  ...
  class fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount, nodeAlignment,
   ...
   public:
    ...
    ///
    fixed_hashtable_allocator(
        const this_type& x)  // No need to copy the overflow allocator, because bEnableOverflow is
  ...
  template <size_t bucketCount, size_t nodeSize, size_t nodeCount, size_t nodeAlignment,
            size_t nodeAlignmentOffset, bool bEnableOverflow, typename OverflowAllocator>
  inline bool operator==(
  ...
  class fixed_vector_allocator {
   public:
    typedef fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset,
                                   bEnableOverflow, OverflowAllocator>
                              this_type;
    typedef OverflowAllocator overflow_allocator_type;
  ...
  template <size_t nodeSize, size_t nodeCount, size_t nodeAlignment, size_t nodeAlignmentOffset,
            typename OverflowAllocator>
  class fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false,
                               OverflowAllocator> {
   public:
    typedef fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false,
                                   OverflowAllocator>
                              this_type;
    typedef OverflowAllocator overflow_allocator_type;
