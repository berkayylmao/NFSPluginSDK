...
#if defined(EA_HAVE_CPP11_MUTEX) && !defined(EA_PLATFORM_MICROSOFT) && \
    !defined(EA_PLATFORM_UNIX)  // We stick with platform-specific mutex support to the extent
                                // possible, as it's currently more reliably available.
...
#if defined(_MSC_VER)
#pragma warning(push)
#pragma warning(disable : 4625)  // copy constructor could not be generated because a base class
...
extern "C" long __stdcall _InterlockedCompareExchange(long volatile* Dest, long Exchange,
                                                      long Comp);
...
#else
      EASTL_FAIL_MSG(
          "EASTL thread safety is not implemented yet. See EAThread for how to do this for the "
          "given platform.");
      return ++*p32;
#endif
    }
...
#elif defined(EA_COMPILER_MSVC)
      return ((int32_t)_InterlockedCompareExchange((volatile long*)p32, (long)newValue,
                                                   (long)condition) == condition);
#elif defined(EA_COMPILER_GNUC)
      // GCC Inline ASM Constraints
      // r  <--> Any general purpose register
      // a  <--> The a register.
      // 1  <--> The constraint '1' for operand 2 says that it must occupy the same location as
      // operand 1. =a <--> output registers =r <--> output registers
...
// mutex
    ...
    class EASTL_API auto_mutex {
     public:
      EA_FORCE_INLINE auto_mutex(mutex& mutex) : pMutex(&mutex) { pMutex->lock(); }

      EA_FORCE_INLINE ~auto_mutex() { pMutex->unlock(); }
