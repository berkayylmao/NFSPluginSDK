...
namespace eastl {
  ...
  inline void fill(ForwardIterator first, ForwardIterator last, const T& value) {
    ...
    //                                       eastl::integral_constant<bool, (sizeof(T) <=
    //                                       16)>::value;
  ...
  inline void fill(char* first, char* last,
  ...
  struct fill_n_imp<true> {
    ...
    static OutputIterator do_fill(OutputIterator first, Size n, const T& value) {
      ...
      for (const T temp = value; n-- > 0; ++first) *first = static_cast<value_type>(temp);
      return first;
    }
  };
...
#ifdef _MSC_VER  // VC++ up to and including VC8 blow up when you pass a 64 bit scalar to the
                 // do_fill function.
    return eastl::fill_n_imp < is_scalar<T>::value &&
           (sizeof(T) <= sizeof(uint32_t)) > ::do_fill(first, n, value);
#else
