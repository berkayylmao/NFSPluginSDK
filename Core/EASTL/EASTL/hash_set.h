...
#define EASTL_HASH_MULTISET_DEFAULT_ALLOCATOR allocator_type(EASTL_HASH_MULTISET_DEFAULT_NAME)
  ...
  template <typename Value, typename Hash = eastl::hash<Value>,
            typename Predicate = eastl::equal_to<Value>, typename Allocator = EASTLAllocatorType,
            bool bCacheHashCode = false>
  class hash_set : public hashtable<Value, Value, Allocator, eastl::use_self<Value>, Predicate,
                                    Hash, mod_range_hashing, default_ranged_hash,
                                    prime_rehash_policy, bCacheHashCode, false, true> {
   public:
    typedef hashtable<Value, Value, Allocator, eastl::use_self<Value>, Predicate, Hash,
   ...
   public:
    ...
    ///
    explicit hash_set(size_type nBucketCount, const Hash& hashFunction = Hash(),
    ...
    hash_set(const this_type& x) : base_type(x) {}

#if EASTL_MOVE_SEMANTICS_ENABLED
    hash_set(this_type&& x) : base_type(eastl::move(x)) {}
    ...
    hash_set(std::initializer_list<value_type> ilist, size_type nBucketCount = 0,
        ...
        : base_type(ilist.begin(), ilist.end(), nBucketCount, hashFunction, mod_range_hashing(),
                    default_ranged_hash(), predicate, eastl::use_self<Value>(), allocator) {
      // Empty
    }
    ...
    hash_set(FowardIterator first, FowardIterator last, size_type nBucketCount = 0,
             const Hash& hashFunction = Hash(), const Predicate& predicate = Predicate(),
             const allocator_type& allocator = EASTL_HASH_SET_DEFAULT_ALLOCATOR)
  ...
  template <typename Value, typename Hash = eastl::hash<Value>,
            typename Predicate = eastl::equal_to<Value>, typename Allocator = EASTLAllocatorType,
            bool bCacheHashCode = false>
  class hash_multiset : public hashtable<Value, Value, Allocator, eastl::use_self<Value>, Predicate,
                                         Hash, mod_range_hashing, default_ranged_hash,
   ...
   public:
    ...
    hash_multiset(const this_type& x) : base_type(x) {}

#if EASTL_MOVE_SEMANTICS_ENABLED
    hash_multiset(this_type&& x) : base_type(eastl::move(x)) {}
    ...
    ///
    hash_multiset(std::initializer_list<value_type> ilist, size_type nBucketCount = 0,
                  const Hash& hashFunction = Hash(), const Predicate& predicate = Predicate(),
                  const allocator_type& allocator = EASTL_HASH_MULTISET_DEFAULT_ALLOCATOR)
        : base_type(ilist.begin(), ilist.end(), nBucketCount, hashFunction, mod_range_hashing(),
                    default_ranged_hash(), predicate, eastl::use_self<Value>(), allocator) {
    ...
    ///
    template <typename FowardIterator>
    hash_multiset(FowardIterator first, FowardIterator last, size_type nBucketCount = 0,
                  const Hash& hashFunction = Hash(), const Predicate& predicate = Predicate(),
                  const allocator_type& allocator = EASTL_HASH_MULTISET_DEFAULT_ALLOCATOR)
        : base_type(first, last, nBucketCount, hashFunction, mod_range_hashing(),
                    default_ranged_hash(), predicate, eastl::use_self<Value>(), allocator) {
  ...
  inline bool operator==(const hash_set<Value, Hash, Predicate, Allocator, bCacheHashCode>& a,
    ...
    typedef typename hash_set<Value, Hash, Predicate, Allocator, bCacheHashCode>::const_iterator
        const_iterator;
    ...
    // as there can be only one such pairing per element. multiset needs to do a something more
    // elaborate.
    for (const_iterator ai = a.begin(), aiEnd = a.end(), biEnd = b.end(); ai != aiEnd; ++ai) {
      ...
      if ((bi == biEnd) ||
          !(*ai ==
            *bi))  // We have to compare values in addition to making sure the lookups succeeded.
                   // This is because the lookup is done via the user-supplised Predicate
        return false;  // which isn't strictly required to be identical to the Value operator==,
                       // though 99% of the time it will be so.
    }
  ...
  inline bool operator==(
    ...
    typedef
        typename hash_multiset<Value, Hash, Predicate, Allocator, bCacheHashCode>::const_iterator
                                                                             const_iterator;
         ...
         ai = aRange.second)  // For each element in a...
    {
      ...
      if (aDistance == 1) {
        if (!(*aRange.first ==
              *bRange.first))  // We have to compare values in addition to making sure the distance
                               // (element count) was equal. This is because the lookup is done via
                               // the user-supplised Predicate
          return false;  // which isn't strictly required to be identical to the Value operator==,
                         // though 99% of the time it will be so. Ditto for the is_permutation usage
                         // below.
      } else {
