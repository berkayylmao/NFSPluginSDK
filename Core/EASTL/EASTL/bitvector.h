...
#include <Core/EASTL/EASTL/vector.h>
#include <Core/EASTL/EASTL/algorithm.h>
#include <Core/EASTL/EASTL/bitset.h>
...
#define EASTL_BITVECTOR_DEFAULT_ALLOCATOR allocator_type(EASTL_BITVECTOR_DEFAULT_NAME)
  ...
  class bitvector {
   public:
    ...
    static const size_type npos =
        container_type::npos;  /// 'npos' means non-valid position or simply non-position.
   ...
   public:
    ...
    bitvector(size_type n, value_type value,
              const allocator_type& allocator = EASTL_BITVECTOR_DEFAULT_ALLOCATOR);
  ...
  template <typename Element>
  void MoveBits(bitvector_iterator<Element> start, bitvector_iterator<Element> end,
  ...
  */
  ...
  bitvector<Allocator, Element, Container>::insert(const_iterator position, value_type value) {
    iterator iPosition(
        position.get_reference_type());  // This is just a non-const version of position.
  ...
  template <typename Allocator, typename Element, typename Container>
  void bitvector<Allocator, Element, Container>::insert(const_iterator position, size_type n,
                                                        value_type value) {
    iterator iPosition(
        position.get_reference_type());  // This is just a non-const version of position.
...
// Verify that no reallocation occurred.
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::reverse_iterator
  bitvector<Allocator, Element, Container>::erase(const_reverse_iterator first,
  ...
  template <typename Allocator, typename Element, typename Container>
  bitvector<Allocator, Element, Container>& bitvector<Allocator, Element, Container>::operator=(
      const bitvector& rhs) {
  ...
  template <typename Allocator, typename Element, typename Container>
  bitvector<Allocator, Element, Container>::bitvector(size_type n, value_type value,
                                                      const allocator_type& allocator)
      : mContainer((n + kBitCount - 1) / kBitCount, value ? ~element_type(0) : element_type(0),
                   allocator) {
    mFreeBitCount = kBitCount - (n % kBitCount);
