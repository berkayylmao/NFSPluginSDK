...
#define EASTL_BASIC_STRING_DEFAULT_ALLOCATOR allocator_type(EASTL_BASIC_STRING_DEFAULT_NAME)
  ...
  class abstract_string {
   public:
    ...
    value_type* mpCapacity;  // End of allocated space, including the space needed to store the
                             // trailing '0' char. mpCapacity is always at least mpEnd + 1.
   ...
   public:
    ...
    size_type capacity() const
        EA_NOEXCEPT;  // Expanded in source code as: (size_type)((mpCapacity - mpBegin) - 1)
    ...
    void clear();
    void reset_lose_memory();  // This is a unilateral reset to an initially empty state. No
                               // destructors are called, no deallocation occurs.
    ...
    this_type& replace(iterator first, iterator last, const value_type* pBegin,
    ...
    // Case-insensitive comparison functions. Not part of C++ basic_string. Only ASCII-level locale
...
// this_type& operator=(this_type&& x);
   ...
   protected:
    ...
    allocator_type
        mAllocator;  // To consider: Use base class optimization to make this go away. However, this
                     // is not easy because doing so requires templating the base class on something
                     // in addition to T, and we'd like to avoid that in order to reduce code bloat.
  };                 // basic_string
  ...
  inline abstract_string<T>::abstract_string(const this_type& x)
      : mpBegin(NULL), mpEnd(NULL), mpCapacity(NULL), mpAllocFreeMethod(x.mpAllocFreeMethod) {
  ...
  inline basic_string<T, Allocator>::basic_string()
      : base_type(&AllocFreeMethod), mAllocator(EASTL_BASIC_STRING_DEFAULT_NAME) {
    // Do nothing.
  }

  template <typename T, typename Allocator>
  inline basic_string<T, Allocator>::basic_string(const allocator_type& allocator)
      : base_type(&AllocFreeMethod), mAllocator(allocator) {
    // Do nothing.
  }
  ...
  inline basic_string<T, Allocator>::basic_string(const this_type& x)
      : base_type(&AllocFreeMethod), mAllocator(x.mAllocator) {
    RangeInitialize(x.mpBegin, x.mpEnd);
  }
  ...
  basic_string<T, Allocator>::basic_string(CtorSprintf /*unused*/, const value_type* pFormat, ...)
      : base_type(&AllocFreeMethod), mAllocator() {
    const size_type n =
  ...
  */
  ...
  template <typename T>
  inline const typename abstract_string<T>::value_type* abstract_string<T>::data() const {
    return mpBegin;
  }

  template <typename T>
  inline const typename abstract_string<T>::value_type* abstract_string<T>::c_str() const {
    return mpBegin;
  }

  template <typename T>
  inline typename abstract_string<T>::iterator abstract_string<T>::begin() EA_NOEXCEPT {
    return mpBegin;
  }

  template <typename T>
  inline typename abstract_string<T>::const_iterator abstract_string<T>::begin() const EA_NOEXCEPT {
    return mpBegin;
  }

  template <typename T>
  inline typename abstract_string<T>::const_iterator abstract_string<T>::cbegin() const
      EA_NOEXCEPT {
    return mpBegin;
  }

  template <typename T>
  inline typename abstract_string<T>::iterator abstract_string<T>::end() EA_NOEXCEPT {
    return mpEnd;
  }

  template <typename T>
  inline typename abstract_string<T>::const_iterator abstract_string<T>::end() const EA_NOEXCEPT {
    return mpEnd;
  }

  template <typename T>
  inline typename abstract_string<T>::const_iterator abstract_string<T>::cend() const EA_NOEXCEPT {
    return mpEnd;
  }

  template <typename T>
  inline typename abstract_string<T>::reverse_iterator abstract_string<T>::rbegin() EA_NOEXCEPT {
    return reverse_iterator(mpEnd);
  }

  template <typename T>
  inline typename abstract_string<T>::const_reverse_iterator abstract_string<T>::rbegin() const
      EA_NOEXCEPT {
    return const_reverse_iterator(mpEnd);
  }

  template <typename T>
  inline typename abstract_string<T>::const_reverse_iterator abstract_string<T>::crbegin() const
      EA_NOEXCEPT {
    return const_reverse_iterator(mpEnd);
  }

  template <typename T>
  inline typename abstract_string<T>::reverse_iterator abstract_string<T>::rend() EA_NOEXCEPT {
    return reverse_iterator(mpBegin);
  }

  template <typename T>
  inline typename abstract_string<T>::const_reverse_iterator abstract_string<T>::rend() const
      EA_NOEXCEPT {
    return const_reverse_iterator(mpBegin);
  }

  template <typename T>
  inline typename abstract_string<T>::const_reverse_iterator abstract_string<T>::crend() const
      EA_NOEXCEPT {
    return const_reverse_iterator(mpBegin);
  }
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::size() const EA_NOEXCEPT {
    return (size_type)(mpEnd - mpBegin);
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::length() const EA_NOEXCEPT {
    return (size_type)(mpEnd - mpBegin);
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::max_size() const EA_NOEXCEPT {
    return kMaxSize;
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::capacity() const EA_NOEXCEPT {
    return (size_type)(
        (mpCapacity - mpBegin) -
  ...
  template <typename T>
  inline typename abstract_string<T>::const_reference abstract_string<T>::operator[](
      size_type n) const {
  ...
  template <typename T>
  inline typename abstract_string<T>::reference abstract_string<T>::operator[](size_type n) {
#if EASTL_ASSERT_ENABLED  // We allow the user to reference the trailing 0 char without asserting.
                          // Perhaps we shouldn't.
    if (EASTL_UNLIKELY(n > (static_cast<size_type>(mpEnd - mpBegin))))
      EASTL_FAIL_MSG("basic_string::operator[] -- out of range");
  ...
  /*
  template <typename T, typename Allocator>
  inline typename basic_string<T, Allocator>::this_type& basic_string<T,
  Allocator>::operator=(this_type&& x)
  {
  ...
  */
  ...
  template <typename T>
  inline typename abstract_string<T>::const_reference abstract_string<T>::at(size_type n) const {
#if EASTL_STRING_OPT_RANGE_ERRORS
  ...
  template <typename T>
  inline typename abstract_string<T>::reference abstract_string<T>::at(size_type n) {
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY(n >= (size_type)(mpEnd - mpBegin))) ThrowRangeException();
#elif EASTL_ASSERT_ENABLED  // We assert if the user references the trailing 0 char.
    if (EASTL_UNLIKELY(n >= (size_type)(mpEnd - mpBegin)))
  ...
  template <typename T>
  inline typename abstract_string<T>::reference abstract_string<T>::front() {
  ...
  template <typename T>
  inline typename abstract_string<T>::const_reference abstract_string<T>::front() const {
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
    // We allow the user to reference the trailing 0 char without asserting.
#elif EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(mpEnd <= mpBegin))  // We assert if the user references the trailing 0 char.
  ...
  template <typename T>
  inline typename abstract_string<T>::reference abstract_string<T>::back() {
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
    // We allow the user to reference the trailing 0 char without asserting.
#elif EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(mpEnd <= mpBegin))  // We assert if the user references the trailing 0 char.
  ...
  template <typename T>
  inline typename abstract_string<T>::const_reference abstract_string<T>::back() const {
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
    // We allow the user to reference the trailing 0 char without asserting.
#elif EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(mpEnd <= mpBegin))  // We assert if the user references the trailing 0 char.
  ...
  template <typename T>
  inline abstract_string<T>& abstract_string<T>::append(const this_type& x, size_type position,
                                                        size_type n) {
#if EASTL_STRING_OPT_RANGE_ERRORS
  ...
  template <typename T>
  abstract_string<T>& abstract_string<T>::append_sprintf_va_list(const value_type* pFormat,
                                                                 va_list           arguments) {
...
// In this case we definitely have C99 Vsnprintf behaviour.
  ...
  template <typename T>
  inline abstract_string<T>& abstract_string<T>::assign(const this_type& x, size_type position,
                                                        size_type n) {
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY(position > (size_type)(x.mpEnd - x.mpBegin))) ThrowRangeException();
#endif
  ...
  template <typename T>
  inline typename abstract_string<T>::iterator abstract_string<T>::insert(iterator   p,
                                                                          value_type c) {
    if (p == mpEnd) {
      push_back(c);
      return mpEnd - 1;
  ...
  template <typename T>
  inline typename abstract_string<T>::iterator abstract_string<T>::erase(iterator p) {
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((p < mpBegin) || (p >= mpEnd)))
      EASTL_FAIL_MSG("abstract_string::erase -- invalid position");
#endif
  ...
  template <typename T>
  typename abstract_string<T>::iterator abstract_string<T>::erase(iterator pBegin, iterator pEnd) {
#if EASTL_ASSERT_ENABLED
  ...
  template <typename T>
  inline typename abstract_string<T>::reverse_iterator abstract_string<T>::erase(
      reverse_iterator position) {
    return reverse_iterator(erase((++position).base()));
  }

  template <typename T>
  typename abstract_string<T>::reverse_iterator abstract_string<T>::erase(reverse_iterator first,
                                                                          reverse_iterator last) {
    return reverse_iterator(erase((++last).base(), (++first).base()));
  }

  template <typename T>
  abstract_string<T>& abstract_string<T>::replace(size_type position, size_type n,
                                                  const this_type& x) {
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY(position > (size_type)(mpEnd - mpBegin))) ThrowRangeException();
#endif
  ...
  template <typename T>
  abstract_string<T>& abstract_string<T>::replace(size_type pos1, size_type n1, const this_type& x,
  ...
  template <typename T>
  abstract_string<T>& abstract_string<T>::replace(size_type position, size_type n1,
  ...
  template <typename T>
  abstract_string<T>& abstract_string<T>::replace(size_type position, size_type n1, size_type n2,
                                                  value_type c) {
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY(position > (size_type)(mpEnd - mpBegin))) ThrowRangeException();
#endif
  ...
  template <typename T>
  inline abstract_string<T>& abstract_string<T>::replace(iterator pBegin, iterator pEnd,
                                                         const this_type& x) {
  ...
  template <typename T>
  abstract_string<T>& abstract_string<T>::replace(iterator pBegin, iterator pEnd, size_type n,
                                                  value_type c) {
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((pBegin < mpBegin) || (pBegin > mpEnd) || (pEnd < mpBegin) ||
                       (pEnd > mpEnd) || (pEnd < pBegin)))
  ...
  template <typename T>
  abstract_string<T>& abstract_string<T>::replace(iterator pBegin1, iterator pEnd1,
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::copy(value_type* p, size_type n,
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find(const this_type& x,
                                                                         size_type position) const {
    return find(x.mpBegin, position, (size_type)(x.mpEnd - x.mpBegin));
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find(const value_type* p,
                                                                         size_type position) const {
    return find(p, position, (size_type)CharStrlen(p));
  }

  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find(const value_type* p,
                                                                  size_type         position,
                                                                  size_type         n) const {
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find(value_type c,
                                                                  size_type  position) const {
    // It is not clear what the requirements are for position, but since the C++ standard
    // appears to be silent it is assumed for now that position can be any value.
    //#if EASTL_ASSERT_ENABLED
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::rfind(
      const this_type& x, size_type position) const {
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::rfind(const value_type* p,
                                                                   size_type         position,
                                                                   size_type         n) const {
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::rfind(value_type c,
                                                                   size_type  position) const {
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_first_of(
      const this_type& x, size_type position) const {
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find_first_of(const value_type* p,
                                                                           size_type position,
                                                                           size_type n) const {
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_first_of(
      value_type c, size_type position) const {
    return find(c, position);
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_last_of(
      const this_type& x, size_type position) const {
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find_last_of(const value_type* p,
                                                                          size_type position,
                                                                          size_type n) const {
    // If n is zero or position is >= size, we return npos.
    const size_type nLength = (size_type)(mpEnd - mpBegin);
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_first_not_of(
      const this_type& x, size_type position) const {
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find_first_not_of(const value_type* p,
                                                                               size_type position,
                                                                               size_type n) const {
    if (EASTL_LIKELY(position <= (size_type)(mpEnd - mpBegin))) {
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find_first_not_of(
      value_type c, size_type position) const {
    if (EASTL_LIKELY(position <= (size_type)(mpEnd - mpBegin))) {
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_last_not_of(
      const this_type& x, size_type position) const {
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find_last_not_of(const value_type* p,
                                                                              size_type position,
                                                                              size_type n) const {
    const size_type nLength = (size_type)(mpEnd - mpBegin);
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find_last_not_of(
      value_type c, size_type position) const {
    const size_type nLength = (size_type)(mpEnd - mpBegin);
  ...
  template <typename T>
  inline int abstract_string<T>::compare(size_type pos1, size_type n1, const this_type& x,
  ...
  template <typename T>
  inline int abstract_string<T>::compare(size_type pos1, size_type n1, const value_type* p,
                                         size_type n2) const {
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY(pos1 > (size_type)(mpEnd - mpBegin))) ThrowRangeException();
#endif
  ...
  template <typename T>
  abstract_string<T>& abstract_string<T>::sprintf_va_list(const value_type* pFormat,
                                                          va_list           arguments) {
    mpEnd = mpBegin;  // Fast truncate to zero length.
  ...
  template <typename T>
  typename abstract_string<T>::iterator abstract_string<T>::InsertInternal(iterator   p,
                                                                           value_type c) {
  ...
  void abstract_string<T>::RangeInitialize(const value_type* pBegin, const value_type* pEnd) {
    ...
    if (EASTL_UNLIKELY(!pBegin && (n != 0))) ThrowInvalidArgumentException();
#endif
  ...
  template <typename T>
  inline typename abstract_string<T>::value_type* abstract_string<T>::DoAllocate(size_type n) {
  ...
  inline void abstract_string<T>::DoFree(value_type* p, size_type n) {
    if (p) (*mpAllocFreeMethod)(n * sizeof(value_type), p, this);
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::GetNewCapacity(
  ...
  template <typename T>
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringFindEnd(
      const value_type* pBegin, const value_type* pEnd, value_type c) {
  ...
  template <typename T>
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringRFind(
      const value_type* pRBegin, const value_type* pREnd, const value_type c) {
    while (pRBegin > pREnd) {
  ...
  template <typename T>
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringSearch(
      const value_type* p1Begin, const value_type* p1End, const value_type* p2Begin,
      const value_type* p2End) {
  ...
  template <typename T>
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringRSearch(
      const value_type* p1Begin, const value_type* p1End, const value_type* p2Begin,
      const value_type* p2End) {
    // Test for zero length strings, in which case we have a match or a failure,
    // but the return value is the same either way.
  ...
  // null-terminated.
  template <typename T>
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringFindFirstOf(
      const value_type* p1Begin, const value_type* p1End, const value_type* p2Begin,
      const value_type* p2End) {
  ...
  // null-terminated.
  template <typename T>
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringRFindFirstOf(
      const value_type* p1RBegin, const value_type* p1REnd, const value_type* p2Begin,
      const value_type* p2End) {
  ...
  template <typename T>
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringFindFirstNotOf(
      const value_type* p1Begin, const value_type* p1End, const value_type* p2Begin,
      const value_type* p2End) {
    for (; p1Begin != p1End; ++p1Begin) {
      const value_type* pTemp;
  ...
  template <typename T>
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringRFindFirstNotOf(
      const value_type* p1RBegin, const value_type* p1REnd, const value_type* p2Begin,
      const value_type* p2End) {
    for (; p1RBegin != p1REnd; --p1RBegin) {
      const value_type* pTemp;
  ...
  inline basic_string<T, Allocator>& basic_string<T, Allocator>::operator=(const base_type& x) {
    if (&x != this) {
      // We are copying from our base_type, which is an interface class that doesn't define an
      // allocator. and may not have one or may not have one like ours. So a copy from the base type
      // does not attempt to copy the allocator. Note that we also have an operator=(const
      // this_type&) which will in fact copy the allocator if the copy source is the same type as
      // us. It would be nice if we had a mechanism to somehow detect that the base_type had a
      // compatible allocator, but that can bet complicated.
      //#if EASTL_ALLOCATOR_COPY_ENABLED
      //    mAllocator = x.mAllocator;
      //#endif
  ...
  inline bool operator==(const abstract_string<T>& a, const abstract_string<T>& b) {
    return ((a.size() == b.size()) &&
            (memcmp(a.data(), b.data(),
                    (size_t)a.size() * sizeof(typename abstract_string<T>::value_type)) == 0));
  }

  template <typename T>
  inline bool operator==(const typename abstract_string<T>::value_type* p,
                         const abstract_string<T>&                      b) {
  ...
  template <typename T>
  inline bool operator==(const abstract_string<T>&                      a,
                         const typename abstract_string<T>::value_type* p) {
    typedef typename abstract_string<T>::size_type size_type;
    const size_type                                n = (size_type)CharStrlen(p);
