...
#include <Core/EASTL/EASTL/sort.h>  // For backwards compatibility due to sorts moved from here to sort.h.
      ...
      for (++iCurrent; iCurrent != last; ++iCurrent) {
        if (compare(*iCurrent, *iMin)) {
          EASTL_VALIDATE_COMPARE(
              !compare(*iMin, *iCurrent));  // Validate that the compare function is sane.
          iMin = iCurrent;
        }
      }
        ...
        if (first != last) {
          for (iCurrent = last; iCurrent != first; iCurrent = iNext) {
            ...
            if (compare(*iCurrent, *iNext)) {
              EASTL_VALIDATE_COMPARE(
                  !compare(*iNext, *iCurrent));  // Validate that the compare function is sane.
              iLastModified = iCurrent;
