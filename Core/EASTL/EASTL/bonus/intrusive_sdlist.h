...
namespace eastl {
  ...
  class intrusive_sdlist : public intrusive_sdlist_base {
   ...
   public:
    ...
    void push_front(
        value_type& value);  ///< Adds an element to the front of the list; O(1). The element is not
                             ///< copied. The element must not be in any other list.
    void push_back(
        value_type& value);  ///< Adds an element to the back of the list; O(N). The element is not
                             ///< copied. The element must not be in any other list.
    void pop_back();  ///< Removes an element from the back of the list; O(N). The element must be
                      ///< present, but is not deallocated.

    bool contains(
        const value_type& value) const;  ///< Returns true if the given element is in the list;
                                         ///< O(n). Equivalent to (locate(x) != end()).

    iterator locate(
        value_type& value);  ///< Converts a reference to an object in the list back to an iterator,
  ...
  template <typename T>
  inline typename intrusive_sdlist<T>::iterator intrusive_sdlist<T>::insert(iterator    position,
                                                                            value_type& value) {
  ...
  template <typename T>
  inline typename intrusive_sdlist<T>::iterator intrusive_sdlist<T>::erase(iterator first,
                                                                           iterator last) {
