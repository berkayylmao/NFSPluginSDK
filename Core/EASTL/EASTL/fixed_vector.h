...
#define EASTL_FIXED_VECTOR_DEFAULT_ALLOCATOR \
  ...
  class fixed_vector
      : public vector<T, fixed_vector_allocator<sizeof(T), nodeCount, EASTL_ALIGN_OF(T), 0,
                                                bEnableOverflow, OverflowAllocator> > {
   public:
    typedef fixed_vector_allocator<sizeof(T), nodeCount, EASTL_ALIGN_OF(T), 0, bEnableOverflow,
                                   OverflowAllocator>
   ...
   public:
    fixed_vector();
    explicit fixed_vector(const overflow_allocator_type&
                              overflowAllocator);  // Only applicable if bEnableOverflow is true.
    explicit fixed_vector(size_type n);            // Currently we don't support overflowAllocator
...
#endif
    fixed_vector(std::initializer_list<T> ilist, const overflow_allocator_type& overflowAllocator =
                                                     EASTL_FIXED_VECTOR_DEFAULT_ALLOCATOR);
    ...
    bool full()
        const;  // Returns true if the fixed space has been fully allocated. Note that if overflow
    ...
    void  push_back(const value_type& value);  // We implement push_back here because we have a
...
// Deprecated:
  ...
  inline fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_vector(this_type&& x)
      : base_type(fixed_allocator_type(mBuffer.buffer)) {
    // Since we are a fixed_vector, we can't swap pointers. We can possibly so something like
    // fixed_swap or we can just do an assignment from x. If we want to do the former then we need
    // to have some complicated code to deal with overflow or no overflow, and whether the memory is
    // in the fixed-size buffer or in the overflow allocator. 90% of the time the memory should be
    // in the fixed buffer, in which case a simple assignment is no worse than the fancy pathway.
    ...
    // if(has_overflowed() && x.has_overflowed() && (get_overflow_allocator() ==
    // x.get_overflow_allocator()))
    //{
    //    We can swap contents and may need to swap the allocators as well.
    //}

    // The following is currently identical to the fixed_vector(const this_type& x) code above. If
    // it stays that way then we may want to make a shared implementation.
    mAllocator.copy_overflow_allocator(x.mAllocator);
  ...
  inline fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_vector(
    ...
    mpBegin = mpEnd = (value_type*)&mBuffer.buffer[0];
    mpCapacity      = mpBegin + nodeCount;
    base_type::template DoAssignFromIterator<InputIterator, false>(ilist.begin(), ilist.end(),
                                                                   IC());
  }

  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  template <typename InputIterator>
  fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_vector(InputIterator first,
                                                                               InputIterator last)
      : base_type(fixed_allocator_type(mBuffer.buffer)) {
#if EASTL_NAME_ENABLED
    mAllocator.set_name(EASTL_FIXED_VECTOR_DEFAULT_NAME);
  ...
  inline void fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::swap(this_type& x) {
    if ((has_overflowed() && x.has_overflowed()) &&
        (get_overflow_allocator() ==
         x.get_overflow_allocator()))  // If both containers are using the heap instead of local
                                       // memory
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline void fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::set_capacity(
      size_type n) {
    ...
    if (n != nPrevCapacity)  // If the request results in a capacity change...
    {
      if (can_overflow() && (((uintptr_t)mpBegin != (uintptr_t)mBuffer.buffer) ||
                             (n > kMaxSize)))  // If we are or would be using dynamically allocated
                                               // memory instead of our fixed-size member buffer...
      {
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline bool fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::has_overflowed()
      const {
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline void*
  fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::push_back_uninitialized() {
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline void* fixed_vector<T, nodeCount, bEnableOverflow,
                            OverflowAllocator>::DoPushBackUninitialized(false_type) {
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline void fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::push_back(
      const value_type& value) {
    DoPushBack(typename type_select<bEnableOverflow, true_type, false_type>::type(), value);
  }

  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline void fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::DoPushBack(
      true_type, const value_type& value) {
    base_type::push_back(value);
  }

  // This template specializes for overflow NOT enabled.
  // In this configuration, there is no need for the heavy weight push_back() which tests to see if
  // the container should grow (it never will)
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline void fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::DoPushBack(
      false_type, const value_type& value) {
    EASTL_ASSERT(mpEnd < mpCapacity);
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::reference
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::reference
  fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::DoPushBack(true_type) {
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::reference
  fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::DoPushBack(false_type) {
    EASTL_ASSERT(mpEnd < mpCapacity);

    ::new ((void*)mpEnd++) value_type;  // Note that this isn't value_type() as that syntax doesn't
                                        // work on all compilers for POD types.
  ...
  inline void fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::push_back(
      value_type&& value) {
    DoPushBackMove(typename type_select<bEnableOverflow, true_type, false_type>::type(),
                   eastl::move(value));
  }

  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline void fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::DoPushBackMove(
      true_type, value_type&& value) {
    base_type::push_back(eastl::move(value));  // This will call vector::push_back(value_type &&),
                                               // and possibly swap value with *mpEnd.
  }
  ...
  inline void fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::DoPushBackMove(
    ...
    ::new ((void*)mpEnd++)
        value_type(eastl::move(value));  // This will call the value_type(value_type&&) constructor,
                                         // and possibly swap value with *mpEnd.
  }
#endif
  ...
  fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::get_overflow_allocator() const
      EA_NOEXCEPT {
  ...
  template <typename T, size_t nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline void
  fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::set_overflow_allocator(
