...
#include <Core/EASTL/EABase/eahave.h>

#include <Core/EASTL/EASTL/internal/integer_sequence.h>
#include <Core/EASTL/EASTL/internal/tuple_fwd_decls.h>
...
#pragma warning(disable : 4217)  // Member template functions cannot be used for copy-assignment or
                                 // copy-construction.
#pragma warning(disable : 4512)  // 'class' : assignment operator could not be generated.  // This
                                 // disabling would best be put elsewhere.
#endif
...
#define EASTL_TYPE_TRAIT_is_swappable_CONFORMANCE 1
  ...
  struct is_swappable
      : public integral_constant<
            bool, !eastl::is_same<decltype(swap(eastl::declval<T&>(), eastl::declval<T&>())),
                                  eastl::unused>::value> {
  };  // Don't prefix swap with eastl:: as we want to allow user-defined swaps via
...
#define EASTL_TYPE_TRAIT_is_nothrow_swappable_CONFORMANCE 0
  ...
  template <typename>
  struct is_nothrow_swappable : public eastl::false_type {};
#else
#define EASTL_TYPE_TRAIT_is_nothrow_swappable_CONFORMANCE EASTL_TYPE_TRAIT_is_swappable_CONFORMANCE
  ...
  struct is_nothrow_swappable_helper
      : public eastl::integral_constant<bool,
                                        is_nothrow_swappable_helper_noexcept_wrapper<T>::value> {
  };  // Don't prefix swap with eastl:: as we want to allow user-defined swaps via
      // argument-dependent lookup.
  ...
  struct iter_swap_impl {
    ...
    static void iter_swap(ForwardIterator1 a, ForwardIterator2 b) {
      ...
      value_type_a temp(
          EASTL_MOVE(*a));  // EASTL_MOVE uses EASTL::move when available, else is a no-op.
      *a = EASTL_MOVE(*b);
      *b = EASTL_MOVE(temp);
    }
  };
  ...
  struct iter_swap_impl<true> {
    template <typename ForwardIterator1, typename ForwardIterator2>
    static void iter_swap(ForwardIterator1 a, ForwardIterator2 b) {
      swap(*a, *b);  // Don't prefix swap with eastl:: as we want to allow user-defined swaps via
                     // argument-dependent lookup.
    }
  };
  ...
  inline void iter_swap(ForwardIterator1 a, ForwardIterator2 b) {
    ...
    eastl::iter_swap_impl<
        eastl::type_and<eastl::is_same<value_type_a, value_type_b>::value,
  ...
  inline ForwardIterator2 swap_ranges(ForwardIterator1 first1, ForwardIterator1 last1,
    ...
    for (; first1 != last1; ++first1, ++first2)
      iter_swap(first1, first2);  // Don't prefix swap with eastl:: as we want to allow user-defined
                                  // swaps via argument-dependent lookup.
    return first2;
  }
  ...
  inline void swap(T (&a)[N], T (&b)[N]) EA_NOEXCEPT_IF(eastl::is_nothrow_swappable<T>::value) {
  ...
  struct pair {
    ...
    EA_CONSTEXPR pair() : first(), second() {}

    // To consider: Use type traits to enable this ctor only if T2 (second
    // is_default_constructible<T2>::value == true.)
    EA_CPP14_CONSTEXPR pair(const T1& x) : first(x), second() {}

    EA_CPP14_CONSTEXPR pair(const T1& x, const T2& y) : first(x), second(y) {}

    template <typename U,
              typename V>  // This would enable_if situation be better solvable with C++14 concepts,
                           // but such compilers aren't currently available.
                           EA_CPP14_CONSTEXPR pair(
                               const pair<U, V>& p,
                               typename eastl::enable_if<
                                   eastl::is_convertible<const U&, T1>::value &&
                                   eastl::is_convertible<const V&, T2>::value>::type* = 0)
    ...
    EA_CPP14_CONSTEXPR pair(
        U&& u, V&& v,
        typename eastl::enable_if<eastl::is_convertible<U, T1>::value &&
                                  eastl::is_convertible<V, T2>::value>::type* = 0)
        : first(eastl::forward<U>(u)), second(eastl::forward<V>(v)) {}

    template <typename U>
    EA_CPP14_CONSTEXPR pair(
        U&& x, const T2& y,
        typename eastl::enable_if<eastl::is_convertible<U, T1>::value>::type* = 0)
        : first(eastl::forward<U>(x)), second(y) {}

    template <typename V>
    EA_CPP14_CONSTEXPR pair(
        const T1& x, V&& y,
        typename eastl::enable_if<eastl::is_convertible<V, T2>::value>::type* = 0)
        : first(x), second(eastl::forward<V>(y)) {}
#else
    template <typename U, typename V,
              typename = typename eastl::enable_if<eastl::is_convertible<U, T1>::value &&
                                                   eastl::is_convertible<V, T2>::value>::type>
    EA_CPP14_CONSTEXPR pair(U&& u, V&& v)
        : first(eastl::forward<U>(u)), second(eastl::forward<V>(v)) {}

    template <typename U,
              typename = typename eastl::enable_if<eastl::is_convertible<U, T1>::value>::type>
    EA_CPP14_CONSTEXPR pair(U&& x, const T2& y) : first(eastl::forward<U>(x)), second(y) {}

    template <typename V,
              typename = typename eastl::enable_if<eastl::is_convertible<V, T2>::value>::type>
    EA_CPP14_CONSTEXPR pair(const T1& x, V&& y) : first(x), second(eastl::forward<V>(y)) {}
#endif

#if (EABASE_VERSION_N >= 20040) && !defined(EA_COMPILER_NO_DEFAULTED_FUNCTIONS) && \
    defined(_MSC_VER) && (EA_COMPILER_VERSION >= 1900)
    pair(pair&& p) = default;
#else
    // The C++11 Standard specifies the following as a defaulted function, but we cannot do that
    // because otherwise we get GCC and MSVC compile errors in the rest of EASTL saying that it's a
    // needed but implicitly deleted function.
    EA_CPP14_CONSTEXPR pair(pair&& p)
        EA_NOEXCEPT_IF(eastl::is_nothrow_move_constructible<T1>::value&&
                           eastl::is_nothrow_move_constructible<T2>::value)
        : first(eastl::forward<T1>(p.first)), second(eastl::forward<T2>(p.second)) {}
#endif

#if defined(EA_COMPILER_NO_FUNCTION_TEMPLATE_DEFAULT_ARGS)
    template <typename U, typename V>
    EA_CPP14_CONSTEXPR pair(
        pair<U, V>&& p, typename eastl::enable_if<eastl::is_convertible<U, T1>::value &&
                                                  eastl::is_convertible<V, T2>::value>::type* = 0)
    ...
    pair& operator=(const pair& p)
        EA_NOEXCEPT_IF(eastl::is_nothrow_copy_assignable<T1>::value&&
                           eastl::is_nothrow_copy_assignable<T2>::value) {
      first  = p.first;
      second = p.second;
      return *this;
    }
...
#if EASTL_MOVE_SEMANTICS_ENABLED
    pair& operator=(pair&& p) EA_NOEXCEPT_IF(eastl::is_nothrow_move_assignable<T1>::value&&
                                                 eastl::is_nothrow_move_assignable<T2>::value) {
    ...
    void swap(pair& p) EA_NOEXCEPT_IF(
        eastl::is_nothrow_swappable<T1>::value&& eastl::is_nothrow_swappable<T2>::value) {
...
#if defined(EA_COMPILER_NO_FUNCTION_TEMPLATE_DEFAULT_ARGS) || \
    !EASTL_MOVE_SEMANTICS_ENABLED  // We aren't concerned about the noexcept conformance above.
...
#define EASTL_PAIR_CONFORMANCE 1
  ...
  ///
  template <typename T>
  struct use_self  // : public unary_function<T, T> // Perhaps we want to make it a subclass of
                   // unary_function.
  {
  ...
  struct use_first  // : public unary_function<Pair, typename Pair::first_type> // Perhaps we want
                    // to make it a subclass of unary_function.
  {
    typedef Pair                      argument_type;
    typedef typename Pair::first_type result_type;

    const result_type& operator()(const Pair& x) const { return x.first; }
  };
  ...
  ///
  template <typename Pair>
  struct use_second  // : public unary_function<Pair, typename Pair::second_type> // Perhaps we want
                     // to make it a subclass of unary_function.
  {
    typedef Pair                       argument_type;
    typedef typename Pair::second_type result_type;

    const result_type& operator()(const Pair& x) const { return x.second; }
  };
...
///
  ...
  EA_CPP14_CONSTEXPR inline pair<
      typename eastl::remove_reference_wrapper<typename eastl::decay<T1>::type>::type,
...
// Without the following, VC++ fails to compile code like this: pair<const char*, int> p =
// eastl::make_pair<const char*, int>("hello", 0); We define a const reference version alternative
// to the above. "hello" is of type char const(&)[6] (array of 6 const chars), but VC++ decays it to
// const char* and allows this make_pair to be called with that. VC++ fails below with
// make_pair("hello", "people") because you can't assign arrays and until we have a better solution
// we just disable this make_pair specialization for when T1 or T2 are of type char const(&)[].
#if defined(_MSC_VER)
  template <typename T1, typename T2>
  EA_CPP14_CONSTEXPR inline pair<T1, T2> make_pair(
      const T1& a, const T2& b,
      typename eastl::enable_if<!eastl::is_array<T1>::value && !eastl::is_array<T2>::value>::type* =
          0) {
