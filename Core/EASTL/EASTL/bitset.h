...
#define EASTL_DISABLE_BITSET_ARRAYBOUNDS_WARNING 0
  ...
  struct BitsetBase<1, WordType> {
   ...
   public:
    word_type
        mWord[1];  // Defined as an array of 1 so that bitset can treat this BitsetBase like others.
   ...
   public:
    ...
    size_type DoFindPrev(
        size_type last_find) const;  // Returns 1 * kBitsPerWord (the bit count) if no bits are set.
  ...
  struct BitsetBase<2, WordType> {
   ...
   public:
    ...
    size_type DoFindPrev(
        size_type last_find) const;  // Returns 2 * kBitsPerWord (the bit count) if no bits are set.
  ...
  class bitset : private BitsetBase<BITSET_WORD_COUNT(N, WordType), WordType> {
   public:
    ...
    enum {
      ...
      kWordSize =
          sizeof(word_type),  // The size of individual words the bitset uses to hold the bits.
      kWordCount =
          BITSET_WORD_COUNT(N, WordType)  // The number of words the bitset uses to hold the bits.
...
#define EASTL_BITSET_COUNT_STRING "\0\1\1\2\1\2\2\3\1\2\2\3\2\3\3\4"
  ...
  inline uint32_t GetFirstBit(
      uint16_t x)  // To do: Update this to use VC++ _BitScanForward, _BitScanForward64; GCC
  ...
  inline uint32_t GetLastBit(eastl_uint128_t x) {
    if (x) {
      ...
      eastl_uint128_t mask(UINT64_C(
          0xFFFFFFFF00000000));  // There doesn't seem to exist compiler support for INT128_C() by
  ...
  */
  ...
  template <size_t NW, typename WordType>
  inline typename BitsetBase<NW, WordType>::word_type& BitsetBase<NW, WordType>::DoGetWord(
      size_type i) {
  ...
  template <size_t NW, typename WordType>
  inline typename BitsetBase<NW, WordType>::size_type BitsetBase<NW, WordType>::DoFindFirst()
      const {
  ...
  template <size_t NW, typename WordType>
  inline typename BitsetBase<NW, WordType>::size_type BitsetBase<NW, WordType>::DoFindNext(
  ...
  template <size_t NW, typename WordType>
  inline typename BitsetBase<NW, WordType>::size_type BitsetBase<NW, WordType>::DoFindPrev(
      size_type last_find) const {
    if (last_find > 0) {
      // Set initial state based on last find.
      size_type word_index = static_cast<size_type>(last_find >> kBitsPerWordShift);
  ...
  */
...
#if EASTL_EXCEPTIONS_ENABLED
#if ((EA_PLATFORM_WORD_SIZE > 4) && \
     defined(EA_PLATFORM_MICROSOFT))  // If we are using 64 bit words but ulong is less than 64
                                      // bits... Microsoft platforms alone use a 32 bit long under
                                      // 64 bit platforms.
  ...
  template <typename WordType>
  inline typename BitsetBase<1, WordType>::word_type& BitsetBase<1, WordType>::DoGetWord(
      size_type) {
    return mWord[0];
  }
  ...
  template <typename WordType>
  inline typename BitsetBase<1, WordType>::size_type BitsetBase<1, WordType>::DoFindNext(
      size_type last_find) const {
  ...
  template <typename WordType>
  inline typename BitsetBase<1, WordType>::size_type BitsetBase<1, WordType>::DoFindPrev(
      size_type last_find) const {
    if (last_find > 0) {
      // Mask off previous bits of word so our search becomes a "find first".
  ...
  */
  ...
  inline void BitsetBase<2, WordType>::operator<<=(size_type n) {
    if (n)  // to avoid a shift by kBitsPerWord, which is undefined
    {
      ...
      mWord[1] =
          (mWord[1] << n) | (mWord[0] >> (kBitsPerWord - n));  // Intentionally use | instead of +.
      mWord[0] <<= n;
      // We let the parent class turn off any upper bits.
    }
  }
  ...
  inline void BitsetBase<2, WordType>::operator>>=(size_type n) {
    if (n)  // to avoid a shift by kBitsPerWord, which is undefined
    {
      ...
      mWord[0] =
          (mWord[0] >> n) | (mWord[1] << (kBitsPerWord - n));  // Intentionally use | instead of +.
      mWord[1] >>= n;
    }
  }
  ...
  template <typename WordType>
  inline typename BitsetBase<2, WordType>::word_type& BitsetBase<2, WordType>::DoGetWord(
      size_type i) {
    return mWord[i >> kBitsPerWordShift];
  }
  ...
  template <typename WordType>
  inline typename BitsetBase<2, WordType>::size_type BitsetBase<2, WordType>::DoFindNext(
      size_type last_find) const {
  ...
  template <typename WordType>
  inline typename BitsetBase<2, WordType>::size_type BitsetBase<2, WordType>::DoFindPrev(
      size_type last_find) const {
  ...
  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::reference& bitset<N, WordType>::reference::operator=(
      bool value) {
    if (value)
  ...
  inline bitset<N, WordType>::bitset(uint32_t value) : base_type(value) {
    if ((N & kBitsPerWordMask) ||
        (N == 0))  // If there are any high bits to clear... (If we didn't have this check, then the
                   // code below would do the wrong thing when N == 32.
  ...
  */
  ...
  inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::operator<<=(size_type n) {
    if (EASTL_LIKELY((intptr_t)n < (intptr_t)N)) {
      ...
      if ((N & kBitsPerWordMask) ||
          (N == 0))  // If there are any high bits to clear... (If we didn't have this check, then
                     // the code below would do the wrong thing when N == 32.
        mWord[kWordCount - 1] &=
  ...
  inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::set() {
    ...
    if ((N & kBitsPerWordMask) ||
      ...
      mWord[kWordCount - 1] &=
          ~(~static_cast<word_type>(0)
            << (N & kBitsPerWordMask));  // This clears any high unused bits. We need to do this so
                                         // that shift operations proceed correctly.
    return *this;
  }

  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::set(size_type i,
                                                                           bool      value) {
    if (i < N)
