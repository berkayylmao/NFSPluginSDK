...
#include <Core/EASTL/EABase/version.h>
...
// would have put this into the compilertraits header.
    ...
    (defined(EA_COMPILER_IS_C99) || defined(INT8_MIN) || defined(EA_COMPILER_HAS_C99_TYPES) || \
...
#pragma warning(pop)
                                  ...
                                  defined(_FEVAL))  // GCC 3.x defines __FLT_EVAL_METHOD__ instead
...
#ifndef INT8_C
#define INT8_C(x) \
  int8_t(x)  // x##i8 doesn't work satisfactorilly because -128i8 generates an out of range warning.
#endif
#ifndef UINT8_C
...
#if defined(EA_COMPILER_MSVC) || \
    defined(EA_COMPILER_BORLAND)  // VC++ 7.1+ understands long long as a data type but doesn't
                                  // accept %ll as a printf specifier.
...
#define EA_CHAR16_NATIVE 1
    ...
    (defined(EA_COMPILER_CPP11_ENABLED) ||                                                       \
     defined(__STDC_VERSION__))  // g++ (C++ compiler) 4.4+ with -std=c++0x or gcc (C compiler) 4.4+
                                 // with -std=gnu99
#define EA_CHAR16_NATIVE 1
#else
#define EA_CHAR16_NATIVE 0
...
#if (defined(_MSC_VER) && \
     (_MSC_VER >= 1600))  // if VS2010+ or using platforms that use Dinkumware under a compiler that
                          // doesn't natively support C++11 char16_t.
#if !defined(_CHAR16T)
#define _CHAR16T
#endif
...
#define static_assert(expression, description)      \
  typedef char EA_STATIC_ASSERT_CONCATENATE_HELPER( \
      compileTimeAssert,                            \
      __COUNTER__)[((expression) != 0) ? 1 : -1] EA_STATIC_ASSERT_UNUSED_ATTRIBUTE
#else
#define static_assert(expression, description)      \
  typedef char EA_STATIC_ASSERT_CONCATENATE_HELPER( \
