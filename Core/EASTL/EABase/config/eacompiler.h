...
#include <Core/EASTL/EABase/config/eaplatform.h>
...
#define EA_COMPILER_CPP11_ENABLED 1
#elif defined( \
    _MSC_VER)  // Microsoft unilaterally enables its C++11 support; there is no way to disable it.
#define EA_COMPILER_CPP11_ENABLED 1
...
#define EA_COMPILER_STRING \
  EA_COMPILER_NAME         \
      " compiler, version " INTERNAL_STRINGIZE(__GNUC__) "." INTERNAL_STRINGIZE(__GNUC_MINOR__)
...
#if (__GNUC__ == 3) &&        \
    ((__GNUC_MINOR__ == 1) || \
     (__GNUC_MINOR__ == 2))  // If GCC 3.1 or 3.2 (but not pre 3.1 or post 3.2)...
...
// Should we enable the following? We probably should do so since enabling it does a lot more good
// than harm for users. The Intel Windows compiler does a pretty good job of emulating VC++ and so
// the user would likely have to handle few special cases where the Intel compiler doesn't emulate
// VC++ correctly.
#if defined(_MSC_VER)
...
// Should we enable the following? This isn't as clear because as of this writing we don't know if
// the Intel compiler truly emulates GCC well enough that enabling this does more good than harm.
#if defined(__GNUC__)
#define EA_COMPILER_GNUC 1
#endif
...
#if defined(_MSC_VER)
#define EA_COMPILER_STRING \
  EA_COMPILER_NAME " compiler, version " INTERNAL_STRINGIZE( EA_COMPILER_VERSION ) ", EDG version " INTERNAL_STRINGIZE( __EDG_VERSION__ ) ", VC++ version " INTERNAL_STRINGIZE( _MSC_VER )
#elif defined(__GNUC__)
#define EA_COMPILER_STRING \
  EA_COMPILER_NAME " compiler, version " INTERNAL_STRINGIZE( EA_COMPILER_VERSION ) ", EDG version " INTERNAL_STRINGIZE( __EDG_VERSION__ ) ", GCC version " INTERNAL_STRINGIZE( __GNUC__ )
#else
#define EA_COMPILER_STRING                                   \
...
#define EA_COMPILER_NO_MEMBER_TEMPLATE_FRIENDS \
  1  // The compiler fails to compile such cases. Fixed in VC7.1
//#define EA_COMPILER_NO_MEMBER_TEMPLATES 1                       // VC6.x supports member templates
//properly 95% of the time. So do we flag the remaining 5%? #define
//EA_COMPILER_NO_MEMBER_TEMPLATE_SPECIALIZATION 1         // VC6.x supports member templates
//properly 95% of the time. So do we flag the remaining 5%?
...
//#define EA_COMPILER_NO_MEMBER_TEMPLATES 1                       // VC7.0 supports member templates
//properly 95% of the time. So do we flag the remaining 5%?
...
#define EA_COMPILER_MSVC_2011 \
  1  // Microsoft changed the name to VS2012 before shipping, despite referring to it as VS2011 up
     // to just a few weeks before shipping.
...
#define EA_COMPILER_STRING \
  EA_COMPILER_NAME " compiler, version " INTERNAL_STRINGIZE(EA_COMPILER_VERSION)
#endif
...
///////////////////////////////////////////////////////////////////////////////
    ...
    (!defined(__i386__) || defined(__x86_64__)) && !defined(__ppc__) && !defined(__PPC__) && \
    !defined(__PPC64__)
...
//
    ...
    defined(_NO_EX)  // GCC on some platforms defines _NO_EX when exceptions are disabled.
#define EA_COMPILER_NO_EXCEPTIONS 1

#elif (defined(EA_COMPILER_CLANG) || defined(EA_COMPILER_GNUC) || defined(EA_COMPILER_INTEL) || \
       defined(EA_COMPILER_RVCT)) &&                                                            \
    !defined(__EXCEPTIONS)  // GCC and most EDG-based compilers define __EXCEPTIONS when exception
                            // handling is enabled.
#define EA_COMPILER_NO_EXCEPTIONS 1
...
#define EA_DISABLE_ALL_VC_WARNINGS()           \
  __pragma(warning(push, 0)) __pragma(warning( \
      disable : 4244 4265 4267 4350 4472 4509 4548 4710 4985 6320 4755))  // Some warnings need to
...
#if defined(_MSC_VER)
// Disabled warning "nonstandard extension used: 'throw (...)'" as this warning is a W4 warning
// which is usually off by default and doesn't convey any important information but will still
// complain when building with /Wall (which most teams do)
#define EA_THROW_SPEC_NEW(X) \
  __pragma(warning(push)) __pragma(warning(disable : 4987)) _THROWS(X) __pragma(warning(pop))
#else
#define EA_THROW_SPEC_NEW(X) _THROW1(X)
#endif
#define EA_THROW_SPEC_NEW_NONE()    _THROW0()
#define EA_THROW_SPEC_DELETE_NONE() _THROW0()
#elif (defined(EA_COMPILER_NO_EXCEPTIONS) || defined(_MSL_NO_THROW_SPECS)) && \
    !defined(EA_COMPILER_RVCT) && !defined(EA_PLATFORM_LINUX) && !defined(EA_PLATFORM_APPLE)
...
// supported. Earlier EDG supported a subset of rvalue references. Implicit move constructors and
// assignment operators aren't supported until EDG 4.5.
...
// supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && defined(__APPLE__) && \
    (EA_COMPILER_VERSION >= 401)
// Extern template is supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    !defined(__APPLE__)  // Clang other than Apple's Clang
// Extern template is supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
...
#if defined(EA_COMPILER_CPP11_ENABLED) && \
    (defined(_MSC_VER) &&                 \
     (EA_COMPILER_VERSION >= 1900))  // VS2015+... Not present in VC++ up to and including VS2013.
                                     // supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
...
//
    ...
    (EA_COMPILER_VERSION >                                     \
     1600)  // VC++ > VS2010, even without C++11 support. VS2010 does support override, however will
            // generate warnings due to the keyword being 'non-standard'
            // supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
...
// supported, though you need to use EA_INHERITANCE_FINAL for it to work with VS versions prior to
// 2012.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 209)  // Clang 2.9+
// supported
...
// supported, but as of VS2013 it isn't supported for defaulted move constructors and move
// assignment operators.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 401)  // EDG 4.1+.
// supported, but as of EDG 4.3 it isn't supported for defaulted move constructors and move
// assignment operators until EDG 4.5.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
...
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && (EA_COMPILER_VERSION >= 401) && \
    defined(__APPLE__)
// supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && (EA_COMPILER_VERSION >= 301) && \
    !defined(__APPLE__)  // Clang 3.1+, not including Apple's Clang.
// supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4004)  // GCC 4.4+
// supported.
...
//
// Refers to C++11 strongly typed enums, which includes enum classes and sized enums. Doesn't
// include forward-declared enums.
//
...
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 405)  // EDG 4.5+.
// supported. EDG 4.3 supports basic forward-declared enums, but not forward-declared strongly typed
// enums.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && (EA_COMPILER_VERSION >= 401) && \
    defined(__APPLE__)
// supported.
...
// supported.
    ...
    (__EDG_VERSION__ >= 402)  // EDG 4.2+.
// supported, though 4.1 has partial support for variadic templates.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && (EA_COMPILER_VERSION >= 300) && \
    !defined(__APPLE__)  // Clang 3.0+, not including Apple's Clang.
// supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4007)  // GCC 4.7+
// supported, though GCC 4.3 has partial support for variadic templates.
...
// This refers to the compiler support for this and not the Standard Library support
...
// supported via __declspec(noreturn). You need to use that or EA_NORETURN. VC++ up to VS2013
// doesn't support any C++11 attribute types.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 402)  // EDG 4.2+.
// supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && (EA_COMPILER_VERSION >= 401) && \
...
#if defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && (EA_COMPILER_VERSION >= 401) && \
    defined(__APPLE__)  // Apple clang 4.1+
                        // supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 402)  // EDG 4.2+.
...
//#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && (EA_COMPILER_VERSION >= 4008) //
//GCC 4.8+
//    // supported.
#else
...
// Not supported by VC++ as of VS2013, though EA_ALIGN_OF is supported on all coompilers as an
// alternative.
...
// Not supported by VC++ as of VS2013.
    ...
    defined(__APPLE__)  // Apple clang 4.1+
                        // supported.
// Versions of EDG prior to 4.5 only support extended sizeof in non-member functions. Full support
// was added in 4.5
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 405)  // EDG 4.5+.
// supported.
...
#pragma warning(disable : 4836)   // nonstandard extension used: local types or unnamed types cannot
...
//
// Refers to char16_t and char32_t as true native types (and not something simply typedef'd from
// uint16_t and uint32_t). http://en.cppreference.com/w/cpp/language/types
//
#if !defined(EA_COMPILER_NO_NEW_CHARACTER_TYPES)
#if defined(EA_COMPILER_NO_UNICODE_STRING_LITERALS)  // Some compilers have had support for char16_t
                                                     // prior to support for u"", but it's not
                                                     // useful to have the former without the
                                                     // latter.
...
// VC++ up till at least VS2013 supports \u and \U but supports them wrong with respect to the C++11
// Standard.
...
//#if defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && __has_feature(cxx_thread_local) &&
//_______
//    // thread_local requires a cooperating standard library, and we yet don't have a means to
