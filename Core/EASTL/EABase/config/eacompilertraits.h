...
#include <Core/EASTL/EABase/config/eacompiler.h>
...
// Metrowerks defines C99 types (e.g. intptr_t) instrinsically when in C99 mode (-lang C99 on the
// command line).
...
//
#if defined(__GNUC__)  // We can't use GCC 4's __builtin_offsetof because it mistakenly complains
                       // about non-PODs that are really PODs.
#define EA_OFFSETOF(struct_, member_)                                                              \
  ((size_t)(((uintptr_t) & reinterpret_cast<const volatile char&>((((struct_*)65536)->member_))) - \
            65536))
#else
...
// access rules still apply. We offer two approaches depending on the compiler's support for
// non-static member initializers although most C++11 compilers support this.
//
// Example usage:
//     struct A{ int x; int y; };
...
//    EA_ALIGN_MAX_STATIC       // The max align value that the compiler will respect for EA_ALIGN
//    for static data (global and static variables). Some compilers allow high values, some allow no
//    more than 8. EA_ALIGN_MIN is assumed to be 1. EA_ALIGN_MAX_AUTOMATIC    // The max align value
//    for automatic variables (variables declared as local to a function). EA_ALIGN(n) // Used as a
//    prefix. n is byte alignment, with being a power of two. Most of the time you can use this and
//    avoid using EA_PREFIX_ALIGN/EA_POSTFIX_ALIGN. EA_ALIGNED(t, v, n)       // Type, variable,
//    alignment. Used to align an instance. You should need this only for unusual compilers.
//    EA_PACKED                 // Specifies that the given structure be packed (and not have its
//    members aligned).
//
// Also we define the following for rare cases that it's needed.
//    EA_PREFIX_ALIGN(n)        // n is byte alignment, with being a power of two. You should need
//    this only for unusual compilers. EA_POSTFIX_ALIGN(n)       // Valid values for n are 1, 2, 4,
//    8, etc. You should need this only for unusual compilers.
//
// Example usage:
//    size_t x = EA_ALIGN_OF(int);                                  Non-aligned equivalents. Meaning
//    EA_PREFIX_ALIGN(8) int x = 5;                                 int x = 5; Align x on 8 for
//    compilers that require prefix attributes. Can just use EA_ALIGN instead. EA_ALIGN(8) int x;
//    int x;                          Align x on 8 for compilers that allow prefix attributes. int x
//    EA_POSTFIX_ALIGN(8);                                    int x;                          Align
//    x on 8 for compilers that require postfix attributes. int x EA_POSTFIX_ALIGN(8) = 5; int x =
//    5;                      Align x on 8 for compilers that require postfix attributes. int x
//    EA_POSTFIX_ALIGN(8)(5);                                 int x(5);                       Align
//    x on 8 for compilers that require postfix attributes. struct EA_PREFIX_ALIGN(8) X { int x; }
//    EA_POSTFIX_ALIGN(8);   struct X { int x; };            Define X as a struct which is aligned
//    on 8 when used. EA_ALIGNED(int, x, 8) = 5;                                    int x = 5; Align
//    x on 8. EA_ALIGNED(int, x, 16)(5);                                    int x(5); Align x on 16.
//    EA_ALIGNED(int, x[3], 16);                                    int x[3]; Align x array on 16.
//    EA_ALIGNED(int, x[3], 16) = { 1, 2, 3 };                      int x[3] = { 1, 2, 3 }; Align x
//    array on 16. int x[3] EA_PACKED;                                           int x[3]; Pack the
//    3 ints of the x array. GCC doesn't seem to support packing of int arrays. struct EA_ALIGN(32)
//    X { int x; int y; };                      struct X { int x; };            Define A as a struct
//    which is aligned on 32 when used. EA_ALIGN(32) struct X { int x; int y; } Z; struct X { int x;
//    } Z;          Define A as a struct, and align the instance Z on 32. struct X { int x
//    EA_PACKED; int y EA_PACKED; };               struct X { int x; int y; };     Pack the x and y
//    members of struct X. struct X { int x; int y; } EA_PACKED;                         struct X {
//    int x; int y; };     Pack the members of struct X. typedef EA_ALIGNED(int, int16, 16); int16
//    n16;                typedef int int16; int16 n16;   Define int16 as an int which is aligned
//    on 16. typedef EA_ALIGNED(X, X16, 16); X16 x16;                      typedef X X16; X16 x16;
//    Define X16 as an X which is aligned on 16.

#if !defined(EA_ALIGN_MAX)     // If the user hasn't globally set an alternative value...
#if defined(EA_PROCESSOR_ARM)  // ARM compilers in general tend to limit automatic variables to 8 or
                               // less.
...
#define EA_PACKED                              __packed
       ...
       // If the given platform/compiler doesn't support alignment specifications, then it's
       // somewhat likely that alignment doesn't matter for that platform. Otherwise they would have
       // defined functionality to manipulate alignment.
#define EA_ALIGN(n)
#define EA_PREFIX_ALIGN(n)
#define EA_POSTFIX_ALIGN(n)
...
#if defined(__GNUC__) && \
    !defined(__EDG__)  // EDG typically #defines __GNUC__ but doesn't implement init_priority.
...
#define EA_ANALYSIS_ASSUME(x) \
  __analysis_assume(!!(x))  // !! because that allows for convertible-to-bool in addition to bool.
#else
#define EA_ANALYSIS_ASSUME(x)
#endif
#endif
...
#if defined(EA_COMPILER_GNUC) && \
    (EA_COMPILER_VERSION >= 4006)  // Can't test directly for __GNUC__ because some compilers lie.
#define EA_DISABLE_GCC_WARNING(w) _Pragma("GCC diagnostic push") _Pragma(EAGCCWHELP2(w))
...
#define EA_DISABLE_CLANG_WARNING(w) _Pragma("clang diagnostic push") _Pragma(EACLANGWHELP2(w))
#else
...
#define EA_DISABLE_EDG_WARNING(w) _Pragma("control %push diag") _Pragma(EAEDGWHELP1(w))
#else
...
//
// Example usage:
//     // Currently we are limited to one warning per line and must also specify the warning in the
//     restore macro. EA_DISABLE_CW_WARNING(10317) EA_DISABLE_CW_WARNING(10324) <code>
...
#define EA_WCHAR_T_NON_NATIVE 1
    ...
    (!defined(_WCHAR_T) &&        \
     (__EDG_VERSION__ <           \
      400))  // EDG prior to v4 uses _WCHAR_T to indicate if wchar_t is native. v4+ may define
             // something else, but we're not currently aware of it.
#define EA_WCHAR_T_NON_NATIVE 1
#endif
#endif
...
#define EA_SSE 2
#elif defined(EA_PROCESSOR_X86) && defined(_MSC_FULL_VER) && !defined(__NOSSE__) && \
    defined(_M_IX86_FP)
...
#endif
#endif
// EA_FP16C may be used to determine the existence of float <-> half conversion operations on an x86
// CPU. (For example to determine if _mm_cvtph_ps or _mm_cvtps_ph could be used.)
#ifndef EA_FP16C
...
#else
#define EA_ONCE()  // _Pragma("once")   It turns out that _Pragma("once") isn't supported by many
                   // compilers.
#endif
#endif
...
// You can use EA_FINAL_OVERRIDE to combine usage of EA_OVERRIDE and EA_INHERITANCE_FINAL in a
// single statement.
//
// Example usage:
...
// You can use EA_FINAL_OVERRIDE to combine usage of EA_OVERRIDE and EA_INHERITANCE_FINAL in a
// single statement. This is not called EA_FINAL because that term is used within EA to denote
// debug/release/final builds.
//
// Example usage:
...
//
// See http://www.cprogramming.com/c++11/c++11-compile-time-processing-with-constexpr.html for more
...
struct EANonCopyable {
    ...
    defined(                                       \
        __EDG__)  // EDG doesn't appear to behave properly for the case of defaulted constructors;
...
#elif defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION > 4004) && \
    (defined(__i386__) ||                                          \
     defined(__x86_64__))  // GCC 4.4+ - Seems to work only on x86/Linux so far. However, GCC 4.4
                           // itself appears broken and screws up parameter passing conventions.
#define EA_OPTIMIZE_OFF() _Pragma("GCC push_options") _Pragma("GCC optimize 0")
#elif defined(EA_COMPILER_CLANG) && \
    !defined(EA_PLATFORM_ANDROID)  // android clang 305 compiler crashes when this pragma is used
