...
// Unix doesn't have Microsoft's <io.h> but has the same functionality in <fcntl.h> and
// <sys/stat.h>.
...
#if (defined(EA_PLATFORM_UNIX) &&                                        \
     !(defined(CS_UNDEFINED_STRING) && defined(EA_PLATFORM_CONSOLE))) || \
...
#define EA_HAVE_PTHREAD_H                                                                   \
  1 /* It can be had under Microsoft/Windows with the http://sourceware.org/pthreads-win32/ \
       library */
#else
...
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(EA_HAVE_LIBSTDCPP_LIBRARY) && \
    defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4005)
#define EA_HAVE_CPP11_FUTURE 1
#elif defined(EA_HAVE_LIBCPP_LIBRARY) && (_LIBCPP_VERSION >= 1)
#define EA_HAVE_CPP11_FUTURE 1
#else
...
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(EA_HAVE_LIBSTDCPP_LIBRARY) && \
    defined(EA_COMPILER_CLANG) && (EA_COMPILER_VERSION >= 301) &&                 \
    !defined(EA_COMPILER_NO_INITIALIZER_LISTS) && !defined(EA_PLATFORM_APPLE)
#define EA_HAVE_CPP11_INITIALIZER_LIST 1
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(EA_HAVE_LIBSTDCPP_LIBRARY) && \
    defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4004) &&                 \
    !defined(EA_COMPILER_NO_INITIALIZER_LISTS) && !defined(EA_PLATFORM_APPLE)
#define EA_HAVE_CPP11_INITIALIZER_LIST 1
#elif defined(EA_HAVE_LIBCPP_LIBRARY) && (_LIBCPP_VERSION >= 1) && \
...
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(EA_HAVE_LIBSTDCPP_LIBRARY) && \
    defined(EA_COMPILER_CLANG) && (EA_COMPILER_VERSION >= 301) && !defined(EA_PLATFORM_APPLE)
#define EA_HAVE_CPP11_SYSTEM_ERROR 1
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(EA_HAVE_LIBSTDCPP_LIBRARY) && \
    defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4004) && !defined(EA_PLATFORM_APPLE)
#define EA_HAVE_CPP11_SYSTEM_ERROR 1
#elif defined(EA_HAVE_LIBCPP_LIBRARY) && (_LIBCPP_VERSION >= 1)
#define EA_HAVE_CPP11_SYSTEM_ERROR 1
#else
...
//#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(EA_HAVE_LIBSTDCPP_LIBRARY) &&
//defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4008)
//    #define EA_HAVE_CPP11_CODECVT 1
#elif defined(EA_HAVE_LIBCPP_LIBRARY) && (_LIBCPP_VERSION >= 1)
#define EA_HAVE_CPP11_CODECVT 1
#else
...
#if defined(EA_PLATFORM_POSIX) /* Posix means Linux, Unix, and Macintosh OSX, among others \
                                  (including Linux-based mobile platforms). */
...
#define EA_HAVE_ISNAN(x)                                                                           \
  __builtin_isnan(x) /* There are a number of standard libraries for Android and it's hard to tell \
                        them apart, so just go with builtins */
...
#define EA_HAVE_WCHAR_IMPL                                                                       \
  1 /* Specifies if wchar_t string functions are provided, such as wcslen, wprintf, etc. Implies \
       EA_HAVE_WCHAR_H */
#else
...
#define EA_HAVE_inet_ntop_IMPL                                                                    \
  1 /* This doesn't identify if the platform SDK has some alternative function that does the same \
       thing; */
#define EA_HAVE_inet_pton_IMPL                                                                  \
  1 /* it identifies strictly the <arpa/inet.h> inet_ntop and inet_pton functions. For example, \
       Microsoft has InetNtop in <Ws2tcpip.h> */
#else
...
#define EA_HAVE_getcwd_IMPL                                                                      \
  1 /* With Microsoft compilers you may need to use _getcwd, as they have deprecated getcwd. And \
       in any case it's present at <direct.h> */
#else
...
// This EA_HAVE is deprecated, as EA_COMPILER_NO_NULLPTR is more appropriate, given that nullptr is
// a compiler-level feature and not a library feature.
...
// The EDG compiler provides nullptr, but uses an older standard library that doesn't support
// std::nullptr_t.
...
// <functional>: std::function, std::mem_fn, std::bad_function_call, std::is_bind_expression,
// std::is_placeholder, std::reference_wrapper, std::hash, std::bind, std::ref, std::cref.
...
// <exception> std::current_exception, std::rethrow_exception, std::exception_ptr,
...
/*
        bcopy   -- Just use memmove or some customized equivalent. bcopy offers no practical
   benefit. strlcpy -- So few platforms have this built-in that we get no benefit from using it. Use
   EA::StdC::Strlcpy instead. strlcat -- "
*/
