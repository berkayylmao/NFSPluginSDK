...
namespace eastl {
  ...
  class intrusive_ptr {
   ...
   public:
    ...
    intrusive_ptr(T* p, bool bAddRef = true) : mpObject(p) {
      if (mpObject && bAddRef)
        intrusive_ptr_add_ref(mpObject);  // Intentionally do not prefix the call with eastl:: but instead allow
                                          // namespace lookup to resolve the namespace.
    }

    /// intrusive_ptr
    /// Construction from self type.
    intrusive_ptr(const intrusive_ptr& ip) : mpObject(ip.mpObject) {
      if (mpObject) intrusive_ptr_add_ref(mpObject);
    }
    ...
    ~intrusive_ptr() {
      if (mpObject) intrusive_ptr_release(mpObject);
    }
    ...
    ///    if(!ptr)
    ///        assert(false);
    bool operator!() const { return (mpObject == NULL); }
