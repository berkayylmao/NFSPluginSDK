...
#include <OpenSpeed/Core/EASTL/EASTL/internal/in_place_t.h>
#include <OpenSpeed/Core/EASTL/EASTL/internal/piecewise_construct_t.h>
...
#define EASTL_TYPE_TRAIT_is_nothrow_swappable_CONFORMANCE EASTL_TYPE_TRAIT_is_swappable_CONFORMANCE
  ...
  template <typename T, typename U>
  struct is_swappable_with : public eastl::bool_constant<is_swappable_with_helper<T, U>::value> {};
...
#define EASTL_TYPE_TRAIT_is_nothrow_swappable_with_CONFORMANCE 1
  ...
  struct is_nothrow_swappable_with_helper {
    // Don't prefix swap with eastl:: as we want to allow user-defined swaps via argument-dependent lookup.
    static const bool value = noexcept(swap(eastl::declval<T>(), eastl::declval<U>()))&& noexcept(
  ...
  inline ForwardIterator2 swap_ranges(ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2) {
    for (; first1 != last1; ++first1, ++first2)
      iter_swap(first1, first2);  // Don't prefix swap with eastl:: as we want to allow user-defined swaps via
  ...
  struct pair {
    ...
    template <
        typename TT1 = T1, typename TT2 = T2,
        class = eastl::enable_if_t<eastl::is_default_constructible_v<TT1> && eastl::is_default_constructible_v<TT2>>>
    EA_CONSTEXPR pair() : first(), second() {}

#if EASTL_ENABLE_PAIR_FIRST_ELEMENT_CONSTRUCTOR
    template <typename TT1 = T1, typename TT2 = T2,
              typename = eastl::enable_if_t<eastl::is_default_constructible_v<TT2>>>
    EA_CPP14_CONSTEXPR pair(const TT1& x) : first(x), second() {}
...
// error: with 'eastl::pair<T1, T2>::pair(const T1&) [with T1 = const int&; T2 = const int&]'
    ...
    EA_CPP14_CONSTEXPR pair(T1&& x) : first(eastl::move(x)), second() {}
#endif
#endif
...
// https://bugs.llvm.org/show_bug.cgi?id=38374
    ...
    EA_CPP14_CONSTEXPR pair(pair_first_construct_t, const TT1& x) : first(x), second() {}
...
// error: with 'eastl::pair<T1, T2>::pair(const T1&) [with T1 = const int&; T2 = const int&]'
    ...
    EA_CPP14_CONSTEXPR pair(pair_first_construct_t, T1&& x) : first(eastl::move(x)), second() {}
#endif
    ...
    EA_CPP14_CONSTEXPR pair(const pair<U, V>& p) : first(p.first), second(p.second) {}
    ...
    pair(eastl::piecewise_construct_t pwc, eastl::tuple<Args1...> first_args, eastl::tuple<Args2...> second_args) :
        pair(pwc, eastl::move(first_args), eastl::move(second_args), eastl::make_index_sequence<sizeof...(Args1)>(),
   ...
   private:
    ...
    pair(eastl::piecewise_construct_t, eastl::tuple<Args1...> first_args, eastl::tuple<Args2...> second_args,
         eastl::index_sequence<I1...>, eastl::index_sequence<I2...>) :
...
#define EASTL_PAIR_CONFORMANCE 1
  ...
  ///
  template <typename Pair>
  struct use_second  // : public unary_function<Pair, typename Pair::second_type> // Perhaps we want to make it a
...
// Without the following, VC++ fails to compile code like this: pair<const char*, int> p = eastl::make_pair<const char*,
// int>("hello", 0); We define a const reference version alternative to the above. "hello" is of type char const(&)[6]
// (array of 6 const chars), but VC++ decays it to const char* and allows this make_pair to be called with that. VC++
// fails below with make_pair("hello", "people") because you can't assign arrays and until we have a better solution we
// just disable this make_pair specialization for when T1 or T2 are of type char const(&)[].
#if defined(_MSC_VER)
  template <typename T1, typename T2>
