...
#include <OpenSpeed/Core/EASTL/EABase/eahave.h>

#include <OpenSpeed/Core/EASTL/EASTL/internal/integer_sequence.h>
#include <OpenSpeed/Core/EASTL/EASTL/internal/tuple_fwd_decls.h>
...
#define EASTL_TYPE_TRAIT_is_swappable_CONFORMANCE 1
  ...
  struct is_swappable
      : public integral_constant<
            bool, !eastl::is_same<decltype(swap(eastl::declval<T&>(), eastl::declval<T&>())), eastl::unused>::value> {
...
#define EASTL_TYPE_TRAIT_is_nothrow_swappable_CONFORMANCE EASTL_TYPE_TRAIT_is_swappable_CONFORMANCE
  ...
  struct is_nothrow_swappable_helper
      : public eastl::integral_constant<bool, is_nothrow_swappable_helper_noexcept_wrapper<T>::value> {
  };  // Don't prefix swap with eastl:: as we want to allow user-defined swaps via
      // argument-dependent lookup.
  ...
  template <typename T>
  struct is_nothrow_swappable : public eastl::is_nothrow_swappable_helper<T, eastl::is_swappable<T>::value> {};
#endif
  ...
  inline void iter_swap(ForwardIterator1 a, ForwardIterator2 b) {
    ...
    eastl::iter_swap_impl<eastl::type_and<eastl::is_same<value_type_a, value_type_b>::value,
  ...
  struct pair {
    ...
    EA_CPP14_CONSTEXPR pair(const pair<U, V>& p,
                            typename eastl::enable_if<eastl::is_convertible<const U&, T1>::value &&
    ...
    EA_CPP14_CONSTEXPR pair(U&& u, V&& v) : first(eastl::forward<U>(u)), second(eastl::forward<V>(v)) {}
...
#if (EABASE_VERSION_N >= 20040) && !defined(EA_COMPILER_NO_DEFAULTED_FUNCTIONS) && defined(_MSC_VER) && \
    (EA_COMPILER_VERSION >= 1900)
    pair(pair&& p) = default;
#else
    ...
    EA_CPP14_CONSTEXPR pair(pair&& p) EA_NOEXCEPT_IF(
        eastl::is_nothrow_move_constructible<T1>::value&& eastl::is_nothrow_move_constructible<T2>::value) :
        first(eastl::forward<T1>(p.first)), second(eastl::forward<T2>(p.second)) {}
#endif

#if defined(EA_COMPILER_NO_FUNCTION_TEMPLATE_DEFAULT_ARGS)
    template <typename U, typename V>
    EA_CPP14_CONSTEXPR pair(pair<U, V>&& p, typename eastl::enable_if<eastl::is_convertible<U, T1>::value &&
                                                                      eastl::is_convertible<V, T2>::value>::type* = 0) :
    ...
    template <typename U, typename V,
              typename = typename eastl::enable_if<eastl::is_convertible<U, T1>::value &&
                                                   eastl::is_convertible<V, T2>::value>::type>
    EA_CPP14_CONSTEXPR pair(pair<U, V>&& p) : first(eastl::forward<U>(p.first)), second(eastl::forward<V>(p.second)) {}
#endif
#endif
