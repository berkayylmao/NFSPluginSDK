...
#define EASTL_VECTOR_MAP_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " vector_map"  // Unless the user overrides something, this is "EASTL vector_map".
#endif
...
#define EASTL_VECTOR_MAP_DEFAULT_ALLOCATOR allocator_type(EASTL_VECTOR_MAP_DEFAULT_NAME)
  ...
  typename vector_map<K, T, C, A, RAC>::iterator vector_map<K, T, C, A, RAC>::insert(const_iterator    position,
    ...
    {
      if ((position == begin()) ||
          mValueCompare(*(position - 1), value))  // If the element before position is less than value...
  ...
  template <typename K, typename T, typename C, typename A, typename RAC>
  inline typename vector_map<K, T, C, A, RAC>::size_type vector_map<K, T, C, A, RAC>::count(const key_type& k) const {
  ...
  template <typename K, typename T, typename C, typename A, typename RAC>
  inline typename vector_map<K, T, C, A, RAC>::mapped_type& vector_map<K, T, C, A, RAC>::operator[](key_type&& k) {
    iterator itLB(lower_bound(k));

    if ((itLB == end()) || key_comp()(k, (*itLB).first)) itLB = insert(itLB, value_type(eastl::move(k), mapped_type()));
    return (*itLB).second;
  }
