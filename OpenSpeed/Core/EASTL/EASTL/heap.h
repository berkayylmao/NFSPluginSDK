...
//    push_heap     -- Adds an entry to a heap.                             Same as C++
//    std::push_heap. pop_heap      -- Removes the top entry from a heap.                   Same as
//    C++ std::pop_heap. make_heap     -- Converts an array to a heap.                         Same
//    as C++ std::make_heap. sort_heap     -- Sorts a heap in place. Same as C++ std::sort_heap.
//    remove_heap   -- Removes an arbitrary entry from a heap.
//    change_heap   -- Changes the priority of an entry in the heap.
//    is_heap       -- Returns true if an array appears is in heap format.   Same as C++11
//    std::is_heap. is_heap_until -- Returns largest part of the range which is a heap.    Same as
//    C++11 std::is_heap_until.
///////////////////////////////////////////////////////////////////////////////
...
namespace eastl {
  ...
  inline void promote_heap(RandomAccessIterator first, Distance topPosition, Distance position, const T& value) {
    for (Distance parentPosition =
             (position - 1) >> 1;  // This formula assumes that (position > 0). // We use '>> 1' instead of '/ 2'
                                   // because we have seen VC++ generate better code with >>.
         (position > topPosition) && (*(first + parentPosition) < value); parentPosition = (position - 1) >> 1) {
  ...
  ///    pop_heap(heap.begin(), heap.end());  // Moves heap[0] to the back of the heap and adjusts
  ///    the heap. heap.pop_back();                     // Remove value that was just at the top of
  ///    the heap
  ///
  template <typename RandomAccessIterator>
  ...
  void make_heap(RandomAccessIterator first, RandomAccessIterator last) {
    ...
    if (heapSize >= 2)  // If there is anything to do... (we need this check because otherwise the
                        // math fails below).
    {
      difference_type parentPosition = ((heapSize - 2) >> 1) + 1;  // We use '>> 1' instead of '/ 2' because we have
                                                                   // seen VC++ generate better code with >>.

      do {
        --parentPosition;
        const value_type temp(*(first + parentPosition));
        eastl::adjust_heap<RandomAccessIterator, difference_type, value_type>(first, parentPosition, heapSize,
                                                                              parentPosition, temp);
  ...
  void make_heap(RandomAccessIterator first, RandomAccessIterator last, Compare compare) {
                        ...
                        // math fails below).
    {
      ...
      do {
        --parentPosition;
        const value_type temp(*(first + parentPosition));
        eastl::adjust_heap<RandomAccessIterator, difference_type, value_type, Compare>(first, parentPosition, heapSize,
                                                                                       parentPosition, temp, compare);
      } while (parentPosition != 0);
    }
  }
  ...
  inline void change_heap(RandomAccessIterator first, Distance heapSize, Distance position) {
    ...
    eastl::promote_heap<RandomAccessIterator, difference_type, value_type>(first, (difference_type)0,
  ...
  inline RandomAccessIterator is_heap_until(RandomAccessIterator first, RandomAccessIterator last, Compare compare) {
    ...
    for (RandomAccessIterator child = first + 1; child < last; ++child, counter ^= 1) {
      if (compare(*first, *child)) return child;
      first += counter;  // counter switches between 0 and 1 every time through.
    }
