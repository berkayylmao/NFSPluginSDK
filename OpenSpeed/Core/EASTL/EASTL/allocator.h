...
#ifndef EASTL_USER_DEFINED_ALLOCATOR  // If the user hasn't declared that he has defined a different
                                      // allocator implementation elsewhere...
...
namespace eastl {
  ...
  inline Allocator* get_default_allocator(const Allocator*) {
    return NULL;  // By default we return NULL; the user must make specialization of this function
                  // in order to provide their own implementation.
  }

  inline EASTLAllocatorType* get_default_allocator(const EASTLAllocatorType*) {
    return EASTLAllocatorDefault();  // For the built-in allocator EASTLAllocatorType, we happen to
                                     // already have a function for returning the default allocator
                                     // instance, so we provide it.
  }
  ...
  inline void* allocate_memory(Allocator& a, size_t n, size_t alignment, size_t alignmentOffset) {
    ...
    if (alignment <= EASTL_ALLOCATOR_MIN_ALIGNMENT) {
      ...
      // EASTL_ALLOCATOR_MIN_ALIGNMENT and the minimum alignment of EASTLAlloc.  If there is a
      // mismatch it may be necessary to define EASTL_ALLOCATOR_MIN_ALIGNMENT to be the minimum
      // alignment of EASTLAlloc, or to increase the alignment of EASTLAlloc to match
      ...
      // Ensure the result is correctly aligned.  An assertion here may indicate a bug in the
      // allocator.
      EASTL_ASSERT((reinterpret_cast<size_t>(result) & ~(alignment - 1)) == reinterpret_cast<size_t>(result));
    }
    return result;
