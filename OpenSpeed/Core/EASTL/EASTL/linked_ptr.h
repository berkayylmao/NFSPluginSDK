...
namespace eastl {
  ...
  class linked_ptr : public linked_ptr_base {
   protected:
    ...
    template <typename U, typename D>
    void link(const linked_ptr<U, D>& linkedPtr) {  // This code can only be called when we are in a reset state.
   ...
   public:
    ...
    /// linked_ptr
    /// Default constructor.
    linked_ptr() : mpValue(NULL) { mpPrev = mpNext = this; }

    /// linked_ptr
    /// Takes ownership of the pointer. It is OK if the input pointer is null.
    template <typename U>
    explicit linked_ptr(U* pValue) : mpValue(pValue) {
      mpPrev = mpNext = this;
    }
    ...
    linked_ptr(const linked_ptr& linkedPtr) : mpValue(linkedPtr.mpValue) {
      if (mpValue)
        link(linkedPtr);
      else
        mpPrev = mpNext = this;
    ...
    template <typename U, typename D>
    linked_ptr(const linked_ptr<U, D>& linkedPtr) : mpValue(linkedPtr.mpValue) {
      if (mpValue)
        link(linkedPtr);
      else
        mpPrev = mpNext = this;
    ...
    linked_ptr& operator=(const linked_ptr& linkedPtr) {
      if (linkedPtr.mpValue != mpValue) {
        ...
        if (linkedPtr.mpValue) link(linkedPtr);
      }
      return *this;
    }
    ...
    T& operator*() const { return *mpValue; }
    ...
    bool unique() const { return (mpNext == static_cast<const linked_ptr_base*>(this)); }
    ...
    bool operator!() { return (mpValue == NULL); }
