...
//      - Because it simply locates an object within the list based on its node entry and doesn't perform before any
//      value-based searches or comparisons.
//
...
namespace eastl {
...
#endif
  } EASTL_MAY_ALIAS;  // It's not clear if this really should be needed. An old GCC compatible compiler is generating
                      // some crashing optimized code when strict aliasing is enabled, but analysis of it seems to blame
                      // the compiler. However, this topic can be tricky.
  ...
  class intrusive_list_iterator {
   ...
   public:
    ...
    explicit intrusive_list_iterator(
        pointer pNode);  // Note that you can also construct an iterator from T via this, since value_type == node_type.
  ...
  class intrusive_list_base {
   ...
   public:
    ...
    void pop_front();  ///< Removes an element from the front of the list; O(1). The element must exist, but is not
                       ///< deallocated.
    void pop_back();   ///< Removes an element from the back of the list; O(1). The element must exist, but is not
                       ///< deallocated.
  ...
  class intrusive_list : public intrusive_list_base {
   ...
   public:
    ...
    // intrusive_list(std::initializer_list<value_type> ilist); To consider: Is this feasible, given how
    // initializer_list works by creating a temporary array? Even if it is feasible, is it a good idea?
    ...
    void push_front(value_type& x);  ///< Adds an element to the front of the list; O(1). The element is not copied. The
                                     ///< element must not be in any other list.
    void push_back(value_type& x);   ///< Adds an element to the back of the list; O(1). The element is not copied. The
                                     ///< element must not be in any other list.

    bool contains(const value_type& x)
        const;  ///< Returns true if the given element is in the list; O(n). Equivalent to (locate(x) != end()).

    iterator locate(value_type& x);  ///< Converts a reference to an object in the list back to an iterator, or returns
                                     ///< end() if it is not part of the list. O(n)
    ...
    static void remove(
        value_type& value);  ///< Erases an element from a list; O(1). Note that this is static so you don't need to
                             ///< know which list the element, although it must be in some list.
  ...
  inline intrusive_list<T>::intrusive_list(const this_type& /*x*/) : intrusive_list_base() {
    ...
    // There should be a comment here about it, though my first guess is that this exists to quell VC++ level 4/-Wall
    // compiler warnings.
  }
