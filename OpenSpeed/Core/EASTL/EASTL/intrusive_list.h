...
//      - So as not to confuse the member function with the well-defined free function from
//      algorithm.h.
//      - Because it is not API-compatible with eastl::find().
//      - Because it simply locates an object within the list based on its node entry and doesn't
//      p[erform before any value-based searches or comparisons.
//
...
namespace eastl {
  ...
  struct intrusive_list_node {
    ...
    ~intrusive_list_node() {
      ...
      if (mpNext || mpPrev) EASTL_FAIL_MSG("~intrusive_list_node(): List is non-empty.");
#endif
    }
#endif
  } EASTL_MAY_ALIAS;  // It's not clear if this really should be needed. An old GCC compatible
                      // compiler is generating some crashing optimized code when strict aliasing is
                      // enabled, but analysis of it seems to blame the compiler. However, this
                      // topic can be tricky.
  ...
  class intrusive_list_iterator {
   ...
   public:
    intrusive_list_iterator();
    explicit intrusive_list_iterator(pointer pNode);  // Note that you can also construct an iterator from T via this,
                                                      // since value_type == node_type.
  ...
  class intrusive_list_base {
   ...
   public:
    ...
    void         pop_front();               ///< Removes an element from the front of the list; O(1). The element must
                                            ///< exist, but is not deallocated.
    void pop_back();                        ///< Removes an element from the back of the list; O(1). The element must
                                            ///< exist, but is not deallocated.
    EASTL_API void reverse() EA_NOEXCEPT;   ///< Reverses a list so that front and back are swapped; O(n).

    EASTL_API bool validate() const;  ///< Scans a list for linkage inconsistencies; O(n) time, O(1) space. Returns
                                      ///< false if errors are detected, such as loops or branching.
  ...
  class intrusive_list : public intrusive_list_base {
   ...
   public:
    ...
    // intrusive_list(std::initializer_list<value_type> ilist); To consider: Is this feasible, given
    // how initializer_list works by creating a temporary array? Even if it is feasible, is it a good
    // idea?
    ...
    reverse_iterator rbegin() EA_NOEXCEPT;              ///< Returns a reverse_iterator pointing at the end of
                                                        ///< the list (start of the reverse sequence).
    const_reverse_iterator rbegin() const EA_NOEXCEPT;  ///< Returns a const_reverse_iterator pointing at the end of the
                                                        ///< list (start of the reverse sequence).
    const_reverse_iterator crbegin() const EA_NOEXCEPT;  ///< Returns a const_reverse_iterator pointing at the end of
                                                         ///< the list (start of the reverse sequence).

    reverse_iterator rend() EA_NOEXCEPT;              ///< Returns a reverse_iterator pointing at the start of
                                                      ///< the list (end of the reverse sequence).
    const_reverse_iterator rend() const EA_NOEXCEPT;  ///< Returns a const_reverse_iterator pointing at the start of the
                                                      ///< list (end of the reverse sequence).
    const_reverse_iterator crend() const EA_NOEXCEPT;  ///< Returns a const_reverse_iterator pointing at the start of
                                                       ///< the list (end of the reverse sequence).
    ...
    void push_front(value_type& x);  ///< Adds an element to the front of the list; O(1). The element is not
                                     ///< copied. The element must not be in any other list.
    void push_back(value_type& x);   ///< Adds an element to the back of the list; O(1). The element
                                     ///< is not copied. The element must not be in any other list.

    bool contains(const value_type& x) const;  ///< Returns true if the given element is in the
                                               ///< list; O(n). Equivalent to (locate(x) != end()).

    iterator locate(value_type& x);  ///< Converts a reference to an object in the list back to an iterator, or
                                     ///< returns end() if it is not part of the list. O(n)
    const_iterator locate(
        const value_type& x) const;  ///< Converts a const reference to an object in the list back to a const iterator,
                                     ///< or returns end() if it is not part of the list. O(n)

    iterator insert(const_iterator pos,
                    value_type&    x);      ///< Inserts an element before the element pointed to by the iterator. O(1)
    iterator erase(const_iterator pos);  ///< Erases the element pointed to by the iterator. O(1)
    iterator erase(const_iterator pos,
                   const_iterator last);  ///< Erases elements within the iterator range [pos, last). O(1)
    ...
    static void remove(value_type& value);  ///< Erases an element from a list; O(1). Note that this
                                            ///< is static so you don't need to know which list the
                                            ///< element, although it must be in some list.
  ...
  template <typename T, typename Pointer, typename Reference>
  inline intrusive_list_iterator<T, Pointer, Reference>::intrusive_list_iterator(pointer pNode) : mpNode(pNode) {
    // Empty
  }

  template <typename T, typename Pointer, typename Reference>
  inline intrusive_list_iterator<T, Pointer, Reference>::intrusive_list_iterator(const iterator& x) : mpNode(x.mpNode) {
    // Empty
  }
  ...
  inline intrusive_list_base::intrusive_list_base() { mAnchor.mpNext = mAnchor.mpPrev = &mAnchor; }
  ...
  inline bool intrusive_list_base::empty() const EA_NOEXCEPT { return mAnchor.mpPrev == &mAnchor; }
...
#if EASTL_VALIDATE_INTRUSIVE_LIST
    if (pNode != &mAnchor) pNode->mpNext = pNode->mpPrev = NULL;
#if EASTL_ASSERT_ENABLED
    else
  ...
  template <typename T>
  inline intrusive_list<T>::intrusive_list(const this_type& /*x*/) : intrusive_list_base() {
    // We intentionally ignore argument x.
    // To consider: Shouldn't this function simply not exist? Is there a useful purpose for having
    // this function? There should be a comment here about it, though my first guess is that this
    // exists to quell VC++ level 4/-Wall compiler warnings.
  }
  ...
  inline typename intrusive_list<T>::reference intrusive_list<T>::front() {
    ...
    if (mAnchor.mpNext == &mAnchor) EASTL_FAIL_MSG("intrusive_list::front(): empty list.");
#endif
...
#if EASTL_VALIDATE_INTRUSIVE_LIST && EASTL_ASSERT_ENABLED
    if (mAnchor.mpNext == &mAnchor) EASTL_FAIL_MSG("intrusive_list::back(): empty list.");
#endif
...
#if EASTL_VALIDATE_INTRUSIVE_LIST && EASTL_ASSERT_ENABLED
    if (x.mpNext || x.mpPrev) EASTL_FAIL_MSG("intrusive_list::push_front(): element already on a list.");
#endif
...
#if EASTL_VALIDATE_INTRUSIVE_LIST && EASTL_ASSERT_ENABLED
    if (x.mpNext || x.mpPrev) EASTL_FAIL_MSG("intrusive_list::push_back(): element already on a list.");
#endif
  ...
  inline typename intrusive_list<T>::iterator intrusive_list<T>::locate(value_type& x) {
    for (intrusive_list_node* p = (T*)mAnchor.mpNext; p != &mAnchor; p = p->mpNext) {
      if (p == &x) return iterator(static_cast<T*>(p));
    }
  ...
  inline typename intrusive_list<T>::const_iterator intrusive_list<T>::locate(const value_type& x) const {
    for (const intrusive_list_node* p = mAnchor.mpNext; p != &mAnchor; p = p->mpNext) {
      if (p == &x) return const_iterator(static_cast<const T*>(p));
    }
...
#if EASTL_VALIDATE_INTRUSIVE_LIST && EASTL_ASSERT_ENABLED
    if (x.mpNext || x.mpPrev) EASTL_FAIL_MSG("intrusive_list::insert(): element already on a list.");
#endif
  ...
  template <typename T>
  inline typename intrusive_list<T>::iterator intrusive_list<T>::erase(const_iterator pos) {
  ...
  template <typename T>
  inline typename intrusive_list<T>::iterator intrusive_list<T>::erase(const_iterator first, const_iterator last) {
  ...
  template <typename T>
  inline typename intrusive_list<T>::reverse_iterator intrusive_list<T>::erase(const_reverse_iterator position) {
    return reverse_iterator(erase((++position).base()));
  }

  template <typename T>
  inline typename intrusive_list<T>::reverse_iterator intrusive_list<T>::erase(const_reverse_iterator first,
  ...
  void intrusive_list<T>::sort() {
    ...
    // function calls to sort the list. The sorting of a very large list may fail due to stack
    // overflow if the stack is exhausted. The limit depends on the platform and the avaialble stack
    // space.

    // Easier-to-understand version of the 'if' statement:
    // iterator i(begin());
    // if((i != end()) && (++i != end())) // If the size is >= 2 (without calling the more expensive
    // size() function)...
