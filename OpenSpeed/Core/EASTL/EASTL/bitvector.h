...
#include <OpenSpeed/Core/EASTL/EASTL/bitset.h>
...
#define EASTL_BITVECTOR_DEFAULT_ALLOCATOR allocator_type(EASTL_BITVECTOR_DEFAULT_NAME)
  ...
  ///
  template <typename Allocator = EASTLAllocatorType, typename Element = BitvectorWordType,
  ...
  class bitvector {
   public:
    ...
    static const size_type kMaxSize =
        container_type::kMaxSize;  /// -1 is reserved for 'npos'. It also happens to be slightly
   ...
   public:
    ...
    bool test(size_type n,
              bool      defaultValue) const;  // Returns true if the bit index is < size() and set.
                                         // Returns defaultValue if the bit is >= size().
    ...
    /*
    Work in progress:
    template <bool value = true> iterator find_first();                                 // Finds the
    lowest "on" bit. template <bool value = true> iterator find_next(const_iterator it); // Finds
    the next lowest "on" bit after it. template <bool value = true> iterator find_last(); // Finds
    the index of the last "on" bit, returns size if none are set. template <bool value = true>
    iterator find_prev(const_iterator it);                 // Finds the index of the last "on" bit
    before last_find, returns size if none are set.

    template <bool value = true> const_iterator find_first() const;                     // Finds the
    lowest "on" bit. template <bool value = true> const_iterator find_next(const_iterator it) const;
    // Finds the next lowest "on" bit after it. template <bool value = true> const_iterator
    find_last() const;                      // Finds the index of the last "on" bit, returns size if
    none are set. template <bool value = true> const_iterator find_prev(const_iterator it) const; //
  ...
  template <typename Element>
  bitvector_reference<Element>::bitvector_reference(Element* p, eastl_size_t i) : mpBitWord(p), mnBitIndex(i) {}

  template <typename Element>
  bitvector_reference<Element>& bitvector_reference<Element>::operator=(bool value) {
  ...
  template <typename Element>
  bitvector_reference<Element>& bitvector_reference<Element>::operator=(const bitvector_reference& rhs) {
  ...
  template <typename Element>
  bitvector_const_iterator<Element>::bitvector_const_iterator() : mReference(0, 0) {}

  template <typename Element>
  bitvector_const_iterator<Element>::bitvector_const_iterator(const Element* p, eastl_size_t i) :
      mReference(const_cast<Element*>(p), i)  // const_cast is safe here because we never let
                                              // mReference leak and we don't modify it.
  {}
  ...
  template <typename Element>
  bitvector_const_iterator<Element> bitvector_const_iterator<Element>::operator++(int) {
  ...
  template <typename Element>
  bitvector_const_iterator<Element>& bitvector_const_iterator<Element>::operator+=(difference_type n) {
  ...
  template <typename Element>
  bitvector_const_iterator<Element> bitvector_const_iterator<Element>::operator+(difference_type n) const {
    bitvector_const_iterator copy(*this);
    copy += n;
    return copy;
  }
  ...
  template <typename Element>
  typename bitvector_const_iterator<Element>::difference_type bitvector_const_iterator<Element>::operator-(
      const this_type& rhs) const {
    return ((mReference.mpBitWord - rhs.mReference.mpBitWord) * kBitCount) + mReference.mnBitIndex -
           rhs.mReference.mnBitIndex;
  }
  ...
  template <typename Element>
  int bitvector_const_iterator<Element>::validate(const Element* pStart, const Element* pEnd,
  ...
  template <typename Element>
  bitvector_iterator<Element>::bitvector_iterator() : base_type() {}

  template <typename Element>
  bitvector_iterator<Element>::bitvector_iterator(Element* p, eastl_size_t i) : base_type(p, i) {}

  template <typename Element>
  bitvector_iterator<Element>::bitvector_iterator(reference_type& reference) : base_type(reference) {}

  template <typename Element>
  typename bitvector_iterator<Element>::reference_type bitvector_iterator<Element>::operator*() const {
    return base_type::mReference;
  }

  template <typename Element>
  typename bitvector_iterator<Element>::reference_type bitvector_iterator<Element>::operator[](
      difference_type n) const {
    return *(*this + n);
  }

  template <typename Element>
  void MoveBits(bitvector_iterator<Element> start, bitvector_iterator<Element> end, bitvector_iterator<Element> dest) {
  ...
  template <typename Element>
  bitvector_iterator<Element> bitvector_iterator<Element>::operator+(difference_type n) const {
    bitvector_iterator copy(*this);
    copy += n;
    return copy;
  }
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::iterator bitvector<Allocator, Element, Container>::begin()
      EA_NOEXCEPT {
    return iterator(&mContainer[0], 0);
  }

  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::const_iterator bitvector<Allocator, Element, Container>::begin()
      const EA_NOEXCEPT {
    return const_iterator(&mContainer[0], 0);
  }

  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::const_iterator bitvector<Allocator, Element, Container>::cbegin()
      const EA_NOEXCEPT {
    return const_iterator(&mContainer[0], 0);
  }

  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::iterator bitvector<Allocator, Element, Container>::end()
      EA_NOEXCEPT {
    return iterator(mContainer.end(), 0) - mFreeBitCount;
  }

  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::const_iterator bitvector<Allocator, Element, Container>::end()
      const EA_NOEXCEPT {
    return const_iterator(mContainer.end(), 0) - mFreeBitCount;
  }

  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::const_iterator bitvector<Allocator, Element, Container>::cend()
      const EA_NOEXCEPT {
    return const_iterator(mContainer.end(), 0) - mFreeBitCount;
  }
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::size_type bitvector<Allocator, Element, Container>::size() const
      EA_NOEXCEPT {
    return (mContainer.size() * kBitCount) - mFreeBitCount;
  }

  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::size_type bitvector<Allocator, Element, Container>::capacity()
      const EA_NOEXCEPT {
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::reverse_iterator bitvector<Allocator, Element, Container>::rbegin()
      EA_NOEXCEPT {
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::reverse_iterator bitvector<Allocator, Element, Container>::rend()
      EA_NOEXCEPT {
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::reference bitvector<Allocator, Element, Container>::front() {
    EASTL_ASSERT(!empty());
    return reference(&mContainer[0], 0);
  }

  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::const_reference bitvector<Allocator, Element, Container>::front()
      const {
    EASTL_ASSERT(!empty());
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::reference bitvector<Allocator, Element, Container>::back() {
    EASTL_ASSERT(!empty());
    return *(--end());
  }

  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::const_reference bitvector<Allocator, Element, Container>::back()
      const {
    EASTL_ASSERT(!empty());
    return *(--end());
  }
  ...
  bool bitvector<Allocator, Element, Container>::test(size_type n, bool defaultValue) const {
    if (n < size()) return *(begin() + (difference_type)n);
  ...
  void bitvector<Allocator, Element, Container>::set(size_type n, bool value) {
    if (EASTL_UNLIKELY(n >= size())) resize(n + 1);

    *(begin() + (difference_type)n) = value;
  }

  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::reference bitvector<Allocator, Element, Container>::at(
      size_type n) {
    ...
    if (EASTL_UNLIKELY(n >= size())) throw std::out_of_range("bitvector::at -- out of range");
#elif EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(n >= size())) EASTL_FAIL_MSG("bitvector::at -- out of range");
#endif
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::const_reference bitvector<Allocator, Element, Container>::at(
      size_type n) const {
#if EASTL_EXCEPTIONS_ENABLED
    if (EASTL_UNLIKELY(n >= size())) throw std::out_of_range("bitvector::at -- out of range");
#elif EASTL_ASSERT_ENABLED
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::reference bitvector<Allocator, Element, Container>::operator[](
      size_type n) {
    return *(begin() + (difference_type)n);
  }
  ...
  */
  ...
  bool bitvector<Allocator, Element, Container>::validate() const {
    if (!mContainer.validate()) return false;

    if ((unsigned)mFreeBitCount >= kBitCount) return false;
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::element_type* bitvector<Allocator, Element, Container>::data()
      EA_NOEXCEPT {
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::iterator bitvector<Allocator, Element, Container>::insert(
  ...
  /*
  The following is a placeholder for a future implementation. It turns out that a correct
  implementation of insert(pos, first, last) is a non-trivial exercise that would take a few hours
  to implement and test. The reasons why involve primarily the problem of handling the case where
  insertion source comes from within the container itself, and the case that first and last (note
  they are templated) might not refer to iterators might refer to a value/count pair. The C++
  Standard requires you to handle this case and I (Paul Pedriana) believe that it applies even for a
  bitvector, given that bool is an integral type. So you have to set up a compile-time type traits
  function chooser. See vector, for example.

  template <typename Allocator, typename Element, typename Container>
  template <typename InputIterator>
  void bitvector<Allocator, Element, Container>::insert(const_iterator position, InputIterator
  first, InputIterator last)
  {
          iterator iPosition(position.get_reference_type()); // This is just a non-const version of
  position.
  ...
  */
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::iterator bitvector<Allocator, Element, Container>::erase(
...
// Verify that no reallocation occurred.
  ...
  template <typename Allocator, typename Element, typename Container>
  typename bitvector<Allocator, Element, Container>::reverse_iterator bitvector<Allocator, Element, Container>::erase(
  ...
  template <typename Allocator, typename Element, typename Container>
  bitvector<Allocator, Element, Container>& bitvector<Allocator, Element, Container>::operator=(const bitvector& rhs) {
  ...
  template <typename Allocator, typename Element, typename Container>
  bitvector<Allocator, Element, Container>::bitvector() : mContainer(), mFreeBitCount(0) {}

  template <typename Allocator, typename Element, typename Container>
  bitvector<Allocator, Element, Container>::bitvector(const allocator_type& allocator) :
      mContainer(allocator), mFreeBitCount(0) {}
  ...
  bitvector<Allocator, Element, Container>::bitvector(size_type n, const allocator_type& allocator) :
    ...
    if (mFreeBitCount == kBitCount) mFreeBitCount = 0;
  }
  ...
  inline bool operator==(const bitvector<Allocator, Element, Container>& a,
    ...
    // To do: Replace this with a smart compare implementation. This is much slower than it needs to
    // be.
  ...
  template <typename Allocator, typename Element, typename Container>
  inline void swap(bitvector<Allocator, Element, Container>& a, bitvector<Allocator, Element, Container>& b) {
    a.swap(b);
  }
