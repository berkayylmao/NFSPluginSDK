...
#pragma warning(disable : 4244)  // Argument: conversion from 'int' to 'const
                                 // eastl::vector<T>::value_type', possible loss of data
...
#define EASTL_VECTOR_DEFAULT_ALLOCATOR allocator_type(EASTL_VECTOR_DEFAULT_NAME)
  ...
  class vector : public VectorBase<T, Allocator> {
   ...
   public:
    ...
    typedef T&       reference;
    typedef const T& const_reference;  // Maintainer note: We want to leave iterator defined as T*
                                       // -- at least in release builds -- as this gives some
                                       // algorithms an advantage that optimizers cannot get around.
    typedef T* iterator;               // Note: iterator is simply T* right now, but this will likely change in
                                       // the future, at least for debug builds.
    typedef const T* const_iterator;   //       Do not write code that relies on iterator being T*.
                                       //       The reason it will
    typedef eastl::reverse_iterator<iterator> reverse_iterator;  //       change in the future is that a debugging
                                                                 //       iterator system will be created.
   ...
   public:
    ...
    void set_capacity(size_type n = base_type::npos);  // Revises the capacity to the user-specified value. Resizes the
                                                       // container to match the capacity if the requested capacity n is
                                                       // less than the current size. If n == npos then the capacity is
   ...
   protected:
    ...
    template <typename ForwardIterator>  // Allocates a pointer of array count n and copy-constructs
                                         // it with [first,last).
  ...
  inline VectorBase<T, Allocator>::VectorBase() :
      mpBegin(NULL), mpEnd(NULL), mpCapacity(NULL), mAllocator(EASTL_VECTOR_DEFAULT_NAME) {}
  ...
  template <typename T, typename Allocator>
  inline VectorBase<T, Allocator>::VectorBase(size_type n, const allocator_type& allocator) : mAllocator(allocator) {
  ...
  template <typename T, typename Allocator>
  inline VectorBase<T, Allocator>::~VectorBase() {
    if (mpBegin) EASTLFree(mAllocator, mpBegin, (mpCapacity - mpBegin) * sizeof(T));
  }
  ...
  inline T* VectorBase<T, Allocator>::DoAllocate(size_type n) {
    ...
    if (EASTL_UNLIKELY(n >= 0x80000000)) EASTL_FAIL_MSG("vector::DoAllocate -- improbably large request.");
#endif
  ...
  inline void VectorBase<T, Allocator>::DoFree(T* p, size_type n) {
    if (p) EASTLFree(mAllocator, p, n * sizeof(T));
  }
  ...
  template <typename T, typename Allocator>
  inline vector<T, Allocator>::vector() : base_type() {
    // Empty
  }

  template <typename T, typename Allocator>
  inline vector<T, Allocator>::vector(const allocator_type& allocator) : base_type(allocator) {
    // Empty
  }
  ...
  template <typename T, typename Allocator>
  inline vector<T, Allocator>::vector(const this_type& x) : base_type(x.size(), x.mAllocator) {
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Allocator>
  inline vector<T, Allocator>::vector(this_type&& x) : base_type(x.mAllocator) {
    DoSwap(x);
  }
  ...
  typename vector<T, Allocator>::this_type& vector<T, Allocator>::operator=(const this_type& x) {
    if (this != &x)  // If not assigning to self...
    {
      ...
      if (bSlowerPathwayRequired) {
        DoClearCapacity();  // Must clear the capacity instead of clear because set_capacity frees
                            // our memory, unlike clear.
  ...
  typename vector<T, Allocator>::this_type& vector<T, Allocator>::operator=(this_type&& x) {
    if (this != &x) {
      DoClearCapacity();  // To consider: Are we really required to clear here? x is going away soon
  ...
  void vector<T, Allocator>::set_capacity(size_type n) {
    ...
    {
      if (n == 0)  // Very often n will be 0, and clear will be faster than resize and use less
                   // stack space.
        clear();
  ...
  inline void vector<T, Allocator>::shrink_to_fit() {
    ...
    this_type temp(*this);
#endif
    // Call DoSwap() rather than swap() as we know our allocators match and we don't want to invoke
    // the code path handling non matching allocators as it imposes additional restrictions on the
    // type of T to be copyable
    DoSwap(temp);
  }
  ...
  inline typename vector<T, Allocator>::reference vector<T, Allocator>::operator[](size_type n) {
                                          ...
                                          // empty container. But this was merely grandfathered in
                                          // and ideally we shouldn't allow such access to [0].
...
#elif EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(n >= (static_cast<size_type>(mpEnd - mpBegin)))) EASTL_FAIL_MSG("vector::at -- out of range");
#endif
...
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(mpEnd <= mpBegin)) EASTL_FAIL_MSG("vector::pop_back -- empty vector");
#endif
  ...
  inline typename vector<T, Allocator>::iterator vector<T, Allocator>::emplace(const_iterator position,
    ...
    if ((mpEnd == mpCapacity) || (position != mpEnd))
      ...
      ++mpEnd;  // Increment this after the construction above in case the construction throws an
                // exception.
    }
  ...
  inline void vector<T, Allocator>::emplace_back(Args&&... args) {
    if (mpEnd < mpCapacity) {
      ::new ((void*)mpEnd)
          value_type(eastl::forward<Args>(args)...);  // If value_type has a move constructor, it will use it
                                                      // and this operation may be faster than otherwise.
      ++mpEnd;  // Increment this after the construction above in case the construction throws an
                // exception.
    } else
  ...
  inline void vector<T, Allocator>::emplace_back(value_type&& value) {
    if (mpEnd < mpCapacity) {
      ::new ((void*)mpEnd) value_type(eastl::move(value));  // If value_type has a move constructor, it will use it
                                                            // and this operation may be faster than otherwise.
      ++mpEnd;  // Increment this after the construction above in case the construction throws an
                // exception.
    } else
  ...
  inline typename vector<T, Allocator>::iterator vector<T, Allocator>::insert(const_iterator    position,
    ...
    // We implment a quick pathway for the case that the insertion position is at the end and we
    // have free capacity for it.
    const ptrdiff_t n = position - mpBegin;  // Save this because we might reallocate.
  ...
  inline typename vector<T, Allocator>::iterator vector<T, Allocator>::erase(const_iterator position) {
    ...
    if ((position + 1) < mpEnd) eastl::move(destPosition + 1, mpEnd, destPosition);
    --mpEnd;
    mpEnd->~value_type();
    return destPosition;
  }
  ...
  // (propagate_on_container_swap is false by default). EASTL doesn't have allocator_traits and so
  // this doesn't directly apply, but EASTL has the effective behavior of
  // propagate_on_container_swap = false for all allocators. So EASTL swap exchanges contents but
  // not allocators, and swap is more efficient if allocators are equivalent.
  template <typename T, typename Allocator>
  ...
  template <typename T, typename Allocator>
  template <typename ForwardIterator>
  inline typename vector<T, Allocator>::pointer vector<T, Allocator>::DoRealloc(size_type n, ForwardIterator first,
                                                                                ForwardIterator last, should_copy_tag) {
  ...
  template <typename ForwardIterator>
  inline typename vector<T, Allocator>::pointer vector<T, Allocator>::DoRealloc(size_type n, ForwardIterator first,
                                                                                ForwardIterator last, should_move_tag) {
    T* const p = DoAllocate(n);  // p is of type T* but is not constructed.
    eastl::uninitialized_move_ptr_if_noexcept(first, last,
                                              p);  // move-constructs p from [first,last).
    return p;
  }
  ...
  inline void vector<T, Allocator>::DoInit(Integer n, Integer value, true_type) {
    ...
    typedef
        typename eastl::remove_const<T>::type non_const_value_type;  // If T is a const type (e.g. const int) then we
                                                                     // need to initialize it as if it were non-const.
  ...
  template <typename T, typename Allocator>
  template <typename InputIterator>
  inline void vector<T, Allocator>::DoInitFromIterator(InputIterator first, InputIterator last,
                                                       EASTL_ITC_NS::input_iterator_tag) {
    // To do: Use emplace_back instead of push_back(). Our emplace_back will work below without any
    // ifdefs.
  ...
  }  // Luckily, InputIterators are in practice almost never used, so this code will likely never
     // get executed.

  template <typename T, typename Allocator>
  template <typename ForwardIterator>
  inline void vector<T, Allocator>::DoInitFromIterator(ForwardIterator first, ForwardIterator last,
  ...
  void vector<T, Allocator>::DoAssignValues(size_type n, const value_type& value) {
    if (n > size_type(mpCapacity - mpBegin))  // If n > capacity ...
    {
      this_type temp(n, value,
                     mAllocator);  // We have little choice but to reallocate with new memory.
      swap(temp);
  ...
  template <typename T, typename Allocator>
  template <typename InputIterator, bool bMove>
  void vector<T, Allocator>::DoAssignFromIterator(InputIterator first, InputIterator last,
                                                  EASTL_ITC_NS::input_iterator_tag) {
  ...
  template <typename T, typename Allocator>
  template <typename RandomAccessIterator, bool bMove>
  void vector<T, Allocator>::DoAssignFromIterator(RandomAccessIterator first, RandomAccessIterator last,
                                                  EASTL_ITC_NS::random_access_iterator_tag) {
    const size_type n = (size_type)eastl::distance(first, last);
    ...
    {
      pointer const pNewEnd =
          eastl::copy(first, last,
                      mpBegin);  // Since we are copying to mpBegin, we don't have to worry about needing
  ...
  template <typename T, typename Allocator>
  template <typename InputIterator>
  inline void vector<T, Allocator>::DoInsert(const_iterator position, InputIterator first, InputIterator last,
                                             false_type) {
    typedef typename eastl::iterator_traits<InputIterator>::iterator_category IC;
  ...
  template <typename T, typename Allocator>
  template <typename InputIterator>
  inline void vector<T, Allocator>::DoInsertFromIterator(const_iterator position, InputIterator first,
                                                         InputIterator last, EASTL_ITC_NS::input_iterator_tag) {
    for (; first != last; ++first, ++position) position = insert(position, *first);
  }

  template <typename T, typename Allocator>
  template <typename BidirectionalIterator>
  void vector<T, Allocator>::DoInsertFromIterator(const_iterator position, BidirectionalIterator first,
    ...
    if (first != last) {
      ...
      if (n <= size_type(mpCapacity - mpEnd))  // If n fits within the existing capacity...
      {
        ...
        if (n < nExtra)  // If the inserted values are entirely within initialized memory (i.e. are
                         // before mpEnd)...
        {
          eastl::uninitialized_copy_ptr(mpEnd - n, mpEnd, mpEnd);
          eastl::copy_backward(destPosition, mpEnd - n,
                               mpEnd);  // We need copy_backward because of potential overlap issues.
  ...
  void vector<T, Allocator>::DoInsertValues(const_iterator position, size_type n, const value_type& value) {
    ...
    if (n <= size_type(mpCapacity - mpEnd))  // If n is <= capacity...
    {
      ...
      {
        // To consider: Make this algorithm work more like DoInsertValue whereby a pointer to value
        // is used.
  ...
  template <typename T, typename Allocator>
  void vector<T, Allocator>::DoClearCapacity()  // This function exists because set_capacity()
                                                // currently indirectly requires value_type to be
                                                // default-constructible,
  {  // and some functions that need to clear our capacity (e.g. operator=) aren't supposed to
     // require default-constructibility.
    clear();
    this_type temp(*this);  // This is the simplest way to accomplish this,
    swap(temp);             // and it is as efficient as any other.
  }
  ...
  // The code duplication between this and the version that takes no value argument and default
  // constructs the values is unfortunate but not easily resolved without relying on C++11 perfect
  // forwarding.
  template <typename T, typename Allocator>
  ...
  void vector<T, Allocator>::DoInsertValue(const_iterator position, Args&&... args) {
    // To consider: It's feasible that the args is from a value_type comes from within the current
    // sequence itself and so we need to be sure to handle that case. This is different from
    // insert(position, const value_type&) because in this case value is potentially being modified.
    ...
    {
      // We need to take into account the possibility that args is a value_type that comes from
      // within the vector itself. creating a temporary value on the stack here is not an optimal
      // way to solve this because sizeof(value_type) may be too much for the given platform. An
      // alternative solution may be to specialize this function for the case of the argument being
      // const value_type& or value_type&&.
      EASTL_ASSERT(position < mpEnd);  // While insert at end() is valid, our design is such that calling code should
                                       // handle that case before getting here, as our streamlined logic directly
                                       // doesn't handle this particular case due to resulting negative ranges.
#if EASTL_USE_FORWARD_WORKAROUND
      auto value = value_type(eastl::forward<Args>(args)...);  // Workaround for compiler bug in VS2013 which results in
                                                               // a compiler internal crash while compiling this code.
#else
      value_type value(eastl::forward<Args>(args)...);  // Need to do this before the move_backward below because maybe
                                                        // args refers to something within the moving range.
#endif
      ::new (static_cast<void*>(mpEnd))
          value_type(eastl::move(*(mpEnd - 1)));  // mpEnd is uninitialized memory, so we must construct into it instead
                                                  // of move into it like we do with the other elements below.
...
#if EASTL_EXCEPTIONS_ENABLED
      pointer pNewEnd = pNewData;
      try {  // To do: We are not handling exceptions properly below.  In particular we don't want
             // to
        // call eastl::destruct on the entire range if only the first part of the range was
        // costructed.
        ::new ((void*)(pNewData + nPosSize))
            value_type(eastl::forward<Args>(args)...);  // Because the old data is potentially being moved
                                                        // rather than copied, we need to move.
        pNewEnd = NULL;  // Set to NULL so that in catch we can tell the exception occurred during
                         // the next call.
        pNewEnd = eastl::uninitialized_move_ptr_if_noexcept(mpBegin, destPosition,
                                                            pNewData);  // the value first, because it might possibly be
                                                                        // a reference to the old data being moved.
        pNewEnd = eastl::uninitialized_move_ptr_if_noexcept(destPosition, mpEnd, ++pNewEnd);
      } catch (...) {
        if (pNewEnd)
          eastl::destruct(pNewData, pNewEnd);  // Destroy what has been constructed so far.
        else
          eastl::destruct(pNewData + nPosSize);  // The exception occurred during the first unintialized move,
...
#else
      ::new ((void*)(pNewData + nPosSize))
          value_type(eastl::forward<Args>(args)...);  // Because the old data is potentially being
                                                      // moved rather than copied, we need to move
      pointer pNewEnd = eastl::uninitialized_move_ptr_if_noexcept(
          mpBegin, destPosition, pNewData);  // the value first, because it might possibly be a
                                             // reference to the old data being moved.
      pNewEnd = eastl::uninitialized_move_ptr_if_noexcept(
          destPosition, mpEnd,
          ++pNewEnd);  // Question: with exceptions disabled, do we asssume all operations are
                       // noexcept and thus there's no need for uninitialized_move_ptr_if_noexcept?
#endif
  ...
  // They are 90% the same as each other with the only difference being the use of
  // eastl::move(value) usage. However, it isn't simple to fold that difference because value_type&
  // and value_type&& are treated significantly differently and constructing objects with them
  // executes different code.

  template <typename T, typename Allocator>
  void vector<T, Allocator>::DoInsertValue(const_iterator position, value_type&& value) {
    // To consider: It's feasible that value comes from within the current sequence itself and so we
    // need to be sure to handle that case. This is different from insert(position, const
    // value_type&) because in this case value is potentially being modified.
    ...
    {
      // We need to take into account the possibility that value may come from within the vector
      // itself.
      EASTL_ASSERT(position < mpEnd);  // While insert at end() is valid, our design is such that calling code should
                                       // handle that case before getting here, as our streamlined logic directly
                                       // doesn't handle this particular case due to resulting negative ranges.
    ...
    } else  // else (size == capacity)
    {
      ...
      try {
        ::new ((void*)(pNewData + nPosSize))
            value_type(eastl::move(value));  // Because the old data is being moved rather than
                                             // copied, we need to move the value first,
        pNewEnd = NULL;                      // Set to NULL so that in catch we can tell the exception occurred during
                                             // the next call.
        pNewEnd = eastl::uninitialized_move_ptr_if_noexcept(
            mpBegin, destPosition,
            pNewData);  // because it might possibly be a reference to the old data being moved.
        pNewEnd = eastl::uninitialized_move_ptr_if_noexcept(destPosition, mpEnd, ++pNewEnd);
      } catch (...) {
        if (pNewEnd)
          eastl::destruct(pNewData, pNewEnd);  // Destroy what has been constructed so far.
  ...
  void vector<T, Allocator>::DoInsertValue(const_iterator position, const value_type& value) {
    ...
    if (mpEnd != mpCapacity)  // If size < capacity ...
    {
      ...
      if ((pValue >= destPosition) && (pValue < mpEnd))  // If value comes from within the range to be moved...
        ++pValue;                                        // Set pValue to be where it will be after the copy.
      ::new ((void*)mpEnd)
          value_type(*(mpEnd - 1));  // mpEnd is uninitialized memory, so we must construct into it instead of
    ...
    } else  // else (size == capacity)
    {
      ...
      pointer pNewEnd = pNewData;
      try {
        ::new ((void*)(pNewData + nPosSize)) value_type(value);  // Because the old data is being moved rather than
                                                                 // copied, we need to move the value first,
        pNewEnd = NULL;  // Set to NULL so that in catch we can tell the exception occurred during
                         // the next call.
        pNewEnd = eastl::uninitialized_move_ptr_if_noexcept(
