...
#include <OpenSpeed/Core/EASTL/EASTL/allocator.h>
#include <OpenSpeed/Core/EASTL/EASTL/algorithm.h>
#include <OpenSpeed/Core/EASTL/EASTL/type_traits.h>
#include <OpenSpeed/Core/EASTL/EASTL/iterator.h>
#include <OpenSpeed/Core/EASTL/EASTL/memory.h>
...
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable : 4267)  // 'argument' : conversion from 'size_t' to 'const uint32_t', possible loss of
...
#if EASTL_EXCEPTIONS_ENABLED
#pragma warning(disable : 4703)  // potentially uninitialized local pointer variable used.   VC++ is
                                 // mistakenly analyzing the possibility of uninitialized variables,
                                 // though it's not easy for it to do so.
...
#define EASTL_DEQUE_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " deque"  // Unless the user overrides something, this is "EASTL deque".
#endif
...
#define DEQUE_DEFAULT_SUBARRAY_SIZE(T) 16
  ...
  struct DequeIterator {
   ...
   protected:
    ...
    template <typename U, typename PointerA, typename ReferenceA, typename PointerB, typename ReferenceB,
    ...
    template <typename U, typename PointerA, typename ReferenceA, typename PointerB, typename ReferenceB,
              unsigned kDequeSubarraySizeU>
    friend typename DequeIterator<U, PointerA, ReferenceA, kDequeSubarraySizeU>::difference_type operator-(
   ...
   protected:
    ...
    T* mpEnd;               // The end of the current subarray. To consider: remove this member, as it is always
                            // equal to 'mpBegin + kDequeSubarraySize'. Given that deque subarrays usually
                            // consist of hundreds of bytes, this isn't a massive win. Also, now that we are
                            // implementing a zero-allocation new deque policy, mpEnd may in fact not be equal to
                            // 'mpBegin + kDequeSubarraySize'.
    T** mpCurrentArrayPtr;  // Pointer to current subarray. We could alternatively implement this as
                            // a list node iterator if the deque used a linked list.
  ...
  struct DequeBase {
    ...
    static const size_type kMaxSize = (size_type)-2;  /// -1 is reserved for 'npos'. It also happens to be slightly
                                                      /// beneficial that kMaxSize is a value less than -1, as it helps
                                                      /// us deal with potential integer wraparound issues.
#endif

    enum {
      kMinPtrArraySize = 8,               /// A new empty deque has a ptrArraySize of 0, but any allocated
                                          /// ptrArrays use this min size.
      kSubarraySize = kDequeSubarraySize  ///
      // kNodeSize        = kDequeSubarraySize * sizeof(T)   /// Disabled because it prevents the
      // ability to do this: struct X{ eastl::deque<X, EASTLAllocatorType, 16> mDequeOfSelf; };
    };
  ...
  class deque : public DequeBase<T, Allocator, kDequeSubarraySize> {
   ...
   public:
    ...
    deque(InputIterator first,
          InputIterator last);  // allocator arg removed because VC7.1 fails on the default arg. To
                                // do: Make a second version of this function without a default arg.
    ...
    template <typename InputIterator>  // It turns out that the C++ std::deque<int, int> specifies a
                                       // two argument
    void assign(InputIterator first,
                InputIterator last);  // version of assign that takes (int size,
                                      // int value). These are not iterators, so we
                                      // need to do a template compiler trick to do
                                      // the right thing.
    ...
    void clear();
    // void reset_lose_memory(); // Disabled until it can be implemented efficiently and cleanly. //
    // This is a unilateral reset to an initially empty state. No destructors are called, no
    // deallocation occurs.
   ...
   protected:
    ...
    template <typename InputIterator>
    void DoInsertFromIterator(const_iterator position, const InputIterator& first, const InputIterator& last,
  ...
  DequeBase<T, Allocator, kDequeSubarraySize>::DequeBase(const allocator_type& allocator) :
      mpPtrArray(NULL), mnPtrArraySize(0), mItBegin(), mItEnd(), mAllocator(allocator) {
  ...
  DequeBase<T, Allocator, kDequeSubarraySize>::DequeBase(size_type n) :
      mpPtrArray(NULL), mnPtrArraySize(0), mItBegin(), mItEnd(), mAllocator(EASTL_DEQUE_DEFAULT_NAME) {
  ...
  void DequeBase<T, Allocator, kDequeSubarraySize>::set_allocator(const allocator_type& allocator) {
    // The only time you can set an allocator is with an empty unused container, such as right after
    // construction.
    if (EASTL_LIKELY(mAllocator != allocator)) {
      if (EASTL_LIKELY(mpPtrArray &&
                       (mItBegin.mpCurrentArrayPtr ==
                        mItEnd.mpCurrentArrayPtr)))  // If we are empty and so can safely deallocate the
                                                     // existing memory... We could also test for empty(),
                                                     // but that's a more expensive calculation and more
                                                     // involved clearing, though it would be more flexible.
      {
        DoFreeSubarrays(mItBegin.mpCurrentArrayPtr, mItEnd.mpCurrentArrayPtr + 1);
        DoFreePtrArray(mpPtrArray, mnPtrArraySize);
  ...
  void DequeBase<T, Allocator, kDequeSubarraySize>::DoFreeSubarray(T* p) {
    if (p) EASTLFree(mAllocator, p, kDequeSubarraySize * sizeof(T));
  }

  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  void DequeBase<T, Allocator, kDequeSubarraySize>::DoFreeSubarrays(T** pBegin, T** pEnd) {
    while (pBegin < pEnd) DoFreeSubarray(*pBegin++);
  }

  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  T** DequeBase<T, Allocator, kDequeSubarraySize>::DoAllocatePtrArray(size_type n) {
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(n >= 0x80000000)) EASTL_FAIL_MSG("deque::DoAllocatePtrArray -- improbably large request.");
#endif
  ...
  void DequeBase<T, Allocator, kDequeSubarraySize>::DoFreePtrArray(T** pp, size_t n) {
    if (pp) EASTLFree(mAllocator, pp, n * sizeof(T*));
  }
  ...
  DequeBase<T, Allocator, kDequeSubarraySize>::DoReallocSubarray(size_type nAdditionalCapacity, Side allocationSide) {
    ...
    if (allocationSide == kSideFront) {
      // There might be some free space (nCurrentAdditionalCapacity) at the front of the existing
      // subarray.
      const size_type nCurrentAdditionalCapacity = (size_type)(mItBegin.mpCurrent - mItBegin.mpBegin);

      if (EASTL_UNLIKELY(nCurrentAdditionalCapacity <
                         nAdditionalCapacity))  // If we need to grow downward into a new subarray...
      {
        ...
        if (nSubarrayIncrease > (mItBegin.mpCurrentArrayPtr - mpPtrArray))  // If there are not enough pointers in front
                                                                            // of the current (first) one...
          DoReallocPtrArray((size_type)(nSubarrayIncrease - (mItBegin.mpCurrentArrayPtr - mpPtrArray)), kSideFront);

#if EASTL_EXCEPTIONS_ENABLED
        try {
#endif
          for (i = 1; i <= nSubarrayIncrease; ++i) mItBegin.mpCurrentArrayPtr[-i] = DoAllocateSubarray();
#if EASTL_EXCEPTIONS_ENABLED
        } catch (...) {
          for (difference_type j = 1; j < i; ++j) DoFreeSubarray(mItBegin.mpCurrentArrayPtr[-j]);
          throw;
        }
#endif
      }
    ...
    } else  // else kSideBack
    {
                         ...
                         nAdditionalCapacity))  // If we need to grow forward into a new subarray...
      {
        ...
        if (nSubarrayIncrease > ((mpPtrArray + mnPtrArraySize) - mItEnd.mpCurrentArrayPtr) -
                                    1)  // If there are not enough pointers after the current (last) one...
          DoReallocPtrArray(
              (size_type)(nSubarrayIncrease - (((mpPtrArray + mnPtrArraySize) - mItEnd.mpCurrentArrayPtr) - 1)),
              kSideBack);

#if EASTL_EXCEPTIONS_ENABLED
        try {
#endif
          for (i = 1; i <= nSubarrayIncrease; ++i) mItEnd.mpCurrentArrayPtr[i] = DoAllocateSubarray();
#if EASTL_EXCEPTIONS_ENABLED
        } catch (...) {
          for (difference_type j = 1; j < i; ++j) DoFreeSubarray(mItEnd.mpCurrentArrayPtr[j]);
          throw;
        }
#endif
      }
  ...
  void DequeBase<T, Allocator, kDequeSubarraySize>::DoReallocPtrArray(size_type nAdditionalCapacity,
    ...
    //
    // Balanced pointer array     Unbalanced pointer array (unused space at front, no free space at
    // back)
    // ----++++++++++++----        ---------+++++++++++
    ...
    if ((allocationSide == kSideBack) &&
        (nAdditionalCapacity <= nUnusedPtrCountAtFront))  // If we can take advantage of unused pointers at the front
                                                          // without doing any reallocation...
    {
      if (nAdditionalCapacity <
          (nUnusedPtrCountAtFront / 2))  // Possibly use more space than required, if there's a lot of extra space.
...
#endif
    } else if ((allocationSide == kSideFront) &&
               (nAdditionalCapacity <= nUnusedPtrCountAtBack))  // If we can take advantage of unused pointers at the
                                                                // back without doing any reallocation...
    {
      if (nAdditionalCapacity <
          (nUnusedPtrCountAtBack / 2))  // Possibly use more space than required, if there's a lot of extra space.
      ...
      const size_type nNewPtrArraySize =
          mnPtrArraySize + eastl::max_alt(mnPtrArraySize, nAdditionalCapacity) + 2;  // Allocate extra capacity.
      ...
      // The following is equivalent to: eastl::copy(mItBegin.mpCurrentArrayPtr,
      // mItEnd.mpCurrentArrayPtr + 1, pPtrArrayBegin); It's OK to use memcpy instead of memmove
      // because the destination is guaranteed to non-overlap the source.
    ...
    // We need to reset the begin and end iterators, as code that calls this expects them to *not*
    // be invalidated.
  ...
  void DequeBase<T, Allocator, kDequeSubarraySize>::DoInit(size_type n) {
    ...
    //{
    const size_type nNewPtrArraySize =
        (size_type)((n / kDequeSubarraySize) + 1);         // Always have at least one, even if n is zero.
    ...
    value_type** const pPtrArrayBegin =
        (mpPtrArray + ((mnPtrArraySize - nNewPtrArraySize) / 2));  // Try to place it in the middle.
...
// I am sorry for the mess of #ifs and indentations below.
    ...
    //    mItBegin.mpEnd             = NULL; // We intentionally create a situation whereby the
    //    subarray that has no capacity. mItBegin.mpCurrent         = NULL;
    //
    //    mItEnd = mItBegin;
    //}
  }
  ...
  DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::operator+=(difference_type n) {
    ...
    if ((size_t)subarrayPosition < (size_t)kDequeSubarraySize)  // If the new position is within the current subarray
                                                                // (i.e. >= 0 && < kSubArraySize)...
      mpCurrent += n;
    else {
  ...
  template <typename T, typename Pointer, typename Reference, unsigned kDequeSubarraySize>
  typename DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::this_type
  DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::copy(const iterator& first, const iterator& last,
                                                                 true_type) {
    ...
    if ((first.mpBegin == last.mpBegin) &&
        (first.mpBegin == mpBegin))  // If all operations are within the same subarray, implement
                                     // the operation as a memmove.
    {
  ...
  template <typename T, typename Pointer, typename Reference, unsigned kDequeSubarraySize>
  void DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::copy_backward(const iterator& first,
                                                                               const iterator& last, true_type) {
    // To do: Implement this as a loop which does memmoves between subarrays appropriately.
    //        Currently we only do memcpy if the entire operation occurs within a single subarray.
    if ((first.mpBegin == last.mpBegin) &&
        (first.mpBegin == mpBegin))  // If all operations are within the same subarray, implement
                                     // the operation as a memcpy.
  ...
  // The C++ defect report #179 requires that we support comparisons between const and non-const
  // iterators. Thus we provide additional template paremeters here to support this. The defect
  // report does not require us to support comparisons between reverse_iterators and
  ...
  inline bool operator<(const DequeIterator<T, PointerA, ReferenceA, kDequeSubarraySize>& a,
                        const DequeIterator<T, PointerB, ReferenceB, kDequeSubarraySize>& b) {
    return (a.mpCurrentArrayPtr == b.mpCurrentArrayPtr) ? (a.mpCurrent < b.mpCurrent)
  ...
  inline DequeIterator<T, Pointer, Reference, kDequeSubarraySize> operator+(
      ptrdiff_t n, const DequeIterator<T, Pointer, Reference, kDequeSubarraySize>& x) {
  ...
  template <typename T, typename PointerA, typename ReferenceA, typename PointerB, typename ReferenceB,
            unsigned kDequeSubarraySize>
  inline typename DequeIterator<T, PointerA, ReferenceA, kDequeSubarraySize>::difference_type operator-(
      const DequeIterator<T, PointerA, ReferenceA, kDequeSubarraySize>& a,
      const DequeIterator<T, PointerB, ReferenceB, kDequeSubarraySize>& b) {
    // This is a fairly clever algorithm that has been used in STL deque implementations since the
    // original HP STL:
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  inline deque<T, Allocator, kDequeSubarraySize>::deque() : base_type((size_type)0) {
    // Empty
  }
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  inline deque<T, Allocator, kDequeSubarraySize>::deque(const this_type& x) : base_type(x.size(), x.mAllocator) {
    eastl::uninitialized_copy(x.mItBegin, x.mItEnd, mItBegin);
  }

#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  inline deque<T, Allocator, kDequeSubarraySize>::deque(this_type&& x) : base_type((size_type)0, x.mAllocator) {
    swap(x);
  }
  ...
  inline deque<T, Allocator, kDequeSubarraySize>::deque(this_type&& x, const allocator_type& allocator) :
      base_type((size_type)0, allocator) {
    swap(x);  // member swap handles the case that x has a different allocator than our allocator by
              // doing a copy.
  }
#endif
  ...
  inline deque<T, Allocator, kDequeSubarraySize>::deque(InputIterator first, InputIterator last) :
      base_type(EASTL_DEQUE_DEFAULT_ALLOCATOR)  // Call the empty base constructor, which does nothing.
                                                // We need to do all the work in our own DoInit.
  {
  ...
  inline deque<T, Allocator, kDequeSubarraySize>::~deque() {
    ...
    for (iterator itCurrent(mItBegin); itCurrent != mItEnd; ++itCurrent) itCurrent.mpCurrent->~value_type();
  }

  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::this_type& deque<T, Allocator, kDequeSubarraySize>::operator=(
      const this_type& x) {
    if (&x != this)  // If not assigning to ourselves...
    {
      ...
      if (bSlowerPathwayRequired) {
        ...
        DoSwap(temp);
        // Now we have an empty container with an allocator equal to x.mAllocator, ready to assign
        // from x.
      }
  ...
  deque<T, Allocator, kDequeSubarraySize>::operator=(this_type&& x) {
    if (this != &x) {
      set_capacity(0);  // To consider: Are we really required to clear here? x is going away soon
                        // and will clear itself in its dtor.
      swap(x);          // member swap handles the case that x has a different allocator than our allocator
                        // by doing a copy.
    }
    return *this;
  }
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  inline typename deque<T, Allocator, kDequeSubarraySize>::iterator deque<T, Allocator, kDequeSubarraySize>::begin()
      EA_NOEXCEPT {
    return mItBegin;
  }
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  inline typename deque<T, Allocator, kDequeSubarraySize>::iterator deque<T, Allocator, kDequeSubarraySize>::end()
      EA_NOEXCEPT {
    return mItEnd;
  }

  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::const_iterator deque<T, Allocator, kDequeSubarraySize>::end() const
      EA_NOEXCEPT {
    return mItEnd;
  }
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::size_type inline deque<T, Allocator, kDequeSubarraySize>::size()
      const EA_NOEXCEPT {
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::reference deque<T, Allocator, kDequeSubarraySize>::operator[](
      size_type n) {
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED  // We allow the user to use a reference to v[0] of an
                                          // empty container.
    if (EASTL_UNLIKELY((n != 0) && n >= (size_type)(mItEnd - mItBegin)))
      EASTL_FAIL_MSG("deque::operator[] -- out of range");
#elif EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(n >= (size_type)(mItEnd - mItBegin))) EASTL_FAIL_MSG("deque::operator[] -- out of range");
#endif
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::const_reference deque<T, Allocator, kDequeSubarraySize>::operator[](
      size_type n) const {
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED  // We allow the user to use a reference to v[0] of an
                                          // empty container.
    if (EASTL_UNLIKELY((n != 0) && n >= (size_type)(mItEnd - mItBegin)))
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::reference deque<T, Allocator, kDequeSubarraySize>::at(size_type n) {
#if EASTL_EXCEPTIONS_ENABLED
    if (n >= (size_type)(mItEnd - mItBegin)) throw std::out_of_range("deque::at -- out of range");
#elif EASTL_ASSERT_ENABLED
    if (n >= (size_type)(mItEnd - mItBegin)) EASTL_FAIL_MSG("deque::at -- out of range");
#endif
    return *(mItBegin.operator+((difference_type)n));
  }

  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::const_reference deque<T, Allocator, kDequeSubarraySize>::at(
      size_type n) const {
#if EASTL_EXCEPTIONS_ENABLED
    if (n >= (size_type)(mItEnd - mItBegin)) throw std::out_of_range("deque::at -- out of range");
#elif EASTL_ASSERT_ENABLED
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::reference deque<T, Allocator, kDequeSubarraySize>::front() {
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
    // We allow the user to reference an empty container.
#elif EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((size_type)(mItEnd == mItBegin))) EASTL_FAIL_MSG("deque::front -- empty deque");
#endif
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::const_reference deque<T, Allocator, kDequeSubarraySize>::front()
      const {
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
    // We allow the user to reference an empty container.
#elif EASTL_ASSERT_ENABLED
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::reference deque<T, Allocator, kDequeSubarraySize>::back() {
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
    // We allow the user to reference an empty container.
#elif EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((size_type)(mItEnd == mItBegin))) EASTL_FAIL_MSG("deque::back -- empty deque");
#endif
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::const_reference deque<T, Allocator, kDequeSubarraySize>::back()
      const {
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
    // We allow the user to reference an empty container.
#elif EASTL_ASSERT_ENABLED
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::reference deque<T, Allocator, kDequeSubarraySize>::push_front() {
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::reference deque<T, Allocator, kDequeSubarraySize>::push_back() {
...
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((size_type)(mItEnd == mItBegin))) EASTL_FAIL_MSG("deque::pop_front -- empty deque");
#endif
...
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((size_type)(mItEnd == mItBegin))) EASTL_FAIL_MSG("deque::pop_back -- empty deque");
#endif
...
// In this case we need to free the subarray and point mItEnd to the previous subarray.
      ...
      mItEnd.mpCurrent->~value_type();      // Thus we need to call the destructor on the item *before*
                                            // that last item.
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  template <class... Args>
  typename deque<T, Allocator, kDequeSubarraySize>::iterator deque<T, Allocator, kDequeSubarraySize>::emplace(
    ...
    if (EASTL_UNLIKELY(position.mpCurrent == mItEnd.mpCurrent))  // If we are doing the same thing as push_back...
    {
      ...
      return iterator(mItEnd, typename iterator::Decrement());  // Unfortunately, we need to make an iterator
                                                                // here, as the above push_back is an operation
                                                                // that can invalidate existing iterators.
    } else if (EASTL_UNLIKELY(position.mpCurrent ==
                              mItBegin.mpCurrent))  // If we are doing the same thing as push_front...
    {
...
#else
    value_type valueSaved(
        eastl::forward<Args>(args)...);  // We need to save this because value may come from within our container. It
                                         // would be somewhat tedious to make a workaround that could avoid this.
#endif
    const difference_type i(itPosition - mItBegin);

#if EASTL_ASSERT_ENABLED
    EASTL_ASSERT(!empty());  // The push_front and push_back calls below assume that we are
                             // non-empty. It turns out this is never called unless so.
    ...
    if (i <
        (difference_type)(size() / 2))  // Should we insert at the front or at the back? We divide the range in half.
    {
      emplace_front(*mItBegin);  // This operation potentially invalidates all existing iterators
                                 // and so we need to assign them anew relative to mItBegin below.
  ...
  void deque<T, Allocator, kDequeSubarraySize>::emplace_front(Args&&... args) {
    if (mItBegin.mpCurrent != mItBegin.mpBegin)  // If we have room in the first subarray... we hope that usually this
                                                 // 'new' pathway gets executed, as it is slightly faster.
      ::new ((void*)--mItBegin.mpCurrent)
          value_type(eastl::forward<Args>(args)...);  // Construct in place. If args is a single arg of type
                                                      // value_type&& then it this will be a move construction.
    else {
// To consider: Detect if value isn't coming from within this container and handle that efficiently.
...
#else
      value_type valueSaved(
          eastl::forward<Args>(args)...);  // We need to make a temporary, because args may be a value_type that comes
                                           // from within our container and the operations below may change the
                                           // container. But we can use move instead of copy.
#endif

      if (mItBegin.mpCurrentArrayPtr ==
      ...
      try {
        ...
        ++mItBegin;  // The exception could only occur in the new operation above, after we have
                     // incremented mItBegin. So we need to undo it.
  ...
  void deque<T, Allocator, kDequeSubarraySize>::emplace_back(Args&&... args) {
    if ((mItEnd.mpCurrent + 1) != mItEnd.mpEnd)  // If we have room in the last subarray... we hope that usually this
                                                 // 'new' pathway gets executed, as it is slightly faster.
      ::new ((void*)mItEnd.mpCurrent++)
          value_type(eastl::forward<Args>(args)...);  // Construct in place. If args is a single arg of type
                                                      // value_type&& then it this will be a move construction.
    else {
// To consider: Detect if value isn't coming from within this container and handle that efficiently.
      ...
      value_type valueSaved(
          eastl::forward<Args>(args)...);  // We need to make a temporary, because args may be a value_type that comes
                                           ...
                                           // container. But we can use move instead of copy.
#endif
      if (((mItEnd.mpCurrentArrayPtr - mpPtrArray) + 1) >=
          (difference_type)mnPtrArraySize)  // If there are no more pointers after the current (last) one.
      ...
      try {
        ...
        // No need to execute '--mItEnd', as the exception could only occur in the new operation
        // above before we set mItEnd.
  ...
  typename deque<T, Allocator, kDequeSubarraySize>::iterator deque<T, Allocator, kDequeSubarraySize>::emplace(
    ...
    iterator itPosition(position, typename iterator::FromConst());
    value_type valueSaved(
        eastl::move(value));  // We need to save this because value may come from within our container. It would
                              // be somewhat tedious to make a workaround that could avoid this.
    const difference_type i(itPosition - mItBegin);
  ...
  void deque<T, Allocator, kDequeSubarraySize>::emplace_front(value_type&& value) {
    if (mItBegin.mpCurrent != mItBegin.mpBegin)  // If we have room in the first subarray... we hope that usually this
      ...
      // To consider: Detect if value isn't coming from within this container and handle that
      // efficiently.
      value_type valueSaved(eastl::move(value));  // We need to make a temporary, because value may come from within
                                                  // our container and the operations below may change the container.
                                                  // But we can use move instead of copy.
  ...
  typename deque<T, Allocator, kDequeSubarraySize>::iterator deque<T, Allocator, kDequeSubarraySize>::emplace(
    ...
    iterator itPosition(position, typename iterator::FromConst());
    value_type valueSaved(value);  // We need to save this because value may come from within our container. It would
                                   // be somewhat tedious to make a workaround that could avoid this.
    const difference_type i(itPosition - mItBegin);
  ...
  void deque<T, Allocator, kDequeSubarraySize>::emplace_front(const value_type& value) {
    if (mItBegin.mpCurrent != mItBegin.mpBegin)               // If we have room in the first subarray...
      ::new ((void*)--mItBegin.mpCurrent) value_type(value);  // We hope that usually this 'new' pathway gets executed,
                                                              // as it is slightly faster.
    else  // Note that in this 'else' case we create a temporary, which is less desirable.
    {
      // To consider: Detect if value isn't coming from within this container and handle that
      // efficiently.
      value_type valueSaved(value);  // We need to make a temporary, because value may come from within our container
                                     // and the operations below may change the container.
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::iterator deque<T, Allocator, kDequeSubarraySize>::insert(
      const_iterator position, const value_type& value) {
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  template <typename InputIterator>
  void deque<T, Allocator, kDequeSubarraySize>::insert(const_iterator position, InputIterator first,
                                                       InputIterator last) {
    DoInsert(position, first, last,
             is_integral<InputIterator>());  // The C++ standard requires this sort of behaviour, as
                                             // InputIterator might actually be Integer and 'first'
                                             // is really 'count' and 'last' is really 'value'.
  }
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::iterator deque<T, Allocator, kDequeSubarraySize>::erase(
      const_iterator position) {
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(!(validate_iterator(position) & isf_valid))) EASTL_FAIL_MSG("deque::erase -- invalid iterator");
#endif
    ...
    if (i < (difference_type)(size() / 2))  // Should we move the front entries forward or the back
                                            // entries backward? We divide the range in half.
    {
...
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(!(validate_iterator(itFirst) & isf_valid))) EASTL_FAIL_MSG("deque::erase -- invalid iterator");
    if (EASTL_UNLIKELY(!(validate_iterator(itLast) & isf_valid))) EASTL_FAIL_MSG("deque::erase -- invalid iterator");
#endif

    if ((itFirst != mItBegin) ||
        (itLast != mItEnd))  // If not erasing everything... (We expect that the user won't call
                             // erase(begin, end) because instead the user would just call clear.)
    {
      const difference_type n(itLast - itFirst);
      const difference_type i(itFirst - mItBegin);

      if (i < (difference_type)((size() - n) / 2))  // Should we move the front entries forward or the back entries
                                                    // backward? We divide the range in half.
      {
        ...
        for (; mItBegin != itNewBegin; ++mItBegin)  // Question: If value_type is a POD type, will
                                                    // the compiler generate this loop at all?
          mItBegin.mpCurrent->~value_type();        //           If so, then we need to make a
                                                    //           specialization for destructing PODs.
      ...
      } else  // Else we will be moving back entries backward.
      {
        ...
        for (iterator itTemp(itNewEnd); itTemp != mItEnd; ++itTemp) itTemp.mpCurrent->~value_type();
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  typename deque<T, Allocator, kDequeSubarraySize>::reverse_iterator deque<T, Allocator, kDequeSubarraySize>::erase(
  ...
  void deque<T, Allocator, kDequeSubarraySize>::clear() {
    ...
    if (mItBegin.mpCurrentArrayPtr != mItEnd.mpCurrentArrayPtr)  // If there are multiple subarrays (more often than
                                                                 // not, this will be so)...
    {
      for (value_type* p1 = mItBegin.mpCurrent; p1 < mItBegin.mpEnd; ++p1) p1->~value_type();
      for (value_type* p2 = mItEnd.mpBegin; p2 < mItEnd.mpCurrent; ++p2) p2->~value_type();
      DoFreeSubarray(mItEnd.mpBegin);  // Leave mItBegin with a valid subarray.
    } else {
      for (value_type* p = mItBegin.mpCurrent; p < mItEnd.mpCurrent; ++p) p->~value_type();
    ...
    for (value_type** pPtrArray = mItBegin.mpCurrentArrayPtr + 1; pPtrArray < mItEnd.mpCurrentArrayPtr; ++pPtrArray) {
      for (value_type *p = *pPtrArray, *pEnd = *pPtrArray + kDequeSubarraySize; p < pEnd; ++p) p->~value_type();
  ...
  void deque<T, Allocator, kDequeSubarraySize>::DoInitFromIterator(InputIterator first, InputIterator last,
    ...
    try {
      ...
      for (; first != last; ++first)  // InputIterators by definition actually only allow you to
                                      // iterate through them once.
      {                               // Thus the standard *requires* that we do this (inefficient) implementation.
        push_back(*first);            // Luckily, InputIterators are in practice almost never used, so this
                                      // code will likely never get executed.
      }
#if EASTL_EXCEPTIONS_ENABLED
    } catch (...) {
      clear();
  ...
  void deque<T, Allocator, kDequeSubarraySize>::DoInitFromIterator(ForwardIterator first, ForwardIterator last,
    ...
    typedef typename eastl::remove_const<ForwardIterator>::type
        non_const_iterator_type;  // If T is a const type (e.g. const int) then we need to
                                  // initialize it as if it were non-const.
    ...
    try {
      ...
      {
        // We implment an algorithm here whereby we use uninitialized_copy() and advance() instead
        // of just iterating from first to last and constructing as we go. The reason for this is
        // that we can take advantage of POD data types and implement construction as memcpy
        // operations.
        ForwardIterator current(first);  // To do: Implement a specialization of this algorithm for
                                         // non-PODs which eliminates the need for 'current'.

        eastl::advance(current, kDequeSubarraySize);
        eastl::uninitialized_copy((non_const_iterator_type)first, (non_const_iterator_type)current,
                                  (non_const_value_type*)*pPtrArrayCurrent);
        first = current;
      }

      eastl::uninitialized_copy((non_const_iterator_type)first, (non_const_iterator_type)last,
  ...
  {
    // Actually, the implementation below requires first/last to be a ForwardIterator and not just
    // an InputIterator. But Paul Pedriana if you somehow need to work with an InputIterator and we
    // can deal with it.
    const size_type n     = (size_type)eastl::distance(first, last);
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  template <typename Integer>
  void deque<T, Allocator, kDequeSubarraySize>::DoInsert(const const_iterator& position, Integer n, Integer value,
                                                         true_type) {
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  template <typename InputIterator>
  void deque<T, Allocator, kDequeSubarraySize>::DoInsert(const const_iterator& position, const InputIterator& first,
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  template <typename InputIterator>
  void deque<T, Allocator, kDequeSubarraySize>::DoInsertFromIterator(const_iterator       position,
    ...
    if (position.mpCurrent == mItBegin.mpCurrent)  // If inserting at the beginning or into an empty container...
    {
      iterator itNewBegin(
          DoReallocSubarray(n,
                            kSideFront));  // itNewBegin to mItBegin refers to memory that isn't initialized yet; so
                                           // it's not truly a valid iterator. Or at least not a dereferencable one.
    ...
    {
      const iterator itNewEnd(
          DoReallocSubarray(n, kSideBack));  // mItEnd to itNewEnd refers to memory that isn't initialized yet; so
                                             // it's not truly a valid iterator. Or at least not a dereferencable one.
      ...
      if (nInsertionIndex < (difference_type)(nSize / 2))  // If the insertion index is in the front half of the
                                                           // deque... grow the deque at the front.
      {
        const iterator itNewBegin(
            DoReallocSubarray(n,
                              kSideFront));  // itNewBegin to mItBegin refers to memory that isn't initialized yet; so
                                             // it's not truly a valid iterator. Or at least not a dereferencable one.
        const iterator itOldBegin(mItBegin);
        const iterator itPosition(mItBegin + nInsertionIndex);  // We need to reset this value because the
                                                                // reallocation above can invalidate iterators.

#if EASTL_EXCEPTIONS_ENABLED
        try {
#endif
          // We have a problem here: we would like to use move instead of copy, but it may be that
          // the range to be inserted comes from
          // this container and comes from the segment we need to move. So we can't use move
          // operations unless we are careful to handle that situation. The newly inserted contents
          // must be contents that were moved to and not moved from. To do: solve this.
          if (nInsertionIndex >=
              (difference_type)n)  // If the newly inserted items will be entirely within the old area...
          {
            iterator itUCopyEnd(mItBegin + (difference_type)n);

            eastl::uninitialized_copy(mItBegin, itUCopyEnd, itNewBegin);  // This can throw.
            itUCopyEnd = eastl::copy(itUCopyEnd, itPosition,
                                     itOldBegin);  // Recycle 'itUCopyEnd' to mean something else.
            eastl::copy(first, last, itUCopyEnd);
          } else  // Else the newly inserted items are going within the newly allocated area at the
                  // front.
          {
        ...
        const iterator        itPosition(mItEnd - nPushedCount);  // We need to reset this value because the
                                                                  // reallocation above can invalidate iterators.
  ...
  template <typename T, typename Allocator, unsigned kDequeSubarraySize>
  void deque<T, Allocator, kDequeSubarraySize>::DoInsertValues(const_iterator position, size_type n,
                                                               const value_type& value) {
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(!(validate_iterator(position) & isf_valid))) EASTL_FAIL_MSG("deque::insert -- invalid iterator");
#endif
        ...
        try {
              ...
              (difference_type)n)  // If the newly inserted items will be entirely within the old area...
          {
            ...
            eastl::uninitialized_move_if_noexcept(mItBegin, itUCopyEnd,
                                                  itNewBegin);  // This can throw.
            itUCopyEnd = eastl::move(itUCopyEnd, itPosition,
                                     itOldBegin);  // Recycle 'itUCopyEnd' to mean something else.
            eastl::fill(itUCopyEnd, itPosition, valueSaved);
          } else  // Else the newly inserted items are going within the newly allocated area at the
                  // front.
          {
            eastl::uninitialized_move_fill(mItBegin, itPosition, itNewBegin, mItBegin,
...
#endif
      } else  // Else the insertion index is in the back half of the deque, so grow the deque at the
              // back.
      {
        const iterator        itNewEnd(DoReallocSubarray(n, kSideBack));
        const iterator        itOldEnd(mItEnd);
        ...
        try {
          ...
          if (nPushedCount > (difference_type)n)  // If the newly inserted items will be entirely within the old area...
          {
            ...
            itUCopyEnd =
                eastl::move_backward(itPosition, itUCopyEnd, itOldEnd);  // Recycle 'itUCopyEnd' to mean something else.
            eastl::fill(itPosition, itUCopyEnd, valueSaved);
          } else  // Else the newly inserted items are going within the newly allocated area at the
                  // back.
          {
            eastl::uninitialized_fill_move(mItEnd, itPosition + (difference_type)n, valueSaved, itPosition,
  ...
  inline int deque<T, Allocator, kDequeSubarraySize>::validate_iterator(const_iterator i) const {
    ...
    if ((i - begin()) < 0) return isf_none;

    if ((end() - i) < 0) return isf_none;

    if (i == end()) return (isf_valid | isf_current);
