...
#define EASTL_STACK_DEFAULT_ALLOCATOR allocator_type(EASTL_STACK_DEFAULT_NAME)
  ...
  class stack {
   ...
   public:
    ...
    stack(std::initializer_list<value_type> ilist);  // The first item in the initializer list is pushed first. C++11
                                                     // doesn't specify that std::stack has initializer list support.
  ...
  template <typename T, typename Container>
  inline const typename stack<T, Container>::container_type& stack<T, Container>::get_container() const {
    return c;
  }
  ...
  inline void swap(stack<T, Container>& a, stack<T, Container>& b)
      EA_NOEXCEPT_IF((eastl::is_nothrow_swappable<typename stack<T, Container>::container_type>::value)) {
    a.swap(b);
  }
