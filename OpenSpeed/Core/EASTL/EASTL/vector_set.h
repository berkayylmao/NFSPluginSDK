...
#define EASTL_VECTOR_SET_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " vector_set"  // Unless the user overrides something, this is "EASTL vector_set".
#endif
...
#define EASTL_VECTOR_SET_DEFAULT_ALLOCATOR allocator_type(EASTL_VECTOR_SET_DEFAULT_NAME)
  ...
  class vector_set : public RandomAccessContainer {
   ...
   public:
    ...
    vector_set(InputIterator first, InputIterator last,
               const key_compare& compare);  // allocator arg removed because VC7.1 fails on the default arg. To do:
                                             // Make a second version of this function without a default arg.
  ...
  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_set<K, C, A, RAC>::size_type vector_set<K, C, A, RAC>::count(const key_type& k) const {
