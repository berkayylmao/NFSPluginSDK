...
#include <OpenSpeed/Core/EASTL/EASTL/bonus/fixed_string_abstract.h>
#else  // 'else' encompasses the entire rest of this file.
#include <OpenSpeed/Core/EASTL/EASTL/string.h>
...
#define EASTL_FIXED_STRING_DEFAULT_NAME \
  ...
  ///
  /// Template parameters:
  ///     T                      The type of object the string holds (char, wchar_t, char8_t,
  ///     char16_t, char32_t). nodeCount              The max number of objects to contain.
  ...
  ///
  /// Example usage:
  ///    fixed_string<char, 128 + 1, true> fixedString("hello world"); // Can hold up to a strlen of
  ///    128.
  ///
  ...
  class fixed_string : public basic_string<T, fixed_vector_allocator<sizeof(T), nodeCount, EASTL_ALIGN_OF(T), 0,
   ...
   public:
    typedef fixed_vector_allocator<sizeof(T), nodeCount, EASTL_ALIGN_OF(T), 0, bEnableOverflow, OverflowAllocator>
   ...
   protected:
    union  // We define a union in order to avoid strict pointer aliasing issues with compilers like
           // GCC.
    {
      value_type          mArray[1];
      aligned_buffer_type mBuffer;  // Question: Why are we doing this aligned_buffer thing? Why not just do an
                                    // array of value_type, given that we are using just strings of char types.
    };

   public:
    fixed_string();
    explicit fixed_string(
        const overflow_allocator_type& overflowAllocator);  // Only applicable if bEnableOverflow is true.
    fixed_string(const base_type& x, size_type position,
                 size_type n = base_type::npos);  // Currently we don't support overflowAllocator
                                                  // specification for other constructors, for simplicity.
    ...
    bool      full() const;       // Returns true if the fixed space has been fully allocated. Note that if overflow
                                  // is enabled, the container size can be greater than nodeCount but full() could
                                  // return true because the fixed space may have a recently freed slot.
    bool has_overflowed() const;  // Returns true if the allocations spilled over into the overflow
                                  // allocator. Meaningful only if overflow is enabled.
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_string(
      const this_type& x, const overflow_allocator_type& overflowAllocator) :
      base_type(fixed_allocator_type(mBuffer.buffer, overflowAllocator)) {
    mAllocator.copy_overflow_allocator(x.mAllocator);
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_string(size_type         n,
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_string(const value_type* pBegin,
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_string(CtorDoNotInitialize,
                                                                                      size_type n) :
      base_type(fixed_allocator_type(mBuffer.buffer)) {
#if EASTL_NAME_ENABLED
    mAllocator.set_name(EASTL_FIXED_STRING_DEFAULT_NAME);
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_string(CtorSprintf,
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_string(
      std::initializer_list<T> ilist, const overflow_allocator_type& overflowAllocator) :
      base_type(fixed_allocator_type(mBuffer.buffer, overflowAllocator)) {
#if EASTL_NAME_ENABLED
    mAllocator.set_name(EASTL_FIXED_STRING_DEFAULT_NAME);
#endif
  ...
  inline fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_string(this_type&& x) :
      base_type(fixed_allocator_type(mBuffer.buffer)) {
// We copy from x instead of trade with it. We need to do so because fixed_ containers use local
// memory buffers.
#if EASTL_NAME_ENABLED
    mAllocator.set_name(x.get_allocator().get_name());
#endif
...
// memory buffers.
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::this_type&
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::operator=(const base_type& x) {
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::this_type&
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::operator=(const value_type* p) {
    if (mpBegin != p) {
      clear();
      append(p);
    }
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::this_type&
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::operator=(const value_type c) {
    clear();
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::this_type&
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::operator=(std::initializer_list<T> ilist) {
    clear();
    append(ilist.begin(), ilist.end());
    return *this;
  }
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::this_type&
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::operator=(this_type&& x) {
    // We copy from x instead of trade with it. We need to do so because fixed_ containers use local
    // memory buffers.

    // if(static_cast<base_type*>(this) != &x) This should be impossible, so we disable it until
    // proven otherwise.
    {
      clear();
  ...
  inline void fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::set_capacity(size_type n) {
    ...
    {
      const size_type allocSize = (n + 1);  // +1 because the terminating 0 isn't included in the supplied capacity
                                            // value. So now n refers the amount of memory we need.

      if (can_overflow() && (((uintptr_t)mpBegin != (uintptr_t)mBuffer.buffer) ||
                             (allocSize > kMaxSize)))  // If we are or would be using dynamically allocated memory
                                                       // instead of our fixed-size member buffer...
      {
        T* const pNewData = (allocSize <= kMaxSize) ? (T*)&mBuffer.buffer[0] : DoAllocate(allocSize);
        T* const pCopyEnd = (n < nPrevSize) ? (mpBegin + n) : mpEnd;
        CharStringUninitializedCopy(mpBegin, pCopyEnd,
                                    pNewData);  // Copy [mpBegin, pCopyEnd) to pNewData.
        if ((uintptr_t)mpBegin != (uintptr_t)mBuffer.buffer) DoFree(mpBegin, (size_type)(mpCapacity - mpBegin));

        mpEnd      = pNewData + (pCopyEnd - mpBegin);
        mpBegin    = pNewData;
        mpCapacity = mpBegin + allocSize;
      }                        // Else the new capacity would be within our fixed buffer.
      else if (n < nPrevSize)  // If the newly requested capacity is less than our size, we do what
                               // vector::set_capacity does and resize, even though we actually
                               // aren't reducing the capacity.
        resize(n);
    }
  }
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::size_type
  ...
  inline bool fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::full() const {
    ...
    // Also, if our size is smaller but we've switched away from mBuffer due to a previous overflow,
    // then we are considered full.
  ...
  inline bool fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::has_overflowed() const {
    // This will be incorrect for the case that bOverflowEnabled is true and the container was
    // resized down to a small size where the fixed buffer could take over ownership of the data
    // again. The only simple fix for this is to take on another member variable which tracks
    // whether this overflow has occurred at some point in the past.
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::this_type
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::substr(size_type position, size_type n) const {
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (position > (size_type)(mpEnd - mpBegin)) base_type::ThrowRangeException();
#endif

    return fixed_string(mpBegin + position,
                        mpBegin + position + eastl::min_alt(n, (size_type)(mpEnd - mpBegin) - position));
  }

  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::this_type
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::left(size_type n) const {
    const size_type nLength = size();
    if (n < nLength) return fixed_string(mpBegin, mpBegin + n);
    return *this;
  }

  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::this_type
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::right(size_type n) const {
    const size_type nLength = size();
    if (n < nLength) return fixed_string(mpEnd - n, mpEnd);
    return *this;
  }

  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline const typename fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::overflow_allocator_type&
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::get_overflow_allocator() const EA_NOEXCEPT {
    return mAllocator.get_overflow_allocator();
  }

  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::overflow_allocator_type&
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::get_overflow_allocator() EA_NOEXCEPT {
    return mAllocator.get_overflow_allocator();
  }

  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline void fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::set_overflow_allocator(
  ...
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator> operator+(
    ...
    // We have a problem here because need to return an fixed_string by value. This will typically
    // result in it using stack space equal to its size. That size may be too large to be workable.
    typedef fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator> this_type;
