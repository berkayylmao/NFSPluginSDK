...
#define EASTL_FIXED_STRING_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " fixed_string"  // Unless the user overrides something, this is "EASTL fixed_string".
#endif
  ...
  ///     OverflowAllocator              Overflow allocator, which is only used if bEnableOverflow == true. Defaults to
  ///     the global heap.
  ///
  /// Notes:
  ...
  class fixed_string : public basic_string<T, fixed_vector_allocator<sizeof(T), nodeCount, EASTL_ALIGN_OF(T), 0,
   ...
   protected:
    ...
    {
      value_type          mArray[1];
      aligned_buffer_type mBuffer;  // Question: Why are we doing this aligned_buffer thing? Why not just do an array of
                                    // value_type, given that we are using just strings of char types.
    };
   ...
   public:
    ...
    fixed_string(const base_type& x, size_type position,
                 size_type n = base_type::npos);  // Currently we don't support overflowAllocator specification for
                                                  // other constructors, for simplicity.
    ...
    bool full() const;  // Returns true if the fixed space has been fully allocated. Note that if overflow is enabled,
    ...
    bool has_overflowed() const;  // Returns true if the allocations spilled over into the overflow allocator.
                                  // Meaningful only if overflow is enabled.
...
// We copy from x instead of trade with it. We need to do so because fixed_ containers use local memory buffers.
  ...
  inline void fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::set_capacity(size_type n) {
    ...
    if (n != nPrevCapacity)  // If the request results in a capacity change...
    {
      ...
      if (can_overflow() && (((uintptr_t)internalLayout().HeapBeginPtr() != (uintptr_t)mBuffer.buffer) ||
                                                       ...
                                                       // instead of our fixed-size member buffer...
      {
        ...
        CharStringUninitializedCopy(internalLayout().HeapBeginPtr(), pCopyEnd,
      ...
      else if (n < nPrevSize)  // If the newly requested capacity is less than our size, we do what vector::set_capacity
                               // does and resize, even though we actually aren't reducing the capacity.
        resize(n);
    }
  }
  ...
  inline bool fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::full() const {
    ...
    // Also, if our size is smaller but we've switched away from mBuffer due to a previous overflow, then we are
    // considered full.
    return ((size_t)(internalLayout().HeapEndPtr() - internalLayout().HeapBeginPtr()) >= kMaxSize) ||
  ...
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::substr(size_type position, size_type n) const {
    ...
    if (position > internalLayout().GetSize()) base_type::ThrowRangeException();
#endif
  ...
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::left(size_type n) const {
    ...
    if (n < nLength) return fixed_string(internalLayout().HeapBeginPtr(), internalLayout().HeapBeginPtr() + n);
    return *this;
  }
  ...
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::right(size_type n) const {
    const size_type nLength = size();
    if (n < nLength) return fixed_string(internalLayout().HeapEndPtr() - n, internalLayout().HeapEndPtr());
    return *this;
  }
