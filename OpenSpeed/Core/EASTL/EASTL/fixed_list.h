...
#define EASTL_FIXED_LIST_DEFAULT_ALLOCATOR overflow_allocator_type(EASTL_FIXED_LIST_DEFAULT_NAME)
  ...
  class fixed_list : public list<T, fixed_node_allocator<sizeof(typename list<T>::node_type), nodeCount,
                                                         EASTL_ALIGN_OF(T), 0, bEnableOverflow, OverflowAllocator> > {
   public:
    typedef fixed_node_allocator<sizeof(typename list<T>::node_type), nodeCount, EASTL_ALIGN_OF(T), 0, bEnableOverflow,
                                 OverflowAllocator>
...
// Deprecated:
  ...
  inline bool fixed_list<T, nodeCount, bEnableOverflow, OverflowAllocator>::full() const {
    ...
    return !mAllocator.can_allocate();  // This is the quickest way of detecting this. has_overflowed uses
