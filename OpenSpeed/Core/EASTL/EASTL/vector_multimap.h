...
#define EASTL_VECTOR_MULTIMAP_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " vector_multimap"  // Unless the user overrides something, this is "EASTL vector_multimap".
#endif
...
#define EASTL_VECTOR_MULTIMAP_DEFAULT_ALLOCATOR allocator_type(EASTL_VECTOR_MULTIMAP_DEFAULT_NAME)
  ...
  inline void vector_multimap<K, T, C, A, RAC>::insert(InputIterator first, InputIterator last) {
    ...
    for (; first != last; ++first) base_type::insert(upper_bound((*first).first), *first);
  }
  ...
  template <typename K, typename T, typename C, typename A, typename RAC>
  inline eastl::pair<typename vector_multimap<K, T, C, A, RAC>::iterator, typename vector_multimap<K, T, C, A,
  RAC>::iterator> vector_multimap<K, T, C, A, RAC>::equal_range_small(const key_type& k)
  {
          const iterator itLower(lower_bound(k));
          iterator       itUpper(itLower);
