...
//
//    EASTL_DECLARE_TRIVIAL_RELOCATE(Widget); // Usually you put this at the Widget class
//    declaration. vector<Widget> wVector; wVector.erase(wVector.begin());         // This operation
//    will be optimized via using memcpy.
//
...
//    is_array                              T is an array type. The templated array container is not
//    an array type. is_enum                               T is an enumeration type. is_union T is a
//    union type. is_class                              T is a class type but not a union type.
//    is_function                           T is a function type.
//    is_pointer                            T is a pointer type. Includes function pointers, but not
//    pointers to (data or function) members. is_rvalue_reference is_lvalue_reference
//    is_member_object_pointer              T is a pointer to data member.
//    is_member_function_pointer            T is a pointer to member function.
//
//    is_fundamental                        T is a fundamental type (void, integral, or floating
//    point). is_arithmetic                         T is an arithmetic type (integral or floating
//    point). is_scalar                             T is a scalar type (arithmetic, enum, pointer,
//    member_pointer) is_object                             T is an object type. is_compound T is a
//    compound type (anything but fundamental). is_reference                          T is a
//    reference type. Includes references to functions. is_member_pointer                     T is a
//    pointer to a member or member function.
//
...
//
//    alignment_of                          An integer value representing the number of bytes of the
//    alignment of objects of type T; an object of type T may be allocated at an address that is a
//    multiple of its alignment. rank                                  An integer value representing
//    the rank of objects of type T. The term 'rank' here is used to describe the number of
//    dimensions of an array type. extent                                An integer value
//    representing the extent (dimension) of the I'th bound of objects of type T. If the type T is
//    not an array type, has rank of less than I, or if I == 0 and T is of type 'array of unknown
//    bound of U,' then value shall evaluate to zero; otherwise value shall evaluate to the number
//    of elements in the I'th array bound of T. The term 'extent' here is used to describe the
//    number of elements in an array type.
//
//    is_same                               T and U name the same type.
//    is_base_of                            Base is a base class of Derived or Base and Derived name
//    the same type. is_convertible                        An imaginary lvalue of type From is
//    implicitly convertible to type To. Special conversions involving string-literals and
//    null-pointer constants are not considered. No function-parameter adjustments are made to type
//    To when determining whether From is convertible to To; this implies that if type To is a
//    function type or an array type, then the condition is false.
//
//    remove_cv
//    remove_const                          The member typedef type shall be the same as T except
//    that any top level const-qualifier has been removed. remove_const<const volatile int>::type
//    evaluates to volatile int, whereas remove_const<const int*> is const int*. remove_volatile
//    add_cv
//    add_const
//    add_volatile
//
...
//    is_aligned                            Defined as true if the type has alignment requirements
//    greater than default alignment, which is taken to be 8. is_aligned is not found in Boost nor
//    C++11, though alignment_of is. union_cast                            Allows for easy-to-read
//    casting between types that are unrelated but have binary equivalence. The classic use case is
//    converting between float and int32_t bit representations. is_array_of_known_bounds
...
//    has_nothrow_constructor               The default constructor for T has an empty exception
//    specification or can otherwise be deduced never to throw an exception. has_nothrow_copy The
//    copy constructor for T has an empty exception specification or can otherwise be deduced never
//    to throw an exception. has_nothrow_assign                    The assignment operator for T has
//    an empty exception specification or can otherwise be deduced never to throw an exception.
//   *has_trivial_relocate                  T can be moved to a new location via bitwise copy. Note
//   that C++11 rvalue/move functionality supercedes this.
//
...
//     GCC 2.96         We used to have a separate set of type traits for this compiler, but removed
//     it due to lack of necessity.
//////////////////////////////////////////////////////////////////////////////
...
namespace eastl {
  ...
  //
  // Example usage:
  //    typedef ChosenType = type_select<is_integral<SomeType>::value, ChoiceAType,
  //    ChoiceBType>::type;
  //
...
#define EASTL_TYPE_TRAIT_is_const_CONFORMANCE 1  // is_const is conforming.
  ...
  template <typename T>
  struct is_const<T&> : public eastl::false_type {};  // Note here that T is const, not the reference to T. So is_const
                                                      // is false. See section 8.3.2p1 of the C++ standard.
...
#define EASTL_TYPE_TRAIT_is_volatile_CONFORMANCE 1  // is_volatile is conforming.
  ...
  template <typename T>
  struct is_volatile<T&> : public eastl::false_type {};  // Note here that T is volatile, not the reference to T. So
                                                         // is_const is false. See section 8.3.2p1 of the C++ standard.
...
#define EASTL_TYPE_TRAIT_is_function_CONFORMANCE \
  0  // Can't handle functions with ellipsis arguments, like int printf(const char*, ...). Could be
     // fixed with yet more specializations below.

  template <typename R>
  struct is_function_ptr_helper : public eastl::false_type {};
  template <typename R>
  struct is_function_ptr_helper<R (*)()> : public eastl::true_type {};
  template <typename R, typename Arg0>
  struct is_function_ptr_helper<R (*)(Arg0)> : public eastl::true_type {};
  template <typename R, typename Arg0, typename Arg1>
  struct is_function_ptr_helper<R (*)(Arg0, Arg1)> : public eastl::true_type {};
  template <typename R, typename Arg0, typename Arg1, typename Arg2>
  struct is_function_ptr_helper<R (*)(Arg0, Arg1, Arg2)> : public eastl::true_type {};
  template <typename R, typename Arg0, typename Arg1, typename Arg2, typename Arg3>
  struct is_function_ptr_helper<R (*)(Arg0, Arg1, Arg2, Arg3)> : public eastl::true_type {};
  template <typename R, typename Arg0, typename Arg1, typename Arg2, typename Arg3, typename Arg4>
  struct is_function_ptr_helper<R (*)(Arg0, Arg1, Arg2, Arg3, Arg4)> : public eastl::true_type {};
  template <typename R, typename Arg0, typename Arg1, typename Arg2, typename Arg3, typename Arg4, typename Arg5>
  struct is_function_ptr_helper<R (*)(Arg0, Arg1, Arg2, Arg3, Arg4, Arg5)> : public eastl::true_type {};
  template <typename R, typename Arg0, typename Arg1, typename Arg2, typename Arg3, typename Arg4, typename Arg5,
            typename Arg6>
  struct is_function_ptr_helper<R (*)(Arg0, Arg1, Arg2, Arg3, Arg4, Arg5, Arg6)> : public eastl::true_type {};
  template <typename R, typename Arg0, typename Arg1, typename Arg2, typename Arg3, typename Arg4, typename Arg5,
            typename Arg6, typename Arg7>
  struct is_function_ptr_helper<R (*)(Arg0, Arg1, Arg2, Arg3, Arg4, Arg5, Arg6, Arg7)> : public eastl::true_type {};

  template <bool is_ref = true>
  struct is_function_chooser {
    template <typename T>
    struct result_ : public eastl::false_type {};
  };

  template <>
  struct is_function_chooser<false> {
    template <typename T>
    struct result_ : public eastl::is_function_ptr_helper<T*> {};
  };

  template <typename T>
  struct is_function_value : public eastl::is_function_chooser<eastl::is_reference<T>::value>::template result_<T> {};

  template <typename T>
  struct is_function : public eastl::integral_constant<bool, is_function_value<T>::value> {};
...
#define EASTL_TYPE_TRAIT_is_function_CONFORMANCE 1  // is_function is conforming.
  ...
  struct is_function<ReturnValue /*FunctionName*/ (ArgPack...)> : public eastl::true_type {};

  template <typename ReturnValue, typename... ArgPack>
  struct is_function<ReturnValue /*FunctionName*/ (
      ArgPack...,
...
#if defined(_MSC_VER) && (_MSC_VER <= 1600)  // VS2010 and earlier mistakenly report: "cannot add a reference to a
                                             // zero-sized array." Actually they are allowed, but there's nothing we can
                                             // do about it under VS2010 and earlier.
  template <typename T>
...
#define EASTL_TYPE_TRAIT_add_lvalue_reference_CONFORMANCE 1  // add_lvalue_reference is conforming.
  ...
  struct add_lvalue_reference<T&> {
    typedef T& type;
  };  // This shouldn't be required for modern compilers, as they recognize that a reference to a
...
#if EASTL_NO_RVALUE_REFERENCES
#define EASTL_TYPE_TRAIT_add_rvalue_reference_CONFORMANCE \
  0  // Arguably this could be 1 since it's never wrong, as rvalue references don't exist for this
     // compiler.
...
#define EASTL_TYPE_TRAIT_add_rvalue_reference_CONFORMANCE 1
  ...
  struct add_rvalue_reference {
    typedef T&& type;
  };  // Dinkumware has this as { typedef typename eastl::remove_reference<T>::type&& type; }, but
      // that doesn't seem right to me.
  template <typename T>
  struct add_rvalue_reference<T&> {
    typedef T& type;
  };  // The Standard section 20.7.9.2 specifies that we do this, though it seems like the compiler
      // ought to not require this, as C++11 stipulates that & + && -> &.
  template <>
  ...
  // decltype expressions without specifying constructors. It has no use outside decltype
  // expressions. By design there is no implementation, as it's never executed but rather is used
  // only in decltype expressions. The C++11 Standard section 20.2.4 states that we must declare
  // this. http://en.cppreference.com/w/cpp/utility/declval
  //
  ///////////////////////////////////////////////////////////////////////
...
#if defined(EA_COMPILER_NO_VARIADIC_TEMPLATES) || \
    defined(EA_COMPILER_MSVC)  // VS2013 fails to compile the variadic code below due to what looks
                               // like a deficiency in their handling of integral variadic template
                               // parameters.
...
#include <OpenSpeed/Core/EASTL/EASTL/internal/type_fundamental.h>
#include <OpenSpeed/Core/EASTL/EASTL/internal/type_transformations.h>
#include <OpenSpeed/Core/EASTL/EASTL/internal/type_properties.h>
#include <OpenSpeed/Core/EASTL/EASTL/internal/type_compound.h>
#include <OpenSpeed/Core/EASTL/EASTL/internal/type_pod.h>
