...
namespace eastl {
  ...
  class intrusive_sdlist_base {
   ...
   public:
    ...
    void pop_front();  ///< Removes an element from the front of the list; O(1). The element must be present, but is not
                       ///< deallocated.
    void reverse();    ///< Reverses a list so that front and back are swapped; O(n).

    // bool    validate() const;         ///< Scans a list for linkage inconsistencies; O(n) time, O(1) space. Returns
  ...
  class intrusive_sdlist : public intrusive_sdlist_base {
   ...
   public:
    ...
    void push_front(value_type& value);  ///< Adds an element to the front of the list; O(1). The element is not copied.
                                         ///< The element must not be in any other list.
    void push_back(value_type& value);   ///< Adds an element to the back of the list; O(N). The element is not copied.
                                         ///< The element must not be in any other list.
    void pop_back();  ///< Removes an element from the back of the list; O(N). The element must be present, but is not
                      ///< deallocated.

    bool contains(const value_type& value)
        const;  ///< Returns true if the given element is in the list; O(n). Equivalent to (locate(x) != end()).

    iterator locate(value_type& value);  ///< Converts a reference to an object in the list back to an iterator, or
    ...
    iterator insert(iterator    position,
                    value_type& value);  ///< Inserts an element before the element pointed to by the iterator. O(1)
    ...
    void splice(iterator position, value_type& value);  ///< Moves the given element into this list before the element
                                                        ///< pointed to by position; O(1). Required: x must be in some
                                                        ///< list or have first/next pointers that point it itself.

    void splice(iterator   position,
                this_type& x);  ///< Moves the contents of a list into this list before the element pointed to by
                                ///< position; O(1). Required: &x != this (same as std::list).

    void splice(iterator position, this_type& x,
                iterator xPosition);  ///< Moves the given element pointed to i within the list x into the current list
                                      ///< before the element pointed to by position; O(1).

    void splice(iterator position, this_type& x, iterator first,
                iterator last);  ///< Moves the range of elements [first, last) from list x into the current list before
