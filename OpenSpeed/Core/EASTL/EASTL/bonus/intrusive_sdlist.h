...
namespace eastl {
  ...
  struct IntrusiveSDListIterator {
   ...
   public:
    IntrusiveSDListIterator();
    explicit IntrusiveSDListIterator(pointer pNode);  // Note that you can also construct an iterator from T via this,
  ...
  class intrusive_sdlist_base {
   ...
   public:
    ...
    void pop_front();  ///< Removes an element from the front of the list; O(1). The element must be
                       ///< present, but is not deallocated.
    void reverse();    ///< Reverses a list so that front and back are swapped; O(n).

    // bool    validate() const;         ///< Scans a list for linkage inconsistencies; O(n) time,
    // O(1) space. Returns false if errors are detected, such as loops or branching.
  ...
  class intrusive_sdlist : public intrusive_sdlist_base {
   ...
   public:
    ...
    void push_front(value_type& value);  ///< Adds an element to the front of the list; O(1). The element is not
                                         ///< copied. The element must not be in any other list.
    void push_back(value_type& value);   ///< Adds an element to the back of the list; O(N). The element is not
                                         ///< copied. The element must not be in any other list.
    void pop_back();                     ///< Removes an element from the back of the list; O(N). The element must be
                                         ///< present, but is not deallocated.

    bool contains(const value_type& value) const;  ///< Returns true if the given element is in the list;
                                                   ///< O(n). Equivalent to (locate(x) != end()).

    iterator locate(value_type& value);  ///< Converts a reference to an object in the list back to an iterator,
                                         ///< or returns end() if it is not part of the list. O(n)
    const_iterator locate(
        const value_type& value) const;  ///< Converts a const reference to an object in the list back to a const
                                         ///< iterator, or returns end() if it is not part of the list. O(n)

    iterator insert(iterator    position,
                    value_type& value);  ///< Inserts an element before the element pointed to by
                                         ///< the iterator. O(1)
    iterator erase(iterator position);   ///< Erases the element pointed to by the iterator. O(1)
    iterator erase(iterator first,
                   iterator last);       ///< Erases elements within the iterator range [first, last). O(1).
    ...
    void splice(iterator    position,
                value_type& value);  ///< Moves the given element into this list before the element
                                     ///< pointed to by position; O(1). Required: x must be in some
                                     ///< list or have first/next pointers that point it itself.

    void splice(iterator   position,
                this_type& x);  ///< Moves the contents of a list into this list before the element pointed
                                ///< to by position; O(1). Required: &x != this (same as std::list).

    void splice(iterator position, this_type& x,
                iterator xPosition);  ///< Moves the given element pointed to i within the list x into the
                                      ///< current list before the element pointed to by position; O(1).

    void splice(iterator position, this_type& x, iterator first,
                iterator last);  ///< Moves the range of elements [first, last) from list x into the
                                 ///< current list before the element pointed to by position; O(1).
                                 ///< Required: position must not be in [first, last). (same as
                                 ///< std::list).
    bool validate() const;
  ...
  template <typename T, typename Pointer, typename Reference>
  inline IntrusiveSDListIterator<T, Pointer, Reference>::IntrusiveSDListIterator(pointer pNode) : mpNode(pNode) {}
  ...
  inline intrusive_sdlist_base::size_type intrusive_sdlist_base::size() const {
    size_type n = 0;
    for (const intrusive_sdlist_node* pCurrent = mpNext; pCurrent; pCurrent = pCurrent->mpNext) n++;
    return n;
  }
  ...
  template <typename T>
  inline intrusive_sdlist<T>::intrusive_sdlist(const this_type& /*x*/) : intrusive_sdlist_base() {
  ...
  inline void intrusive_sdlist<T>::push_front(value_type& value) {
    ...
    if (mpNext) mpNext->mppPrevNext = &value.mpNext;
    mpNext = &value;
  }
  ...
  inline void intrusive_sdlist<T>::pop_back() {
    ...
    while (pCurrent->mpNext) pCurrent = static_cast<node_type*>(pCurrent->mpNext);
  ...
  template <typename T>
  inline typename intrusive_sdlist<T>::iterator intrusive_sdlist<T>::insert(iterator position, value_type& value) {
  ...
  template <typename T>
  inline typename intrusive_sdlist<T>::iterator intrusive_sdlist<T>::erase(iterator position) {
  ...
  template <typename T>
  inline typename intrusive_sdlist<T>::iterator intrusive_sdlist<T>::erase(iterator first, iterator last) {
  ...
  inline void intrusive_sdlist<T>::remove(value_type& value) {
    *value.mppPrevNext = value.mpNext;
    if (value.mpNext) value.mpNext->mppPrevNext = value.mppPrevNext;
  }
  ...
  void intrusive_sdlist<T>::swap(intrusive_sdlist& x) {
    ...
    if (x.mpNext) x.mpNext->mppPrevNext = &mpNext;

    if (mpNext) mpNext->mppPrevNext = &x.mpNext;
  }
  ...
  void intrusive_sdlist<T>::splice(iterator /*position*/, value_type& /*value*/) {
    EASTL_ASSERT(false);  // If you need this working, ask Paul Pedriana or submit a working version
                          // for inclusion.
  }
