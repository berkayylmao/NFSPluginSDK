...
#define EASTL_BASIC_STRING_DEFAULT_ALLOCATOR allocator_type(EASTL_BASIC_STRING_DEFAULT_NAME)
  ...
  class abstract_string {
   ...
   public:
    ...
    void      set_capacity(size_type n = npos);  // Revises the capacity to the user-specified value. Resizes the
    ...
    void force_size(size_type n);  // Unilaterally moves the string end position (mpEnd) to the given location.
  ...
  class basic_string : public abstract_string<T> {
   public:
    ...
    EASTL_STRING_EXPLICIT basic_string(const value_type*     p,
                                       const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);
...
// this_type& operator=(this_type&& x);
   ...
   protected:
    ...
    allocator_type mAllocator;  // To consider: Use base class optimization to make this go away. However, this
  ...
  inline const char8_t* Find(const char8_t* p, char8_t c, size_t n) { return (const char8_t*)memchr(p, c, n); }
  ...
  */
  ...
  template <typename T, typename Allocator>
  inline typename basic_string<T, Allocator>::allocator_type& basic_string<T, Allocator>::get_allocator() EA_NOEXCEPT {
    return mAllocator;
  }
  ...
  template <typename T>
  inline typename abstract_string<T>::const_iterator abstract_string<T>::cbegin() const EA_NOEXCEPT {
    return mpBegin;
  }
  ...
  template <typename T>
  inline typename abstract_string<T>::const_reverse_iterator abstract_string<T>::rbegin() const EA_NOEXCEPT {
    return const_reverse_iterator(mpEnd);
  }

  template <typename T>
  inline typename abstract_string<T>::const_reverse_iterator abstract_string<T>::crbegin() const EA_NOEXCEPT {
    return const_reverse_iterator(mpEnd);
  }
  ...
  template <typename T>
  inline typename abstract_string<T>::const_reverse_iterator abstract_string<T>::rend() const EA_NOEXCEPT {
    return const_reverse_iterator(mpBegin);
  }

  template <typename T>
  inline typename abstract_string<T>::const_reverse_iterator abstract_string<T>::crend() const EA_NOEXCEPT {
    return const_reverse_iterator(mpBegin);
  }
  ...
  template <typename T>
  inline typename abstract_string<T>::const_reference abstract_string<T>::operator[](size_type n) const {
  ...
  */
  ...
  void abstract_string<T>::reserve(size_type n) {
    ...
    n = eastl::max_alt(
        n,
        (size_type)(mpEnd - mpBegin));  // Calculate the new capacity, which needs to be >= container size.

    if (n >= (size_type)(mpCapacity - mpBegin))  // If there is something to do... // We use >= because
  ...
  inline typename abstract_string<T>::const_reference abstract_string<T>::at(size_type n) const {
    ...
    if (EASTL_UNLIKELY(n >= (size_type)(mpEnd - mpBegin))) EASTL_FAIL_MSG("abstract_string::at -- out of range");
#endif
  ...
  abstract_string<T>& abstract_string<T>::append(size_type n, value_type c) {
    ...
    if ((s + n) > nCapacity) reserve(eastl::max_alt((size_type)GetNewCapacity(nCapacity), (size_type)(s + n)));
...
#if EASTL_STRING_OPT_LENGTH_ERRORS
      if (EASTL_UNLIKELY(((size_t)n > kMaxSize) || (nOldSize > (kMaxSize - n)))) ThrowLengthException();
#endif

      const size_type nCapacity = (size_type)((mpCapacity - mpBegin) - 1);

      if ((nOldSize + n) > nCapacity) {
        const size_type nLength = eastl::max_alt((size_type)GetNewCapacity(nCapacity), (size_type)(nOldSize + n)) +
  ...
  abstract_string<T>& abstract_string<T>::append_sprintf_va_list(const value_type* pFormat, va_list arguments) {
    ...
    if (mpBegin == GetEmptyString(value_type()))  // We need to do this because non-standard vsnprintf implementations
...
// In this case we definitely have C99 Vsnprintf behaviour.
    ...
    } else if (nReturnValue < 0)                   // If vsnprintf is non-C99-standard (e.g. it is VC++ _vsnprintf)...
    {
      ...
      size_type n =
          eastl::max_alt((size_type)(EASTL_STRING_INITIAL_CAPACITY - 1),
                         (size_type)(size() * 2));  // '-1' because the resize call below will add one for NULL
                                                    // terminator and we want to keep allocations on fixed block sizes.
    ...
    if (nReturnValue >= 0)
      mpEnd =
          mpBegin + nInitialSize + nReturnValue;  // We are guaranteed from the above logic that mpEnd <= mpCapacity.
  ...
  inline void abstract_string<T>::pop_back() {
    ...
    if (EASTL_UNLIKELY(mpEnd <= mpBegin)) EASTL_FAIL_MSG("abstract_string::pop_back -- empty string");
#endif
  ...
  template <typename T>
  inline typename abstract_string<T>::iterator abstract_string<T>::insert(iterator p, value_type c) {
    if (p == mpEnd) {
      push_back(c);
      return mpEnd - 1;
    }
  ...
  void abstract_string<T>::insert(iterator p, size_type n, value_type c) {
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((p < mpBegin) || (p > mpEnd))) EASTL_FAIL_MSG("abstract_string::insert -- invalid position");
#endif
    ...
    if (n)  // If there is anything to insert...
    {
      if (size_type(mpCapacity - mpEnd) >= (n + 1))  // If we have enough capacity...
      {
        ...
        const size_type nLength  = eastl::max_alt((size_type)GetNewCapacity(nOldCap), (size_type)(nOldSize + n)) +
                                  1;  // + 1 to accomodate the trailing 0.
...
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((p < mpBegin) || (p >= mpEnd))) EASTL_FAIL_MSG("abstract_string::erase -- invalid position");
#endif
  ...
  template <typename T>
  inline typename abstract_string<T>::reverse_iterator abstract_string<T>::erase(reverse_iterator position) {
  ...
  template <typename T>
  abstract_string<T>& abstract_string<T>::replace(size_type pos1, size_type n1, const this_type& x, size_type pos2,
                                                  size_type n2) {
#if EASTL_STRING_OPT_RANGE_ERRORS
...
#if EASTL_STRING_OPT_LENGTH_ERRORS
    if (EASTL_UNLIKELY(((size_type)(mpEnd - mpBegin) - nLength1) >= (kMaxSize - nLength2))) ThrowLengthException();
#endif
  ...
  template <typename T>
  abstract_string<T>& abstract_string<T>::replace(iterator pBegin1, iterator pEnd1, const value_type* pBegin2,
                                                  const value_type* pEnd2) {
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((pBegin1 < mpBegin) || (pBegin1 > mpEnd) || (pEnd1 < mpBegin) || (pEnd1 > mpEnd) ||
                       (pEnd1 < pBegin1)))
      EASTL_FAIL_MSG("abstract_string::replace -- invalid position");
#endif
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find(const this_type& x, size_type position) const {
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find(const value_type* p, size_type position,
                                                                  size_type n) const {
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find(value_type c, size_type position) const {
    // It is not clear what the requirements are for position, but since the C++ standard
    // appears to be silent it is assumed for now that position can be any value.
    //#if EASTL_ASSERT_ENABLED
    //    if(EASTL_UNLIKELY(position > (size_type)(mpEnd - mpBegin)))
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::rfind(const this_type& x,
                                                                          size_type        position) const {
    return rfind(x.mpBegin, position, (size_type)(x.mpEnd - x.mpBegin));
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::rfind(const value_type* p,
                                                                          size_type         position) const {
    return rfind(p, position, (size_type)CharStrlen(p));
  }

  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::rfind(const value_type* p, size_type position,
                                                                   size_type n) const {
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::rfind(value_type c, size_type position) const {
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_first_of(const this_type& x,
                                                                                  size_type        position) const {
    return find_first_of(x.mpBegin, position, (size_type)(x.mpEnd - x.mpBegin));
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_first_of(const value_type* p,
                                                                                  size_type         position) const {
    return find_first_of(p, position, (size_type)CharStrlen(p));
  }

  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find_first_of(const value_type* p, size_type position,
                                                                           size_type n) const {
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_first_of(value_type c,
                                                                                  size_type  position) const {
    return find(c, position);
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_last_of(const this_type& x,
                                                                                 size_type        position) const {
    return find_last_of(x.mpBegin, position, (size_type)(x.mpEnd - x.mpBegin));
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_last_of(const value_type* p,
                                                                                 size_type         position) const {
    return find_last_of(p, position, (size_type)CharStrlen(p));
  }

  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find_last_of(const value_type* p, size_type position,
                                                                          size_type n) const {
    // If n is zero or position is >= size, we return npos.
    const size_type nLength = (size_type)(mpEnd - mpBegin);
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_last_of(value_type c,
                                                                                 size_type  position) const {
    return rfind(c, position);
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_first_not_of(const this_type& x,
                                                                                      size_type        position) const {
    return find_first_not_of(x.mpBegin, position, (size_type)(x.mpEnd - x.mpBegin));
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_first_not_of(const value_type* p,
                                                                                      size_type position) const {
    return find_first_not_of(p, position, (size_type)CharStrlen(p));
  }

  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find_first_not_of(const value_type* p, size_type position,
                                                                               size_type n) const {
    if (EASTL_LIKELY(position <= (size_type)(mpEnd - mpBegin))) {
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find_first_not_of(value_type c, size_type position) const {
    if (EASTL_LIKELY(position <= (size_type)(mpEnd - mpBegin))) {
  ...
  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_last_not_of(const this_type& x,
                                                                                     size_type        position) const {
    return find_last_not_of(x.mpBegin, position, (size_type)(x.mpEnd - x.mpBegin));
  }

  template <typename T>
  inline typename abstract_string<T>::size_type abstract_string<T>::find_last_not_of(const value_type* p,
                                                                                     size_type         position) const {
    return find_last_not_of(p, position, (size_type)CharStrlen(p));
  }

  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find_last_not_of(const value_type* p, size_type position,
                                                                              size_type n) const {
    const size_type nLength = (size_type)(mpEnd - mpBegin);
  ...
  template <typename T>
  typename abstract_string<T>::size_type abstract_string<T>::find_last_not_of(value_type c, size_type position) const {
    const size_type nLength = (size_type)(mpEnd - mpBegin);
  ...
  template <typename T>
  inline int abstract_string<T>::compare(size_type pos1, size_type n1, const this_type& x, size_type pos2,
                                         size_type n2) const {
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY((pos1 > (size_type)(mpEnd - mpBegin)) || (pos2 > (size_type)(x.mpEnd - x.mpBegin))))
      ThrowRangeException();
  ...
  template <typename T>
  int abstract_string<T>::compare(const value_type* pBegin1, const value_type* pEnd1, const value_type* pBegin2,
                                  const value_type* pEnd2) {
  ...
  template <typename T>
  int abstract_string<T>::comparei(const value_type* pBegin1, const value_type* pEnd1, const value_type* pBegin2,
                                   const value_type* pEnd2) {
    const ptrdiff_t n1   = pEnd1 - pBegin1;
    const ptrdiff_t n2   = pEnd2 - pBegin2;
    const ptrdiff_t nMin = eastl::min_alt(n1, n2);
  ...
  template <typename T>
  typename abstract_string<T>::iterator abstract_string<T>::InsertInternal(iterator p, value_type c) {
    ...
    if ((mpEnd + 1) < mpCapacity) {
      ...
      *p = c;
      ...
      const size_type nOldSize = (size_type)(mpEnd - mpBegin);
      const size_type nOldCap  = (size_type)((mpCapacity - mpBegin) - 1);
      const size_type nLength  = eastl::max_alt((size_type)GetNewCapacity(nOldCap), (size_type)(nOldSize + 1)) +
...
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(n >= 0x40000000)) EASTL_FAIL_MSG("abstract_string::AllocateSelf -- improbably large request.");
#endif
  ...
  template <typename T>
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringFindEnd(const value_type* pBegin,
  ...
  template <typename T>
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringRFind(const value_type* pRBegin,
  ...
  template <typename T>
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringSearch(const value_type* p1Begin,
  ...
  template <typename T>
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringRSearch(const value_type* p1Begin,
                                                                                           const value_type* p1End,
                                                                                           const value_type* p2Begin,
                                                                                           const value_type* p2End) {
    // Test for zero length strings, in which case we have a match or a failure,
  ...
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringFindFirstOf(
      const value_type* p1Begin, const value_type* p1End, const value_type* p2Begin, const value_type* p2End) {
  ...
  const typename abstract_string<T>::value_type* abstract_string<T>::CharTypeStringRFindFirstOf(
      const value_type* p1RBegin, const value_type* p1REnd, const value_type* p2Begin, const value_type* p2End) {
  ...
  inline bool operator==(const abstract_string<T>& a, const abstract_string<T>& b) {
    return ((a.size() == b.size()) &&
            (memcmp(a.data(), b.data(), (size_t)a.size() * sizeof(typename abstract_string<T>::value_type)) == 0));
  }
  ...
  ///
  /// Example usage:
  ///    #include <OpenSpeed/Core/EASTL/EASTL/hash_set.h>
