...
namespace eastl {
  ...
  class intrusive_slist : public intrusive_slist_base {
   ...
   public:
    ...
    // intrusive_slist(const this_type& x);      ///< Creates an empty list; ignores the argument. To consider: Is this
    // a useful function? this_type& operator=(const this_type& x); ///< Clears the list; ignores the argument.       To
    // consider: Is this a useful function?
    ...
    void pop_front();  ///< Removes an element from the back of the list; O(n). The element must be present, but is not
                       ///< deallocated.
    ...
    iterator insert(iterator    position,
                    value_type& value);  ///< Inserts an element before the element pointed to by the iterator. O(n)
    iterator insert_after(
        iterator    position,
        value_type& value);  ///< Inserts an element after the element pointed to by the iterator. O(1)
    ...
    void splice_after(
        iterator    position,
        value_type& value);  ///< Moves the given element into this list after the element pointed to by position; O(1).
                             ///< Required: x must be in some list or have first/next pointers that point it itself.

    void splice_after(iterator   position,
                      this_type& x);  ///< Moves the contents of a list into this list after the element pointed to by
                                      ///< position; O(n). Required: &x != this (same as std::list).
    ...
    void splice_after(iterator position, this_type& x, iterator before_first,
                      iterator before_last);  ///< Moves the elements in the range of [before_first+1, before_last+1) to
                                              ///< be after position. O(1).
