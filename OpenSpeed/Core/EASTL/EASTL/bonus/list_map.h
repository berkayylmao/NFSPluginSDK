...
#define EASTL_LIST_MAP_DEFAULT_ALLOCATOR allocator_type(EASTL_LIST_MAP_DEFAULT_NAME)
  ...
  struct use_value_first {
    ...
    const result_type& operator()(const Object& x) const { return x.mValue.first; }
  };
  ...
  /// counterparts in list and map.  However, as the node count increases, list_map quickly
  /// outperforms either a list or a map when find [by-index] and front/back type operations are
  /// required.
  ///
  /// In essence, list_map avoids O(n) iterations at the expense of additional costs to quick (O(1)
  /// and O(log n) operations:
  ///     push_front(), push_back(), pop_front() and pop_back() have O(log n) operation time,
  ///     similar to map::insert(), rather than O(1) time like a list, however, front() and back()
  ///     maintain O(1) operation time.
  ///
  /// As a canonical example, consider a large backlog of player group invites, which are removed
  /// when either:
  ///     The invitation times out - in main loop:  while( !listMap.empty() &&
  ///     listMap.front().IsExpired() ) { listMap.pop_front(); } The player rejects the outstanding
  ///     invitation - on rejection:  iter = listMap.find(playerId);  if (iter != listMap.end()) {
  ///     listMap.erase(iter); }
  ///
  ...
  /// can quickly result in unacceptable performance when executing even infrequenty O(n)
  /// operations.
  ///
  ...
  ///     typedef list_map<Widget, int, less<Widget>, MemoryPool> WidgetMap;  // Delare your
  ///     WidgetMap type. MemoryPool myPool(sizeof(WidgetMap::node_type), 100);               //
  ///     Make a pool of 100 Widget nodes. WidgetMap myMap(&myPool); // Create a map that uses the
  ///     pool.
  ///
  template <typename Key, typename T, typename Compare = eastl::less<Key>, typename Allocator = EASTLAllocatorType>
  class list_map
      : protected rbtree<Key, eastl::list_map_data<eastl::pair<const Key, T> >, Compare, Allocator,
                         eastl::use_value_first<eastl::list_map_data<eastl::pair<const Key, T> > >, true, true> {
   public:
    ...
    typedef typename base_type::node_type node_type;  // Despite the internal and external values being different, we're
                                                      // keeping the node type the same as the base in order to allow
                                                      // for pool allocation.  See EASTL/map.h for more information.
    typedef typename eastl::list_map_iterator<value_type, value_type*, value_type&>
        iterator;  // This is intentionally different from base_type::iterator
    typedef typename eastl::list_map_iterator<value_type, const value_type*, const value_type&>
    ...
    typedef typename eastl::pair<iterator, bool>
        insert_return_type;  // This is intentionally removed, as list_map doesn't support insert()
                             // functions, in favor of list like push_back and push_front
   ...
   public:
    ...
    //#endif
    // list_map(std::initializer_list<mapped_type> ilist, const Compare& compare = Compare(), const
    // allocator_type& allocator = EASTL_LIST_MAP_DEFAULT_ALLOCATOR);
   ...
   public:
// map like functionality which list_map does not support, due to abmiguity with list like
// functionality:
#if !defined(EA_COMPILER_NO_DELETED_FUNCTIONS)
    template <typename InputIterator>
    list_map(InputIterator first, InputIterator last, const Compare& compare,
             const allocator_type& allocator = EASTL_RBTREE_DEFAULT_ALLOCATOR) = delete;
   ...
   public:
// list like functionality which list_map does not support, due to ambiguity with map like
// functionality:
#if 0
  ...
  template <typename Value>
  inline list_map_data<Value>::list_map_data(const Value& value) : mValue(value) {
    mpNext = NULL;  // GCC 4.8 is generating warnings about referencing these values in
                    // list_map::push_front unless we
    mpPrev = NULL;  // initialize them here. The compiler seems to be mistaken, as our code isn't
                    // actually using them unintialized.
  }
  ...
  template <typename T, typename Pointer, typename Reference>
  inline list_map_iterator<T, Pointer, Reference>::list_map_iterator() : mpNode(NULL) {
    // Empty
  }
  ...
  // We provide additional template paremeters here to support comparisons between const and
  // non-const iterators. See C++ defect report #179, or EASTL/list.h for more information.
  ...
  inline list_map<Key, T, Compare, Allocator>::list_map(const allocator_type& allocator) : base_type(allocator) {
    mNode.mpNext = &mNode;
    mNode.mpPrev = &mNode;
  }
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::iterator list_map<Key, T, Compare, Allocator>::begin()
      EA_NOEXCEPT {
    return iterator(mNode.mpNext);
  }

  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::const_iterator list_map<Key, T, Compare, Allocator>::begin()
      const EA_NOEXCEPT {
    return const_iterator(mNode.mpNext);
  }

  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::const_iterator list_map<Key, T, Compare, Allocator>::cbegin()
      const EA_NOEXCEPT {
    return const_iterator(mNode.mpNext);
  }

  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::iterator list_map<Key, T, Compare, Allocator>::end()
      EA_NOEXCEPT {
    return iterator(&mNode);
  }

  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::const_iterator list_map<Key, T, Compare, Allocator>::end() const
      EA_NOEXCEPT {
    return const_iterator(&mNode);
  }

  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::const_iterator list_map<Key, T, Compare, Allocator>::cend()
      const EA_NOEXCEPT {
    return const_iterator(&mNode);
  }

  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::reverse_iterator list_map<Key, T, Compare, Allocator>::rbegin()
      EA_NOEXCEPT {
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::reverse_iterator list_map<Key, T, Compare, Allocator>::rend()
      EA_NOEXCEPT {
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::reference list_map<Key, T, Compare, Allocator>::front() {
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::const_reference list_map<Key, T, Compare, Allocator>::front()
      const {
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
    // We allow the user to reference an empty container.
#elif EASTL_ASSERT_ENABLED
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::reference list_map<Key, T, Compare, Allocator>::back() {
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
    // We allow the user to reference an empty container.
#elif EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(static_cast<internal_value_type*>(mNode.mpNext) == &mNode))
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::const_reference list_map<Key, T, Compare, Allocator>::back()
      const {
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
    // We allow the user to reference an empty container.
#elif EASTL_ASSERT_ENABLED
  ...
  void list_map<Key, T, Compare, Allocator>::pop_front() {
    ...
    if (EASTL_UNLIKELY(empty())) EASTL_FAIL_MSG("list_map::pop_front -- empty container");
#endif
...
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(empty())) EASTL_FAIL_MSG("list_map::pop_back -- empty container");
#endif
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::iterator list_map<Key, T, Compare, Allocator>::find(
      const key_type& key) {
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::const_iterator list_map<Key, T, Compare, Allocator>::find(
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  template <typename U, typename Compare2>
  inline typename list_map<Key, T, Compare, Allocator>::iterator list_map<Key, T, Compare, Allocator>::find_as(
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  template <typename U, typename Compare2>
  inline typename list_map<Key, T, Compare, Allocator>::const_iterator list_map<Key, T, Compare, Allocator>::find_as(
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::size_type list_map<Key, T, Compare, Allocator>::count(
      const key_type& key) const {
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::size_type list_map<Key, T, Compare, Allocator>::erase(
      const key_type& key) {
    typename base_type::iterator baseIter = base_type::find(key);
    if (baseIter != base_type::end()) {
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::iterator list_map<Key, T, Compare, Allocator>::erase(
  ...
  template <typename Key, typename T, typename Compare, typename Allocator>
  inline typename list_map<Key, T, Compare, Allocator>::reverse_iterator list_map<Key, T, Compare, Allocator>::erase(
  ...
  int list_map<Key, T, Compare, Allocator>::validate_iterator(const_iterator iter) const {
    ...
    if (iter == end()) return (isf_valid | isf_current);
