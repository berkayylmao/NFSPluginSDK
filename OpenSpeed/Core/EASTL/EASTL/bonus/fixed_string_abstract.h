...
#define EASTL_FIXED_STRING_DEFAULT_NAME \
  ...
  class fixed_string : public basic_string<T, fixed_vector_allocator<sizeof(T), nodeCount, EASTL_ALIGN_OF(T), 0,
   ...
   public:
    ...
    fixed_string(const this_type& x);  // Currently we don't support overflowAllocator specification
                                       // for other constructors, for simplicity.
  ...
  inline fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixed_string(const abstract_type& x) :
      base_type(fixed_allocator_type(mBuffer.buffer)) {
#if EASTL_NAME_ENABLED  // abstract_string doesn't have a predefined allocator type we can copy
                        // from.
    mAllocator.set_name(EASTL_FIXED_STRING_DEFAULT_NAME);
#endif
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline typename fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::this_type&
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::operator=(const abstract_type& x) {
  ...
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::substr(size_type position, size_type n) const {
    ...
    if (position > (size_type)(mpEnd - mpBegin)) ThrowRangeException();
#endif
  ...
  template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
  inline void* fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::AllocFreeMethod(size_t n, void* pBuffer,
                                                                                               void* pContext) {
    // Use the basic_string method to reuse the same allocator code
    // In reality, we should have a specific implementation
    // We could also handle the overflow code at this level (instead of doing it at the allocator
    // level)
