...
namespace eastl {
  ...
  class ring_buffer {
   ...
   public:        // We declare public so that global comparison operators can be implemented without adding
                  // an inline level and without tripping up GCC 2.x friend declaration failures. GCC
                  // (through at least v4.0) is poor at inlining and performance wins over correctness.
    Container c;  // We follow the naming convention established for stack, queue, priority_queue
                  // and name this 'c'. This variable must always have a size of at least 1, as even
                  // an empty ring_buffer has an unused terminating element.
   ...
   public:
    // There currently isn't a ring_buffer constructor that specifies and initial size, unlike other
    // containers.
    ...
    ring_buffer(std::initializer_list<value_type> ilist,
                const allocator_type& allocator = allocator_type());  // This function sets the capacity to be equal to
    ...
    void      set_capacity(size_type n);  // Sets the capacity to the given value, including values less
                                          // than the current capacity. Adjusts the size downward if n <
                                          // size, by throwing out the oldest elements in the buffer.
    void reserve(size_type n);            // Reserve a given capacity. Doesn't decrease the capacity; it only
                                          // increases it (for compatibility with other containers' behavior).
    ...
    // size_type read(iterator** pPosition1, iterator** pPosition2, size_type& nCount1, size_type&
    // nCount2);
  ...
  ring_buffer_iterator<T, Pointer, Reference, Container>::operator++() {
    if (EASTL_UNLIKELY(++mContainerIterator == mpContainer->end())) mContainerIterator = mpContainer->begin();
    return *this;
  }
  ...
  typename ring_buffer_iterator<T, Pointer, Reference, Container>::this_type&
  ring_buffer_iterator<T, Pointer, Reference, Container>::operator--() {
    if (EASTL_UNLIKELY(mContainerIterator == mpContainer->begin())) mContainerIterator = mpContainer->end();
    --mContainerIterator;
    return *this;
  }
  ...
  template <typename T, typename PointerA, typename ReferenceA, typename PointerB, typename ReferenceB,
            typename Container>
  inline typename ring_buffer_iterator<T, PointerA, ReferenceA, Container>::difference_type operator-(
      const ring_buffer_iterator<T, PointerA, ReferenceA, Container>& a,
    ...
    for (ring_buffer_iterator<T, PointerA, ReferenceA, Container> temp(b); temp != a; ++temp) ++d;
      ...
      c()  // Default construction with default allocator for the container.
  {
    ...
    c.resize(cap + 1);  // Possibly we could construct 'c' with size, but c may not have such a
                        // ctor, though we rely on it having a resize function.
    mBegin = c.begin();
    mEnd   = mBegin;
    mSize  = 0;
  }
  ...
  template <typename T, typename Container, typename Allocator>
  ring_buffer<T, Container, Allocator>::ring_buffer(size_type cap, const allocator_type& allocator) : c(allocator) {
    // To do: This code needs to be amended to deal with possible exceptions
    // that could occur during the resize call below.
  ...
  ring_buffer<T, Container, Allocator>::ring_buffer(const Container& x) :
      c(x)  // This copies elements from x, but unless the user is doing some tricks, the only
            // thing that matters is that c.size() == x.size().
  {
    // To do: This code needs to be amended to deal with possible exceptions
    // that could occur during the resize call below.
    if (c.empty()) c.resize(1);
    mBegin = c.begin();
    mEnd   = mBegin;
    mSize  = 0;
  }
  ...
  template <typename T, typename Container, typename Allocator>
  ring_buffer<T, Container, Allocator>::ring_buffer(const allocator_type& allocator) : c(allocator) {
    // To do: This code needs to be amended to deal with possible exceptions
    // that could occur during the resize call below.

    // We add one because the element at mEnd is necessarily unused.
    c.resize(1);  // Possibly we could construct 'c' with size, but c may not have such a ctor,
                  // though we rely on it having a resize function.
    mBegin = c.begin();
    mEnd   = mBegin;
    mSize  = 0;
  }
  ...
  ring_buffer<T, Container, Allocator>::ring_buffer(const this_type& x) : c(x.c) {
    ...
    eastl::advance(mBegin, eastl::distance(const_cast<this_type&>(x).c.begin(),
      ...
      c()  // Default construction with default allocator for the container.
  {
    ...
    swap(x);  // We are leaving x in an unusual state by swapping default-initialized members with
              // it, as it won't be usable and can be only destructible.
  }

  template <typename T, typename Container, typename Allocator>
  ring_buffer<T, Container, Allocator>::ring_buffer(this_type&& x, const allocator_type& allocator) : c(allocator) {
    c.resize(1);  // Possibly we could construct 'c' with size, but c may not have such a ctor,
                  // though we rely on it having a resize function.
    mBegin = c.begin();
    mEnd   = mBegin;
  ...
  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::this_type& ring_buffer<T, Container, Allocator>::operator=(
      const this_type& x) {
    if (&x != this) {
      c = x.c;
  ...
  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::iterator ring_buffer<T, Container, Allocator>::begin() EA_NOEXCEPT {
    return iterator(&c, mBegin);
  }

  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::const_iterator ring_buffer<T, Container, Allocator>::begin() const
      EA_NOEXCEPT {
  ...
  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::const_iterator ring_buffer<T, Container, Allocator>::cbegin() const
      EA_NOEXCEPT {
    return const_iterator(const_cast<Container*>(&c),
                          mBegin);  // We trust that the const_iterator will respect const-ness.
  }
  ...
  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::iterator ring_buffer<T, Container, Allocator>::end() EA_NOEXCEPT {
    return iterator(&c, mEnd);
  }

  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::const_iterator ring_buffer<T, Container, Allocator>::end() const
      EA_NOEXCEPT {
    return const_iterator(const_cast<Container*>(&c),
  ...
  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::const_iterator ring_buffer<T, Container, Allocator>::cend() const
      EA_NOEXCEPT {
    return const_iterator(const_cast<Container*>(&c),
                          mEnd);  // We trust that the const_iterator will respect const-ness.
  }
  ...
  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::reverse_iterator ring_buffer<T, Container, Allocator>::rbegin()
      EA_NOEXCEPT {
    return reverse_iterator(iterator(&c, mEnd));
  }

  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::const_reverse_iterator ring_buffer<T, Container, Allocator>::rbegin()
      const EA_NOEXCEPT {
    return const_reverse_iterator(const_iterator(const_cast<Container*>(&c), mEnd));
  }

  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::const_reverse_iterator ring_buffer<T, Container, Allocator>::crbegin()
      const EA_NOEXCEPT {
    return const_reverse_iterator(const_iterator(const_cast<Container*>(&c), mEnd));
  }

  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::reverse_iterator ring_buffer<T, Container, Allocator>::rend()
      EA_NOEXCEPT {
    return reverse_iterator(iterator(&c, mBegin));
  }

  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::const_reverse_iterator ring_buffer<T, Container, Allocator>::rend()
      const EA_NOEXCEPT {
    return const_reverse_iterator(const_iterator(const_cast<Container*>(&c), mBegin));
  }

  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::const_reverse_iterator ring_buffer<T, Container, Allocator>::crend()
      const EA_NOEXCEPT {
    return const_reverse_iterator(const_iterator(const_cast<Container*>(&c), mBegin));
  }
  ...
  bool ring_buffer<T, Container, Allocator>::full() const EA_NOEXCEPT {
    ...
    // return mSize == (c.size() - 1); // (c.size() - 1) == capacity(); we are attempting to reduce
    // function calls.
  ...
  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::size_type ring_buffer<T, Container, Allocator>::size() const
      EA_NOEXCEPT {
    return mSize;
  ...
  */
  ...
  ring_buffer<T, Container, Allocator>::DoGetSize(EASTL_ITC_NS::random_access_iterator_tag) const
  {
          ...
          //
          const difference_type dBegin = eastl::distance(const_cast<Container&>(c).begin(), mBegin);
  // const_cast here solves a little compiler const difference_type dEnd   =
  eastl::distance(const_cast<Container&>(c).begin(), mEnd);   // argument matching problem.
  ...
  */
  ...
  void ring_buffer<T, Container, Allocator>::resize(size_type n) {
    ...
    if (n > cap)  // If we need to grow in capacity...
    {
      ...
      mBegin = c.begin();
      mEnd   = mBegin;
      eastl::advance(mEnd, n);  // We can do a simple advance algorithm on this because we know that
                                // mEnd will not wrap around.
    } else                      // We could do a check here for n != size(), but that would be costly and people don't
                                // usually resize things to their same size.
    {
      mEnd = mBegin;

      // eastl::advance(mEnd, n); // We *cannot* use this because there may be wraparound involved.

      // To consider: Possibly we should implement some more detailed logic to optimize the code
      // here. We'd need to do different behaviour dending on whether the container iterator type is
      // a random access iterator or otherwise.

      while (n--) {
        if (EASTL_UNLIKELY(++mEnd == c.end())) mEnd = c.begin();
      }
    }
  }

  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::size_type ring_buffer<T, Container, Allocator>::capacity() const
      EA_NOEXCEPT {
    EASTL_ASSERT(c.size() >= 1);  // This is required because even an empty ring_buffer has one unused
  ...
  void ring_buffer<T, Container, Allocator>::set_capacity(size_type n) {
    ...
    if (n != capacity)  // If we need to change capacity...
    {
      ...
      eastl::copy(itCopyBegin, end(),
                  cTemp.get().begin());  // The begin-end range may in fact be larger than n, in
                                         // which case values will be overwritten.
      eastl::swap(c, cTemp.get());

      mBegin = c.begin();
      mEnd   = mBegin;
      eastl::advance(mEnd, mSize);  // We can do a simple advance algorithm on this because we know
                                    // that mEnd will not wrap around.
    }
  }
  ...
  void ring_buffer<T, Container, Allocator>::reserve(size_type n) {
    ...
    if (n > (c.size() - 1))  // If we need to grow in capacity... // (c.size() - 1) == capacity();
                             // we are attempting to reduce function calls.
    {
      ContainerTemporary<Container, sizeof(Container) >= EASTL_MAX_STACK_USAGE> cTemp;
      cTemp.get().resize(n + 1);
      eastl::copy(begin(), end(), cTemp.get().begin());
  ...
  void ring_buffer<T, Container, Allocator>::push_back(const value_type& value) {
    *mEnd = value;
    ...
    if (++mEnd == c.end()) mEnd = c.begin();

    if (mEnd == mBegin) {
      if (++mBegin == c.end()) mBegin = c.begin();
    } else
      ++mSize;
  }
  ...
  void ring_buffer<T, Container, Allocator>::pop_back() {
    ...
    if (EASTL_UNLIKELY(mEnd == c.begin())) mEnd = c.end();
    --mEnd;
    --mSize;
  }
  ...
  void ring_buffer<T, Container, Allocator>::push_front(const value_type& value) {
    if (EASTL_UNLIKELY(mBegin == c.begin())) mBegin = c.end();
  ...
  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::reference ring_buffer<T, Container, Allocator>::operator[](
      size_type n) {
  ...
  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::const_reference ring_buffer<T, Container, Allocator>::operator[](
      size_type n) const {
    // return *(begin() + n); // Can't use this because not all iterators support operator+.
  ...
  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::iterator ring_buffer<T, Container, Allocator>::insert(
  ...
  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::iterator ring_buffer<T, Container, Allocator>::erase(
  ...
  template <typename T, typename Container, typename Allocator>
  typename ring_buffer<T, Container, Allocator>::reverse_iterator ring_buffer<T, Container, Allocator>::erase(
  ...
  inline bool ring_buffer<T, Container, Allocator>::validate() const {
    if (!c.validate())  // This requires that the container implement the validate function. That
                        // pretty much
      return false;     // means that the container is an EASTL container and not a std STL container.

    if (c.empty())  // c must always have a size of at least 1, as even an empty ring_buffer has an
                    // unused terminating element.
      return false;

    if (size() > capacity()) return false;

    if ((validate_iterator(begin()) & (isf_valid | isf_current)) != (isf_valid | isf_current)) return false;

    if ((validate_iterator(end()) & (isf_valid | isf_current)) != (isf_valid | isf_current)) return false;
