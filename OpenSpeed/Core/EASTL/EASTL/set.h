...
#define EASTL_MULTISET_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " multiset"  // Unless the user overrides something, this is "EASTL multiset".
#endif
...
#define EASTL_MULTISET_DEFAULT_ALLOCATOR allocator_type(EASTL_MULTISET_DEFAULT_NAME)
  ...
  class set : public rbtree<Key, Key, Compare, Allocator, eastl::use_self<Key>, false, true> {
   ...
   public:
    ...
    set(Iterator itBegin, Iterator itEnd);  // allocator arg removed because VC7.1 fails on the default arg. To do: Make
                                            // a second version of this function without a default arg.
  ...
  class multiset : public rbtree<Key, Key, Compare, Allocator, eastl::use_self<Key>, false, false> {
   ...
   public:
    ...
    template <typename Iterator>
    multiset(Iterator itBegin, Iterator itEnd);  // allocator arg removed because VC7.1 fails on the default arg. To do:
  ...
  multiset<Key, Compare, Allocator>::equal_range_small(const Key& k) {
    ...
    while ((itUpper != end()) && !compare(k, itUpper.mpNode->mValue)) ++itUpper;
  ...
  multiset<Key, Compare, Allocator>::equal_range_small(const Key& k) const {
    ...
    while ((itUpper != end()) && !compare(k, *itUpper)) ++itUpper;
