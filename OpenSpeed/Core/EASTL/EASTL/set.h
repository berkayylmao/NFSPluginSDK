...
#include <OpenSpeed/Core/EASTL/EASTL/internal/red_black_tree.h>
...
#define EASTL_SET_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " set"  // Unless the user overrides something, this is "EASTL set".
#endif
...
#define EASTL_MULTISET_DEFAULT_ALLOCATOR allocator_type(EASTL_MULTISET_DEFAULT_NAME)
  ...
  ///     MemoryPool myPool(sizeof(WidgetSet::node_type), 100);       // Make a pool of 100 Widget
  ///     nodes. WidgetSet mySet(&myPool);                                   // Create a map that
  ///     uses the pool.
  ///
  template <typename Key, typename Compare = eastl::less<Key>, typename Allocator = EASTLAllocatorType>
  class set : public rbtree<Key, Key, Compare, Allocator, eastl::use_self<Key>, false, true> {
   public:
...
#endif
    set(std::initializer_list<value_type> ilist, const Compare& compare = Compare(),
        const allocator_type& allocator = EASTL_SET_DEFAULT_ALLOCATOR);

    template <typename Iterator>
    set(Iterator itBegin,
        Iterator itEnd);  // allocator arg removed because VC7.1 fails on the default arg. To do:
                          // Make a second version of this function without a default arg.

#if EASTL_MOVE_SEMANTICS_ENABLED  // The (this_type&& x) ctor above has the side effect of forcing
                                  // us to make operator= visible in this subclass.
  ...
  /// count of pool items, you would do this (assuming that MemoryPool implements
  /// the Allocator interface):
  ///     typedef multiset<Widget, less<Widget>, MemoryPool> WidgetSet;   // Delare your WidgetSet
  ///     type. MemoryPool myPool(sizeof(WidgetSet::node_type), 100);           // Make a pool of
  ///     100 Widget nodes. WidgetSet mySet(&myPool);                                       //
  ///     Create a map that uses the pool.
  ///
  template <typename Key, typename Compare = eastl::less<Key>, typename Allocator = EASTLAllocatorType>
  class multiset : public rbtree<Key, Key, Compare, Allocator, eastl::use_self<Key>, false, false> {
   public:
...
#endif
    multiset(std::initializer_list<value_type> ilist, const Compare& compare = Compare(),
             const allocator_type& allocator = EASTL_MULTISET_DEFAULT_ALLOCATOR);

    template <typename Iterator>
    multiset(Iterator itBegin,
             Iterator itEnd);  // allocator arg removed because VC7.1 fails on the default arg. To
  ...
  inline set<Key, Compare, Allocator>::set(const allocator_type& allocator) : base_type(allocator) {}
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline set<Key, Compare, Allocator>::set(const this_type& x) : base_type(x) {}

#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename Key, typename Compare, typename Allocator>
  inline set<Key, Compare, Allocator>::set(this_type&& x) : base_type(eastl::move(x)) {}
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline set<Key, Compare, Allocator>::set(std::initializer_list<value_type> ilist, const Compare& compare,
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline typename set<Key, Compare, Allocator>::size_type set<Key, Compare, Allocator>::erase(const Key& k) {
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline typename set<Key, Compare, Allocator>::iterator set<Key, Compare, Allocator>::erase(const_iterator position) {
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline typename set<Key, Compare, Allocator>::iterator set<Key, Compare, Allocator>::erase(const_iterator first,
                                                                                             const_iterator last) {
    // We need to provide this version because we override another version
    // and C++ hiding rules would make the base version of this hidden.
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline typename set<Key, Compare, Allocator>::size_type set<Key, Compare, Allocator>::count(const Key& k) const {
    const const_iterator it(find(k));
    return (it != end()) ? (size_type)1 : (size_type)0;
  }

  template <typename Key, typename Compare, typename Allocator>
  inline typename set<Key, Compare, Allocator>::reverse_iterator set<Key, Compare, Allocator>::erase(
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline eastl::pair<typename set<Key, Compare, Allocator>::iterator, typename set<Key, Compare, Allocator>::iterator>
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline multiset<Key, Compare, Allocator>::multiset(const allocator_type& allocator) : base_type(allocator) {}
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline multiset<Key, Compare, Allocator>::multiset(const this_type& x) : base_type(x) {}

#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename Key, typename Compare, typename Allocator>
  inline multiset<Key, Compare, Allocator>::multiset(this_type&& x) : base_type(eastl::move(x)) {}
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline multiset<Key, Compare, Allocator>::multiset(std::initializer_list<value_type> ilist, const Compare& compare,
                                                     const allocator_type& allocator) :
      base_type(ilist.begin(), ilist.end(), compare, allocator) {}
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline typename multiset<Key, Compare, Allocator>::value_compare multiset<Key, Compare, Allocator>::value_comp()
      const {
    return mCompare;
  }

  template <typename Key, typename Compare, typename Allocator>
  inline typename multiset<Key, Compare, Allocator>::size_type multiset<Key, Compare, Allocator>::erase(const Key& k) {
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline typename multiset<Key, Compare, Allocator>::iterator multiset<Key, Compare, Allocator>::erase(
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline typename multiset<Key, Compare, Allocator>::size_type multiset<Key, Compare, Allocator>::count(
      const Key& k) const {
  ...
  template <typename Key, typename Compare, typename Allocator>
  inline typename multiset<Key, Compare, Allocator>::reverse_iterator multiset<Key, Compare, Allocator>::erase(
      const_reverse_iterator position) {
    return reverse_iterator(erase((++position).base()));
  }
  ...
  multiset<Key, Compare, Allocator>::equal_range_small(const Key& k) {
    ...
    while ((itUpper != end()) && !mCompare(k, itUpper.mpNode->mValue)) ++itUpper;
