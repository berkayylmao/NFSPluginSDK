...
namespace eastl {
  ...
  class safe_object {
   public:
    bool is_unreferenced() const;       /// Returns true if there are zero references (by a smart_ptr) to
                                        /// this object (mpSafePtrList is NULL).
   ...
   protected:
    ...
    void clear_references();  /// Forcibly removes any references (by smart_ptrs) to this object.
                              /// All safe_ptr mpObject values are set to NULL.
  ...
  class safe_ptr_base {
   public:
    bool unique() const;                     /// Returns true if there are no other smart pointers pointing to our
                                             /// object except us. True if mpObject is NUll
  ...
  class safe_ptr : public safe_ptr_base {
   ...
   public:
    ...
    //~safe_ptr() {}                                    /// No need to implement this; the
   ...
   public:
    ...
    typedef T* (this_type::*bool_)() const;  /// Allows for a more portable version of testing an
                                             /// instance of this class as a bool.
    operator bool_() const                   // A bug in the CodeWarrior compiler forces us to implement this inline
                                             // instead of below.
    {
      if (mpObject) return &this_type::get;
      return NULL;
    }
  };
...
inline eastl::safe_object::safe_object() : mpSafePtrList(0) {}

inline bool eastl::safe_object::is_unreferenced() const { return (mpSafePtrList == NULL); }
...
inline void eastl::safe_object::remove(safe_ptr_base* pBase) const {
  ...
  if (pBase == mpSafePtrList)
    ...
    for (safe_ptr_base *pPrev = mpSafePtrList, *pCurrent = mpSafePtrList->mpNext; pCurrent;
...
inline eastl::safe_ptr_base::safe_ptr_base(const safe_ptr_base& safePtrBase) :
    mpObject(safePtrBase.mpObject), mpNext(NULL) {
  EASTL_ASSERT(this != &safePtrBase);

  if (mpObject) mpObject->add(this);
}

inline eastl::safe_ptr_base::safe_ptr_base() : mpObject(NULL), mpNext(NULL) {}

inline eastl::safe_ptr_base::safe_ptr_base(const safe_object* pObject) : mpObject(pObject), mpNext(NULL) {
  if (mpObject) mpObject->add(this);
}

inline eastl::safe_ptr_base::~safe_ptr_base() {
  if (mpObject) mpObject->remove(this);
}
...
inline bool eastl::safe_ptr_base::empty() const { return (mpObject == NULL); }
...
inline bool eastl::safe_object::has_unique_reference() const { return mpSafePtrList ? mpSafePtrList->unique() : false; }

// Deprecated:
inline bool eastl::safe_object::has_references() const { return mpSafePtrList ? mpSafePtrList->unique() : false; }
...
template <class T>
inline eastl::safe_ptr<T>::safe_ptr() : safe_ptr_base() {}

template <class T>
inline eastl::safe_ptr<T>::safe_ptr(T* pObject) : safe_ptr_base(pObject) {}

template <class T>
inline eastl::safe_ptr<T>::safe_ptr(const this_type& safePtr) : safe_ptr_base(safePtr) {}

template <class T>
inline typename eastl::safe_ptr<T>::this_type& eastl::safe_ptr<T>::operator=(const this_type& safePtr) {
  if (this != &safePtr) reset(safePtr.mpObject);
  return *this;
}
...
// A bug in the CodeWarrior compiler forces us to implement this inline in the class instead of
// here. template<class T> inline eastl::safe_ptr<T>::operator bool_() const
// {
//     if(mpObject)
