  ...
  namespace detail {
    ...
    struct function_table_inplace_specialization<
      ...
      static void store_functor(total_storage_type& self, T to_store) {
        ...
        FunctorPointer* ptr       = new (&get_functor_ptr_ref(self)) FunctorPointer(
            reinterpret_cast<FunctorPointer>(eastl::allocator_traits<Allocator>::allocate(allocator, sizeof(Functor))));
    ...
    struct function_table {
      ...
      EA_CONSTEXPR function_table(call_move_and_destroy_t cmad, call_copy_t cc, call_copy_functor_only_t ccfo,
                                  call_destroy_t cd
#ifndef FUNC_NO_RTTI
                                      call_type_id_t cti,
                                  call_target_t      ct
...
//
  ...
  class function<Result(Arguments...)> : public detail::typedeffer<Result, Arguments...> {
   public:
    ...
    template <typename T>
    function(T functor, typename eastl::enable_if<detail::is_valid_function_argument<T, Result(Arguments...)>::value,
    ...
    function(eastl::allocator_arg_t, const Allocator& allocator, const function& other) : call(other.call) {
      ...
      // would allocate more and more memory
        ...
        if (other.storage.function_table == function_table_for_function) {
          ...
          detail::function_table_inplace_specialization<function, Allocator>::store_allocator(storage,
   ...
   private:
    ...
    void initialize(T functor, Allocator&& allocator) {
      ...
      detail::function_table_inplace_specialization<T, Allocator>::store_allocator(storage,
                                                                                   EASTL_FORWARD(Allocator, allocator));
  ...
  class function<Result()> : public detail::typedeffer<Result> {
   public:
    ...
    template <typename T>
    function(T functor, typename eastl::enable_if<detail::is_valid_function_argument<T, Result()>::value,
                                                  detail::empty_struct>::type = detail::empty_struct())
        FUNC_TEMPLATE_NOEXCEPT(T, EASTLAllocatorType) {
      if (detail::is_null(functor)) {
        initialize_empty();
  ...
  class function<Result(Argument0)> : public detail::typedeffer<Result, Argument0> {
   public:
    ...
    template <typename T>
    function(T functor, typename eastl::enable_if<detail::is_valid_function_argument<T, Result(Argument0)>::value,
                                                  detail::empty_struct>::type = detail::empty_struct())
        FUNC_TEMPLATE_NOEXCEPT(T, EASTLAllocatorType) {
      if (detail::is_null(functor)) {
        initialize_empty();
