...
namespace eastl {
  namespace Internal {
    ...
    inline int32_t atomic_increment(int32_t* p32) EA_NOEXCEPT {
      ...
      int32_t result;
      __asm__ __volatile__("lock; xaddl %0, %1" : "=r"(result), "=m"(*p32) : "0"(1), "m"(*p32) : "memory");
      return result + 1;
#else
      EASTL_FAIL_MSG(
    ...
    inline int32_t atomic_decrement(int32_t* p32) EA_NOEXCEPT {
      ...
      return _InterlockedDecrement(
          (volatile long*)p32);  // volatile long cast is OK because int32_t == long on Microsoft platforms.
#elif defined(EA_COMPILER_GNUC)
      int32_t result;
      __asm__ __volatile__("lock; xaddl %0, %1" : "=r"(result), "=m"(*p32) : "0"(-1), "m"(*p32) : "memory");
      return result - 1;
