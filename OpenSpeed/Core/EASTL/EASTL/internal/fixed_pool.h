...
#if defined(_MSC_VER)
#pragma warning(push)
#pragma warning(disable : 4275)  // non dll-interface class used as base for DLL-interface classkey
                                 // 'identifier'
#endif
...
#define EASTL_FIXED_POOL_DEFAULT_NAME \
  ...
  struct fixed_pool_base {
   public:
    ...
    ///
    EASTL_API void init(void* pMemory, size_t memorySize, size_t nodeSize, size_t alignment,
    ...
    ///
    bool can_allocate() const { return (mpHead != NULL) || (mpNext != mpCapacity); }
  ...
  class EASTL_API fixed_pool : public fixed_pool_base {
   public:
    ...
    ///
    fixed_pool(void* pMemory = NULL) : fixed_pool_base(pMemory) {}
    ...
    void* allocate() {
      ...
      {
#if EASTL_FIXED_SIZE_TRACKING_ENABLED
        if (++mnCurrentSize > mnPeakSize) mnPeakSize = mnCurrentSize;
#endif
    ...
    void* allocate(size_t /*alignment*/, size_t /*offset*/) { return allocate(); }
    ...
    const char* get_name() const { return EASTL_FIXED_POOL_DEFAULT_NAME; }
  ...
  class fixed_pool_with_overflow : public fixed_pool_base {
   public:
    ...
    fixed_pool_with_overflow(void* pMemory = NULL) :
        fixed_pool_base(pMemory), mOverflowAllocator(EASTL_FIXED_POOL_DEFAULT_NAME) {
      // Leave mpPoolBegin, mpPoolEnd uninitialized.
    }

    fixed_pool_with_overflow(void* pMemory, const overflow_allocator_type& allocator) :
        fixed_pool_base(pMemory), mOverflowAllocator(allocator) {
      // Leave mpPoolBegin, mpPoolEnd uninitialized.
    }

    fixed_pool_with_overflow(void* pMemory, size_t memorySize, size_t nodeSize, size_t alignment,
                             size_t alignmentOffset = 0) :
    ...
    fixed_pool_with_overflow(void* pMemory, size_t memorySize, size_t nodeSize, size_t alignment,
                             size_t alignmentOffset, const overflow_allocator_type& allocator) :
...
#if EASTL_FIXED_SIZE_TRACKING_ENABLED
      if (p && (++mnCurrentSize > mnPeakSize)) mnPeakSize = mnCurrentSize;
#endif
    ...
    const char* get_name() const { return mOverflowAllocator.get_name(); }

    void set_name(const char* pName) { mOverflowAllocator.set_name(pName); }

    const overflow_allocator_type& get_overflow_allocator() const { return mOverflowAllocator; }

    overflow_allocator_type& get_overflow_allocator() { return mOverflowAllocator; }
   ...
   public:
    ...
    void*             mpPoolBegin;  // Ideally we wouldn't need this member variable. he problem is that the
                                    // information about the pool buffer and object size is stored in the owning
                                    // container and we can't have access to it without increasing the amount of
                                    // code we need and by templating more code. It may turn out that simply
                                    // storing data here is smaller in the end.
  ...
  ///     bEnableOverflow        Whether or not we should use the overflow heap if our object pool
  ///     is exhausted. OverflowAllocator      Overflow allocator, which is only used if
  ///     bEnableOverflow == true. Defaults to the global heap.
  ///
  template <size_t nodeSize, size_t nodeCount, size_t nodeAlignment, size_t nodeAlignmentOffset, bool bEnableOverflow,
            typename OverflowAllocator = EASTLAllocatorType>
  class fixed_node_allocator {
   public:
    typedef
        typename type_select<bEnableOverflow, fixed_pool_with_overflow<OverflowAllocator>, fixed_pool>::type pool_type;
    typedef fixed_node_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, bEnableOverflow,
                                 OverflowAllocator>
                              this_type;
    typedef OverflowAllocator overflow_allocator_type;
    ...
    enum {
      ...
      kNodesSize = nodeCount * nodeSize,  // Note that the kBufferSize calculation assumes that the
                                          // compiler sets sizeof(T) to be a multiple alignof(T),
                                          // and so sizeof(T) is always >= alignof(T).
   ...
   public:
    ...
    void deallocate(void* p, size_t) { mPool.deallocate(p); }

    /// can_allocate
    ///
    /// Returns true if there are any free links.
    ///
    bool can_allocate() const { return mPool.can_allocate(); }
    ...
    const char* get_name() const { return mPool.get_name(); }

    void set_name(const char* pName) { mPool.set_name(pName); }

    const overflow_allocator_type& get_overflow_allocator() const EA_NOEXCEPT { return mPool.mOverflowAllocator; }

    overflow_allocator_type& get_overflow_allocator() EA_NOEXCEPT { return mPool.mOverflowAllocator; }

    void set_overflow_allocator(const overflow_allocator_type& allocator) { mPool.mOverflowAllocator = allocator; }

    void copy_overflow_allocator(const this_type& x)  // This function exists so we can write generic code that works
                                                      // for allocators that do and don't have overflow allocators.
    {
  ...
  class fixed_node_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false, OverflowAllocator> {
   public:
    ...
    typedef fixed_node_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false, OverflowAllocator>
                              this_type;
    typedef OverflowAllocator overflow_allocator_type;
   ...
   public:
    ...
    fixed_node_allocator(void* pNodeBuffer,
                         const overflow_allocator_type& /*allocator*/)  // allocator is unused because
                                                                        // bEnableOverflow is false in this
                                                                        // specialization.
        :
        mPool(pNodeBuffer, kNodesSize, kNodeSize, kNodeAlignment, kNodeAlignmentOffset) {}
    ...
    /// Perhaps some day we'll find a more elegant yet costless way around this.
    ///
    fixed_node_allocator(const this_type& x)  // No need to copy the overflow allocator, because
                                              // bEnableOverflow is false in this specialization.
        :
    ...
    const overflow_allocator_type& get_overflow_allocator() const EA_NOEXCEPT {
      ...
      return *pNULL;  // This is not pretty, but it should never execute. This is here only to allow
                      // this to compile.
    }
    ...
    void copy_overflow_allocator(const this_type&)  // This function exists so we can write generic code that works for
                                                    // allocators that do and don't have overflow allocators.
    {
      // We don't have an overflow allocator.
    }
  ...
  template <size_t nodeSize, size_t nodeCount, size_t nodeAlignment, size_t nodeAlignmentOffset, bool bEnableOverflow,
            typename OverflowAllocator>
  inline bool operator==(const fixed_node_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset,
                                                    bEnableOverflow, OverflowAllocator>& a,
                         const fixed_node_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset,
  ...
  class fixed_hashtable_allocator {
   public:
    typedef
        typename type_select<bEnableOverflow, fixed_pool_with_overflow<OverflowAllocator>, fixed_pool>::type pool_type;
    typedef fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset,
    ...
    enum {
      ...
      kBufferSize = kNodesSize + ((nodeAlignment > 1) ? nodeSize - 1 : 0) +
                    nodeAlignmentOffset,  // Don't need to include kBucketsSize in this calculation, as
                                          // fixed_hash_xxx containers have a separate buffer for buckets.
      kNodeAlignment       = nodeAlignment,
      kNodeAlignmentOffset = nodeAlignmentOffset,
      kAllocFlagBuckets    = 0x00400000  // Flag to allocator which indicates that we are allocating
                                         // buckets and not nodes.
    };
   ...
   public:
    ...
    fixed_hashtable_allocator(void* pNodeBuffer) :
        mPool(pNodeBuffer, kBufferSize, kNodeSize, kNodeAlignment, kNodeAlignmentOffset), mpBucketBuffer(NULL) {
    ...
    void* allocate(size_t n, int flags = 0) {
      // We expect that the caller uses kAllocFlagBuckets when it wants us to allocate buckets
      // instead of nodes.
      EASTL_CT_ASSERT(kAllocFlagBuckets == 0x00400000);  // Currently we expect this to be so, because the hashtable has
                                                         // a copy of this enum.
    ...
    const overflow_allocator_type& get_overflow_allocator() const { return mPool.mOverflowAllocator; }
  ...
  template <size_t bucketCount, size_t nodeSize, size_t nodeCount, size_t nodeAlignment, size_t nodeAlignmentOffset,
            typename OverflowAllocator>
  class fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false,
                                  OverflowAllocator> {
   public:
    typedef fixed_pool pool_type;
    typedef fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false,
                                      OverflowAllocator>
                              this_type;
    typedef OverflowAllocator overflow_allocator_type;
   ...
   public:
    ...
    /// See the discussion above in fixed_node_allocator for important information about this.
    ///
    fixed_hashtable_allocator(const this_type& x)  // No need to copy the overflow allocator, because bEnableOverflow is
                                                   // false in this specialization.
        :
  ...
  inline bool operator==(const fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount, nodeAlignment,
                                                         nodeAlignmentOffset, bEnableOverflow, OverflowAllocator>& a,
                         const fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount, nodeAlignment,
                                                         nodeAlignmentOffset, bEnableOverflow, OverflowAllocator>& b) {
    return (&a == &b);  // They are only equal if they are the same object.
  }
  ...
  class fixed_vector_allocator {
   public:
    typedef fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, bEnableOverflow,
                                   OverflowAllocator>
                              this_type;
    typedef OverflowAllocator overflow_allocator_type;
   ...
   public:
    ...
    void*                   mpPoolBegin;  // To consider: Find some way to make this data unnecessary, without
                                          // increasing template proliferation.
   ...
   public:
    ...
    void* allocate(size_t n, int flags = 0) { return mOverflowAllocator.allocate(n, flags); }
    ...
    void deallocate(void* p, size_t n) {
      if (p != mpPoolBegin) mOverflowAllocator.deallocate(p, n);  // Can't do this to our own allocation.
    }
    ...
    const overflow_allocator_type& get_overflow_allocator() const EA_NOEXCEPT { return mOverflowAllocator; }

    overflow_allocator_type& get_overflow_allocator() EA_NOEXCEPT { return mOverflowAllocator; }

    void set_overflow_allocator(const overflow_allocator_type& allocator) { mOverflowAllocator = allocator; }
  ...
  class fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false, OverflowAllocator> {
   public:
    typedef fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false, OverflowAllocator>
                              this_type;
    typedef OverflowAllocator overflow_allocator_type;
    ...
    // Disabled because it causes compile conflicts.
    // fixed_vector_allocator(const char* = NULL) // This char* parameter is present so that this
    // class can be like the other version.
    //{
    //}
    ...
    /// fixed_vector_allocator
    ///
    // Disabled because there is nothing to do. No member data. And the default for this is
    // sufficient. fixed_vector_allocator(const fixed_vector_allocator&)
    // {
    // }
  ...
  template <size_t nodeSize, size_t nodeCount, size_t nodeAlignment, size_t nodeAlignmentOffset, bool bEnableOverflow,
            typename OverflowAllocator>
  inline bool operator==(const fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset,
                                                      bEnableOverflow, OverflowAllocator>& a,
                         const fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset,
                                                      bEnableOverflow, OverflowAllocator>& b) {
    return (&a == &b);  // They are only equal if they are the same object.
  }
