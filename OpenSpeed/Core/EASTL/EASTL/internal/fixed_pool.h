...
#define EASTL_FIXED_POOL_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " fixed_pool"  // Unless the user overrides something, this is "EASTL fixed_pool".
#endif
  ...
  class fixed_pool_with_overflow : public fixed_pool_base {
   ...
   public:
    ...
    void* mpPoolBegin;  // Ideally we wouldn't need this member variable. he problem is that the information about the
                        // pool buffer and object size is stored in the owning container and we can't have access to it
                        // without increasing the amount of code we need and by templating more code. It may turn out
                        // that simply storing data here is smaller in the end.
  ...
  ///     OverflowAllocator      Overflow allocator, which is only used if bEnableOverflow == true. Defaults to the
  ///     global heap.
  ///
  ...
  class fixed_node_allocator {
   public:
    ...
    enum {
      ...
      kNodesSize =
          nodeCount * nodeSize,  // Note that the kBufferSize calculation assumes that the compiler sets sizeof(T) to be
                                 // a multiple alignof(T), and so sizeof(T) is always >= alignof(T).
  ...
  class fixed_node_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false, OverflowAllocator> {
   ...
   public:
    ...
    fixed_node_allocator(void* pNodeBuffer,
                         const overflow_allocator_type& /*allocator*/)  // allocator is unused because bEnableOverflow
                                                                        // is false in this specialization.
        :
        mPool(pNodeBuffer, kNodesSize, kNodeSize, kNodeAlignment, kNodeAlignmentOffset) {}
    ...
    /// Perhaps some day we'll find a more elegant yet costless way around this.
    ///
    fixed_node_allocator(const this_type& x)  // No need to copy the overflow allocator, because bEnableOverflow is
  ...
  class fixed_hashtable_allocator {
   public:
    ...
    enum {
      ...
      kBufferSize = kNodesSize + ((nodeAlignment > 1) ? nodeSize - 1 : 0) +
                    nodeAlignmentOffset,  // Don't need to include kBucketsSize in this calculation, as fixed_hash_xxx
                                          // containers have a separate buffer for buckets.
      kNodeAlignment       = nodeAlignment,
      kNodeAlignmentOffset = nodeAlignmentOffset,
   ...
   public:
    ...
    void* allocate(size_t n, int flags = 0) {
      ...
      EASTL_CT_ASSERT(kAllocFlagBuckets ==
  ...
  class fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false,
   ...
   public:
    ...
    fixed_hashtable_allocator(
        void* pNodeBuffer, const overflow_allocator_type& /*allocator*/)  // allocator is unused because bEnableOverflow
                                                                          // is false in this specialization.
        :
        mPool(pNodeBuffer, kBufferSize, kNodeSize, kNodeAlignment, kNodeAlignmentOffset), mpBucketBuffer(NULL) {
      // EASTL_ASSERT(false); // As it stands now, this is not supposed to be called.
    ...
    fixed_hashtable_allocator(
        ...
        const overflow_allocator_type& /*allocator*/)  // allocator is unused because bEnableOverflow is false in this
                                                       // specialization.
        :
        mPool(pNodeBuffer, kBufferSize, kNodeSize, kNodeAlignment, kNodeAlignmentOffset),
        mpBucketBuffer(pBucketBuffer) {}
  ...
  class fixed_vector_allocator {
   ...
   public:
    ...
    void* mpPoolBegin;  // To consider: Find some way to make this data unnecessary, without increasing template
                        // proliferation.
   ...
   public:
    ...
    fixed_vector_allocator(void* pNodeBuffer = nullptr) : mpPoolBegin(pNodeBuffer) {}
  ...
  class fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false, OverflowAllocator> {
   public:
    ...
    // Disabled because it causes compile conflicts.
    // fixed_vector_allocator(const char* = NULL) // This char* parameter is present so that this class can be like the
    // other version.
    //{
    //}
