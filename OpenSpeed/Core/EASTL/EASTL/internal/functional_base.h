...
namespace eastl {
  ...
  EA_CONSTEXPR auto invoke_impl(R C::*func, T&& obj, Args&&... args)
      EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR((eastl::forward<T>(obj).*func)(eastl::forward<Args>(args)...))) ->
  ...
  EA_CONSTEXPR auto invoke_impl(F&& func, Args&&... args)
      EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR(eastl::forward<F>(func)(eastl::forward<Args>(args)...)))
  ...
  EA_CONSTEXPR auto invoke_impl(M C::*member, T&& obj) EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR(eastl::forward<T>(obj).*member))
      -> typename enable_if<is_base_of<C, decay_t<T>>::value, decltype(eastl::forward<T>(obj).*member)>::type {
    return eastl::forward<T>(obj).*member;
  }

  template <typename M, typename C, typename T>
  EA_CONSTEXPR auto invoke_impl(M C::*member, T&& obj)
      EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR((*eastl::forward<T>(obj)).*member))
          -> decltype((*eastl::forward<T>(obj)).*member) {
    return (*eastl::forward<T>(obj)).*member;
  }

  template <typename F, typename... Args>
  EA_CONSTEXPR decltype(auto) invoke(F&& func, Args&&... args)
      EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR(invoke_impl(eastl::forward<F>(func), eastl::forward<Args>(args)...))) {
  ...
  template <typename F, typename... Args>
  struct is_invocable_impl<F, void_t<typename eastl::invoke_result<F, Args...>::type>, Args...>
  ...
  struct is_nothrow_invocable_r_impl<R, F, void_t<typename eastl::invoke_result<F, Args...>::type>, Args...> {
    static EA_CONSTEXPR_OR_CONST bool value =
        eastl::is_convertible<typename eastl::invoke_result<F, Args...>::type, R>::value &&
  ...
  template <typename T>
  reference_wrapper<T>::reference_wrapper(T& v) EA_NOEXCEPT : val(eastl::addressof(v)) {}

  template <typename T>
  reference_wrapper<T>::reference_wrapper(const reference_wrapper<T>& other) EA_NOEXCEPT : val(other.val) {}
  ...
  template <typename T>
  struct is_reference_wrapper_helper<eastl::reference_wrapper<T>> : public eastl::true_type {};

  template <typename T>
  struct is_reference_wrapper : public eastl::is_reference_wrapper_helper<typename eastl::remove_cv<T>::type> {};
  ...
  template <typename R, typename C, typename T, typename... Args>
  EA_CONSTEXPR auto invoke_impl(R C::*func, T&& obj, Args&&... args)
      EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR((obj.get().*func)(eastl::forward<Args>(args)...))) ->
  ...
  EA_CONSTEXPR auto invoke_impl(M C::*member, T&& obj) EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR(obj.get().*member)) ->
      typename enable_if<is_reference_wrapper<eastl::decay_t<T>>::value, decltype(obj.get().*member)>::type {
