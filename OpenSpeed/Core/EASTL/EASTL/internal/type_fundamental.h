...
#define EASTL_TYPE_TRAIT_is_null_pointer_CONFORMANCE 1
  ...
  template <typename T>
  struct is_null_pointer : public eastl::is_same<typename eastl::remove_cv<T>::type, decltype(nullptr)> {
...
#define EASTL_TYPE_TRAIT_is_fundamental_CONFORMANCE 1  // is_fundamental is conforming.
  ...
  template <typename T>
  struct is_fundamental : public integral_constant<bool, is_void<T>::value || is_integral<T>::value ||
                                                             is_floating_point<T>::value || is_null_pointer<T>::value> {
  };
