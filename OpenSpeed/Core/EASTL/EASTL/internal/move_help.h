...
#define EASTL_FORWARD_INLINE(T, x) \
  eastl::forward<T>(x)  // Need to investigate how to properly make a macro for this.
                        // (eastl::is_reference<T>::value ? static_cast<T&&>(static_cast<T&>(x)) :
                        // static_cast<T&&>(x))
#else
...
/// Note to be confused with the single argument move: (typename remove_reference<T>::type&&
/// move(T&& x)) http://en.cppreference.com/w/cpp/algorithm/move
...
namespace eastl {
  ...
  // to another function. move is specifically about making a type be an rvalue reference (i.e.
  // movable) type.
  //
  // Example usage:
  //     template <class T>
  ...
  //
  template <typename T>
  EA_CPP14_CONSTEXPR typename eastl::remove_reference<T>::type&& move(T&& x) EA_NOEXCEPT {
...
//
  ...
  template <typename T>
  EA_CPP14_CONSTEXPR typename eastl::conditional<
      !eastl::is_nothrow_move_constructible<T>::value && eastl::is_copy_constructible<T>::value, const T&, T&&>::type
