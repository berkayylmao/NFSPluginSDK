...
#define DISABLE_IF_TRUETYPE(T) typename eastl::enable_if<!T::value>::type*
  ...
  class hashtable
   ...
   public:
    ...
    enum {
      ...
      kAllocFlagBuckets = eastl::kHashtableAllocFlagBuckets  // Flag to allocator which indicates that we are allocating
   ...
   public:
    ...
    void clear();
    void clear(bool clearBuckets);  // If clearBuckets is true, we free the bucket memory and set the bucket count back
                                    // to the newly constructed count.
    ...
    /// It returns an iterator to the first element that matches the given hash. However, there may be multiple elements
   ...
   protected:
    ...
    iterator DoGetResultIterator(BoolConstantT, const insert_return_type& irt,
    ...
    template <typename BoolConstantT>
    eastl::pair<iterator, bool> DoInsertValueExtra(BoolConstantT, const key_type& k, hash_code_t c, node_type* pNodeNew,
                                                   value_type&& value, ENABLE_IF_TRUETYPE(BoolConstantT) = nullptr);

    template <typename BoolConstantT>
    eastl::pair<iterator, bool> DoInsertValue(BoolConstantT, value_type&& value,
                                              ENABLE_IF_TRUETYPE(BoolConstantT) = nullptr);

    template <typename BoolConstantT>
    iterator DoInsertValueExtra(BoolConstantT, const key_type& k, hash_code_t c, node_type* pNodeNew,
                                value_type&& value, DISABLE_IF_TRUETYPE(BoolConstantT) = nullptr);
    ...
    template <typename BoolConstantT>
    eastl::pair<iterator, bool> DoInsertValue(BoolConstantT, const value_type& value,
                                              ENABLE_IF_TRUETYPE(BoolConstantT) = nullptr);

    template <typename BoolConstantT>
    iterator DoInsertValueExtra(BoolConstantT, const key_type& k, hash_code_t c, node_type* pNodeNew,
                                const value_type& value, DISABLE_IF_TRUETYPE(BoolConstantT) = nullptr);
    ...
    // it is done in case application has it's own hashset/hashmap-like containter, where hash code is for some reason
    // known prior the insert this allows to save some performance, especially with heavy hash functions
    ...
    // We keep DoInsertKey overload without third parameter, for compatibility with older revisions of EASTL (3.12.07
    // and earlier) It used to call get_hash_code as a first call inside the DoInsertKey.
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::hashtable(this_type&& x) :
      ...
      mRehashPolicy(x.mRehashPolicy),
      mAllocator(x.mAllocator) {
    reset_lose_memory();  // We do this here the same as we do it in the default ctor because it puts the container in a
                          // proper initial empty state. This code would be cleaner if we could rely on being able to
                          // use C++11 delegating constructors and just call the default ctor here.
    swap(x);
  }
  ...
  inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoFreeBuckets(node_type** pBucketArray,
    ...
    if (n > 1)
      EASTLFree(mAllocator, pBucketArray,
                (n + 1) * sizeof(node_type*));  // '+1' because DoAllocateBuckets allocates nBucketCount + 1 buckets in
                                                // order to have a NULL sentinel at the end.
  }
      ...
      BoolConstantT, Args&&... args)  // true_type means bUniqueKeys is true.
  {
    ...
    // argument. We need the value_type in order to proceed, but that entails getting an instance of a value_type from
    // the args. And it may turn out that the value is already present in the hashtable and we need to cancel the
    // insertion, despite having obtained a value_type to put into the hashtable. We have mitigated this problem
    // somewhat by providing specializations of the insert function for const value_type& and value_type&&, and so the
    // only time this function should get called is when args refers to arguments to construct a value_type.
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValueExtra(
      BoolConstantT, const key_type& k, hash_code_t c, node_type* pNodeNew, value_type&& value,
...
// do a rehash if the allocation throws.
#if EASTL_EXCEPTIONS_ENABLED
      bool nodeAllocated;  // If exceptions are enabled then we we need to track if we allocated the node so we can free
  ...
  eastl::pair<typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator, bool>
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(
      BoolConstantT, value_type&& value, ENABLE_IF_TRUETYPE(BoolConstantT))  // true_type means bUniqueKeys is true.
  {
  ...
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(
      BoolConstantT, value_type&& value, DISABLE_IF_TRUETYPE(BoolConstantT))  // false_type means bUniqueKeys is false.
  {
    const key_type&   k = mExtractKey(value);
    const hash_code_t c = get_hash_code(k);
  ...
  eastl::pair<typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator, bool>
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValueExtra(
      BoolConstantT, const key_type& k, hash_code_t c, node_type* pNodeNew, const value_type& value,
      ENABLE_IF_TRUETYPE(BoolConstantT))  // true_type means bUniqueKeys is true.
  {
    // Adds the value to the hash table if not already present.
    // If already present then the existing value is returned via an iterator/bool pair.
...
// do a rehash if the allocation throws.
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertKey(
      true_type, const key_type& key, const hash_code_t c)  // true_type means bUniqueKeys is true.
  {
  ...
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertKey(
      false_type, const key_type& key, const hash_code_t c)  // false_type means bUniqueKeys is false.
  {
    const eastl::pair<bool, uint32_t> bRehash =
        mRehashPolicy.GetRehashRequired((uint32_t)mnBucketCount, (uint32_t)mnElementCount, (uint32_t)1);
  ...
  eastl::pair<typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator, bool>
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertKey(
      true_type, key_type&& key, const hash_code_t c)  // true_type means bUniqueKeys is true.
  {
    size_type        n     = (size_type)bucket_index(key, c, (uint32_t)mnBucketCount);
    node_type* const pNode = DoFindNode(mpBucketArray[n], key, c);
  ...
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertKey(
      false_type, key_type&& key, const hash_code_t c)  // false_type means bUniqueKeys is false.
  {
    const eastl::pair<bool, uint32_t> bRehash =
        mRehashPolicy.GetRehashRequired((uint32_t)mnBucketCount, (uint32_t)mnElementCount, (uint32_t)1);
  ...
  template <class... Args>
  inline typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::try_emplace(const_iterator, const key_type& key,
                                                                     Args&&... args) {
    insert_return_type result =
        DoInsertValue(has_unique_keys_type(), value_type(piecewise_construct, eastl::forward_as_tuple(key),
                                                         eastl::forward_as_tuple(eastl::forward<Args>(args)...)));
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert(hash_code_t c, node_type* pNodeNew, P&& otherValue) {
    ...
    value_type value(eastl::forward<P>(otherValue));  // Need to use forward instead of move because P&& is a "universal
