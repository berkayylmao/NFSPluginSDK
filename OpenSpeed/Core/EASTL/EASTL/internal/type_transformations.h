...
#define EASTL_TYPE_TRAIT_add_const_CONFORMANCE 1  // add_const is conforming.
  ...
  template <typename T,
            bool = eastl::is_const<T>::value || eastl::is_reference<T>::value || eastl::is_function<T>::value>
...
#define EASTL_TYPE_TRAIT_add_volatile_CONFORMANCE 1  // add_volatile is conforming.
  ...
  template <typename T,
            bool = eastl::is_volatile<T>::value || eastl::is_reference<T>::value || eastl::is_function<T>::value>
...
#define EASTL_TYPE_TRAIT_make_signed_CONFORMANCE 0  // make_signed is only partially conforming.
  ...
  struct make_signed<const unsigned long long> {
    typedef const signed long long type;
     ...
     (CHAR_MAX == UCHAR_MAX))  // If char is unsigned, we convert char to signed char. However, if char is
...
#define EASTL_TYPE_TRAIT_aligned_union_CONFORMANCE 1  // aligned_union is conforming.
  ...
  DestType union_cast(SourceType sourceValue) {
    EASTL_CT_ASSERT(
        ...
        (EA_ALIGN_OF(DestType) == EA_ALIGN_OF(SourceType)));  // To support differening alignments, we would need to use
                                                              // a memcpy-based solution or find a way to make the two
                                                              // union members align with each other.
