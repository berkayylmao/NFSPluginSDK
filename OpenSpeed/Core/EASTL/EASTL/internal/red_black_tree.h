...
#include <OpenSpeed/Core/EASTL/EASTL/tuple.h>
...
#define EASTL_RBTREE_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " rbtree"  // Unless the user overrides something, this is "EASTL rbtree".
#endif
...
//
// Potentially we could provide a constructor that would satisfy the compiler and change the code to use this
// constructor instead of constructing mValue in place within an unconstructed rbtree_node.
#if defined(_MSC_VER)
  ...
  class rbtree : public rb_base<Key, Value, Compare, ExtractKey, bUniqueKeys,
   ...
   public:
    rbtree_node_base mAnchor;   /// This node acts as end() and its mpLeft points to begin(), and mpRight points to
                                /// rbegin() (the last node on the right).
   ...
   public:
    ...
    void clear();
    void reset_lose_memory();  // This is a unilateral reset to an initially empty state. No destructors are called, no
                               // deallocation occurs.
  ...
  inline rbtree<K, V, C, A, E, bM, bU>::rbtree(const this_type& x) :
      base_type(x.get_compare()), mAnchor(), mnSize(0), mAllocator(x.mAllocator) {
    reset_lose_memory();
  ...
  inline rbtree<K, V, C, A, E, bM, bU>::rbtree(this_type&& x, const allocator_type& allocator) :
      base_type(x.get_compare()), mAnchor(), mnSize(0), mAllocator(allocator) {
    reset_lose_memory();
  ...
  inline typename rbtree<K, V, C, A, E, bM, bU>::this_type& rbtree<K, V, C, A, E, bM, bU>::operator=(this_type&& x) {
    if (this != &x) {
      clear();  // To consider: Are we really required to clear here? x is going away soon and will clear itself in its
                // dtor.
  ...
  template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
  template <class... Args>
  inline typename rbtree<K, V, C, A, E, bM, bU>::insert_return_type  // map/set::insert return a pair,
  ...
  template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
  template <class... Args>
  inline eastl::pair<typename rbtree<K, V, C, A, E, bM, bU>::iterator, bool> rbtree<K, V, C, A, E, bM, bU>::try_emplace(
  ...
  template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
  template <class... Args>
  inline typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::try_emplace(
      const_iterator position, const key_type& key, Args&&... args) {
    return DoInsertValueHint(has_unique_keys_type(), position, piecewise_construct, eastl::forward_as_tuple(key),
                             eastl::forward_as_tuple(eastl::forward<Args>(args)...));
  }
  ...
  template <class... Args>
  inline typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::try_emplace(
      const_iterator position, key_type&& key, Args&&... args) {
    return DoInsertValueHint(has_unique_keys_type(), position, piecewise_construct,
                             eastl::forward_as_tuple(eastl::move(key)),
                             eastl::forward_as_tuple(eastl::forward<Args>(args)...));
  }
  ...
  template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
  template <class M>
  eastl::pair<typename rbtree<K, V, C, A, E, bM, bU>::iterator, bool> rbtree<K, V, C, A, E, bM, bU>::insert_or_assign(
      const key_type& k, M&& obj) {
    auto iter = find(k);

    if (iter == end()) {
      return insert(
          value_type(piecewise_construct, eastl::forward_as_tuple(k), eastl::forward_as_tuple(eastl::forward<M>(obj))));
    } else {
  ...
  template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
  template <class M>
  typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::insert_or_assign(const_iterator  hint,
                                                                                                   const key_type& k,
                                                                                                   M&& obj) {
    auto iter = find(k);

    if (iter == end()) {
      return insert(hint, value_type(piecewise_construct, eastl::forward_as_tuple(k),
                                     eastl::forward_as_tuple(eastl::forward<M>(obj))));
    } else {
      iter->second = eastl::forward<M>(obj);
      return iter;
  ...
  typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::insert_or_assign(const_iterator hint,
    ...
    if (iter == end()) {
      return insert(hint, value_type(piecewise_construct, eastl::forward_as_tuple(eastl::move(k)),
                                     eastl::forward_as_tuple(eastl::forward<M>(obj))));
    } else {
      iter->second = eastl::forward<M>(obj);
      return iter;
  ...
  typename rbtree<K, V, C, A, E, bM, bU>::node_type* rbtree<K, V, C, A, E, bM, bU>::DoGetKeyInsertionPositionUniqueKeys(
    ...
    while (EASTL_LIKELY(pCurrent))  // Do a walk down the tree.
    {
      ...
      if (bValueLessThanNode) {
        EASTL_VALIDATE_COMPARE(
            !compare(extractKey(pCurrent->mValue), key));  // Validate that the compare function is sane.
        pCurrent = (node_type*)pCurrent->mpNodeLeft;
      } else
        pCurrent = (node_type*)pCurrent->mpNodeRight;
    }

    pParent = pLowerBound;  // pLowerBound is actually upper bound right now (i.e. it is > value instead of <=), but we
                            // will make it the lower bound below.
    ...
    {
      EASTL_VALIDATE_COMPARE(
          !compare(key, extractKey(pLowerBound->mValue)));  // Validate that the compare function is sane.
      canInsert = true;
      return pParent;
    }
  ...
  template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
  typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::DoInsertValue(false_type,
                                                                                                value_type&& value) {
    extract_key extractKey;
    key_type    key(extractKey(value));
  ...
  {
    // We have a problem here if sizeof(value_type) is too big for the stack. We may want to consider having a
    // specialization for large value_types. To do: Change this so that we call
    // DoCreateNode(eastl::forward<Args>(args)...) here and use the value from the resulting pNode to get the key, and
    // make DoInsertValueImpl take that node as an argument. That way there is no value created on the stack.
  ...
  template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
  typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::DoInsertValueImpl(
      node_type* pNodeParent, bool bForceToLeft, const key_type& key, node_type* pNodeNew) {
  ...
  rbtree<K, V, C, A, E, bM, bU>::DoGetKeyInsertionPositionUniqueKeysHint(const_iterator position, bool& bForceToLeft,
        ...
        (position.mpNode != &mAnchor))  // If the user specified a specific insertion position...
    {
      ...
      {
        EASTL_VALIDATE_COMPARE(
            !compare(key, extractKey(position.mpNode->mValue)));  // Validate that the compare function is sane.
        ...
        {
          EASTL_VALIDATE_COMPARE(
              !compare(extractKey(itNext.mpNode->mValue), key));  // Validate that the compare function is sane.
  ...
  rbtree<K, V, C, A, E, bM, bU>::DoGetKeyInsertionPositionNonuniqueKeysHint(const_iterator position, bool& bForceToLeft,
        ...
        (position.mpNode != &mAnchor))  // If the user specified a specific insertion position...
    {
      ...
      {
        if (position.mpNode->mpNodeRight)  // If there are any nodes to the right... [this expression will always be
                                           // true as long as we aren't at the end()]
        {
    ...
    if (mnSize && !compare(key, extractKey(((node_type*)mAnchor.mpNodeRight)
  ...
  typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::DoInsertValueHint(
      true_type, const_iterator position, Args&&... args)  // true_type means keys are unique.
  {
    // This is the pathway for insertion of unique keys (map and set, but not multimap and multiset).
    //
    // We follow the same approach as SGI STL/STLPort and use the position as
  ...
  template <class... Args>
  typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::DoInsertValueHint(
      false_type, const_iterator position, Args&&... args)  // false_type means keys are not unique.
  {
    // This is the pathway for insertion of non-unique keys (multimap and multiset, but not map and set).
    //
    // We follow the same approach as SGI STL/STLPort and use the position as
  ...
  template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
  typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::DoInsertValueHint(
      true_type, const_iterator position, value_type&& value)  // true_type means keys are unique.
  {
    // This is the pathway for insertion of unique keys (map and set, but not multimap and multiset).
    //
    // We follow the same approach as SGI STL/STLPort and use the position as
  ...
  template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
  typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::DoInsertValueHint(
      false_type, const_iterator position, value_type&& value)  // false_type means keys are not unique.
  {
    // This is the pathway for insertion of non-unique keys (multimap and multiset, but not map and set).
    //
    // We follow the same approach as SGI STL/STLPort and use the position as
  ...
  template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
  typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::DoInsertKey(
      false_type, const_iterator position, const key_type& key)  // false_type means keys are not unique.
  {
    // This is the pathway for insertion of non-unique keys (multimap and multiset, but not map and set).
    //
    // We follow the same approach as SGI STL/STLPort and use the position as
  ...
  void rbtree<K, V, C, A, E, bM, bU>::insert(InputIterator first, InputIterator last) {
    for (; first != last; ++first)
      DoInsertValue(has_unique_keys_type(), *first);  // Or maybe we should call 'insert(end(), *first)' instead. If the
  ...
  inline typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::erase(
    ...
    --mnSize;  // Interleave this between the two references to itNext. We expect no exceptions to occur during the code
               // below.
    ++position;
  ...
  typename rbtree<K, V, C, A, E, bM, bU>::iterator rbtree<K, V, C, A, E, bM, bU>::find(const key_type& key) {
    ...
    while (EASTL_LIKELY(pCurrent))  // Do a walk down the tree.
    {
      if (EASTL_LIKELY(!compare(extractKey(pCurrent->mValue), key)))  // If pCurrent is >= key...
      {
        ...
        EASTL_VALIDATE_COMPARE(
            !compare(key, extractKey(pCurrent->mValue)));  // Validate that the compare function is sane.
        pCurrent = (node_type*)pCurrent->mpNodeRight;
      }
    }
  ...
  bool rbtree<K, V, C, A, E, bM, bU>::validate() const {
    ...
    //   7 The comparison function is sane; it obeys strict weak ordering. If compare(a,b) is true, then compare(b,a)
    //   must be false. Both cannot be true.
    ...
    if (mnSize) {
      ...
      for (const_iterator it = begin(); it != end(); ++it, ++nIteratedSize) {
        ...
        // Verify #7 above.
        if (pNodeLeft && compare(extractKey(pNodeLeft->mValue), extractKey(pNode->mValue)) &&
            compare(extractKey(pNode->mValue),
        ...
        // Verify item #6 above.
        if (pNodeRight && compare(extractKey(pNodeRight->mValue), extractKey(pNode->mValue))) return false;

        if (pNodeLeft && compare(extractKey(pNode->mValue), extractKey(pNodeLeft->mValue))) return false;
  ...
  template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
  template <class... Args>
  typename rbtree<K, V, C, A, E, bM, bU>::node_type* rbtree<K, V, C, A, E, bM, bU>::DoCreateNode(Args&&... args) {
    // Note that this function intentionally leaves the node pointers uninitialized.
    // The caller would otherwise just turn right around and modify them, so there's
    // no point in us initializing them to anything (except in a debug build).
    node_type* const pNode = DoAllocateNode();
