...
#define EASTL_TYPE_TRAIT_is_empty_CONFORMANCE 1  // is_empty is fully conforming.
  ...
  struct is_empty_helper<T, true>
      : public eastl::integral_constant<bool, sizeof(is_empty_helper_t1<T>) == sizeof(is_empty_helper_t2)> {};
...
#define EASTL_TYPE_TRAIT_is_pod_CONFORMANCE \
  1  // is_pod is conforming. Actually as of VS2008 it is apparently not fully conforming, as it
     // flags the following as a non-pod: struct Pod{ Pod(){} };

  EA_DISABLE_VC_WARNING(4647)
  template <typename T>  // We check for has_trivial_constructor only because the VC++ is_pod does.
                         // Is it due to some compiler bug?
  struct is_pod : public eastl::integral_constant<bool, (__has_trivial_constructor(T) && __is_pod(T)) ||
                                                            eastl::is_void<T>::value || eastl::is_scalar<T>::value> {};
  EA_RESTORE_VC_WARNING()

#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
    (defined(EA_COMPILER_GNUC) || (defined(EA_COMPILER_CLANG) && EA_COMPILER_HAS_FEATURE(is_pod)))
#define EASTL_TYPE_TRAIT_is_pod_CONFORMANCE 1  // is_pod is conforming.

  template <typename T>
  struct is_pod
      : public eastl::integral_constant<bool, __is_pod(T) || eastl::is_void<T>::value || eastl::is_scalar<T>::value> {};
#else
#define EASTL_TYPE_TRAIT_is_pod_CONFORMANCE 0  // is_pod is not conforming. Can return false negatives.
  ...
  struct is_pod
      : public eastl::integral_constant<
            bool, eastl::is_void<T>::value || eastl::is_scalar<typename eastl::remove_all_extents<T>::type>::value> {};
#endif
...
#define EASTL_DECLARE_IS_POD(T, isPod)                                                 \
  namespace eastl {                                                                    \
    ...
    template <>                                                                        \
    struct is_pod<const volatile T> : public eastl::integral_constant<bool, isPod> {}; \
  }
...
//
    ...
    ((defined(EA_COMPILER_MSVC) && (_MSC_VER >= 1700)) ||            \
     ...
     (defined(EA_COMPILER_CLANG) && EA_COMPILER_HAS_FEATURE(is_standard_layout)))
#define EASTL_TYPE_TRAIT_is_standard_layout_CONFORMANCE 1  // is_standard_layout is conforming.

  template <typename T>
  struct is_standard_layout
      : public eastl::integral_constant<bool, __is_standard_layout(T) || eastl::is_void<T>::value ||
...
#define EASTL_DECLARE_IS_STANDARD_LAYOUT(T, isStandardLayout)                                                 \
  namespace eastl {                                                                                           \
    template <>                                                                                               \
    struct is_standard_layout<T> : public eastl::integral_constant<bool, isStandardLayout> {};                \
    template <>                                                                                               \
    struct is_standard_layout<const T> : public eastl::integral_constant<bool, isStandardLayout> {};          \
    template <>                                                                                               \
    struct is_standard_layout<volatile T> : public eastl::integral_constant<bool, isStandardLayout> {};       \
    template <>                                                                                               \
    struct is_standard_layout<const volatile T> : public eastl::integral_constant<bool, isStandardLayout> {}; \
  }
...
#define EASTL_TYPE_TRAIT_has_trivial_constructor_CONFORMANCE 1  // has_trivial_constructor is conforming.
  ...
  struct has_trivial_constructor
      : public eastl::integral_constant<bool, __has_trivial_constructor(T) || eastl::is_pod<T>::value> {};
#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
    (defined(_MSC_VER) || defined(EA_COMPILER_GNUC) || defined(EA_COMPILER_CLANG))
#define EASTL_TYPE_TRAIT_has_trivial_constructor_CONFORMANCE 1  // has_trivial_constructor is conforming.
...
#define EASTL_DECLARE_HAS_TRIVIAL_CONSTRUCTOR(T, hasTrivialConstructor)                                  \
  namespace eastl {                                                                                      \
    template <>                                                                                          \
    struct has_trivial_constructor<T> : public eastl::integral_constant<bool, hasTrivialConstructor> {}; \
  }
...
#define EASTL_TYPE_TRAIT_has_trivial_copy_CONFORMANCE 1  // has_trivial_copy is conforming.
  ...
  template <typename T>
  struct has_trivial_copy : public eastl::integral_constant<bool, (__has_trivial_copy(T) || eastl::is_pod<T>::value) &&
...
#define EASTL_TYPE_TRAIT_has_trivial_copy_CONFORMANCE 1  // has_trivial_copy is conforming.
  ...
  struct has_trivial_copy
      : public eastl::integral_constant<bool, (__has_trivial_copy(T) || eastl::is_pod<T>::value) &&
...
#define EASTL_TYPE_TRAIT_has_trivial_copy_CONFORMANCE \
  ...
  template <typename T>
  struct has_trivial_copy
      : public eastl::integral_constant<bool, eastl::is_pod<T>::value && !eastl::is_volatile<T>::value> {};
#endif

#define EASTL_DECLARE_HAS_TRIVIAL_COPY(T, hasTrivialCopy)                                  \
  namespace eastl {                                                                        \
    template <>                                                                            \
    struct has_trivial_copy<T> : public eastl::integral_constant<bool, hasTrivialCopy> {}; \
  }
...
#define EASTL_TYPE_TRAIT_has_trivial_assign_CONFORMANCE 1  // has_trivial_assign is conforming.
  ...
  struct has_trivial_assign
      : public integral_constant<bool, (__has_trivial_assign(T) || eastl::is_pod<T>::value) &&
...
#define EASTL_TYPE_TRAIT_has_trivial_assign_CONFORMANCE \
  ...
  template <typename T>
  struct has_trivial_assign
      : public integral_constant<bool, is_pod<T>::value && !is_const<T>::value && !is_volatile<T>::value> {};
#endif

#define EASTL_DECLARE_HAS_TRIVIAL_ASSIGN(T, hasTrivialAssign)                                  \
  namespace eastl {                                                                            \
    template <>                                                                                \
    struct has_trivial_assign<T> : public eastl::integral_constant<bool, hasTrivialAssign> {}; \
  }
...
#define EASTL_TYPE_TRAIT_has_trivial_destructor_CONFORMANCE 1  // has_trivial_destructor is conforming.
  ...
  struct has_trivial_destructor
      : public eastl::integral_constant<bool, __has_trivial_destructor(T) || eastl::is_pod<T>::value> {};
#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
    (defined(_MSC_VER) || defined(EA_COMPILER_GNUC) || defined(EA_COMPILER_CLANG))
#define EASTL_TYPE_TRAIT_has_trivial_destructor_CONFORMANCE 1  // has_trivial_destructor is conforming.
...
#define EASTL_DECLARE_HAS_TRIVIAL_DESTRUCTOR(T, hasTrivialDestructor)                                  \
  namespace eastl {                                                                                    \
    template <>                                                                                        \
    struct has_trivial_destructor<T> : public eastl::integral_constant<bool, hasTrivialDestructor> {}; \
  }
...
#define EASTL_TYPE_TRAIT_has_nothrow_constructor_CONFORMANCE 1
  ...
  template <typename T>
  struct has_nothrow_constructor : public eastl::integral_constant<bool, __has_nothrow_constructor(T)> {};

#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && defined(_MSC_VER)
// Microsoft's implementation of __has_nothrow_constructor is crippled and returns true only if T is
// a class that has an explicit constructor. "Returns true if the default constructor has an empty
...
#define EASTL_TYPE_TRAIT_has_nothrow_constructor_CONFORMANCE \
  ...
  struct has_nothrow_constructor  // To do: Improve this to include other types that can work.
  {
    static const bool value =
        eastl::is_scalar<typename eastl::remove_all_extents<T>::type>::value || eastl::is_reference<T>::value;
  };
#endif
...
#define EASTL_DECLARE_HAS_NOTHROW_CONSTRUCTOR(T, hasNothrowConstructor)                                  \
  namespace eastl {                                                                                      \
    template <>                                                                                          \
    struct has_nothrow_constructor<T> : public eastl::integral_constant<bool, hasNothrowConstructor> {}; \
  }
...
#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && defined(_MSC_VER)
// Microsoft's implementation of __has_nothrow_copy is crippled and returns true only if T is a
// class that has a copy constructor. "Returns true if the copy constructor has an empty exception
// specification."
#define EASTL_TYPE_TRAIT_has_nothrow_copy_CONFORMANCE 0
  ...
  struct has_nothrow_copy
      : public eastl::integral_constant<
            bool, __has_nothrow_copy(T) || eastl::is_scalar<typename eastl::remove_all_extents<T>::type>::value ||
                      eastl::is_reference<T>::value> {};
...
#define EASTL_DECLARE_HAS_NOTHROW_COPY(T, hasNothrowCopy)                                  \
  namespace eastl {                                                                        \
    template <>                                                                            \
    struct has_nothrow_copy<T> : public eastl::integral_constant<bool, hasNothrowCopy> {}; \
  }
...
#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && defined(_MSC_VER)
// Microsoft's implementation of __has_nothrow_assign is crippled and returns true only if T is a
// class that has an assignment operator. "Returns true if a copy assignment operator has an empty
// exception specification."
#define EASTL_TYPE_TRAIT_has_nothrow_assign_CONFORMANCE 0
  ...
  struct has_nothrow_assign
      : public eastl::integral_constant<
            bool, __has_nothrow_assign(T) || eastl::is_scalar<typename eastl::remove_all_extents<T>::type>::value ||
                      eastl::is_reference<T>::value> {};
#else
...
#define EASTL_DECLARE_HAS_NOTHROW_ASSIGN(T, hasNothrowAssign)                                  \
  namespace eastl {                                                                            \
    template <>                                                                                \
    struct has_nothrow_assign<T> : public eastl::integral_constant<bool, hasNothrowAssign> {}; \
  }
...
#define EASTL_DECLARE_HAS_VIRTUAL_DESTRUCTOR(T, hasVirtualDestructor)                                                 \
  namespace eastl {                                                                                                   \
    template <>                                                                                                       \
    struct has_virtual_destructor<T> : public eastl::integral_constant<bool, hasVirtualDestructor> {};                \
    template <>                                                                                                       \
    struct has_virtual_destructor<const T> : public eastl::integral_constant<bool, hasVirtualDestructor> {};          \
    template <>                                                                                                       \
    struct has_virtual_destructor<volatile T> : public eastl::integral_constant<bool, hasVirtualDestructor> {};       \
    template <>                                                                                                       \
    struct has_virtual_destructor<const volatile T> : public eastl::integral_constant<bool, hasVirtualDestructor> {}; \
  }
  ...
  //         - every constructor call and full-expression in the brace-or-equal-initializer s for
  //         non-static data members (if any) is a constant expression (5.19),
  //         - it is an aggregate type (8.5.1) or has at least one constexpr constructor or
...
#define EASTL_TYPE_TRAIT_is_literal_type_CONFORMANCE \
  0  // It seems that in this case GCC supports the compiler intrinsic but reports it as false when
     // it's true.
#else
#define EASTL_TYPE_TRAIT_is_literal_type_CONFORMANCE 1
#endif
...
#define EASTL_TYPE_TRAIT_is_literal_type_CONFORMANCE 0
  ...
  template <typename T>  // This is not a complete implementation and will be true for only some
                         // literal types (the basic ones).
  struct is_literal_type
      : public eastl::integral_constant<bool, eastl::is_scalar<typename eastl::remove_reference<
...
#define EASTL_TYPE_TRAIT_is_abstract_CONFORMANCE 0
  ...
  class is_abstract_helper {
    ...
    static eastl::no_type test(T1 (*)[1]);  // The following: 'typedef SomeAbstractClass (*SomeFunctionType)[1];' is
                                            // invalid (can't have an array of abstract types) and thus doesn't choose
                                            // this path.
  ...
  template <typename T>
  struct is_abstract : public integral_constant<bool, is_abstract_helper<T>::value> {};
...
#define EASTL_DECLARE_IS_ABSTRACT(T, isAbstract)                                                 \
  namespace eastl {                                                                              \
    ...
    template <>                                                                                  \
    struct is_abstract<const T> : public eastl::integral_constant<bool, isAbstract> {};          \
    template <>                                                                                  \
    struct is_abstract<volatile T> : public eastl::integral_constant<bool, isAbstract> {};       \
    template <>                                                                                  \
    struct is_abstract<const volatile T> : public eastl::integral_constant<bool, isAbstract> {}; \
  }
  ...
  ///////////////////////////////////////////////////////////////////////
    ...
    ((defined(_MSC_VER) && (_MSC_VER >= 1700)) ||     \
     (defined(EA_COMPILER_CLANG) && EA_COMPILER_HAS_FEATURE(is_trivially_copyable)))
#define EASTL_TYPE_TRAIT_is_trivially_copyable_CONFORMANCE 1
  ...
  //
  // From Microsoft:
  //   We're working on fixing this. When overhauling <type_traits> in VC 2013, I incorrectly
  //   believed that is_trivially_copyable was a synonym for is_trivially_copy_constructible. I've
  //   asked the compiler team to provide a compiler hook with 100% accurate answers. (Currently,
  //   the compiler hook has incorrect answers for volatile scalars, volatile data members, and
  //   various scenarios for defaulted/deleted/private special member functions - I wrote an
  //   exhaustive test case to exercise the complicated Standardese.) When the compiler hook is
  //   fixed, I'll change <type_traits> to invoke it.
  //
...
#define EASTL_TYPE_TRAIT_is_trivially_copyable_CONFORMANCE 1
  ...
  // Micrsoft (prior to VS2012) and GCC have __has_trivial_copy, but it may not be identical with
  // the goals of this type trait.
  template <typename T>
  struct is_trivially_copyable
      : public integral_constant<
            bool, (__has_trivial_copy(T) || eastl::is_pod<typename eastl::remove_all_extents<T>::type>::value) &&
                      (!eastl::is_void<T>::value && !eastl::is_volatile<T>::value && !eastl::is_reference<T>::value)> {
  };
#else
...
#define EASTL_DECLARE_IS_TRIVIALLY_COPYABLE(T, isTriviallyCopyable)                                                 \
  namespace eastl {                                                                                                 \
    template <>                                                                                                     \
    struct is_trivially_copyable<T> : public eastl::integral_constant<bool, isTriviallyCopyable> {};                \
    template <>                                                                                                     \
    struct is_trivially_copyable<const T> : public eastl::integral_constant<bool, isTriviallyCopyable> {};          \
    template <>                                                                                                     \
    struct is_trivially_copyable<volatile T> : public eastl::integral_constant<bool, isTriviallyCopyable> {};       \
    template <>                                                                                                     \
    struct is_trivially_copyable<const volatile T> : public eastl::integral_constant<bool, isTriviallyCopyable> {}; \
  }
  ...
  // We currently take a simplified approach whereby we lump all deficient compilers together here,
  // rather than implementing a version for compilers that support rvalue references but not
  // variadic templates. However, we should eventually implement such a thing because VS2010 and
  // VS2012 are exactly that: compilers that support decltype and rvalue references but not variadic
  // templates.

#define EASTL_TYPE_TRAIT_is_constructible_CONFORMANCE 0

  // Start by assuming the type is constructible, then below we subtract types that are known
  // not-constructible. This can result in false-positives, which we normally avoid, but the use
  // cases of this trait are such that we'd rather have false positives than false negatives. But we
  // can revisit that if you have a problem.

  template <typename T>
  struct is_constructible_helper {
    static const bool value =
        !eastl::is_void<T>::value && !eastl::is_function<T>::value && !eastl::is_abstract<T>::value;
  };
  ...
  struct is_constructible {
    static const bool value = eastl::is_constructible_helper<typename eastl::remove_all_extents<T>::type>::value &&
                              ...
                              !eastl::is_reference<T>::value &&  // references can be copy constructed, but here we are
                                                                 // testing for only default construction, which isn't
                                                                 // possible for references.
                              eastl::is_same<Arg0, eastl::unused>::value &&  // We have no way of checking argument
                                                                             // matches without decltype support,
                              eastl::is_same<Arg1, eastl::unused>::value;    // so disallow them. This will result in
                                                                           // cases being potentially false negatives.
  };  // (To do:) However, we could easily add support for constructing a scalar from another single
      // scalar or reference from something.

#else
#define EASTL_TYPE_TRAIT_is_constructible_CONFORMANCE 1

  // We implement a copy of move here has move_internal. We are currently stuck doing this because
  // our move implementation is in <utility.h> and <utility.h> currently #includes us, and so we
  // have a header chicken-and-egg problem. To do: Resolve this, probably by putting eastl::move
  // somewhere else.
  template <typename T>
  ...
  typename first_type_select<eastl::true_type, decltype(eastl::move_internal(T(eastl::declval<Args>()...)))>::type is(
      T&&, Args&&...);
  ...
  struct is_constructible_helper_2  // argument_sink will catch all T that is not constructible from
                                    // the Args and denote false_type
  ...
  // Scalars and references can be constructed only with 0 or 1 argument. e.g the following is an
  // invalid expression: int(17, 23)
  template <typename T, typename Arg0, typename... Args>
  struct is_constructible_helper_2<true, T, Arg0, Args...> : public eastl::false_type {};
  ...
  // Unilaterally dismiss void, abstract, unknown bound arrays, and function types as not
  // constructible.
  ...
  // Array types are constructible if constructed with no arguments and if their element type is
  ...
  // Arrays with arguments are not constructible. e.g. the following is an invalid expression:
  // int[3](37, 34, 12)
  template <typename Array, size_t N, typename... Args>
  struct is_constructible_helper_2<false, Array[N], Args...> : public eastl::false_type {};
...
#define EASTL_DECLARE_IS_CONSTRUCTIBLE(T, U, isConstructible)                                  \
  namespace eastl {                                                                            \
    template <>                                                                                \
    struct is_constructible<T, U> : public eastl::integral_constant<bool, isConstructible> {}; \
  }
...
// If the compiler has this trait built-in (which ideally all compilers would have since it's
// necessary for full conformance) use it.
#if EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
  ...
  template <typename T>
  struct is_trivially_constructible<T, T> : public eastl::integral_constant<bool, __is_trivially_constructible(T)> {};
  ...
  struct is_trivially_constructible<T, eastl::unused>
      : public eastl::integral_constant<
            bool, eastl::is_constructible<T>::value &&
                      eastl::has_trivial_constructor<typename eastl::remove_all_extents<T>::type>::value> {};
...
#define EASTL_TYPE_TRAIT_is_trivially_constructible_CONFORMANCE 1
  ...
  // We have a problem with clang here as of clang 3.4: __is_trivially_constructible(int[]) is
  // false, yet I believe it should be true. Until it gets resolved, what we do is check for
  // is_constructible along with __is_trivially_constructible().
  template <typename T, typename... Args>
  struct is_trivially_constructible
      : public eastl::integral_constant<bool, eastl::is_constructible<T, Args...>::value&& __is_trivially_constructible(
                                                  T, Args...)> {};

#else

#define EASTL_TYPE_TRAIT_is_trivially_constructible_CONFORMANCE \
  0  // This is 0 but in fact it will work for most real-world cases due to the
     // has_trivial_constructor specialization below.
  ...
  // It's questionable whether we can use has_trivial_copy here, as it could theoretically create a
  // false-positive.
  template <typename T>
  struct is_trivially_constructible<T, T>
      : public eastl::integral_constant<bool, eastl::is_constructible<T>::value && eastl::has_trivial_copy<T>::value> {
...
#define EASTL_DECLARE_IS_TRIVIALLY_CONSTRUCTIBLE(T, isTriviallyConstructible)                                         \
  namespace eastl {                                                                                                   \
    template <>                                                                                                       \
    struct is_trivially_constructible<T> : public eastl::integral_constant<bool, isTriviallyConstructible> {};        \
    template <>                                                                                                       \
    struct is_trivially_constructible<const T> : public eastl::integral_constant<bool, isTriviallyConstructible> {};  \
    template <>                                                                                                       \
    struct is_trivially_constructible<volatile T> : public eastl::integral_constant<bool, isTriviallyConstructible> { \
    };                                                                                                                \
    template <>                                                                                                       \
...
#define EASTL_TYPE_TRAIT_is_trivially_default_constructible_CONFORMANCE \
  ...
  template <typename T>
  struct is_trivially_default_constructible : public eastl::is_trivially_constructible<T> {};
...
#define EASTL_TYPE_TRAIT_is_trivial_CONFORMANCE                        \
  ...
  struct is_trivial_helper
      : public eastl::integral_constant<bool, eastl::is_trivially_copyable<T>::value &&
  ...
  template <typename T>
  struct is_trivial : public is_trivial_helper<(EA_ALIGN_OF(T) > EA_PLATFORM_MIN_MALLOC_ALIGNMENT), T>::type {};
#else
  // All other compilers seem to be able to handle aligned types passed as value
  template <typename T>
  struct is_trivial : public eastl::integral_constant<bool, eastl::is_trivially_copyable<T>::value &&
                                                                eastl::is_trivially_default_constructible<T>::value> {};
#endif
...
#define EASTL_TYPE_TRAIT_is_nothrow_constructible_CONFORMANCE 0
  ...
  template <typename T>
  struct is_nothrow_constructible<T> : public eastl::integral_constant<bool, eastl::has_nothrow_constructor<T>::value> {
  };

  template <typename T>
  struct is_nothrow_constructible<T, T> : public eastl::integral_constant<bool, eastl::has_nothrow_copy<T>::value> {};
...
#define EASTL_TYPE_TRAIT_is_nothrow_constructible_CONFORMANCE \
  0  // GCC up to v4.7's noexcept is broken and fails to generate true for the case of
...
#define EASTL_TYPE_TRAIT_is_nothrow_constructible_CONFORMANCE EASTL_TYPE_TRAIT_is_constructible_CONFORMANCE
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // *_noexcept_wrapper implements a workaround for VS2015 preview.  A standards conforming noexcept
  // operator allows variadic template expansion. There appears to be an issue with VS2015 preview
  // that prevents variadic template expansion into a noexcept operator that is passed directly to a
  // template parameter.
  //
  // The fix hoists the noexcept expression into a separate struct and caches the result of the
  // expression.  This result is then passed to integral_constant.
  //
  // Example code from Clang libc++
  // template <class _Tp, class... _Args>
  // struct __libcpp_is_nothrow_constructible<[>is constructible*/true, /*is reference<]false, _Tp,
  // _Args...>
  ...
  template <typename T>
  struct is_nothrow_constructible_helper<true, T> : public eastl::integral_constant<bool, noexcept(T())> {};

  template <typename T, typename... Args>
  struct is_nothrow_constructible_helper<false, T, Args...> : public eastl::false_type {};
...
#define EASTL_DECLARE_IS_NOTHROW_CONSTRUCTIBLE(T, isNothrowConstructible)                                  \
  namespace eastl {                                                                                        \
    template <>                                                                                            \
    struct is_nothrow_constructible<T> : public eastl::integral_constant<bool, isNothrowConstructible> {}; \
  }
...
#define EASTL_TYPE_TRAIT_is_trivially_copy_constructible_CONFORMANCE \
  ...
  struct is_trivially_copy_constructible
      : public eastl::is_trivially_constructible<
            T, typename eastl::add_lvalue_reference<typename eastl::add_const<T>::type>::type> {};
...
#define EASTL_TYPE_TRAIT_is_move_constructible_CONFORMANCE 0
  ...
  template <typename T>
  struct is_move_constructible : public eastl::is_copy_constructible<T> {};
#else
#define EASTL_TYPE_TRAIT_is_move_constructible_CONFORMANCE EASTL_TYPE_TRAIT_is_constructible_CONFORMANCE

  template <typename T>
  struct is_move_constructible : public eastl::is_constructible<T, typename eastl::add_rvalue_reference<T>::type> {};
#endif
...
#define EASTL_TYPE_TRAIT_is_trivially_move_constructible_CONFORMANCE 0
  ...
  template <typename T>
  struct is_trivially_move_constructible : public eastl::is_trivially_copy_constructible<T> {};
#else
...
#define EASTL_DECLARE_IS_TRIVIALLY_MOVE_CONSTRUCTIBLE(T, isTrivallyMoveConstructible)                                 \
  namespace eastl {                                                                                                   \
    template <>                                                                                                       \
    struct is_trivially_move_constructible<T> : public eastl::integral_constant<bool, isTriviallyMoveConstructible> { \
    };                                                                                                                \
  }
  ...
  // is_assignable
  //
  // The expression declval<T>() = declval<U>() is well-formed when treated as an unevaluated
  // operand. Access checking is performed as if in a context unrelated to T and U. Only the
  // validity of the immediate context of the assignment expression is considered. The compilation
  // of the expression can result in side effects such as the instantiation of class template
  // specializations and function template specializations, the generation of implicitly-defined
  // functions, and so on. Such side effects are not in the "immediate context" and can result in
  // the program being ill-formed.
  //
  // Note:
  // This type trait has a misleading and counter-intuitive name. It does not indicate whether an
  // instance of U can be assigned to an instance of T (e.g. t = u). Instead it indicates whether
  // the assignment can be done after adding rvalue references to both, as in
  // add_rvalue_reference<T>::type = add_rvalue_reference<U>::type. A counterintuitive result of
  // this is that is_assignable<int, int>::value == false. The is_copy_assignable trait indicates if
  // a type can be assigned to its own type, though there isn't a standard C++ way to tell if an
  // arbitrary type is assignable to another type.
  // http://stackoverflow.com/questions/19920213/why-is-stdis-assignable-counter-intuitive
  //
  // Note:
  // A true is_assignable value doesn't guarantee that the expression is compile-able, the compiler
  // checks only that the assignment matches before compilation. In particular, if you have
  // templated operator= for a class, the compiler will always say is_assignable is true, regardless
  // of what's being tested on the right hand side of the expression. It may actually turn out
  // during compilation that the templated operator= fails to compile because in practice it doesn't
  // accept every possible type for the right hand side of the expression.
  //
  // Expected results:
  ...
  //
  // Note:
  // Our implementation here yields different results than does the std::is_assignable from
  // Dinkumware-based Standard Libraries, but yields similar results to the std::is_assignable from
  // GCC's libstdc++ and clang's libc++. It may possibly be that the Dinkumware results are
  // intentionally different for some practical purpose or because they represent the spirit or the
  // Standard but not the letter of the Standard.
  //
  ///////////////////////////////////////////////////////////////////////

#if (defined(EA_COMPILER_NO_DECLTYPE) && !EASTL_TYPE_TRAIT_declval_CONFORMANCE) || \
    defined(_MSC_VER)  // VS2012 mis-compiles the conforming code below and so must be placed here.
#define EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE 0
  ...
  struct is_assignable_helper {
    ...
    static const bool isSameScalar =
        (eastl::is_same<T1, U1>::value && eastl::is_scalar<T1>::value && eastl::is_scalar<U1>::value);
...
#define EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE 1
  ...
  template <typename T, typename U>
  struct is_assignable : public eastl::integral_constant<bool, eastl::is_assignable_helper<T, U>::value> {};
...
#define EASTL_DECLARE_IS_ASSIGNABLE(T, U, isAssignable)                                  \
  namespace eastl {                                                                      \
    template <>                                                                          \
    struct is_assignable<T, U> : public eastl::integral_constant<bool, isAssignable> {}; \
  }
...
#define EASTL_DECLARE_IS_LVALUE_ASSIGNABLE(T, U, isLvalueAssignable)                                  \
  namespace eastl {                                                                                   \
    template <>                                                                                       \
    struct is_lvalue_assignable<T, U> : public eastl::integral_constant<bool, isLvalueAssignable> {}; \
  }
...
#define EASTL_TYPE_TRAIT_is_trivially_assignable_CONFORMANCE 1
  ...
  template <typename T, typename U>
  struct is_trivially_assignable : eastl::integral_constant<bool, __is_trivially_assignable(T, U)> {};
...
#define EASTL_TYPE_TRAIT_is_trivially_assignable_CONFORMANCE EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE
  ...
  // This code path is attempting to work around the issue with VS2013 __is_trivially_assignable
  // compiler intrinsic documented in the link below.  todo: Re-evaluate in VS2014.
  //
...
#define EASTL_TYPE_TRAIT_is_trivially_assignable_CONFORMANCE EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE
  ...
  // Micrsoft (up till at least VS2012) and GCC have __has_trivial_assign, but it may not be
  // identical with the goals of this type trait. The Microsoft type trait headers suggest that a
  // future version of VS will have a __is_trivially_assignable intrinsic, but we need to come up
  // with something in the meantime. To do: Re-evalulate this for VS2013+ when it becomes available.
  template <typename T, typename U>
  struct is_trivially_assignable
...
#define EASTL_TYPE_TRAIT_is_trivially_assignable_CONFORMANCE 0  // Generates false negatives.
  ...
  template <typename T>
  struct is_trivially_assignable<T&, T> : public eastl::integral_constant<bool, eastl::is_scalar<T>::value> {};

  template <typename T>
  struct is_trivially_assignable<T&, T&> : public eastl::integral_constant<bool, eastl::is_scalar<T>::value> {};

  template <typename T>
  struct is_trivially_assignable<T&, const T&> : public eastl::integral_constant<bool, eastl::is_scalar<T>::value> {};
...
#define EASTL_DECLARE_IS_TRIVIALLY_ASSIGNABLE(T, U, isTriviallyAssignable)                                  \
  namespace eastl {                                                                                         \
    template <>                                                                                             \
    struct is_trivially_assignable<T, U> : public eastl::integral_constant<bool, isTriviallyAssignable> {}; \
  }
...
#define EASTL_TYPE_TRAIT_is_nothrow_assignable_CONFORMANCE 1
  ...
  template <typename T, typename U>
  struct is_nothrow_assignable : eastl::integral_constant<bool, __is_nothrow_assignable(T, U)> {};

#elif defined(EA_COMPILER_NO_NOEXCEPT) || defined(__EDG_VERSION__)  // EDG mis-compiles the conforming code below and so
                                                                    // must be placed here.
#define EASTL_TYPE_TRAIT_is_nothrow_assignable_CONFORMANCE 0
  ...
  // Note that the following are crippled in that they support only assignment of T types to other T
  // types.
  template <typename T>
  struct is_nothrow_assignable<T&, T> : public eastl::integral_constant<bool, eastl::has_nothrow_assign<T>::value> {};
...
#define EASTL_TYPE_TRAIT_is_nothrow_assignable_CONFORMANCE 1
  ...
  template <typename T, typename U>
  struct is_nothrow_assignable_helper<true, T, U>  // Set to true if the assignment (same as
                                                   // is_assignable) cannot generate an exception.
      : public eastl::integral_constant<bool, noexcept(eastl::declval<T>() = eastl::declval<U>())> {};

  template <typename T, typename U>
  struct is_nothrow_assignable : public eastl::is_nothrow_assignable_helper<eastl::is_assignable<T, U>::value, T, U> {};
#endif

#define EASTL_DECLARE_IS_NOTHROW_ASSIGNABLE(T, isNothrowAssignable)                                                 \
  namespace eastl {                                                                                                 \
    template <>                                                                                                     \
    struct is_nothrow_assignable<T> : public eastl::integral_constant<bool, isNothrowAssignable> {};                \
    template <>                                                                                                     \
    struct is_nothrow_assignable<const T> : public eastl::integral_constant<bool, isNothrowAssignable> {};          \
    template <>                                                                                                     \
    struct is_nothrow_assignable<volatile T> : public eastl::integral_constant<bool, isNothrowAssignable> {};       \
    template <>                                                                                                     \
    struct is_nothrow_assignable<const volatile T> : public eastl::integral_constant<bool, isNothrowAssignable> {}; \
  }
      ...
      : public integral_constant<
            bool, eastl::is_scalar<T>::value || eastl::is_pod<T>::value ||
                      eastl::is_trivially_assignable<
                          typename eastl::add_lvalue_reference<T>::type,
                          typename eastl::add_lvalue_reference<typename eastl::add_const<T>::type>::type>::value> {};
#endif

#define EASTL_DECLARE_IS_TRIVIALLY_COPY_ASSIGNABLE(T, isTriviallyCopyAssignable)                                  \
  namespace eastl {                                                                                               \
    template <>                                                                                                   \
    struct is_trivially_copy_assignable<T> : public eastl::integral_constant<bool, isTriviallyCopyAssignable> {}; \
  }
...
#define EASTL_TYPE_TRAIT_is_move_assignable_CONFORMANCE EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE
  ...
  template <typename T>
  struct is_move_assignable : public eastl::is_assignable<typename eastl::add_lvalue_reference<T>::type,
...
#define EASTL_DECLARE_IS_MOVE_ASSIGNABLE(T, isMoveAssignable)                                                 \
  namespace eastl {                                                                                           \
    template <>                                                                                               \
    struct is_move_assignable<T> : public eastl::integral_constant<bool, isMoveAssignable> {};                \
    template <>                                                                                               \
    struct is_move_assignable<const T> : public eastl::integral_constant<bool, isMoveAssignable> {};          \
    template <>                                                                                               \
    struct is_move_assignable<volatile T> : public eastl::integral_constant<bool, isMoveAssignable> {};       \
    template <>                                                                                               \
    struct is_move_assignable<const volatile T> : public eastl::integral_constant<bool, isMoveAssignable> {}; \
  }
...
#if 0  // defined(_MSC_VER) && (_MSC_VER >= 1800) // VS2013+ -- Disabled due to __is_destructible
       // being broken in VC++ versions up to at least VS2013. A ticket will be submitted for this
...
#elif defined(EA_COMPILER_NO_DECLTYPE) || defined(EA_COMPILER_NO_FUNCTION_TEMPLATE_DEFAULT_ARGS) || \
    defined(_MSC_VER) || defined(__EDG_VERSION__)  // VS2012 and EDG mis-compile the conforming code below and so must
                                                   // be placed here.
...
#define EASTL_TYPE_TRAIT_is_destructible_CONFORMANCE 1
  ...
  template <typename T,
            typename U = decltype(eastl::declval<eastl::destructible_test_helper<T> >().~destructible_test_helper<T>())>
  eastl::true_type destructible_test_function(int);

  template <typename T,
            bool = eastl::is_array_of_unknown_bounds<T>::value ||  // Exclude these types from being
                                                                   // considered destructible.
                   eastl::is_void<T>::value || eastl::is_function<T>::value || eastl::is_abstract<T>::value>
  struct is_destructible_helper : public eastl::identity<decltype(eastl::destructible_test_function<T>(0))>::type {
  };  // Need to wrap decltype with identity because some compilers otherwise don't like the bare
      // decltype usage.
...
#define EASTL_DECLARE_IS_DESTRUCTIBLE(T, isDestructible)                                                 \
  namespace eastl {                                                                                      \
    template <>                                                                                          \
    struct is_destructible<T> : public eastl::integral_constant<bool, isDestructible> {};                \
    template <>                                                                                          \
    struct is_destructible<const T> : public eastl::integral_constant<bool, isDestructible> {};          \
    template <>                                                                                          \
    struct is_destructible<volatile T> : public eastl::integral_constant<bool, isDestructible> {};       \
    template <>                                                                                          \
    struct is_destructible<const volatile T> : public eastl::integral_constant<bool, isDestructible> {}; \
  }
...
#if 0  // defined(_MSC_VER) && (_MSC_VER >= 1800) // VS2013+ -- Disabled due to
       // __is_trivially_destructible being broken in VC++ versions up to at least VS2013. A ticket
...
#define EASTL_TYPE_TRAIT_is_trivially_destructible_CONFORMANCE EASTL_TYPE_TRAIT_is_destructible_CONFORMANCE
  ...
  template <typename T>
  struct is_trivially_destructible  // Can't use just __has_trivial_destructor(T) because some
                                    // compilers give it slightly different meaning, and are just
                                    // plain broken, such as VC++'s __has_trivial_destructor, which
                                    // says false for fundamental types.
      : public integral_constant<bool, eastl::is_destructible<T>::value &&
...
#define EASTL_TYPE_TRAIT_is_trivially_destructible_CONFORMANCE 0
  ...
  struct is_trivially_destructible_helper
      : public integral_constant<bool, (eastl::is_pod<T>::value || eastl::is_scalar<T>::value ||
                                        eastl::is_reference<T>::value) &&
...
#define EASTL_DECLARE_IS_TRIVIALLY_DESTRUCTIBLE(T, isTriviallyDestructible)                                           \
  namespace eastl {                                                                                                   \
    template <>                                                                                                       \
    struct is_trivially_destructible<T> : public eastl::integral_constant<bool, isTriviallyDestructible> {};          \
    template <>                                                                                                       \
    struct is_trivially_destructible<const T> : public eastl::integral_constant<bool, isTriviallyDestructible> {};    \
    template <>                                                                                                       \
    struct is_trivially_destructible<volatile T> : public eastl::integral_constant<bool, isTriviallyDestructible> {}; \
    template <>                                                                                                       \
...
#if 0  // defined(_MSC_VER) && (_MSC_VER >= 1800) // VS2013+ -- Disabled due to
       // __is_nothrow_destructible being broken in VC++ versions up to at least VS2013. A ticket
       // will be submitted for this
#define EASTL_TYPE_TRAIT_is_nothrow_destructible_CONFORMANCE \
  ((_MSC_VER >= 1900) ? 1 : 0)  // VS2013 (1800) doesn't support noexcept and so can't support all
                                // usage of this properly (in particular default exception
                                // specifications defined in [C++11 Standard, 15.4 paragraph 14].
...
#define EASTL_TYPE_TRAIT_is_nothrow_destructible_CONFORMANCE EASTL_TYPE_TRAIT_is_destructible_CONFORMANCE
#endif
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // *_noexcept_wrapper implements a workaround for VS2015.  A standards conforming noexcept
  // operator allows variadic template expansion. There appears to be an issue with VS2015 that
  // prevents variadic template expansion into a noexcept operator that is passed directly to a
  // template parameter.
  //
  // The fix hoists the noexcept expression into a separate struct and caches the result of the
  // expression.  This result is then passed to integral_constant.
  ...
  template <typename T>
  struct is_nothrow_destructible_helper<T, true>  // If the expression T::~T is a noexcept
                                                  // expression then it's nothrow.
      : public eastl::integral_constant<bool, is_nothrow_destructible_helper_noexcept_wrapper<T>::value> {};

  template <typename T>
  struct is_nothrow_destructible  // A type needs to at least be destructible before it could be
                                  // nothrow destructible.
      : public eastl::is_nothrow_destructible_helper<T, eastl::is_destructible<T>::value> {};

  template <typename T,
            size_t N>  // An array is nothrow destructible if its element type is nothrow
                       // destructible.
  struct is_nothrow_destructible<T[N]>  // To consider: Replace this with a
                                        // remove_all_extents pathway.
      : public eastl::is_nothrow_destructible<T> {};

  template <typename T>
  struct is_nothrow_destructible<T&>  // A reference type cannot throw while being destructed. It's
                                      // just a reference.
      : public eastl::true_type {};

#if !EASTL_NO_RVALUE_REFERENCES
  template <typename T>
  struct is_nothrow_destructible<T&&>  // An rvalue reference type cannot throw while being
                                       // destructed.
      : public eastl::true_type {};
#endif
...
#define EASTL_TYPE_TRAIT_is_nothrow_default_constructible_CONFORMANCE \
  ...
  template <typename T>
  struct is_nothrow_default_constructible : public eastl::is_nothrow_constructible<T> {};
...
#define EASTL_TYPE_TRAIT_is_nothrow_move_constructible_CONFORMANCE 0
  ...
  template <typename T>
  struct is_nothrow_move_constructible : public eastl::is_nothrow_copy_constructible<T> {};
#else
