...
///////////////////////////////////////////////////////////////////////
    ...
    (defined(_MSC_VER) || defined(EA_COMPILER_GNUC) || (defined(__clang__) && EA_COMPILER_HAS_FEATURE(is_empty)))
...
#define EASTL_TYPE_TRAIT_is_pod_CONFORMANCE \
  1  // is_pod is conforming. Actually as of VS2008 it is apparently not fully conforming, as it flags the following as
     // a non-pod: struct Pod{ Pod(){} };

  EA_DISABLE_VC_WARNING(4647)
  template <typename T>  // We check for has_trivial_constructor only because the VC++ is_pod does. Is it due to some
                         // compiler bug?
                         struct is_pod
  ...
  EA_RESTORE_VC_WARNING()
    ...
    (defined(EA_COMPILER_GNUC) || (defined(__clang__) && EA_COMPILER_HAS_FEATURE(is_pod)))
...
//
    ...
    ((defined(EA_COMPILER_MSVC) && (_MSC_VER >= 1700)) ||            \
     ...
     (defined(__clang__) && EA_COMPILER_HAS_FEATURE(is_standard_layout)))
...
#define EASTL_TYPE_TRAIT_has_trivial_constructor_CONFORMANCE 1                 // has_trivial_constructor is conforming.
  ...
  struct has_trivial_constructor
      : public eastl::integral_constant<bool, (__has_trivial_constructor(T) || eastl::is_pod<T>::value) &&
                                                  !eastl::is_hat_type<T>::value> {};
#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
    (defined(_MSC_VER) || defined(EA_COMPILER_GNUC) || defined(__clang__))
#define EASTL_TYPE_TRAIT_has_trivial_constructor_CONFORMANCE 1  // has_trivial_constructor is conforming.
...
#define EASTL_TYPE_TRAIT_has_trivial_copy_CONFORMANCE 1  // has_trivial_copy is conforming.
  ...
  struct has_trivial_copy
      : public eastl::integral_constant<bool, (__has_trivial_copy(T) || eastl::is_pod<T>::value) &&
                                                  !eastl::is_volatile<T>::value && !eastl::is_hat_type<T>::value> {};
...
// Microsoft's implementation of __has_nothrow_constructor is crippled and returns true only if T is a class that has an
// explicit constructor. "Returns true if the default constructor has an empty exception specification."
...
#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && defined(_MSC_VER)
// Microsoft's implementation of __has_nothrow_copy is crippled and returns true only if T is a class that has a copy
// constructor. "Returns true if the copy constructor has an empty exception specification."
...
#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && defined(_MSC_VER)
// Microsoft's implementation of __has_nothrow_assign is crippled and returns true only if T is a class that has an
// assignment operator. "Returns true if a copy assignment operator has an empty exception specification."
...
#define EASTL_DECLARE_HAS_VIRTUAL_DESTRUCTOR(T, hasVirtualDestructor)                                                 \
  ...
  //         - every constructor call and full-expression in the brace-or-equal-initializer s for non-static data
...
#define EASTL_TYPE_TRAIT_is_literal_type_CONFORMANCE 0
  ...
  template <typename T>  // This is not a complete implementation and will be true for only some literal types (the
                         // basic ones).
...
#if EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
    (defined(_MSC_VER) || defined(EA_COMPILER_GNUC) || (defined(__clang__) && EA_COMPILER_HAS_FEATURE(is_abstract)))
...
#define EASTL_TYPE_TRAIT_is_abstract_CONFORMANCE 0
  ...
  class is_abstract_helper {
    ...
    static eastl::no_type test(
        T1 (*)[1]);  // The following: 'typedef SomeAbstractClass (*SomeFunctionType)[1];' is invalid (can't have an
                     // array of abstract types) and thus doesn't choose this path.
...
///////////////////////////////////////////////////////////////////////
#if EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE &&                                                             \
    ((defined(_MSC_VER) && (_MSC_VER >= 1700)) || (defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 5003)) || \
     (defined(__clang__) && EA_COMPILER_HAS_FEATURE(is_trivially_copyable)))
#define EASTL_TYPE_TRAIT_is_trivially_copyable_CONFORMANCE 1
  ...
  //
  // From Microsoft:
  //   We're working on fixing this. When overhauling <type_traits> in VC 2013, I incorrectly believed that
  //   is_trivially_copyable was a synonym for is_trivially_copy_constructible. I've asked the compiler team to provide
  //   a compiler hook with 100% accurate answers. (Currently, the compiler hook has incorrect answers for volatile
  //   scalars, volatile data members, and various scenarios for defaulted/deleted/private special member functions - I
  //   wrote an exhaustive test case to exercise the complicated Standardese.) When the compiler hook is fixed, I'll
  //   change <type_traits> to invoke it.
  //
...
#define EASTL_TYPE_TRAIT_is_trivially_copyable_CONFORMANCE 1
  ...
  // Micrsoft (prior to VS2012) and GCC have __has_trivial_copy, but it may not be identical with the goals of this type
  // trait.
  template <typename T>
  struct is_trivially_copyable
...
#define EASTL_TYPE_TRAIT_is_constructible_CONFORMANCE 1
...
#if EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
    (defined(_MSC_VER) || (defined(__clang__) && EA_COMPILER_HAS_FEATURE(is_constructible)))
  ...
  struct is_constructible_helper_2  // argument_sink will catch all T that is not constructible from the Args and denote
                                    // false_type
...
// If the compiler has this trait built-in (which ideally all compilers would have since it's necessary for full
// conformance) use it.
#if EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
    (defined(__clang__) && EA_COMPILER_HAS_FEATURE(is_trivially_constructible))
...
#define EASTL_TYPE_TRAIT_is_trivially_constructible_CONFORMANCE 1
  ...
  // We have a problem with clang here as of clang 3.4: __is_trivially_constructible(int[]) is false, yet I believe it
  // should be true. Until it gets resolved, what we do is check for is_constructible along with
...
#define EASTL_TYPE_TRAIT_is_trivially_constructible_CONFORMANCE \
  0  // This is 0 but in fact it will work for most real-world cases due to the has_trivial_constructor specialization
     // below.
...
#define EASTL_TYPE_TRAIT_is_nothrow_constructible_CONFORMANCE EASTL_TYPE_TRAIT_is_constructible_CONFORMANCE
  ...
  // variadic template expansion. There appears to be an issue with VS2015 preview that prevents variadic template
  // expansion into a noexcept operator that is passed directly to a template parameter.
  //
  // The fix hoists the noexcept expression into a separate struct and caches the result of the expression.  This result
  // is then passed to integral_constant.
  //
...
#if EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
    (defined(__clang__) && EA_COMPILER_HAS_FEATURE(is_trivially_assignable))
...
#define EASTL_TYPE_TRAIT_is_trivially_assignable_CONFORMANCE EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE
  ...
  // This code path is attempting to work around the issue with VS2013 __is_trivially_assignable compiler intrinsic
  // documented in the link below.  todo: Re-evaluate in VS2014.
  //
...
#define EASTL_TYPE_TRAIT_is_trivially_assignable_CONFORMANCE EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE
  ...
  // Micrsoft (up till at least VS2012) and GCC have __has_trivial_assign, but it may not be identical with the goals of
  // this type trait. The Microsoft type trait headers suggest that a future version of VS will have a
  // __is_trivially_assignable intrinsic, but we need to come up with something in the meantime. To do: Re-evalulate
  // this for VS2013+ when it becomes available.
  template <typename T, typename U>
  struct is_trivially_assignable
...
#define EASTL_TYPE_TRAIT_is_nothrow_assignable_CONFORMANCE 1
  ...
  struct is_nothrow_assignable : eastl::integral_constant<bool, __is_nothrow_assignable(T, U)> {};
    ...
    defined(__EDG_VERSION__)  // EDG mis-compiles the conforming code below and so must be placed here.
...
#define EASTL_TYPE_TRAIT_is_nothrow_assignable_CONFORMANCE 1
  ...
  template <typename T, typename U>
  struct is_nothrow_assignable_helper<true, T, U>  // Set to true if the assignment (same as is_assignable) cannot
...
#if 0  // defined(_MSC_VER) && (_MSC_VER >= 1800) // VS2013+ -- Disabled due to __is_destructible being broken in VC++
       // versions up to at least VS2013. A ticket will be submitted for this
#define EASTL_TYPE_TRAIT_is_destructible_CONFORMANCE 1
    ...
    defined(_MSC_VER) ||                                                                            \
    defined(__EDG_VERSION__)  // VS2012 and EDG mis-compile the conforming code below and so must be placed here.
...
#define EASTL_TYPE_TRAIT_is_destructible_CONFORMANCE 1
  ...
  template <typename T, bool = eastl::is_array_of_unknown_bounds<T>::value ||  // Exclude these types from being
...
#if 0  // defined(_MSC_VER) && (_MSC_VER >= 1800) // VS2013+ -- Disabled due to __is_trivially_destructible being broken
       // in VC++ versions up to at least VS2013. A ticket will be submitted for this
...
#define EASTL_TYPE_TRAIT_is_trivially_destructible_CONFORMANCE EASTL_TYPE_TRAIT_is_destructible_CONFORMANCE
  ...
  template <typename T>
  struct is_trivially_destructible  // Can't use just __has_trivial_destructor(T) because some compilers give it
                                    // slightly different meaning, and are just plain broken, such as VC++'s
                                    // __has_trivial_destructor, which says false for fundamental types.
...
#if 0  // defined(_MSC_VER) && (_MSC_VER >= 1800) // VS2013+ -- Disabled due to __is_nothrow_destructible being broken
       // in VC++ versions up to at least VS2013. A ticket will be submitted for this
#define EASTL_TYPE_TRAIT_is_nothrow_destructible_CONFORMANCE \
  ((_MSC_VER >= 1900)                                        \
       ? 1                                                   \
       : 0)  // VS2013 (1800) doesn't support noexcept and so can't support all usage of this properly (in particular
             // default exception specifications defined in [C++11 Standard, 15.4 paragraph 14].
...
#endif
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // *_noexcept_wrapper implements a workaround for VS2015.  A standards conforming noexcept operator allows variadic
  // template expansion. There appears to be an issue with VS2015 that prevents variadic template expansion into a
  // noexcept operator that is passed directly to a template parameter.
  //
  // The fix hoists the noexcept expression into a separate struct and caches the result of the expression.  This result
  // is then passed to integral_constant.
  //
