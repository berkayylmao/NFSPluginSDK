...
namespace eastl {
  ...
  ///////////////////////////////////////////////////////////////////////
    ...
    ((defined(_MSC_VER) && (_MSC_VER >= 1700)) || (defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4007)) || \
...
#define EASTL_TYPE_TRAIT_is_signed_CONFORMANCE 1  // is_signed is conforming.
  ...
  struct is_signed_helper<char> : public true_type {};
    ...
    ((__WCHAR_MAX__ == 2147483647) || (__WCHAR_MAX__ == 32767))  // GCC defines __WCHAR_MAX__ for most platforms.
  template <>
...
#define EASTL_TYPE_TRAIT_is_unsigned_CONFORMANCE 1  // is_unsigned is conforming.
  ...
  struct is_unsigned_helper<char> : public true_type {};
    ...
    (defined(__WCHAR_MAX__) && \
     ((__WCHAR_MAX__ == 4294967295U) || (__WCHAR_MAX__ == 65535)))  // GCC defines __WCHAR_MAX__ for most platforms.
  template <>
...
#define EASTL_TYPE_TRAIT_rank_CONFORMANCE 1  // rank is conforming.
  ...
  // For a given array type T and a given dimension I where I >= rank<T>::value, extent<T, I>::value
  // == 0. For a given array type of unknown extent T[], extent<T[], 0>::value == 0. For a given
  // non-array type T and an arbitrary dimension I, extent<T, I>::value == 0.
  //
  ///////////////////////////////////////////////////////////////////////
...
#define EASTL_TYPE_TRAIT_is_base_of_CONFORMANCE 1  // is_base_of is conforming.
  ...
  struct is_base_of
      : public eastl::integral_constant<bool, __is_base_of(Base, Derived) || eastl::is_same<Base, Derived>::value> {};
#else
      // Not implemented yet.
...
#define EASTL_TYPE_TRAIT_is_rvalue_reference_CONFORMANCE \
  0  // Given that the compiler doesn't support rvalue references, maybe the conformance here should
     // be 1, since the result of this is always correct.
...
#define EASTL_TYPE_TRAIT_common_type_CONFORMANCE 1  // common_type is conforming.
  ...
  struct common_type<T> {
    typedef T type;
  };  // Question: Should we use T or decay_t<T> here? The C++11 Standard specifically (20.9.7.6,p3)
      // specifies that it be without decay, but libc++ uses decay.

  template <typename T, typename U>
  struct common_type<T, U> {
    static T&&                         t();
    static U&&                         u();
    typedef decltype(true ? t() : u()) type;  // The type of a tertiary expression is set by the compiler
                                              // to be the common type of the two result types.
  };
