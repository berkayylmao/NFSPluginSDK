...
namespace eastl {
  ...
  ///     virtually synonymous). is_trivially_copyable<T>::value is true. i.e. the constructor T(const T& t) (or T(T&&
  ///     t) if present) can be replaced by memmove(this, &t, sizeof(T))
  ///
  ...
  // InputIterator last-first to an integral type. There's simple way for us to take advantage of a random access output
  // iterator, as the range is specified by the input instead of the output, and distance(first, last) for a
  ...
  struct move_and_copy_helper<EASTL_ITC_NS::random_access_iterator_tag, isMove, true> {
    ...
    static T* move_or_copy(const T* first, const T* last, T* result) {
      if (EASTL_UNLIKELY(first == last)) return result;
  ...
  inline OutputIterator move_and_copy_chooser(InputIterator first, InputIterator last, OutputIterator result) {
    ...
    return eastl::move_and_copy_helper<IIC, isMove, canBeMemmoved>::move_or_copy(
        first, last, result);  // Need to chose based on the input iterator tag and not the output iterator tag, because
