...
#include <OpenSpeed/Core/EASTL/EASTL/internal/hashtable.h>
...
namespace eastl {
  ...
  struct intrusive_node_iterator {
   ...
   public:
    intrusive_node_iterator() : mpNode(NULL) {}

    explicit intrusive_node_iterator(value_type* pNode) : mpNode(pNode) {}

    intrusive_node_iterator(const intrusive_node_iterator<Value, true>& x) : mpNode(x.mpNode) {}

    reference operator*() const { return *mpNode; }

    pointer operator->() const { return mpNode; }
  ...
  struct intrusive_hashtable_iterator_base {
   ...
   public:
    intrusive_hashtable_iterator_base(value_type* pNode, value_type** pBucket) : mpNode(pNode), mpBucket(pBucket) {}
  ...
  struct intrusive_hashtable_iterator : public intrusive_hashtable_iterator_base<Value> {
   ...
   public:
    intrusive_hashtable_iterator() : base_type(NULL, NULL) {}

    explicit intrusive_hashtable_iterator(value_type* pNode, value_type** pBucket) : base_type(pNode, pBucket) {}

    explicit intrusive_hashtable_iterator(value_type** pBucket) : base_type(*pBucket, pBucket) {}

    intrusive_hashtable_iterator(const this_type_non_const& x) : base_type(x.mpNode, x.mpBucket) {}

    reference operator*() const { return *base_type::mpNode; }

    pointer operator->() const { return base_type::mpNode; }
  ...
  struct use_intrusive_key  // : public unary_function<T, T> // Perhaps we want to make it a
                            // subclass of unary_function.
  {
    typedef Key result_type;

    const result_type& operator()(const Node& x) const { return x.mKey; }
  };

  ///////////////////////////////////////////////////////////////////////////
  /// intrusive_hashtable
  ///
  template <typename Key, typename Value, typename Hash, typename Equal, size_t bucketCount, bool bConstIterators,
            bool bUniqueKeys>
  class intrusive_hashtable {
   public:
    ...
    typedef
        typename type_select<bConstIterators, eastl::use_self<Value>, eastl::use_intrusive_key<Value, key_type> >::type
            extract_key;

    enum { kBucketCount = bucketCount };

   protected:
    node_type* mBucketArray[kBucketCount + 1];  // '+1' because we have an end bucket which is
                                                // non-NULL so iterators always stop on it.
   ...
   public:
    ...
    iterator end() EA_NOEXCEPT { return iterator(mBucketArray + kBucketCount); }
    ...
    local_iterator begin(size_type n) EA_NOEXCEPT { return local_iterator(mBucketArray[n]); }

    const_local_iterator begin(size_type n) const EA_NOEXCEPT { return const_local_iterator(mBucketArray[n]); }

    const_local_iterator cbegin(size_type n) const EA_NOEXCEPT { return const_local_iterator(mBucketArray[n]); }
    ...
    size_type bucket(const key_type& k) const EA_NOEXCEPT { return (size_type)(mHash(k) % kBucketCount); }

   public:
    float load_factor() const EA_NOEXCEPT { return (float)mnElementCount / (float)kBucketCount; }
   ...
   public:
    ...
    insert_return_type insert(const_iterator, value_type& value) {
      return insert(value);
    }  // To consider: We might be able to use the iterator argument to specify a specific insertion
       // location.
   ...
   public:
    ...
    iterator  remove(value_type& value);  // Removes by value instead of by iterator. This is an O(1)
                                          // operation, due to this hashtable being 'intrusive'.
   ...
   public:
    Hash hash_function() const { return mHash; }
    ...
    const key_equal& key_eq() const { return mEqual; }

    key_equal& key_eq() { return mEqual; }

   protected:
    eastl::pair<iterator, bool> DoInsertValue(value_type&,
                                              true_type);                // true_type means bUniqueKeys is true.
  ...
  inline intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::intrusive_hashtable(const H& h, const Eq& eq) :
      mnElementCount(0), mHash(h), mEqual(eq) {
  ...
  void intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::swap(this_type& x) {
    for (size_t i = 0; i < kBucketCount; i++) eastl::swap(mBucketArray[i], x.mBucketArray[i]);
  ...
  intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::find(const key_type& k) const {
    const size_type  n     = (size_type)(mHash(k) % kBucketCount);
    node_type* const pNode = DoFindNode(mBucketArray[n], k);
    return pNode ? const_iterator(pNode, const_cast<node_type**>(mBucketArray) + n)
  ...
  template <typename K, typename V, typename H, typename Eq, size_t bC, bool bM, bool bU>
  typename intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::size_type intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::count(
    ...
    for (node_type* pNode = mBucketArray[n]; pNode; pNode = static_cast<node_type*>(pNode->mpNext)) {
      if (mEqual(k, extractKey(*pNode))) ++result;
    }
    return result;
  }
  ...
  intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::equal_range(const key_type& k) {
    ...
    if (pNode) {
      ...
      for (; p1; p1 = static_cast<node_type*>(p1->mpNext)) {
        if (!mEqual(k, extractKey(*p1))) break;
      }
  ...
  intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::DoFindNode(node_type* pNode, const key_type& k) const {
    ...
    for (; pNode; pNode = static_cast<node_type*>(pNode->mpNext)) {
      if (mEqual(k, extractKey(*pNode))) return pNode;
    }
    return NULL;
  }
  ...
  template <typename U, typename BinaryPredicate>
  inline typename intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::node_type*
  intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::DoFindNode(node_type* pNode, const U& other,
                                                           BinaryPredicate predicate) const {
    extract_key extractKey;  // extract_key is empty and thus this ctor is a no-op.

    for (; pNode; pNode = static_cast<node_type*>(pNode->mpNext)) {
      if (predicate(extractKey(*pNode),
  ...
  intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::DoInsertValue(value_type& value,
                                                              true_type)  // true_type means bUniqueKeys is true.
  {
  ...
  template <typename K, typename V, typename H, typename Eq, size_t bC, bool bM, bool bU>
  typename intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::iterator intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::erase(
      const_iterator i) {
  ...
  template <typename K, typename V, typename H, typename Eq, size_t bC, bool bM, bool bU>
  typename intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::size_type intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::erase(
      const key_type& k) {
    const size_type n                  = (size_type)(mHash(k) % kBucketCount);
