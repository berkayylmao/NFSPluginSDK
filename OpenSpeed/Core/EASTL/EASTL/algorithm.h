...
//    BinaryPredicate              A function which takes two arguments and returns true if some criteria is met (e.g.
//    they are equal). StrickWeakOrdering           A BinaryPredicate that compares two objects, returning true if the
//    first precedes the second. Like Compare but has additional requirements. Used for sorting routines. Function A
//    function which takes one argument and applies some operation to the target. Size                         A count
//    or size. Generator                    A function which takes no arguments and returns a value (which will usually
//    be assigned to an object). UnaryOperation               A function which takes one argument and returns a value
//    (which will usually be assigned to second object). BinaryOperation              A function which takes two
//    arguments and returns a value (which will usually be assigned to a third object). InputIterator                An
//    input iterator (iterator you read from) which allows reading each element only once and only in a forward
//    direction. ForwardIterator              An input iterator which is like InputIterator except it can be reset back
//    to the beginning. BidirectionalIterator        An input iterator which is like ForwardIterator except it can be
//    read in a backward direction as well. RandomAccessIterator         An input iterator which can be addressed like
//    an array. It is a superset of all other input iterators. OutputIterator               An output iterator (iterator
//    you write to) which allows writing each element only once in only in a forward direction.
//
...
//      max
//      max<Compare>
//     +min_alt                                     Exists to work around the problem of conflicts with min/max #defines
//     on some systems. +min_alt<Compare> +max_alt +max_alt<Compare> +median +median<Compare>
...
namespace eastl {
  ...
  template <typename T>
  inline EA_CONSTEXPR typename eastl::enable_if<!eastl::is_scalar<T>::value, const T&>::type min(const T& a,
                                                                                                 const T& b) {
    return b < a ? b : a;
  }
  ...
  template <typename T>
  inline EA_CONSTEXPR typename eastl::enable_if<!eastl::is_scalar<T>::value, const T&>::type max(const T& a,
                                                                                                 const T& b) {
    return a < b ? b : a;
  }
  ...
  template <typename T>
  inline EA_CONSTEXPR typename eastl::enable_if<!eastl::is_scalar<T>::value, const T&>::type max_alt(const T& a,
                                                                                                     const T& b) {
    return a < b ? b : a;
  }
  ...
  inline T&& median(T&& a, T&& b, T&& c, Compare compare) {
    return eastl::forward<T>(
        median_impl<T&&, Compare>(eastl::forward<T>(a), eastl::forward<T>(b), eastl::forward<T>(c), compare));
  }
  ...
  ///
  /// Example usage:
  ///     struct Rand{ eastl_size_t operator()(eastl_size_t n) { return (eastl_size_t)(rand() % n); } }; // Note: The C
  ///     rand function is poor and slow. Rand randInstance; shuffle(pArrayBegin, pArrayEnd, randInstance);
  ///
  ...
  ///
  /// Example usage:
  ///     eastl_size_t Rand(eastl_size_t n) { return (eastl_size_t)(rand() % n); } // Note: The C rand function is poor
  ///     and slow. pointer_to_unary_function<eastl_size_t, eastl_size_t> randInstance(Rand);
  ///     random_shuffle(pArrayBegin, pArrayEnd, randInstance);
  ///
  /// Example usage:
  ///     struct Rand{ eastl_size_t operator()(eastl_size_t n) { return (eastl_size_t)(rand() % n); } }; // Note: The C
  ///     rand function is poor and slow. Rand randInstance; random_shuffle(pArrayBegin, pArrayEnd, randInstance);
  ///
  ...
  /// Effects: Copies exactly count values from the range beginning at first to the range beginning at result, if count
  /// > 0. Does nothing otherwise. Returns: Iterator in the destination range, pointing past the last element copied if
  /// count>0 or first otherwise. Complexity: Exactly count assignments, if count > 0.
  ///
  template <typename InputIterator, typename Size, typename OutputIterator>
  ...
  inline OutputIterator copy_if(InputIterator first, InputIterator last, OutputIterator result, Predicate predicate) {
    // This implementation's performance could be improved by taking a more complicated approach like with the copy
    // algorithm.
    for (; first != last; ++first) {
  ...
  // BidirectionalIterator1 last-first to an integral type. There's simple way for us to take advantage of a random
  // access output iterator, as the range is specified by the input instead of the output, and distance(first, last) for
  // a non-random-access iterator is potentially slow.
  template <>
  ...
  inline BidirectionalIterator2 move_and_copy_backward_chooser(BidirectionalIterator1 first,
    ...
    return eastl::move_and_copy_backward_helper<IIC, isMove, canBeMemmoved>::move_or_copy_backward(
        first, last, resultEnd);  // Need to chose based on the input iterator tag and not the output iterator tag,
                                  // because containers accept input ranges of iterator types different than self.
  }

  // We have a second layer of unwrap_iterator calls because the original iterator might be something like
  // move_iterator<generic_iterator<int*> > (i.e. doubly-wrapped).
  template <bool isMove, typename BidirectionalIterator1, typename BidirectionalIterator2>
  ...
  EA_CPP14_CONSTEXPR inline InputIterator for_each_n(InputIterator first, Size n, Function function) {
    for (Size i = 0; i < n; ++first, i++) function(*first);
    return first;
  }
  ...
  */
  ...
  {
    while ((first1 != last1) &&
           (*first1 == *first2))  // && (first2 != last2) <- C++ standard mismatch function doesn't check first2/last2.
    {
      ++first1;
      ++first2;
    }
  ...
  search_n_impl(ForwardIterator first, ForwardIterator last, Size count, const T& value,
    ...
    Size d1 = (Size)eastl::distance(
        first, last);  // Should d1 be of type Size, ptrdiff_t, or iterator_traits<ForwardIterator>::difference_type?
  ...
  ///
  template <typename InputIterator1, typename InputIterator2, typename OutputIterator, typename Compare>
  void set_difference_2(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2,
  ...
  template <typename InputIterator1, typename InputIterator2, typename OutputIterator1, typename OutputIterator2,
            typename OutputIterator3, typename Compare>
  OutputIterator3 set_decomposition(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2,
                                    InputIterator2 last2, OutputIterator1 result1, OutputIterator2 result2,
                                    OutputIterator3 result3, Compare compare) {
    while ((first1 != last1) && (first2 != last2)) {
      if (compare(*first1, *first2)) {
        EASTL_VALIDATE_COMPARE(!compare(*first2, *first1));  // Validate that the compare function is sane.
        *result1++ = *first1++;
    ...
    if (first != last)  // If there is anything in the range...
    {
      ...
      if (first != --i)  // If the range has more than one item...
      {
        for (;;) {
          ...
          if (i == first)  // There are no two consecutive values where the first is less than the second, meaning the
                           // range is in reverse order. The reverse ordered range is always the last permutation.
          {
  ...
  namespace Internal {
    ...
    ForwardIterator move_rotate_left_by_one(ForwardIterator first, ForwardIterator last) {
      ...
      *result =
          eastl::move(temp);  // iterator is a RandomAccessIterator then this move will be a memmove for trivial types.
    ...
    BidirectionalIterator move_rotate_right_by_one(BidirectionalIterator first, BidirectionalIterator last) {
      ...
      BidirectionalIterator result = eastl::move_backward(
          first, beforeLast, last);  // Note that while our template type is BidirectionalIterator, if the actual
      *first =
          eastl::move(temp);  // iterator is a RandomAccessIterator then this move will be a memmove for trivial types.
    ...
    struct rotate_helper<EASTL_ITC_NS::bidirectional_iterator_tag, false> {
      ...
      // Smarter "flipping hands" implementation, but still disabled because benchmarks are showing it to be slower than
      rotate_general_impl. template <typename BidirectionalIterator> static BidirectionalIterator
      rotate_impl(BidirectionalIterator first, BidirectionalIterator middle, BidirectionalIterator last)
      {
              ...
              eastl::reverse_impl(middle, last,   EASTL_ITC_NS::bidirectional_iterator_tag()); // Reverse the right
      side.
      ...
      if (middle != last) {
        ...
        return Internal::rotate_helper < IC,
               eastl::is_trivially_move_assignable<
                   value_type>::value ||  // This is the best way of telling if we can move types via memmove, but
                                          // without a conforming C++11 compiler it usually returns false.
                   ...
                   eastl::is_scalar<value_type>::value >  // This is the most conservative means and works with all
                                                          // compilers, but works only for scalars.
  ...
  EA_CONSTEXPR const T& clamp(const T& v, const T& lo, const T& hi, Compare comp) {
    ...
    return [&] { EASTL_ASSERT(!comp(hi, lo)); }(), comp(v, lo) ? lo : comp(hi, v) ? hi : v;
  }
