...
#if defined(EA_HAVE_CPP11_INITIALIZER_LIST)  // If the compiler can generate calls to
...
// If you get an error here about initializer_list being already defined, then the
// EA_HAVE_CPP11_INITIALIZER_LIST define from <EABase/eahave.h> needs to be updated.
namespace std {
  ...
  class initializer_list {
   public:
    ...
    typedef const E* iterator;  // Must be const, as initializer_list (and its mpArray) is an
   ...
   private:
    ...
    // This constructor is private, but the C++ compiler has the ability to call it, as per the
    // C++11 Standard.
