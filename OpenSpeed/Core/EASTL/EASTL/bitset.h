...
namespace eastl {
// To consider: Enable this for backwards compatibility with any user code that might be using
// BitsetWordType: #define BitsetWordType EASTL_BITSET_WORD_TYPE_DEFAULT

/// BITSET_WORD_COUNT
///
/// Defines the number of words we use, based on the number of bits.
/// nBitCount refers to the number of bits in a bitset.
/// WordType refers to the type of integer word which stores bitet data. By default it is
/// BitsetWordType.
///
...
/// Before GCC 4.7 the '-Warray-bounds' buggy and was very likely to issue false positives for loops
/// that are difficult to evaluate. https://gcc.gnu.org/bugzilla/show_bug.cgi?id=45978
///
...
#define EASTL_DISABLE_BITSET_ARRAYBOUNDS_WARNING 0
  ...
  ///
  template <size_t NW, typename WordType>  // Templated on the number of words used to hold the
                                           // bitset and the word type.
  struct BitsetBase {
    ...
    enum {
      ...
      kBitsPerWordShift =
          ((kBitsPerWord == 8)
               ? 3
               : ((kBitsPerWord == 16) ? 4 : ((kBitsPerWord == 32) ? 5 : (((kBitsPerWord == 64) ? 6 : 7)))))
    };
   ...
   public:
    BitsetBase();
    BitsetBase(uint32_t value);  // This exists only for compatibility with std::bitset, which has a
                                 // 'long' constructor.
    // BitsetBase(uint64_t value); // Disabled because it causes conflicts with the 32 bit version
    // with existing user code. Use from_uint64 to init from a uint64_t instead.
  ...
  struct BitsetBase<1, WordType> {
   ...
   public:
    ...
    // BitsetBase(uint64_t value); // Disabled because it causes conflicts with the 32 bit version
    // with existing user code. Use from_uint64 instead.
  ...
  class bitset : private BitsetBase<BITSET_WORD_COUNT(N, WordType), WordType> {
   public:
    ...
    enum {
      ...
      kWordCount = BITSET_WORD_COUNT(N, WordType)  // The number of words the bitset uses to hold the bits.
                                                   // sizeof(bitset<N, WordType>) == kWordSize * kWordCount.
    };
   ...
   public:
    ...
    // bitset(uint64_t value); // Disabled because it causes conflicts with the 32 bit version with
    // existing user code. Use from_uint64 instead.
  ...
  inline uint32_t BitsetCountBits(uint16_t x) { return BitsetCountBits((uint32_t)x); }

  inline uint32_t BitsetCountBits(uint8_t x) { return BitsetCountBits((uint32_t)x); }
...
#define EASTL_BITSET_COUNT_STRING "\0\1\1\2\1\2\2\3\1\2\2\3\2\3\3\4"
  ...
  inline uint32_t GetFirstBit(uint16_t x)  // To do: Update this to use VC++ _BitScanForward, _BitScanForward64; GCC
                                           // __builtin_ctz, __builtin_ctzl. VC++ __lzcnt16, __lzcnt, __lzcnt64 requires
                                           // recent CPUs (2013+) and probably can't be used.
  ...
  inline uint32_t GetLastBit(eastl_uint128_t x) {
    if (x) {
      ...
      eastl_uint128_t mask(
          UINT64_C(0xFFFFFFFF00000000));  // There doesn't seem to exist compiler support for INT128_C() by
                                          // any compiler. EAStdC's int128_t supports it though.
      mask <<= 64;
  ...
  inline BitsetBase<NW, WordType>::BitsetBase(uint32_t value) {
    ...
    // EASTL_CT_ASSERT(sizeof(value) <= sizeof(word_type)); Disabled because we now have support for
    // uint8_t and uint16_t word types. It would be nice to have a runtime assert that tested this.
  ...
  inline BitsetBase<NW, WordType>::BitsetBase(uint64_t value)
  {
          reset();
                  ...
                  EASTL_CT_ASSERT(NW > 2); // We can assume this because we have specializations of
  BitsetBase for <1> and <2>.
                  //if(NW > 1) // NW is a template constant, but it would be a little messy to take
  advantage of it's const-ness. mWord[1] = static_cast<word_type>(value >> 32); #else mWord[0] =
  static_cast<word_type>(value); #endif
  }
  */

  template <size_t NW, typename WordType>
  inline void BitsetBase<NW, WordType>::operator&=(const this_type& x) {
    for (size_t i = 0; i < NW; i++) mWord[i] &= x.mWord[i];
  }
  ...
  inline void BitsetBase<NW, WordType>::operator>>=(size_type n) {
    ...
    if (nWordShift) {
      for (size_t i = 0; i < NW; ++i) mWord[i] = ((nWordShift < (NW - i)) ? mWord[i + nWordShift] : (word_type)0);
    }
  ...
  inline void BitsetBase<NW, WordType>::flip() {
    for (size_t i = 0; i < NW; i++) mWord[i] = ~mWord[i];
    // We let the parent class turn off any upper bits.
  }

  template <size_t NW, typename WordType>
  inline void BitsetBase<NW, WordType>::set() {
    for (size_t i = 0; i < NW; i++) mWord[i] = static_cast<word_type>(~static_cast<word_type>(0));
    // We let the parent class turn off any upper bits.
  }
  ...
  inline void BitsetBase<NW, WordType>::reset() {
    if (NW > 16)  // This is a constant expression and should be optimized away.
    {
      ...
      for (size_t i = 0; i < NW; i++) mWord[i] = 0;
    }
  }
  ...
  inline bool BitsetBase<NW, WordType>::operator==(const this_type& x) const {
    for (size_t i = 0; i < NW; i++) {
      if (mWord[i] != x.mWord[i]) return false;
    }
    return true;
  }
  ...
  inline bool BitsetBase<NW, WordType>::any() const {
    for (size_t i = 0; i < NW; i++) {
      if (mWord[i]) return true;
    }
    return false;
  }

  template <size_t NW, typename WordType>
  inline typename BitsetBase<NW, WordType>::size_type BitsetBase<NW, WordType>::count() const {
    size_type n = 0;
    ...
    !defined(EA_PLATFORM_ANDROID)  // GCC 3.4 or later
      ...
      for (word_type w = mWord[i]; w; w >>= 4) n += EASTL_BITSET_COUNT_STRING[w & 0xF];
  ...
  inline void BitsetBase<NW, WordType>::from_uint64(uint64_t value) {
    ...
    // if(NW > 1) // NW is a template constant, but it would be a little messy to take advantage of
    // it's const-ness.
  ...
  inline unsigned long BitsetBase<NW, WordType>::to_ulong() const {
    ...
    for (size_t i = 1; i < NW; ++i) {
      if (mWord[i]) throw std::overflow_error("BitsetBase::to_ulong");
    }
#endif
    return (unsigned long)mWord[0];  // Todo: We need to deal with the case whereby
                                     // sizeof(word_type) < sizeof(unsigned long)
  }
  ...
  inline uint32_t BitsetBase<NW, WordType>::to_uint32() const {
    ...
    for (size_t i = 1; i < NW; ++i) {
      if (mWord[i]) throw std::overflow_error("BitsetBase::to_uint32");
    }

#if (EA_PLATFORM_WORD_SIZE > 4)  // if we have 64 bit words...
    if (mWord[0] >> 32) throw std::overflow_error("BitsetBase::to_uint32");
#endif
#endif
  ...
  inline uint64_t BitsetBase<NW, WordType>::to_uint64() const {
    ...
    for (size_t i = 2; i < NW; ++i) {
      if (mWord[i]) throw std::overflow_error("BitsetBase::to_uint64");
    }
#endif
  ...
  template <size_t NW, typename WordType>
  inline typename BitsetBase<NW, WordType>::word_type& BitsetBase<NW, WordType>::DoGetWord(size_type i) {
    return mWord[i >> kBitsPerWordShift];
  }

  template <size_t NW, typename WordType>
  inline typename BitsetBase<NW, WordType>::word_type BitsetBase<NW, WordType>::DoGetWord(size_type i) const {
    return mWord[i >> kBitsPerWordShift];
  }

  template <size_t NW, typename WordType>
  inline typename BitsetBase<NW, WordType>::size_type BitsetBase<NW, WordType>::DoFindFirst() const {
    for (size_type word_index = 0; word_index < NW; ++word_index) {
      const size_type fbiw = GetFirstBit(mWord[word_index]);

      if (fbiw != kBitsPerWord) return (word_index * kBitsPerWord) + fbiw;
    }
  ...
  template <size_t NW, typename WordType>
  inline typename BitsetBase<NW, WordType>::size_type BitsetBase<NW, WordType>::DoFindNext(size_type last_find) const {
  ...
  template <size_t NW, typename WordType>
  inline typename BitsetBase<NW, WordType>::size_type BitsetBase<NW, WordType>::DoFindLast() const {
    for (size_type word_index = (size_type)NW; word_index > 0; --word_index) {
      const size_type lbiw = GetLastBit(mWord[word_index - 1]);

      if (lbiw != kBitsPerWord) return ((word_index - 1) * kBitsPerWord) + lbiw;
    }
  ...
  template <size_t NW, typename WordType>
  inline typename BitsetBase<NW, WordType>::size_type BitsetBase<NW, WordType>::DoFindPrev(size_type last_find) const {
    if (last_find > 0) {
      // Set initial state based on last find.
      size_type word_index = static_cast<size_type>(last_find >> kBitsPerWordShift);
      size_type bit_index  = static_cast<size_type>(last_find & kBitsPerWordMask);
      ...
      word_type mask = (~static_cast<word_type>(0) >> (kBitsPerWord - 1 - bit_index)) >>
                       1;  // We do two shifts here because many CPUs ignore requests to shift 32
                           // bit integers by 32 bits, which could be the case above.
      word_type this_word = mWord[word_index] & mask;

      for (;;) {
        const size_type lbiw = GetLastBit(this_word);

        if (lbiw != kBitsPerWord) return (word_index * kBitsPerWord) + lbiw;
  ...
  inline BitsetBase<1, WordType>::BitsetBase(uint64_t value)
  {
                  ...
                  mWord[0] = static_cast<word_type>(value);   // This potentially loses data, but
  that's what the user is requesting. #else mWord[0] = static_cast<word_type>(value); #endif
  }
  */
  ...
  template <typename WordType>
  inline typename BitsetBase<1, WordType>::size_type BitsetBase<1, WordType>::count() const {
#if defined(__GNUC__) && (((__GNUC__ * 100) + __GNUC_MINOR__) >= 304) && \
    !defined(EA_PLATFORM_ANDROID)  // GCC 3.4 or later
#if (EA_PLATFORM_WORD_SIZE == 4)
...
#else
    size_type n = 0;
    for (word_type w = mWord[0]; w; w >>= 4) n += EASTL_BITSET_COUNT_STRING[w & 0xF];
    return n;
#endif
  }
...
#if EASTL_EXCEPTIONS_ENABLED
#if ((EA_PLATFORM_WORD_SIZE > 4) && defined(EA_PLATFORM_MICROSOFT))  // If we are using 64 bit words but ulong is less
                                                                     // than 64 bits... Microsoft platforms alone use a
                                                                     // 32 bit long under 64 bit platforms.
    // Verify that high bits are not set and thus that to_ulong doesn't lose information.
    if (mWord[0] >> 32) throw std::overflow_error("BitsetBase::to_ulong");
#endif
#endif
  ...
  inline uint64_t BitsetBase<1, WordType>::to_uint64() const {
    // This implementation is the same regardless of the word size, and there is no possibility of
    // overflow_error.
    return static_cast<uint64_t>(mWord[0]);
  }

  template <typename WordType>
  inline typename BitsetBase<1, WordType>::word_type& BitsetBase<1, WordType>::DoGetWord(size_type) {
    return mWord[0];
  }

  template <typename WordType>
  inline typename BitsetBase<1, WordType>::word_type BitsetBase<1, WordType>::DoGetWord(size_type) const {
    return mWord[0];
  }

  template <typename WordType>
  inline typename BitsetBase<1, WordType>::size_type BitsetBase<1, WordType>::DoFindFirst() const {
    return GetFirstBit(mWord[0]);
  }

  template <typename WordType>
  inline typename BitsetBase<1, WordType>::size_type BitsetBase<1, WordType>::DoFindNext(size_type last_find) const {
  ...
  template <typename WordType>
  inline typename BitsetBase<1, WordType>::size_type BitsetBase<1, WordType>::DoFindLast() const {
    return GetLastBit(mWord[0]);
  }

  template <typename WordType>
  inline typename BitsetBase<1, WordType>::size_type BitsetBase<1, WordType>::DoFindPrev(size_type last_find) const {
    if (last_find > 0) {
      // Mask off previous bits of word so our search becomes a "find first".
  ...
  */
  ...
  template <typename WordType>
  inline typename BitsetBase<2, WordType>::size_type BitsetBase<2, WordType>::count() const {
#if defined(__GNUC__) && (((__GNUC__ * 100) + __GNUC_MINOR__) >= 304) && \
    !defined(EA_PLATFORM_ANDROID)  // GCC 3.4 or later
#if (EA_PLATFORM_WORD_SIZE == 4)
...
#if EASTL_EXCEPTIONS_ENABLED
    if (mWord[1]) throw std::overflow_error("BitsetBase::to_ulong");
#endif
    return (unsigned long)mWord[0];  // Todo: We need to deal with the case whereby
                                     // sizeof(word_type) < sizeof(unsigned long)
  }
...
#if (EA_PLATFORM_WORD_SIZE == 4)
    if (mWord[1]) throw std::overflow_error("BitsetBase::to_uint32");
#else
    if (mWord[1] || (mWord[0] >> 32)) throw std::overflow_error("BitsetBase::to_uint32");
#endif
#endif
...
#else
#if EASTL_EXCEPTIONS_ENABLED
    if (mWord[1]) throw std::overflow_error("BitsetBase::to_uint64");
#endif
  ...
  template <typename WordType>
  inline typename BitsetBase<2, WordType>::word_type& BitsetBase<2, WordType>::DoGetWord(size_type i) {
    return mWord[i >> kBitsPerWordShift];
  }

  template <typename WordType>
  inline typename BitsetBase<2, WordType>::word_type BitsetBase<2, WordType>::DoGetWord(size_type i) const {
    return mWord[i >> kBitsPerWordShift];
  }

  template <typename WordType>
  inline typename BitsetBase<2, WordType>::size_type BitsetBase<2, WordType>::DoFindFirst() const {
    size_type fbiw = GetFirstBit(mWord[0]);

    if (fbiw != kBitsPerWord) return fbiw;

    fbiw = GetFirstBit(mWord[1]);

    if (fbiw != kBitsPerWord) return kBitsPerWord + fbiw;

    return 2 * kBitsPerWord;
  }

  template <typename WordType>
  inline typename BitsetBase<2, WordType>::size_type BitsetBase<2, WordType>::DoFindNext(size_type last_find) const {
  ...
  template <typename WordType>
  inline typename BitsetBase<2, WordType>::size_type BitsetBase<2, WordType>::DoFindLast() const {
    size_type lbiw = GetLastBit(mWord[1]);

    if (lbiw != kBitsPerWord) return kBitsPerWord + lbiw;

    lbiw = GetLastBit(mWord[0]);

    if (lbiw != kBitsPerWord) return lbiw;

    return 2 * kBitsPerWord;
  }

  template <typename WordType>
  inline typename BitsetBase<2, WordType>::size_type BitsetBase<2, WordType>::DoFindPrev(size_type last_find) const {
  ...
  inline bitset<N, WordType>::reference::reference(const bitset& x, size_type i) :
      ...
      mnBitIndex(i & kBitsPerWordMask) {  // We have an issue here because the above is casting
                                          // away the const-ness of the source bitset. Empty
  }

  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::reference& bitset<N, WordType>::reference::operator=(bool value) {
    if (value)
  ...
  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::reference& bitset<N, WordType>::reference::operator=(const reference& x) {
  ...
  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::reference& bitset<N, WordType>::reference::flip() {
    *mpBitWord ^= static_cast<word_type>(1) << (mnBitIndex & kBitsPerWordMask);
    return *this;
  }
  ...
  template <size_t N, typename WordType>
  inline bitset<N, WordType>::bitset() : base_type() {
    // Empty. The base class will set all bits to zero.
  }

  EA_DISABLE_VC_WARNING(6313)
  template <size_t N, typename WordType>
  inline bitset<N, WordType>::bitset(uint32_t value) : base_type(value) {
    if ((N & kBitsPerWordMask) || (N == 0))  // If there are any high bits to clear... (If we didn't have this check,
                                             // then the code below would do the wrong thing when N == 32.
      mWord[kWordCount - 1] &=
          ~(~static_cast<word_type>(0) << (N & kBitsPerWordMask));  // This clears any high unused bits.
  }
          ...
          : base_type(value)
  {
          if((N & kBitsPerWordMask) || (N == 0)) // If there are any high bits to clear...
                  mWord[kWordCount - 1] &= ~(~static_cast<word_type>(0) << (N & kBitsPerWordMask));
  // This clears any high unused bits.
  }
  */

  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::operator&=(const this_type& x) {
  ...
  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::operator<<=(size_type n) {
    if (EASTL_LIKELY((intptr_t)n < (intptr_t)N)) {
      EA_DISABLE_VC_WARNING(6313)
      base_type::operator<<=(n);
      if ((N & kBitsPerWordMask) || (N == 0))  // If there are any high bits to clear... (If we didn't have this check,
                                               // then the code below would do the wrong thing when N == 32.
        mWord[kWordCount - 1] &= ~(~static_cast<word_type>(0)
                                   << (N & kBitsPerWordMask));  // This clears any high unused bits. We need to do this
                                                                // so that shift operations proceed correctly.
      EA_RESTORE_VC_WARNING()
    } else
      base_type::reset();
    return *this;
  ...
  inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::set() {
    ...
    if ((N & kBitsPerWordMask) || (N == 0))  // If there are any high bits to clear... (If we didn't have this check,
                                             // then the code below would do the wrong thing when N == 32.
      mWord[kWordCount - 1] &=
          ~(~static_cast<word_type>(0) << (N & kBitsPerWordMask));  // This clears any high unused bits. We need to do
                                                                    // this so that shift operations proceed correctly.
    return *this;
  }

  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::set(size_type i, bool value) {
    if (i < N)
      base_type::set(i, value);
    else {
#if EASTL_ASSERT_ENABLED
      if (EASTL_UNLIKELY(!(i < N))) EASTL_FAIL_MSG("bitset::set -- out of range");
#endif
  ...
  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::reset(size_type i) {
    if (EASTL_LIKELY(i < N))
      DoGetWord(i) &= ~(static_cast<word_type>(1) << (i & kBitsPerWordMask));
    else {
#if EASTL_ASSERT_ENABLED
      if (EASTL_UNLIKELY(!(i < N))) EASTL_FAIL_MSG("bitset::reset -- out of range");
#endif
  ...
  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::flip(size_type i) {
    if (EASTL_LIKELY(i < N))
      DoGetWord(i) ^= (static_cast<word_type>(1) << (i & kBitsPerWordMask));
    else {
#if EASTL_ASSERT_ENABLED
      if (EASTL_UNLIKELY(!(i < N))) EASTL_FAIL_MSG("bitset::flip -- out of range");
#endif
  ...
  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::this_type bitset<N, WordType>::operator~() const {
    return this_type(*this).flip();
  }

  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::reference bitset<N, WordType>::operator[](size_type i) {
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(!(i < N))) EASTL_FAIL_MSG("bitset::operator[] -- out of range");
#endif
  ...
  inline bool bitset<N, WordType>::test(size_type i) const {
    if (EASTL_UNLIKELY(i < N)) return (DoGetWord(i) & (static_cast<word_type>(1) << (i & kBitsPerWordMask))) != 0;
  ...
  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::this_type bitset<N, WordType>::operator<<(size_type n) const {
  ...
  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::size_type bitset<N, WordType>::find_first() const {
  ...
  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::size_type bitset<N, WordType>::find_next(size_type last_find) const {
  ...
  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::size_type bitset<N, WordType>::find_last() const {
  ...
  template <size_t N, typename WordType>
  inline typename bitset<N, WordType>::size_type bitset<N, WordType>::find_prev(size_type last_find) const {
