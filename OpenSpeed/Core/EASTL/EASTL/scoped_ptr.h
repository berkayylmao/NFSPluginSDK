...
namespace eastl {
  ...
  class scoped_ptr {
   ...
   public:
    ...
    explicit scoped_ptr(T* pValue = NULL) : mpValue(pValue) {}
    ...
    ///    if(!ptr)
    ///        assert(false);
    bool operator!() const { return (mpValue == NULL); }
  ...
  inline bool operator<(const scoped_ptr<T, D>& scopedPtr1, const scoped_ptr<T, D>& scopedPtr2) {
    return (scopedPtr1.get() <
            scopedPtr2.get());  // Alternatively use: std::less<T*>(scopedPtr1.get(), scopedPtr2.get());
  }
