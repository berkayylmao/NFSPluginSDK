...
namespace EA {
  namespace Allocator {
    ...
    /// Allocates memory from an instance of ICoreAllocator or another class with an equivalent
    /// interface. ICoreAllocator is a pure-virtual memory allocation interface used by a number of
    /// EA games and shared libraries. It's completely unrelated to EASTL, but it's prevalent enough
    /// that it's useful for EASTL to have a built-in adapter for this interface. ICoreAllocator is
    /// declared in the CoreAllocator package icoreallocator_interface.h header, but
    /// CoreAllocatorAdapter can work with any equivalent interface, as defined below.
    ///
    /// Expected interface:
    ///     enum AllocFlags {
    ///         kFlagTempMemory = 0,
    ...
    ///         void* Alloc(size_t size, const char* name, unsigned int allocFlags,     // Not
    ///         required unless you are working with types that require custom alignment.
    ...
    ///
    /// Example usage:
    ///     #include <OpenSpeed/Coreallocator/icoreallocator_interface.h>
    ...
    ///     CoreAllocatorFixed<WidgetList::node_type>
    ///     widgetCoreAllocator(pFixedAllocatorForWidgetListValueType); // CoreAllocatorFixed is a
    ///     hypothetical implementation of the ICoreAllocator interface. WidgetList
    ///     widgetList(Adapter("UI/WidgetList", &widgetCoreAllocator)); // Note that the
    ///     widgetCoreAllocator is declared before and thus destroyed after the widget list.
    ///
    ...
    class CoreAllocatorAdapter {
     ...
     public:
      // To do: Make this constructor explicit, when there is no known code dependent on it being
      // otherwise.
      CoreAllocatorAdapter(const char*    pName      = EASTL_NAME_VAL(EASTL_ALLOCATOR_DEFAULT_NAME),
                           AllocatorType* pAllocator = EASTL_CORE_ALLOCATOR_ADAPTER_GET_DEFAULT_CORE_ALLOCATOR());
     ...
     public:  // Public because otherwise VC++ generates (possibly invalid) warnings about inline
    ...
    ///
    /// Example usage:
    ///     eastl::list<Widget, EASTLICoreAllocator> widgetList("UI/WidgetList",
    ///     pSomeCoreAllocator); widgetList.push_back(Widget());
    ///
    ...
    class CoreDeleterAdapter {
     ...
     public:
      CoreDeleterAdapter(AllocatorType* pAllocator = EASTL_CORE_ALLOCATOR_ADAPTER_GET_DEFAULT_CORE_ALLOCATOR())
          EA_NOEXCEPT : mpCoreAllocator(pAllocator) {}
...
namespace EA {
  namespace Allocator {
    ...
    template <class AllocatorType>
    inline CoreAllocatorAdapter<AllocatorType>::CoreAllocatorAdapter(const char*    EASTL_NAME(pName),
                                                                     AllocatorType* pCoreAllocator, int flags) :
    ...
    template <class AllocatorType>
    inline CoreAllocatorAdapter<AllocatorType>& CoreAllocatorAdapter<AllocatorType>::operator=(
    ...
    template <class AllocatorType>
    inline void* CoreAllocatorAdapter<AllocatorType>::allocate(size_t n, size_t alignment, size_t offset,
                                                               int /*flags*/) {
      // It turns out that EASTL itself doesn't use the flags parameter,
      // whereas the user here might well want to specify a flags
      // parameter. So we use ours instead of the one passed in.
      return mpCoreAllocator->Alloc(n, EASTL_NAME_VAL(mpName), (unsigned)mnFlags, (unsigned)alignment,
                                    (unsigned)offset);
    }
