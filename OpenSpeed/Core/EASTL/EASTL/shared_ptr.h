...
#define EASTL_SHARED_PTR_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " shared_ptr"  // Unless the user overrides something, this is "EASTL shared_ptr".
#endif
...
#define EASTL_SHARED_PTR_DEFAULT_ALLOCATOR EASTLAllocatorType(EASTL_SHARED_PTR_DEFAULT_NAME)
  ...
  struct ref_count_sp {
    ...
    int32_t mWeakRefCount;  /// Reference count on contained pointer plus this ref_count_sp object itself. Starts as 1
                            /// by default.
  ...
  inline ref_count_sp::ref_count_sp(int32_t refCount, int32_t weakRefCount) EA_NOEXCEPT : mRefCount(refCount),
  ...
  inline void ref_count_sp::release() {
    ...
    if (Internal::atomic_decrement(&mRefCount) == 0) free_value();
...
#else
    void*         get_deleter() const EA_NOEXCEPT { return nullptr; }
#endif
  };
  ...
  class shared_ptr {
   public:
    ...
    typedef typename shared_ptr_traits<T>::reference_type
        reference_type;  // This defines what a reference to a T is. It's always simply T&, except for the case where T
                         // is void, whereby the reference is also just void.
   ...
   public:
    ...
    shared_ptr() EA_NOEXCEPT : mpValue(nullptr), mpRefCount(nullptr) {
    ...
    shared_ptr(std::nullptr_t) EA_NOEXCEPT : mpValue(nullptr), mpRefCount(nullptr) {
      // Intentionally leaving mpRefCount as NULL. Can't allocate here due to noexcept.
    }
    ...
    /// use_count
    /// Returns: the number of shared_ptr objects, *this included, that share ownership with *this, or 0 when *this is
    /// empty.
    ...
    bool operator!() const EA_NOEXCEPT { return (mpValue == nullptr); }
#else
    ...
    ///    if(ptr)
    ///        ++*ptr;
    explicit operator bool() const EA_NOEXCEPT { return (mpValue != nullptr); }
#endif
   ...
   protected:
    ...
    void alloc_internal(U pValue, Allocator allocator, Deleter deleter) {
      ...
      } catch (...)  // The exception would usually be std::bad_alloc.
      {
        ...
        throw;  // Throws: bad_alloc, or an implementation-defined exception when a resource other than memory could not
                // be obtained.
      }
#else
      void* const pMemory = EASTLAlloc(allocator, sizeof(ref_count_type));
  ...
  inline bool operator<(const shared_ptr<T>& a, const shared_ptr<U>& b) EA_NOEXCEPT {
    ...
    CPointer pU = b.get();  // "in instantiation of function template specialization 'eastl::operator<<int, int>, no
                            // known conversion from 'element_type *' (aka 'int *') to 'int *&&&' for 1st argument"
  ...
  ///
  /// Requires: The expression dynamic_cast<T*>(sharedPtr.get()) shall be well formed and shall have well defined
  /// behavior. Returns: When dynamic_cast<T*>(sharedPtr.get()) returns a nonzero value, a shared_ptr<T> object that
  /// stores
  ...
  /// It simply returns eastl::hash(x.get()). If your unique_ptr pointer type (the return value of shared_ptr<T>::get)
  ...
  // same pointer and refer to the same pointer), assigns sharedPtrNew into *pSharedPtr using the memory ordering
  // constraints specified by success and returns true. If they are not equivalent, assigns *pSharedPtr into
  // *pSharedPtrCondition using the memory ordering constraints specified by failure and returns false.
  template <typename T>
...
#define EASTL_WEAK_PTR_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " weak_ptr"  // Unless the user overrides something, this is "EASTL weak_ptr".
#endif
...
#define EASTL_WEAK_PTR_DEFAULT_ALLOCATOR allocator_type(EASTL_WEAK_PTR_DEFAULT_NAME)
  ...
  class weak_ptr {
   ...
   public:
    /// weak_ptr
    weak_ptr() EA_NOEXCEPT : mpValue(nullptr), mpRefCount(nullptr) {}
