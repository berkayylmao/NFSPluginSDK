...
//       Multiple shared_ptrs that refer to the same object can be used arbitrarily by multiple
//       threads.
...
#include <OpenSpeed/Core/EASTL/EASTL/internal/smart_ptr.h>
#include <OpenSpeed/Core/EASTL/EASTL/internal/thread_support.h>
#include <OpenSpeed/Core/EASTL/EASTL/unique_ptr.h>
...
#define EASTL_SHARED_PTR_DEFAULT_ALLOCATOR EASTLAllocatorType(EASTL_SHARED_PTR_DEFAULT_NAME)
  ...
  struct bad_weak_ptr : std::exception {
    const char* what() const EA_NOEXCEPT EA_OVERRIDE { return "bad weak_ptr"; }
  };
#endif
  ...
  struct ref_count_sp {
    ...
    int32_t mWeakRefCount;  /// Reference count on contained pointer plus this ref_count_sp object
                            /// itself. Starts as 1 by default.
  ...
  inline ref_count_sp::ref_count_sp(int refCount, int weakRefCount) EA_NOEXCEPT : mRefCount(refCount),
  ...
  inline void ref_count_sp::release() {
    ...
    if (Internal::atomic_decrement(&mRefCount) > 0)
      ...
      if (Internal::atomic_decrement(&mWeakRefCount) == 0) free_ref_count_sp();
    }
  }

  inline void ref_count_sp::weak_addref() EA_NOEXCEPT { Internal::atomic_increment(&mWeakRefCount); }
...
#else
    void*         get_deleter() const EA_NOEXCEPT { return (void*)&mDeleter; }
#endif
  };
  ...
  class ref_count_sp_t_inst : public ref_count_sp {
   public:
    ...
    ref_count_sp_t_inst(allocator_type allocator, Args&&... args) : ref_count_sp(), mAllocator(eastl::move(allocator)) {
      new (&mMemory) value_type(eastl::forward<Args>(args)...);
    }
#else
    ref_count_sp_t_inst(allocator_type allocator) : ref_count_sp(), mAllocator(eastl::move(allocator)) {
      new (&mMemory) value_type();  // For consistency with the variadic version, we use
                                    // value_type() instead of value_type alone.
    }

#if EASTL_MOVE_SEMANTICS_ENABLED
    template <typename A1>
    ref_count_sp_t_inst(allocator_type allocator, A1&& a1) : ref_count_sp(), mAllocator(eastl::move(allocator)) {
    ...
    ref_count_sp_t_inst(allocator_type allocator, A1&& a1, A2&& a2, A3&& a3, A4&& a4) :
        ref_count_sp(), mAllocator(eastl::move(allocator)) {
      new (&mMemory)
          value_type(eastl::forward<A1>(a1), eastl::forward<A2>(a2), eastl::forward<A3>(a3), eastl::forward<A4>(a4));
    }
#endif

    template <typename A1>
    ref_count_sp_t_inst(allocator_type allocator, const A1& a1) : ref_count_sp(), mAllocator(allocator) {
      new (&mMemory) value_type(a1);
    }

    template <typename A1, typename A2>
    ref_count_sp_t_inst(allocator_type allocator, const A1& a1, const A2& a2) : ref_count_sp(), mAllocator(allocator) {
    ...
    void free_value() EA_NOEXCEPT { GetValue()->~value_type(); }
...
#else
    void* get_deleter() const EA_NOEXCEPT { return NULL; }
#endif
  };
  ...
  }  // Empty specialization. This no-op version is called by shared_ptr when shared_ptr's T type is
     // anything but an enabled_shared_from_this class.
  ...
  class shared_ptr {
   public:
    ...
    typedef typename shared_ptr_traits<T>::reference_type
        reference_type;  // This defines what a reference to a T is. It's always simply T&, except
                         // for the case where T is void, whereby the reference is also just void.
   ...
   public:
    ...
    {
      // We explicitly use default_delete<U>. You can use the other version of this constructor to
      // provide a custom version.
      alloc_internal(pValue, default_allocator_type(),
                     default_delete<U>());  // Problem: We want to be able to use default_deleter_type()
                                            // instead of default_delete<U>, but if default_deleter_type's type
                                            // is void or otherwise mismatched then this will fail to compile.
                                            // What we really want to be able to do is "rebind"
                                            // default_allocator_type to U instead of its original type.
    }
    ...
    explicit shared_ptr(U* pValue, Deleter deleter, const Allocator& allocator,
                        typename eastl::enable_if<eastl::is_convertible<U*, element_type*>::value>::type* = 0) :
        mpValue(NULL),
        mpRefCount(NULL)  // alloc_internal will set this.
    {
    ...
    shared_ptr(const shared_ptr& sharedPtr) EA_NOEXCEPT : mpValue(sharedPtr.mpValue), mpRefCount(sharedPtr.mpRefCount) {
      if (mpRefCount) mpRefCount->addref();
    }
    ...
    /// ensure that pValue remains valid at least until the ownership group of sharedPtr is
    /// destroyed. This constructor allows creation of an empty shared_ptr instance with a non-NULL
    /// stored pointer.
    ///
    /// Shares ownership of a pointer with another instance of shared_ptr while storing a
    /// potentially different pointer. This function increments the shared reference count on the
    /// sharedPtr if it exists. If sharedPtr has no shared reference then a shared reference is not
    /// created an pValue is not deleted in our destructor and effectively the pointer is not
    /// actually shared.
    ///
    /// To accomplish this in a thread-safe way requires the user to maintain the lifetime of
    /// sharedPtr as described above.
    ///
    template <typename U>
    shared_ptr(const shared_ptr<U>& sharedPtr, element_type* pValue) EA_NOEXCEPT : mpValue(pValue),
                                                                                   mpRefCount(sharedPtr.mpRefCount) {
      if (mpRefCount) mpRefCount->addref();
    }

#if EASTL_MOVE_SEMANTICS_ENABLED
    shared_ptr(shared_ptr&& sharedPtr) EA_NOEXCEPT : mpValue(sharedPtr.mpValue), mpRefCount(sharedPtr.mpRefCount) {
    ...
    template <typename U, typename Deleter>
#if EASTL_MOVE_SEMANTICS_ENABLED  // We don't (yet) support shared_ptr for arrays. We don't (yet)
                                  // support Deleters that are lvalue references. To do so we need
                                  // to remove_reference on Deleter type.
    ...
    template <typename U, typename Deleter, typename Allocator>
                                  ...
                                  // support Deleters that are lvalue references. To do so we need
                                  // to remove_reference on Deleter type.
    shared_ptr(unique_ptr<U, Deleter>&& uniquePtr, const Allocator& allocator,
               typename eastl::enable_if<!eastl::is_array<U>::value && !is_lvalue_reference<Deleter>::value &&
                                         eastl::is_convertible<U*, element_type*>::value>::type* = 0)
#else
    shared_ptr(unique_ptr<U, Deleter> uniquePtr, const Allocator& allocator,
    ...
    // template <typename U>
    // #if EASTL_MOVE_SEMANTICS_ENABLED
    //     shared_ptr(auto_ptr<U>&& autoPtr, typename eastl::enable_if<eastl::is_convertible<U*,
    //     element_type*>::value>::type* = 0)
    // #else
    //     shared_ptr(auto_ptr<U>   autoPtr, typename eastl::enable_if<eastl::is_convertible<U*,
    //     element_type*>::value>::type* = 0)
    // #endif
    ...
    //       mpRefCount(NULL)
    // {
    //     alloc_internal(autoPtr.release(), default_allocator_type(), eastl::default_delete<U>());
    //     // Use of default_delete here assumes that default_delete matches global operator delete.
    // }
    //
    // template <typename U, typename Allocator>
    // #if EASTL_MOVE_SEMANTICS_ENABLED
    //     shared_ptr(auto_ptr<U>&& autoPtr, const Allocator& allocator, typename
    //     eastl::enable_if<eastl::is_convertible<U*, element_type*>::value>::type* = 0)
    // #else
    //     shared_ptr(auto_ptr<U>   autoPtr, const Allocator& allocator, typename
    //     eastl::enable_if<eastl::is_convertible<U*, element_type*>::value>::type* = 0)
    // #endif
    ...
    //       mpRefCount(NULL)
    // {
    //     alloc_internal(autoPtr.release(), allocator, eastl::default_delete<U>()); // Use of
    //     default_delete here assumes that default_delete matches global operator delete.
    // }
    ...
    explicit shared_ptr(const weak_ptr<U>& weakPtr,
        ...
        mpRefCount(weakPtr.mpRefCount ? weakPtr.mpRefCount->lock()
    ...
    ~shared_ptr() {
      if (mpRefCount) mpRefCount->release();
        // else if mpValue is non-NULL then we just lose it because it wasn't actually shared (can
        // happen with shared_ptr(const shared_ptr<U>& sharedPtr, element_type* pValue)
        // constructor).
    ...
    shared_ptr& operator=(const shared_ptr& sharedPtr) EA_NOEXCEPT {
      if (&sharedPtr != this) this_type(sharedPtr).swap(*this);
    ...
    template <typename U>
    typename eastl::enable_if<eastl::is_convertible<U*, element_type*>::value, this_type&>::type operator=(
        const shared_ptr<U>& sharedPtr) EA_NOEXCEPT {
      if (!equivalent_ownership(sharedPtr)) this_type(sharedPtr).swap(*this);
      return *this;
    }
    ...
    this_type& operator=(shared_ptr&& sharedPtr) EA_NOEXCEPT {
      if (&sharedPtr != this) this_type(eastl::move(sharedPtr)).swap(*this);
    ...
    template <typename U>
    typename eastl::enable_if<eastl::is_convertible<U*, element_type*>::value, this_type&>::type operator=(
        shared_ptr<U>&& sharedPtr) EA_NOEXCEPT {
      if (!equivalent_ownership(sharedPtr)) shared_ptr(eastl::move(sharedPtr)).swap(*this);
      return *this;
    }

    // unique_ptr operator=
    template <typename U, typename Deleter>
    typename eastl::enable_if<!eastl::is_array<U>::value && eastl::is_convertible<U*, element_type*>::value,
                              this_type&>::type
#if EASTL_MOVE_SEMANTICS_ENABLED
    ...
    // To consider: Enable this support for auto_ptr, though it's deprecated by the C++11 Standard.
    // template <typename U>
    // inline typename eastl::enable_if<!eastl::is_array<U>::value && eastl::is_convertible<U*,
    // element_type*>::value, this_type&>::type #if EASTL_MOVE_SEMANTICS_ENABLED
    ...
    void reset() EA_NOEXCEPT { this_type().swap(*this); }

    /// reset
    /// Releases the owned pointer and takes ownership of the
    /// passed in pointer.
    template <typename U>
    typename eastl::enable_if<eastl::is_convertible<U*, element_type*>::value, void>::type reset(U* pValue) {
    ...
    template <typename U, typename Deleter, typename Allocator>
    typename eastl::enable_if<eastl::is_convertible<U*, element_type*>::value, void>::type reset(
    ...
    /// This function is not intrinsically thread-safe. You must use atomic_exchange(shared_ptr<T>*,
    /// shared_ptr<T>) or manually coordinate the swap.
    ...
    ///    int x = *ptr;
    reference_type operator*() const EA_NOEXCEPT { return *mpValue; }
    ...
    ///    X* pX = ptr.get();
    ///    pX->DoSomething();
    element_type* get() const EA_NOEXCEPT { return mpValue; }

    /// use_count
    /// Returns: the number of shared_ptr objects, *this included, that share ownership with *this,
    /// or 0 when *this is empty.
    int use_count() const EA_NOEXCEPT { return mpRefCount ? mpRefCount->mRefCount : 0; }

    /// unique
    /// Returns: use_count() == 1.
    bool unique() const EA_NOEXCEPT { return (mpRefCount && (mpRefCount->mRefCount == 1)); }
...
#if EASTL_RTTI_ENABLED
      return mpRefCount ? static_cast<Deleter*>(mpRefCount->get_deleter(typeid(typename remove_cv<Deleter>::type)))
                        : NULL;
#else
    ...
    operator bool_() const EA_NOEXCEPT {
      if (mpValue) return &this_type::get;
      return NULL;
    }

    bool operator!() const EA_NOEXCEPT { return (mpValue == NULL); }
#else
    ...
    ///    if(ptr)
    ///        ++*ptr;
    explicit operator bool() const EA_NOEXCEPT { return (mpValue != NULL); }
#endif
    ...
    bool equivalent_ownership(const shared_ptr<U>& sharedPtr) const {
      // We compare mpRefCount instead of mpValue, because it's feasible that there are two sets of
      // shared_ptr objects that are unconnected to each other but happen to own the same value
      // pointer.
      return (mpRefCount == sharedPtr.mpRefCount);
    }
   ...
   protected:
    ...
    // The provided pValue may be NULL, as with constructing with a deleter and allocator but NULL
    // pointer.
    ...
    void alloc_internal(U* pValue, Allocator allocator, Deleter deleter) {
      ...
      } catch (...)  // The exception would usually be std::bad_alloc.
      {
        ...
        throw;            // Throws: bad_alloc, or an implementation-defined exception when a resource other
  ...
  inline bool operator<(const shared_ptr<T>& a, const shared_ptr<U>& b) EA_NOEXCEPT {
    ...
    typedef
        typename eastl::common_type<T*, U*>::type CPointer;  // We currently need to make these temporary variables, as
                                                             // otherwise clang complains about CPointer being int*&&&.
    CPointer pT = a.get();            // I wonder if there's something wrong with our common_type type trait
                                      // implementation.
    CPointer pU = b.get();            // "in instantiation of function template specialization
                                      // 'eastl::operator<<int, int>, no known conversion from 'element_type
                                      // *' (aka 'int *') to 'int *&&&' for 1st argument"
    return less<CPointer>()(pT, pU);  // It looks like common_type is making CPointer be (e.g.) int*&& instead of int*,
                                      // though the problem may be in how less<> deals with that.
  }
  ...
  ///          object that stores const_cast<T*>(sharedPtr.get()) and shares ownership with
  ///          sharedPtr.
  /// Postconditions: w.get() == const_cast<T*>(sharedPtr.get()) and w.use_count() ==
  /// sharedPtr.use_count(),
  ...
  ///
  /// Requires: The expression dynamic_cast<T*>(sharedPtr.get()) shall be well formed and shall have
  /// well defined behavior. Returns: When dynamic_cast<T*>(sharedPtr.get()) returns a nonzero
  /// value, a shared_ptr<T> object that stores
  ///          a copy of it and shares ownership with sharedPtr; Otherwise, an empty shared_ptr<T>
  ///          object.
  ...
  inline shared_ptr<T> dynamic_pointer_cast(const shared_ptr<U>& sharedPtr) EA_NOEXCEPT {
    if (T* p = dynamic_cast<T*>(sharedPtr.get())) return shared_ptr<T>(sharedPtr, p);
    return shared_ptr<T>();
  }
  ...
  /// It simply returns eastl::hash(x.get()). If your unique_ptr pointer type (the return value of
  /// shared_ptr<T>::get) is a custom type and not a built-in pointer type then you will need to
  /// independently define eastl::hash for that type.
  template <typename T>
  struct hash<shared_ptr<T> > {
    size_t operator()(const shared_ptr<T>& x) const EA_NOEXCEPT { return eastl::hash<T*>()(x.get()); }
  };
  ...
  shared_ptr<T> allocate_shared(const Allocator& allocator, A1&& a1, A2&& a2) {
    ...
    if (pMemory) {
      ref_count_type* pRefCount =
          ::new (pMemory) ref_count_type(allocator, eastl::forward<A1>(a1), eastl::forward<A2>(a2));
      allocate_shared_helper(ret, pRefCount, pRefCount->GetValue());
    }
    return ret;
  }
  ...
  shared_ptr<T> allocate_shared(const Allocator& allocator, A1&& a1, A2&& a2, A3&& a3) {
    ...
    void* const pMemory = EASTLAlloc(const_cast<Allocator&>(allocator), sizeof(ref_count_type));
    if (pMemory) {
      ref_count_type* pRefCount = ::new (pMemory)
          ref_count_type(allocator, eastl::forward<A1>(a1), eastl::forward<A2>(a2), eastl::forward<A3>(a3));
      allocate_shared_helper(ret, pRefCount, pRefCount->GetValue());
    }
    return ret;
  }
  ...
  shared_ptr<T> allocate_shared(const Allocator& allocator, A1&& a1, A2&& a2, A3&& a3, A4&& a4) {
    ...
    void* const pMemory = EASTLAlloc(const_cast<Allocator&>(allocator), sizeof(ref_count_type));
    if (pMemory) {
      ref_count_type* pRefCount = ::new (pMemory) ref_count_type(
          allocator, eastl::forward<A1>(a1), eastl::forward<A2>(a2), eastl::forward<A3>(a3), eastl::forward<A4>(a4));
      allocate_shared_helper(ret, pRefCount, pRefCount->GetValue());
    }
    return ret;
  }
  ...
  shared_ptr<T> make_shared(A1&& a1, A2&& a2, A3&& a3, A4&& a4) {
    return allocate_shared<T>(EASTL_SHARED_PTR_DEFAULT_ALLOCATOR, eastl::forward<A1>(a1), eastl::forward<A2>(a2),
                              eastl::forward<A3>(a3), eastl::forward<A4>(a4));
  }
  ...
  inline bool atomic_is_lock_free(const shared_ptr<T>*) {
    // Return true if atomic access to the provided shared_ptr instance is lock-free, false
    // otherwise. For this to be lock-free, we would have to be able to copy shared_ptr objects in
    // an atomic way as opposed to wrapping it with a mutex like we do below. Given the nature of
    // shared_ptr, it's probably not feasible to implement these operations without a mutex.
    // atomic_is_lock_free exists in the C++11 Standard because it also applies to other types such
    // as built-in types which can be lock-free in their access.
    return false;
  }
  ...
  template <typename T>
  inline void atomic_store_explicit(shared_ptr<T>* pSharedPtrA, shared_ptr<T> sharedPtrB,
  ...
  template <typename T>
  inline shared_ptr<T> atomic_exchange_explicit(shared_ptr<T>* pSharedPtrA, shared_ptr<T> sharedPtrB,
                                                ... /*std::memory_order memoryOrder*/) {
    atomic_exchange(pSharedPtrA, sharedPtrB);
  }

  // Compares the shared pointers pointed-to by p and expected. If they are equivalent (share
  // ownership of the same pointer and refer to the same pointer), assigns sharedPtrNew into
  // *pSharedPtr using the memory ordering constraints specified by success and returns true. If
  // they are not equivalent, assigns *pSharedPtr into *pSharedPtrCondition using the memory
  // ordering constraints specified by failure and returns false.
  template <typename T>
  bool atomic_compare_exchange_strong(shared_ptr<T>* pSharedPtr, shared_ptr<T>* pSharedPtrCondition,
  ...
  template <typename T>
  inline bool atomic_compare_exchange_weak(shared_ptr<T>* pSharedPtr, shared_ptr<T>* pSharedPtrCondition,
                                           shared_ptr<T> sharedPtrNew) {
  ...
  inline bool atomic_compare_exchange_strong_explicit(
      shared_ptr<T>* pSharedPtr, shared_ptr<T>* pSharedPtrCondition, shared_ptr<T> sharedPtrNew,
...
#define EASTL_WEAK_PTR_DEFAULT_ALLOCATOR allocator_type(EASTL_WEAK_PTR_DEFAULT_NAME)
  ...
  class weak_ptr {
   ...
   public:
    ...
    /// weak_ptr
    /// Construction with self type.
    weak_ptr(const this_type& weakPtr) EA_NOEXCEPT : mpValue(weakPtr.mpValue), mpRefCount(weakPtr.mpRefCount) {
      if (mpRefCount) mpRefCount->weak_addref();
    }

#if EASTL_MOVE_SEMANTICS_ENABLED
    /// weak_ptr
    /// Move construction with self type.
    weak_ptr(this_type&& weakPtr) EA_NOEXCEPT : mpValue(weakPtr.mpValue), mpRefCount(weakPtr.mpRefCount) {
    ...
    /// ~weak_ptr
    ~weak_ptr() {
      if (mpRefCount) mpRefCount->weak_release();
    }
    ...
    template <typename U>
    typename eastl::enable_if<eastl::is_convertible<U*, element_type*>::value, this_type&>::type operator=(
        const weak_ptr<U>& weakPtr) EA_NOEXCEPT {
      assign(weakPtr);
      return *this;
    }
    ...
    template <typename U>
    typename eastl::enable_if<eastl::is_convertible<U*, element_type*>::value, this_type&>::type operator=(
        weak_ptr<U>&& weakPtr) EA_NOEXCEPT {
      weak_ptr(eastl::move(weakPtr)).swap(*this);
      return *this;
    }
#endif
    ...
    shared_ptr<T> lock() const EA_NOEXCEPT {
      // We can't just return shared_ptr<T>(*this), as the object may go stale while we are doing
      // this.
      shared_ptr<T> temp;
      temp.mpRefCount =
          mpRefCount ? mpRefCount->lock() : mpRefCount;  // mpRefCount->lock() addref's the return value for us.
      if (temp.mpRefCount) temp.mpValue = mpValue;
      return temp;
    }

    // Returns: 0 if *this is empty ; otherwise, the number of shared_ptr instances that share
    // ownership with *this.
    int use_count() const EA_NOEXCEPT { return mpRefCount ? mpRefCount->mRefCount : 0; }

    // Returns: use_count() == 0
    bool expired() const EA_NOEXCEPT { return (!mpRefCount || (mpRefCount->mRefCount == 0)); }
  ...
  template <typename T>
  struct owner_less<shared_ptr<T> > : public eastl::binary_function<shared_ptr<T>, shared_ptr<T>, bool> {
    typedef bool result_type;

    bool operator()(shared_ptr<T> const& a, shared_ptr<T> const& b) const { return a.owner_before(b); }

    bool operator()(shared_ptr<T> const& a, weak_ptr<T> const& b) const { return a.owner_before(b); }

    bool operator()(weak_ptr<T> const& a, shared_ptr<T> const& b) const { return a.owner_before(b); }
  };

  template <typename T>
  struct owner_less<weak_ptr<T> > : public eastl::binary_function<weak_ptr<T>, weak_ptr<T>, bool> {
    typedef bool result_type;

    bool operator()(weak_ptr<T> const& a, weak_ptr<T> const& b) const { return a.owner_before(b); }
...
// We have to either #include enable_shared.h here or we need to move the enable_shared source code
// to here.
#include <OpenSpeed/Core/EASTL/EASTL/internal/enable_shared.h>
