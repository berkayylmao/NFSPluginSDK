...
#define EASTL_MULTIMAP_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " multimap"  // Unless the user overrides something, this is "EASTL multimap".
#endif
...
#define EASTL_MULTIMAP_DEFAULT_ALLOCATOR allocator_type(EASTL_MULTIMAP_DEFAULT_NAME)
  ...
  class map : public rbtree<Key, eastl::pair<const Key, T>, Compare, Allocator,
   ...
   public:
    ...
    map(Iterator itBegin, Iterator itEnd);  // allocator arg removed because VC7.1 fails on the default arg. To
  ...
  class multimap : public rbtree<Key, eastl::pair<const Key, T>, Compare, Allocator,
   ...
   public:
    ...
    multimap(Iterator itBegin,
             Iterator itEnd);  // allocator arg removed because VC7.1 fails on the default arg. To consider: Make a
  ...
  multimap<Key, T, Compare, Allocator>::equal_range_small(const Key& key) {
    ...
    while ((itUpper != end()) && !compare(key, itUpper.mpNode->mValue.first)) ++itUpper;
