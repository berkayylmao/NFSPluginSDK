...
#pragma warning(disable : 4217)  // Member template functions cannot be used for copy-assignment or
                                 // copy-construction.
#endif
...
namespace eastl {
  ...
  enum iterator_status_flag {
    isf_none = 0x00,            /// This is called none and not called invalid because it is not strictly the
                                /// opposite of invalid.
    ...
    isf_can_dereference = 0x04  /// The iterator is dereferencable, which means it is in the range
                                /// of [begin, end). It may or may not be current.
  };
...
// we define here. These categories come directly from the C++ standard.
#if !EASTL_STD_ITERATOR_CATEGORY_ENABLED  // If we are to use our own iterator category
                                          // definitions...
  ...
  };  // Extension to the C++ standard. Contiguous ranges are more than random access, they are
  ...
  class is_iterator_wrapper {
    ...
    template <typename U>
    static eastl::yes_type test(typename U::wrapped_iterator_type*,
  ...
  /// and returns the wrapped iterator type. If Iterator is not a wrapper (including being a
  /// pointer), or is not an iterator, then this function returns it as-is. unwrap_iterator unwraps
  /// only a single layer of iterator at a time. You need to call it twice, for example, to unwrap
  /// two layers of iterators.
  ///
  /// Example usage:
  ...
  struct is_iterator_wrapper_helper {
    ...
    static iterator_type get_base(Iterator it) { return it; }
  };
  ...
  struct is_iterator_wrapper_helper<Iterator, true> {
    ...
    static iterator_type get_base(Iterator it) { return it.base(); }
  };

  template <typename Iterator>
  inline typename is_iterator_wrapper_helper<Iterator, eastl::is_iterator_wrapper<Iterator>::value>::iterator_type
  ...
  class reverse_iterator : public iterator<typename eastl::iterator_traits<Iterator>::iterator_category,
   ...
   public:
    typedef Iterator      iterator_type;
    typedef iterator_type wrapped_iterator_type;  // This is not in the C++ Standard; it's used by
                                                  // use to identify it as a wrapping iterator type.
   ...
   public:
    ...
    reverse_iterator operator+(difference_type n) const { return reverse_iterator(mIterator - n); }
    ...
    // random_access_iterator operator[] is merely required to return something convertible to
    // reference. reverse_iterator operator[] can't necessarily know what to return as the
    // underlying iterator operator[] may return something other than reference. reference
  ...
  inline bool operator==(const reverse_iterator<Iterator1>& a, const reverse_iterator<Iterator2>& b) {
  ...
  template <typename Iterator1, typename Iterator2>
  inline typename reverse_iterator<Iterator1>::difference_type operator-(const reverse_iterator<Iterator1>& a,
  ...
  template <typename Iterator>
  struct is_reverse_iterator<eastl::reverse_iterator<Iterator> > : public eastl::true_type {};
  ...
  /// template <typename Iterator>
  /// inline typename eastl::is_iterator_wrapper_helper<Iterator,
  /// eastl::is_reverse_iterator<Iterator>::value>::iterator_type unwrap_reverse_iterator(Iterator
  /// it)
  ///     { return eastl::is_iterator_wrapper_helper<Iterator,
  ///     eastl::is_reverse_iterator<Iterator>::value>::get_base(it); }

#if EASTL_MOVE_SEMANTICS_ENABLED
  /// move_iterator
  ///
  /// From the C++11 Standard, section 24.5.3.1:
  /// Class template move_iterator is an iterator adaptor with the same behavior as the underlying
  /// iterator except that its dereference operator implicitly converts the value returned by the
  /// underlying iterator's dereference operator to an rvalue reference. Some generic algorithms can
  ...
  class move_iterator  // Don't inherit from iterator.
  {
   ...
   public:
    ...
    reference operator*() const { return eastl::move(*mIterator); }
    ...
    move_iterator operator+(difference_type n) const { return move_iterator(mIterator + n); }
    ...
    reference operator[](difference_type n) const { return eastl::move(mIterator[n]); }
  };

  template <typename Iterator1, typename Iterator2>
  inline bool operator==(const move_iterator<Iterator1>& a, const move_iterator<Iterator2>& b) {
    return a.base() == b.base();
  }
  ...
  template <typename Iterator1, typename Iterator2>
  inline auto operator-(const move_iterator<Iterator1>& a, const move_iterator<Iterator2>& b)
      -> decltype(a.base() - b.base()) {
    return a.base() - b.base();
  }

  template <typename Iterator>
  inline move_iterator<Iterator> operator+(typename move_iterator<Iterator>::difference_type n,
...
// make_move_if_noexcept_iterator returns move_iterator<Iterator> if the Iterator is of a noexcept
// type; otherwise returns Iterator as-is. The point of this is to be able to avoid moves that can
// generate exceptions and instead fall back to copies or whatever the default
// IteratorType::operator* returns for use by copy/move algorithms. To consider: merge the
// conditional expression usage here with the one used by move_if_noexcept, as they are the same
// condition.
  ...
  namespace Internal {
    ...
    struct mminei_helper_1 {
      ...
      static inline ReturnType               mminei(Iterator i) { return eastl::move_iterator<Iterator>(i); }
    };
    template <typename Iterator>
    ...
    struct mminei_helper {
      static const bool is_noexcept =
          eastl::is_nothrow_move_constructible<typename eastl::iterator_traits<Iterator>::value_type>::value ||
  ...
  template <typename Iterator>
  inline typename Internal::mminei_helper<Iterator>::ReturnType make_move_if_noexcept_iterator(Iterator i) {
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename Iterator>
  struct is_move_iterator<eastl::move_iterator<Iterator> > : public eastl::true_type {};
#endif
  ...
  ///
  template <typename Iterator>
  inline typename eastl::is_iterator_wrapper_helper<Iterator, eastl::is_move_iterator<Iterator>::value>::iterator_type
  ...
  class back_insert_iterator : public iterator<EASTL_ITC_NS::output_iterator_tag, void, void, void, void> {
   ...
   public:
    ...
    back_insert_iterator& operator++() { return *this; }  // This is by design.
  ...
  class front_insert_iterator : public iterator<EASTL_ITC_NS::output_iterator_tag, void, void, void, void> {
   ...
   public:
    ...
    front_insert_iterator& operator++() { return *this; }  // This is by design.
  ...
  class insert_iterator : public iterator<EASTL_ITC_NS::output_iterator_tag, void, void, void, void> {
   ...
   public:
    ...
    insert_iterator& operator++() { return *this; }  // This is by design.
  ...
  template <typename Iterator>
  struct is_insert_iterator<eastl::insert_iterator<Iterator> > : public eastl::true_type {};
  ...
  inline typename eastl::iterator_traits<InputIterator>::difference_type distance_impl(
  ...
  inline typename eastl::iterator_traits<RandomAccessIterator>::difference_type distance_impl(
      RandomAccessIterator first, RandomAccessIterator last, EASTL_ITC_NS::random_access_iterator_tag) {
    return last - first;
  }
  ...
  template <typename InputIterator>
  inline typename eastl::iterator_traits<InputIterator>::difference_type distance(InputIterator first,
                                                                                  InputIterator last) {
  ...
  inline void advance_impl(InputIterator& i, Distance n, EASTL_ITC_NS::input_iterator_tag) {
    while (n--) ++i;
  }
  ...
  struct advance_bi_impl {
    ...
    static void advance_impl(BidirectionalIterator& i,
  ...
  template <typename BidirectionalIterator, typename Distance>
  inline void advance_impl(BidirectionalIterator& i, Distance n, EASTL_ITC_NS::bidirectional_iterator_tag) {
    advance_bi_impl<eastl::is_signed<Distance>::value>::advance_impl(i, n);
  }

  template <typename RandomAccessIterator, typename Distance>
  inline void advance_impl(RandomAccessIterator& i, Distance n, EASTL_ITC_NS::random_access_iterator_tag) {
    i += n;
  }
  ...
  //
  template <typename InputIterator>
  inline InputIterator next(InputIterator it, typename eastl::iterator_traits<InputIterator>::difference_type n = 1) {
    eastl::advance(it, n);
    return it;
  }

  template <typename InputIterator>
  inline InputIterator prev(InputIterator it, typename eastl::iterator_traits<InputIterator>::difference_type n = 1) {
    eastl::advance(it, -n);
    return it;
  }
