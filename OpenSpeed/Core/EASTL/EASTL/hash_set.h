...
#define EASTL_HASH_MULTISET_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " hash_multiset"  // Unless the user overrides something, this is "EASTL hash_multiset".
#endif
...
#define EASTL_HASH_MULTISET_DEFAULT_ALLOCATOR allocator_type(EASTL_HASH_MULTISET_DEFAULT_NAME)
  ...
  inline bool operator==(const hash_set<Value, Hash, Predicate, Allocator, bCacheHashCode>& a,
    ...
    for (const_iterator ai = a.begin(), aiEnd = a.end(), biEnd = b.end(); ai != aiEnd; ++ai) {
      ...
      if ((bi == biEnd) ||
          !(*ai == *bi))  // We have to compare values in addition to making sure the lookups succeeded. This is because
                          // the lookup is done via the user-supplised Predicate
        return false;  // which isn't strictly required to be identical to the Value operator==, though 99% of the time
                       // it will be so.
    }
  ...
  inline bool operator==(const hash_multiset<Value, Hash, Predicate, Allocator, bCacheHashCode>& a,
    ...
    for (const_iterator ai = a.begin(), aiEnd = a.end(); ai != aiEnd; ai = aRange.second)  // For each element in a...
    {
      ...
      if (aDistance == 1) {
        if (!(*aRange.first ==
              *bRange.first))  // We have to compare values in addition to making sure the distance (element count) was
                               // equal. This is because the lookup is done via the user-supplised Predicate
          return false;  // which isn't strictly required to be identical to the Value operator==, though 99% of the
                         // time it will be so. Ditto for the is_permutation usage below.
      } else {
