...
namespace eastl {
   ...
   public:
    ...
    EA_CPP14_CONSTEXPR unique_ptr() EA_NOEXCEPT : mPair(pointer()) {
      static_assert(!eastl::is_pointer<deleter_type>::value,
                    "unique_ptr deleter default-constructed with null pointer. Use a different constructor or change "
                    "your deleter to a class.");
    }
    ...
    unique_ptr(pointer pValue,
               typename eastl::conditional<eastl::is_lvalue_reference<deleter_type>::value, deleter_type,
    ...
    unique_ptr(pointer pValue, typename eastl::remove_reference<deleter_type>::type&& deleter) EA_NOEXCEPT
      ...
      static_assert(!eastl::is_lvalue_reference<deleter_type>::value,
                    "deleter_type reference refers to an rvalue deleter. The reference will probably become invalid "
                    "before used. Change the deleter_type to not be a reference or construct with permanent deleter.");
    }
    ...
    void reset(pointer pValue = pointer()) EA_NOEXCEPT {
      if (pValue != mPair.first()) {
        if (auto first = eastl::exchange(mPair.first(), pValue)) get_deleter()(first);
      }
    }
  ...
  class unique_ptr<T[], Deleter> {
   ...
   public:
    ...
    template <typename P, typename = eastl::enable_if_t<Internal::is_array_cv_convertible<
                              P, pointer>::value>>  // Pointers to types derived from T are rejected by the
                                                    // constructors, and by reset.
    ...
    unique_ptr(
        ...
        typename enable_if<
            ...
            (!eastl::is_lvalue_reference<deleter_type>::value || eastl::is_same<E, deleter_type>::value)>::type* = 0)
        EA_NOEXCEPT : mPair(u.release(), eastl::forward<E>(u.get_deleter())) {}
    ...
    void reset(pointer pArray = pointer()) EA_NOEXCEPT {
      if (pArray != mPair.first()) {
        if (auto first = eastl::exchange(mPair.first(), pArray)) get_deleter()(first);
      }
    }
  ...
  inline bool operator<(const unique_ptr<T1, D1>& a, const unique_ptr<T2, D2>& b) {
    ...
    // D2>::pointer P2;       // I think there's something wrong with our common_type type trait implementation. typedef
    // typename eastl::common_type<P1, P2>::type   PCommon;  // "in instantiation of function template specialization
    // 'eastl::operator<<int, int>, no known conversion from 'element_type *' (aka 'int *') to 'int *&&&' for 1st
    // argument" return less<PCommon>()(a.get(), b.get());                     // It looks like common_type is making
    // CPointer be (e.g.) int*&& instead of int*, though the problem may be in how less<> deals with that.
