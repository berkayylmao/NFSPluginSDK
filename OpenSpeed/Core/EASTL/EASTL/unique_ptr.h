...
#include <OpenSpeed/Core/EASTL/EABase/nullptr.h>
#include <OpenSpeed/Core/EASTL/EASTL/internal/config.h>
#include <OpenSpeed/Core/EASTL/EASTL/internal/smart_ptr.h>  // Defines smart_ptr_deleter
#include <OpenSpeed/Core/EASTL/EASTL/internal/move_help.h>  // Defines EASTL_MOVE
...
namespace eastl {
  ...
  /// Here is a table which documents several members of shared_ptr and unique_ptr with respect to
  /// completeness requirements. If the member requires a complete type, the entry has a "C",
  /// otherwise the table entry is filled with "I".
  ///
   ...
   public:
    ...
    EA_CPP14_CONSTEXPR unique_ptr() EA_NOEXCEPT : mPair(pointer()) {
      static_assert(!eastl::is_pointer<deleter_type>::value,
                    "unique_ptr deleter default-constructed with null pointer. Use a different "
                    "constructor or change your deleter to a class.");
    }
    ...
    unique_ptr(pointer pValue,
               typename eastl::conditional<eastl::is_reference<deleter_type>::value, deleter_type,
    ...
    unique_ptr(pointer pValue, typename eastl::remove_reference<deleter_type>::type&& deleter) EA_NOEXCEPT
      ...
      static_assert(!eastl::is_reference<deleter_type>::value,
                    "deleter_type reference refers to an rvalue deleter. The reference will "
                    "probably become invalid before used. Change the deleter_type to not be a "
                    "reference or construct with permanent deleter.");
    }
    ...
    unique_ptr(
        ...
        typename enable_if<!is_array<U>::value && is_convertible<typename unique_ptr<U, E>::pointer, pointer>::value &&
                           ...
                           (is_same<deleter_type, E>::value || !is_reference<deleter_type>::value)>::type* = 0)
    ...
    operator bool_() const EA_NOEXCEPT {
      if (mPair.first()) return &this_type::get;
      return NULL;
    }
  ...
  ///
  /// It's not possible to create a unique_ptr for arrays of a known bound (e.g. int[4] as opposed
  /// to int[]).
  ///
  /// Example usage:
  ...
  class unique_ptr<T[], Deleter> {
   ...
   public:
    ...
    template <typename P>  // Pointers to types derived from T are rejected by the constructors, and
                           // by reset.
    explicit unique_ptr(P pArray,
                        typename eastl::enable_if<Internal::is_array_cv_convertible<P, pointer>::value>::type* = 0)
...
#if EASTL_MOVE_SEMANTICS_ENABLED
    template <typename P>
    unique_ptr(P pArray, typename eastl::remove_reference<deleter_type>::type&& deleter,
               typename eastl::enable_if<Internal::is_array_cv_convertible<P, pointer>::value>::type* = 0) EA_NOEXCEPT
    ...
    unique_ptr(unique_ptr<U, E>&& u,
               typename enable_if<
                   ...
                   (!eastl::is_reference<deleter_type>::value || eastl::is_same<E, deleter_type>::value)>::type* = 0)
        EA_NOEXCEPT : mPair(u.release(), eastl::forward<E>(u.get_deleter())) {}
    ...
    template <typename U, typename E>
    typename enable_if<Internal::is_safe_array_conversion<T, pointer, U, typename unique_ptr<U, E>::pointer>::value &&
                           is_assignable<deleter_type&, E&&>::value,
                       this_type&>::type
    operator=(unique_ptr<U, E>&& u) EA_NOEXCEPT {
      reset(u.release());
...
// It's not possible to create a unique_ptr for arrays of a known bound (e.g. int[4] as opposed to
// int[]).
  ...
  inline typename Internal::unique_type<T>::unique_type_single make_unique(A1&& a1, A2&& a2, A3&& a3, A4&& a4) {
    return unique_ptr<T>(
        new T(eastl::forward<A1>(a1), eastl::forward<A2>(a2), eastl::forward<A3>(a3), eastl::forward<A4>(a4)));
  }
...
// int[]).
#if !defined(EA_COMPILER_NO_DELETED_FUNCTIONS)
  template <typename T>
  typename Internal::unique_type<T>::unique_type_bounded_array make_unique() = delete;
#endif
#endif
#else
  ...
  /// unique_ptr<T>::get) is a custom type and not a built-in pointer type then you will need to
  ...
  template <typename T1, typename D1, typename T2, typename D2>
  inline bool operator<(const unique_ptr<T1, D1>& a, const unique_ptr<T2, D2>& b) {
    // typedef typename eastl::unique_ptr<T1, D1>::pointer P1;       // We currently need to make
    // these temporary variables, as otherwise clang complains about CPointer being int*&&&. typedef
    // typename eastl::unique_ptr<T2, D2>::pointer P2;       // I think there's something wrong with
    // our common_type type trait implementation. typedef typename eastl::common_type<P1, P2>::type
    // PCommon;  // "in instantiation of function template specialization 'eastl::operator<<int,
    // int>, no known conversion from 'element_type *' (aka 'int *') to 'int *&&&' for 1st argument"
    // return less<PCommon>()(a.get(), b.get());                     // It looks like common_type is
    // making CPointer be (e.g.) int*&& instead of int*, though the problem may be in how less<>
    // deals with that.
