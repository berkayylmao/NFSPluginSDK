...
#include <OpenSpeed/Core/EASTL/EASTL/internal/memory_base.h>
...
#define EASTL_TEMP_DEFAULT_NAME \
  ...
  class late_constructed {
   public:
    ...
    ~late_constructed() {
      if (autoDestruct && mpValue) (*mpValue).~value_type();
    }
    ...
    bool is_constructed() const EA_NOEXCEPT { return mpValue != nullptr; }
   ...
   protected:
    storage_type mStorage;  // Declared first because it may have aligment requirements, and it would be more
                            // space-efficient if it was first.
    value_type* mpValue;
  };
  ...
  class late_constructed<T, false, autoDestruct> : public late_constructed<T, true, autoDestruct> {
   public:
    ...
    const typename base_type::value_type* get() const EA_NOEXCEPT { return base_type::mpValue; }
  };
  ...
  namespace Internal {
    ...
    struct uninitialized_relocate_impl {
      ...
      static ForwardIteratorDest do_move_start(ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest) {
        ...
        try {
#endif
          for (; first != last; ++first, ++dest) ::new ((void*)eastl::addressof(*dest)) value_type(*first);
#if EASTL_EXCEPTIONS_ENABLED
        } catch (...) {
    ...
    struct uninitialized_relocate_impl<true, EASTL_ITC_NS::random_access_iterator_tag> {
      ...
      static T* do_move_start(T* first, T* last, T* dest) {
        if (EASTL_UNLIKELY(first == last)) return dest;
  ...
  /// Declaration:
  ///    template <typename InputIterator, typename ForwardIterator>
  ///    ForwardIterator uninitialized_copy(InputIterator sourceFirst, InputIterator sourceLast, ForwardIterator
  ///    destination);
  ///
  /// Example usage:
  ...
  namespace Internal {
    ...
    struct uninitialized_copy_n_impl {
      static ForwardIterator impl(InputIterator first, Count n, ForwardIterator dest) {
        ...
        try {
#endif
          for (; n > 0; --n, ++first, ++currentDest) ::new ((void*)(eastl::addressof(*currentDest))) value_type(*first);
#if EASTL_EXCEPTIONS_ENABLED
        } catch (...) {
          for (; dest < currentDest; ++dest) (*dest).~value_type();
          throw;
  ...
  namespace Internal {
    ...
    inline ForwardIterator uninitialized_move_impl(InputIterator first, InputIterator last, ForwardIterator dest,
                                                   true_type) {
      return eastl::copy(
          first, last, dest);  // The copy() in turn will use memcpy for is_trivially_copy_assignable (e.g. POD) types.
    }
...
// We must run a loop over every element and move-construct it at the new location.
...
#endif
        for (; first != last; ++first, ++currentDest)
          ::new ((void*)eastl::addressof(*currentDest))
  ...
  inline Result uninitialized_move_ptr(First first, Last last, Result dest) {
    ...
    const generic_iterator<Result, void> i(Internal::uninitialized_move_impl(
        ...
        eastl::is_trivially_copy_assignable<
            value_type>()));  // is_trivially_copy_assignable identifies if copy assignment would be as valid as move
                              // assignment, which means we have the opportunity to memcpy/memmove optimization.
    return i.base();
  }
  ...
  /// Declaration:
  ///    template <typename InputIterator, typename ForwardIterator>
  ///    ForwardIterator uninitialized_move(InputIterator sourceFirst, InputIterator sourceLast, ForwardIterator
  ///    destination);
  ///
  /// Example usage:
  ///    SomeClass* pArray = malloc(10 * sizeof(SomeClass));
  ...
  inline void uninitialized_default_fill(ForwardIterator first, ForwardIterator last) {
    ...
    try {
#endif
      for (; currentDest != last; ++currentDest) ::new (eastl::addressof(*currentDest)) value_type();
#if EASTL_EXCEPTIONS_ENABLED
    } catch (...) {
  ...
  namespace Internal {
    ...
    inline void uninitialized_default_fill_n_impl(ForwardIterator first, Count n, false_type) {
      ...
      try {
#endif
        for (; n > 0; --n, ++currentDest) ::new (eastl::addressof(*currentDest)) value_type();
#if EASTL_EXCEPTIONS_ENABLED
      } catch (...) {
        for (; first < currentDest; ++first) (*first).~value_type();
        throw;
  ...
  namespace Internal {
    ...
    void uninitialized_fill_impl(ForwardIterator first, ForwardIterator last, const T& value, false_type) {
      ...
      try {
#endif
        for (; currentDest != last; ++currentDest) ::new ((void*)eastl::addressof(*currentDest)) value_type(value);
#if EASTL_EXCEPTIONS_ENABLED
      } catch (...) {
        for (; first < currentDest; ++first) (*first).~value_type();
        throw;
  ...
  namespace Internal {
    ...
    void uninitialized_fill_n_impl(ForwardIterator first, Count n, const T& value, false_type) {
      ...
      try {
#endif
        for (; n > 0; --n, ++currentDest) ::new ((void*)eastl::addressof(*currentDest)) value_type(value);
#if EASTL_EXCEPTIONS_ENABLED
      } catch (...) {
        for (; first < currentDest; ++first) (*first).~value_type();
        throw;
  ...
  inline void destroy(ForwardIterator first, ForwardIterator last) {
    for (; first != last; ++first) eastl::destroy_at(eastl::addressof(*first));
  }
  ...
  ForwardIterator destroy_n(ForwardIterator first, Size n) {
    for (; n > 0; ++first, --n) eastl::destroy_at(eastl::addressof(*first));
  ...
  inline void* align(size_t alignment, size_t size, void*& ptr, size_t& space) {
    if (space >= size) {
      ...
      if ((space - size) >= offset)  // Have to implement this in terms of subtraction instead of addition in order to
  ...
  ///     size_t space = sizeof(buffer);
  ///     void*  p  = buffer;
  ///     void*  p1 = eastl::align_advance(16,  3, p, space, &p, &space); // p is advanced and space reduced
  ///     accordingly. void*  p2 = eastl::align_advance(32, 78, p, space, &p, &space); void*  p3 =
  ///     eastl::align_advance(64,  9, p, space, &p, &space); void*  p4 = eastl::align_advance(16, 33, p, space);
   ...
   public:
    static pointer pointer_to(
        typename eastl::conditional<eastl::is_void<element_type>::value, void, element_type>::type&
            r)  // 20.6.3.2: if element_type is (possibly cv-qualified) void, the type of r is unspecified; otherwise,
                // it is T&.
    {
