...
#include <OpenSpeed/Core/EASTL/EASTL/internal/pair_fwd_decls.h>
#include <OpenSpeed/Core/EASTL/EASTL/internal/functional_base.h>
#include <OpenSpeed/Core/EASTL/EASTL/internal/allocator_traits_fwd_decls.h>
...
#include <OpenSpeed/Core/EASTL/EASTL/numeric_limits.h>
...
#define EASTL_TEMP_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " temp"  // Unless the user overrides something, this is "EASTL temp".
#endif

  /// addressof
  ///
  /// From the C++11 Standard, section 20.6.12.1
  /// Returns the actual address of the object or function referenced by r, even in the presence of
  ...
  ///
  /// Our implementation here differs slightly in that we have alignment, alignmentOffset, and pName
  /// arguments. Note that you can use the EASTL_NAME_VAL macro to make names go away in release
  /// builds.
  ///
  /// Example usage:
  ///    pair<int*, ptrdiff_t> pr = get_temporary_buffer<int>(100, 0, 0, EASTL_NAME_VAL("Temp int
  ///    array")); memset(pr.first, 0, 100 * sizeof(int)); return_temporary_buffer(pr.first);
  ///
  template <typename T>
  eastl::pair<T*, ptrdiff_t> get_temporary_buffer(ptrdiff_t n, size_t alignment = 1, size_t alignmentOffset = 0,
    ...
    return eastl::pair<T*, ptrdiff_t>(
        static_cast<T*>(EASTLAllocAligned(allocator, n * sizeof(T), alignment, alignmentOffset)), n);
  }
  ...
  ///
  /// While construction can be automatic or manual, automatic destruction support is always
  /// present. Thus you aren't required in any case to manually call destruct. However, you may
  /// safely manually destruct the object at any time before the late_constructed destructor is
  /// executed.
  ///
  ...
  ///     void main(){
  ///         gWidget->SetValue(15);
  ///         // You may want to call destruct here, but aren't required to do so unless the Widget
  ///         type requires it.
  ///     }
  ///
  ...
  class late_constructed {
   public:
    ...
    typedef
        typename eastl::aligned_storage<sizeof(value_type), eastl::alignment_of<value_type>::value>::type storage_type;

    late_constructed() EA_NOEXCEPT  // In the case of the late_constructed instance being at global
                                    // scope, we rely on the
        : mpValue(NULL) {}          // compiler executing this constructor or placing the instance in
                                    // auto-zeroed-at-startup memory.

    ~late_constructed() {
      if (mpValue) (*mpValue).~value_type();
    }

#if EASTL_MOVE_SEMANTICS_ENABLED && EASTL_VARIADIC_TEMPLATES_ENABLED
    template <typename... Args>
    void construct(Args&&... args) {
      if (!mpValue) mpValue = new (&mStorage) value_type(eastl::forward<Args>(args)...);
    }
#else
    void construct() {
      if (!mpValue) mpValue = new (&mStorage) value_type;
    }

#if EASTL_MOVE_SEMANTICS_ENABLED
    template <typename A1>
    void construct(A1&& a1) {
      if (!mpValue) mpValue = new (&mStorage) value_type(eastl::forward<A1>(a1));
    }

    template <typename A1, typename A2>
    void construct(A1&& a1, A2&& a2) {
      if (!mpValue) mpValue = new (&mStorage) value_type(eastl::forward<A1>(a1), eastl::forward<A2>(a2));
    }
    ...
    void construct(const A1& a1) {
      if (!mpValue) mpValue = new (&mStorage) value_type(a1);
    }

    template <typename A1, typename A2>
    void construct(const A1& a1, const A2& a2) {
      if (!mpValue) mpValue = new (&mStorage) value_type(a1, a2);
    }

    template <typename A1, typename A2, typename A3>
    void construct(const A1& a1, const A2& a2, const A3& a3) {
      if (!mpValue) mpValue = new (&mStorage) value_type(a1, a2, a3);
    }
#endif

    bool is_constructed() const EA_NOEXCEPT { return mpValue != NULL; }
   ...
   protected:
    storage_type mStorage;  // Declared first because it may have aligment requirements, and it
                            // would be more space-efficient if it was first.
    value_type* mpValue;
  };
  ...
  class late_constructed<T, false> : public late_constructed<T, true> {
   public:
    ...
    typename base_type::value_type* get() EA_NOEXCEPT { return base_type::mpValue; }
  };

  /// raw_storage_iterator
  ///
  /// From the C++11 Standard, section 20.6.10 p1
  /// raw_storage_iterator is provided to enable algorithms to store their results into
  /// uninitialized memory. The formal template parameter OutputIterator is required to have its
  /// operator* return an object for which operator& is defined and returns a pointer to T, and is
  /// also required to satisfy the requirements of an output iterator (24.2.4).
  ...
  class raw_storage_iterator : public iterator<EASTL_ITC_NS::output_iterator_tag, void, void, void, void> {
   ...
   public:
    explicit raw_storage_iterator(OutputIterator iterator) : mIterator(iterator) {}
  ...
  namespace Internal {
    ...
    struct uninitialized_relocate_impl {
      ...
      static ForwardIteratorDest do_move_start(ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest) {
        ...
        try {
          for (; first != last; ++first, ++dest) ::new ((void*)&*dest) value_type(*first);
        } catch (...) {
          for (; origDest < dest; ++origDest) (*origDest).~value_type();
          throw;
        }
#else
        for (; first != last; ++first, ++dest) ::new ((void*)&*dest) value_type(*first);
      ...
      {
        typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
        for (; first != last; ++first, ++dest) (*first).~value_type();

        return dest;
      }

      template <typename ForwardIterator, typename ForwardIteratorDest>
      static ForwardIteratorDest do_move_abort(ForwardIterator first, ForwardIterator last,
                                               ForwardIteratorDest dest)  // throw()
      {
        typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
        for (; first != last; ++first, ++dest) (*dest).~value_type();
        return dest;
      }
    };
  ...
  /// is as it was before.
  ///
  /// In order to make the iterators valid again you need to call either
  /// uninitialized_relocate_abort or uninitialized_relocate_commit. The abort call makes the
  /// original source iterator valid again, and commit makes the destination valid. Both abort and
  /// commit are guaranteed to not throw C++ exceptions.
  ///
  /// Example usage:
  ...
  ///
  template <typename ForwardIterator, typename ForwardIteratorDest>
  inline ForwardIteratorDest uninitialized_relocate_start(ForwardIterator first, ForwardIterator last,
    ...
    const bool bHasTrivialMove =
        type_and<has_trivial_relocate<value_type_input>::value, is_pointer<ForwardIterator>::value,
                 is_pointer<ForwardIteratorDest>::value, is_same<value_type_input, value_type_output>::value>::value;

    return Internal::uninitialized_relocate_impl<bHasTrivialMove, IC>::do_move_start(first, last, dest);
  }

  template <typename ForwardIterator, typename ForwardIteratorDest>
  inline ForwardIteratorDest uninitialized_relocate_commit(ForwardIterator first, ForwardIterator last,
                                                           ForwardIteratorDest dest) {
    typedef typename eastl::iterator_traits<ForwardIterator>::iterator_category IC;
    typedef typename eastl::iterator_traits<ForwardIterator>::value_type        value_type_input;
    typedef typename eastl::iterator_traits<ForwardIteratorDest>::value_type    value_type_output;
  ...
  template <typename ForwardIterator, typename ForwardIteratorDest>
  inline ForwardIteratorDest uninitialized_relocate_abort(ForwardIterator first, ForwardIterator last,
                                                          ForwardIteratorDest dest) {
    typedef typename eastl::iterator_traits<ForwardIterator>::iterator_category IC;
    typedef typename eastl::iterator_traits<ForwardIterator>::value_type        value_type_input;
    typedef typename eastl::iterator_traits<ForwardIteratorDest>::value_type    value_type_output;
  ...
  ///
  template <typename ForwardIterator, typename ForwardIteratorDest>
  inline ForwardIteratorDest uninitialized_relocate(ForwardIterator first, ForwardIterator last,
                                                    ForwardIteratorDest dest) {
...
#if EASTL_EXCEPTIONS_ENABLED
      try {
        for (; first != last; ++first, ++currentDest) ::new ((void*)&*currentDest) value_type(*first);
      } catch (...) {
        for (; dest < currentDest; ++dest) (*dest).~value_type();
        throw;
      }
#else
      for (; first != last; ++first, ++currentDest) ::new ((void*)&*currentDest) value_type(*first);
  ...
  inline ForwardIterator uninitialized_copy(InputIterator first, InputIterator last, ForwardIterator result) {
    ...
    // We use is_trivial, which in the C++11 Standard means is_trivially_copyable and
  ...
  /// beginning at dest. The elements in the uninitialized area are constructed using copy
  /// constructor. If an exception is thrown during the initialization, the function has no final
  /// effects.
  ///
...
#if EASTL_EXCEPTIONS_ENABLED
        try {
          for (; n > 0; --n, ++first, ++currentDest) ::new ((void*)&*currentDest) value_type(*first);
        } catch (...) {
          for (; dest < currentDest; ++dest) (*dest).~value_type();
          throw;
        }
  ...
  ///
  /// This is a specialization of uninitialized_copy for iterators that are pointers. We use it
  /// because internally it uses generic_iterator to make pointers act like regular eastl::iterator.
  ///
  ...
  inline Result uninitialized_copy_ptr(First first, Last last, Result result) {
    ...
    const generic_iterator<Result, void> i(Internal::uninitialized_copy_impl(
        eastl::generic_iterator<First, void>(first),  // generic_iterator makes a pointer act like an iterator.
        eastl::generic_iterator<Last, void>(last), eastl::generic_iterator<Result, void>(result),
...
///
/// This is a specialization of uninitialized_move for iterators that are pointers. We use it
/// because internally it uses generic_iterator to make pointers act like regular eastl::iterator.
///
#if EASTL_MOVE_SEMANTICS_ENABLED
  namespace Internal {
    template <typename InputIterator, typename ForwardIterator>
    inline ForwardIterator uninitialized_move_impl(InputIterator first, InputIterator last, ForwardIterator dest,
                                                   true_type) {
      return eastl::copy(first, last, dest);  // The copy() in turn will use memcpy for
                                              // is_trivially_copy_assignable (e.g. POD) types.
    }
...
// We must run a loop over every element and move-construct it at the new location.
      ...
      try {
        for (; first != last; ++first, ++currentDest)
          ::new ((void*)&*currentDest)
              value_type(eastl::move(*first));  // If value_type has a move constructor then it will be used here.
      } catch (...) {
        ...
        // exception while moving them back? We may want to trace through a conforming C++11
        // Standard Library to see what it does and do something similar. Given that rvalue
        // references are objects that are going away, we may not need to move the values back,
        // though that has the side effect of a certain kind of lost elements problem.
        for (; dest < currentDest; ++dest) (*dest).~value_type();
        throw;
      }
#else
  ...
  inline Result uninitialized_move_ptr(First first, Last last, Result dest) {
    ...
    const generic_iterator<Result, void> i(Internal::uninitialized_move_impl(
        eastl::generic_iterator<First, void>(first),  // generic_iterator makes a pointer act like an iterator.
        eastl::generic_iterator<Last, void>(last), eastl::generic_iterator<Result, void>(dest),
        eastl::is_trivially_copy_assignable<value_type>()));  // is_trivially_copy_assignable identifies if copy
                                                              // assignment would be as valid as move assignment, which
                                                              // means we have the opportunity to memcpy/memmove
                                                              // optimization.
    return i.base();
  }
#else
  ...
  /// Declaration:
  ///    template <typename InputIterator, typename ForwardIterator>
  ///    ForwardIterator uninitialized_move(InputIterator sourceFirst, InputIterator sourceLast,
  ///    ForwardIterator destination);
  ///
  /// Example usage:
  ///    SomeClass* pArray = malloc(10 * sizeof(SomeClass));
  ...
  ///
  /// If the iterated type can be moved without exceptions, move construct the dest with the input.
  /// Else copy-construct the dest witih the input. If move isn't supported by the compiler, do
  /// regular copy.
  ///
  template <typename InputIterator, typename ForwardIterator>
  inline ForwardIterator uninitialized_move_if_noexcept(InputIterator first, InputIterator last, ForwardIterator dest) {
    ...
    return eastl::uninitialized_copy(eastl::make_move_if_noexcept_iterator(first),
                                     eastl::make_move_if_noexcept_iterator(last), dest);
#else
    return eastl::uninitialized_copy(first, last, dest);
#endif
  }
  ...
  // Disable warning C4345 - behavior change: an object of POD type constructed with an initializer
  // of the form () will be default-initialized. This is the behavior we intend below.
  ...
  ///    void uninitialized_default_fill(ForwardIterator destinationFirst, ForwardIterator
  ///    destinationLast);
  ///
...
#if EASTL_EXCEPTIONS_ENABLED
    try {
      for (; currentDest != last; ++currentDest) ::new (static_cast<void*>(&*currentDest)) value_type();
    } catch (...) {
      for (; first < currentDest; ++first) (*first).~value_type();
      throw;
    }
#else
    for (; currentDest != last; ++currentDest) ::new (static_cast<void*>(&*currentDest)) value_type();
...
#if EASTL_EXCEPTIONS_ENABLED
    try {
      for (; n > 0; --n, ++currentDest) ::new (static_cast<void*>(&*currentDest)) value_type();
    } catch (...) {
      for (; first < currentDest; ++first) (*first).~value_type();
      throw;
    }
  ...
  /// Declaration:
  ///    template <typename ForwardIterator, typename T>
  ///    void uninitialized_fill(ForwardIterator destinationFirst, ForwardIterator destinationLast,
  ///    const T& value);
  ///
  namespace Internal {
    template <typename ForwardIterator, typename T>
...
#if EASTL_EXCEPTIONS_ENABLED
      try {
        for (; currentDest != last; ++currentDest) ::new ((void*)&*currentDest) value_type(value);
      } catch (...) {
        for (; first < currentDest; ++first) (*first).~value_type();
        throw;
      }
  ...
  inline void uninitialized_fill_ptr(T* first, T* last, const T& value) {
    ...
    Internal::uninitialized_fill_impl(eastl::generic_iterator<T*, void>(first), eastl::generic_iterator<T*, void>(last),
                                      value, eastl::is_trivially_copy_assignable<value_type>());
  }
...
#if EASTL_EXCEPTIONS_ENABLED
      try {
        for (; n > 0; --n, ++currentDest) ::new ((void*)&*currentDest) value_type(value);
      } catch (...) {
        for (; first < currentDest; ++first) (*first).~value_type();
        throw;
      }
  ...
  inline void uninitialized_fill_n_ptr(T* first, Count n, const T& value) {
    ...
    Internal::uninitialized_fill_n_impl(eastl::generic_iterator<T*, void>(first), n, value,
                                        eastl::is_trivially_copy_assignable<value_type>());
  }
  ...
  inline void uninitialized_copy_fill(InputIterator first1, InputIterator last1, ForwardIterator first2,
    ...
    try {
      ...
      for (; first2 < mid; ++first2) (*first2).~value_type();
      throw;
    }
#endif
  }
  ...
  ///
  template <typename InputIterator, typename ForwardIterator, typename T>
  inline void uninitialized_move_fill(InputIterator first1, InputIterator last1, ForwardIterator first2,
                                      ForwardIterator last2, const T& value) {
  ...
  inline ForwardIterator uninitialized_fill_copy(ForwardIterator result, ForwardIterator mid, const T& value,
    ...
    try {
      ...
      for (; result < mid; ++result) (*result).~value_type();
      throw;
    }
#endif
  }
  ...
  ///
  template <typename ForwardIterator, typename T, typename InputIterator>
  inline ForwardIterator uninitialized_fill_move(ForwardIterator result, ForwardIterator mid, const T& value,
                                                 InputIterator first, InputIterator last) {
    eastl::uninitialized_fill(result, mid, value);
  ...
  /// If it is possible to fit size bytes of storage aligned by alignment into the buffer pointed to
  /// by ptr with length space, the function updates ptr to point to the first possible address of
  /// such storage, decreases space by the number of bytes used for alignment, and returns the new
  /// ptr value. Otherwise, the function returns NULL and leaves ptr and space unmodified.
  ///
  /// Example usage:
  ///     char   buffer[512];
  ...
  inline void* align(size_t alignment, size_t size, void*& ptr, size_t& space) {
    if (space >= size) {
      ...
      if ((space - size) >= offset)  // Have to implement this in terms of subtraction instead of
                                     // addition in order to handle possible overflow.
      {
        ptr = ptrAligned;
        space -= offset;
  ...
  ///     size_t space = sizeof(buffer);
  ///     void*  p  = buffer;
  ///     void*  p1 = eastl::align_advance(16,  3, p, space, &p, &space); // p is advanced and space
  ///     reduced accordingly. void*  p2 = eastl::align_advance(32, 78, p, space, &p, &space); void*
  ///     p3 = eastl::align_advance(64,  9, p, space, &p, &space); void*  p4 =
  ///     eastl::align_advance(16, 33, p, space);
  ...
  inline void* align_advance(size_t alignment, size_t size, void* ptr, size_t space, void** ptrAdvanced = NULL,
    ...
    if (space >= size) {
      ...
      if ((space - size) >= offset)  // Have to implement this in terms of subtraction instead of
                                     // addition in order to handle possible overflow.
      {
        if (ptrAdvanced) *ptrAdvanced = (ptrAligned + size);
        if (spaceReduced) *spaceReduced = (space - (offset + size));
  ...
  namespace Internal {
    ...
    template <typename Pointer>
    struct has_element_type  // has_element_type<T>::value is true if T has an element_type member
                             // typedef.
    {
     private:
      template <typename U>
...
#if EASTL_VARIADIC_TEMPLATES_ENABLED  // See 20.6.3.1 p3 for why we need to support this. Pointer
                                      // may be a template with various arguments as opposed to a
                                      // non-templated class.
...
#else
    // To consider: solve this via multiple declarations. For our uses this may not matter a lot, as
    // this is an uncommon use-case.
#endif

    // pointer_difference_type
    template <typename Pointer>
    struct has_difference_type  // has_difference_type<T>::value is true if T has an difference_type
                                // member typedef.
    {
     private:
      template <typename U>
  ...
  struct pointer_traits {
   ...
   public:
    static pointer pointer_to(
        typename eastl::conditional<eastl::is_void<element_type>::value, void, element_type>::type&
            r)  // 20.6.3.2: if element_type is (possibly cv-qualified) void, the type of r is
                // unspecified; otherwise, it is T&.
    {
  ...
  struct pointer_traits<T*> {
   ...
   public:
    static pointer pointer_to(
        typename eastl::conditional<eastl::is_void<element_type>::value, void, element_type>::type& r) EA_NOEXCEPT {
      return eastl::addressof(r);
    }  // 20.6.3.2: if element_type is (possibly cv-qualified) void, the type of r is unspecified;
       // otherwise, it is T&.
  };

}  // namespace eastl

#include <OpenSpeed/Core/EASTL/EASTL/internal/allocator_traits.h>
