...
#define EASTL_VECTOR_MULTISET_DEFAULT_ALLOCATOR allocator_type(EASTL_VECTOR_MULTISET_DEFAULT_NAME)
  ...
  class vector_multiset : public RandomAccessContainer {
   public:
    ...
    typedef typename base_type::iterator
        iterator;  // **Currently typedefing from iterator instead of const_iterator due to const
                   // issues **: Note that we typedef from const_iterator. This is by design, as
                   // sets are sorted and values cannot be modified. To consider: allow values to be
                   // modified and thus risk changing their sort values.
    typedef typename base_type::const_iterator const_iterator;
    typedef
        typename base_type::reverse_iterator reverse_iterator;  // See notes directly above regarding const_iterator.
   ...
   public:
    ...
    explicit vector_multiset(const key_compare&    comp,
...
#endif
    vector_multiset(std::initializer_list<value_type> ilist, const key_compare& compare = key_compare(),
                    const allocator_type& allocator = EASTL_VECTOR_MULTISET_DEFAULT_ALLOCATOR);

    template <typename InputIterator>
    vector_multiset(InputIterator first,
    ...
    vector_multiset(InputIterator first, InputIterator last,
                    const key_compare& compare);  // allocator arg removed because VC7.1 fails on
                                                  // the default arg. To do: Make a second version
                                                  // of this function without a default arg.
    ...
    iterator insert(const value_type& value);  // The signature of this function was change in EASTL v2.05.00 from
                                               // (the mistaken) pair<iterator, bool> to (the correct) iterator.
#if EASTL_MOVE_SEMANTICS_ENABLED
    template <typename P>
    ...
    eastl::pair<iterator, iterator> equal_range_small(const key_type& k) {
      ...
      while ((itUpper != end()) && !mCompare(k, *itUpper)) ++itUpper;
    ...
    // Functions which are disallowed due to being unsafe. We are looking for a way to disable these
    // at compile-time. Declaring but not defining them doesn't work due to explicit template
    // instantiations.
  ...
  inline vector_multiset<K, C, A, RAC>::vector_multiset() : base_type(), mCompare(C()) {
  ...
  template <typename K, typename C, typename A, typename RAC>
  inline vector_multiset<K, C, A, RAC>::vector_multiset(const this_type& x) : base_type(x), mCompare(x.mCompare) {
    // Empty
  }
  ...
  inline vector_multiset<K, C, A, RAC>::vector_multiset(this_type&& x) :
    ...
    // Empty. Note: x is left with empty contents but its original mValueCompare instead of the
    // default one.
  }
  ...
  template <typename K, typename C, typename A, typename RAC>
  inline vector_multiset<K, C, A, RAC>::vector_multiset(std::initializer_list<value_type> ilist,
                                                        const key_compare& compare, const allocator_type& allocator) :
      base_type(allocator), mCompare(compare) {
    insert(ilist.begin(), ilist.end());
  }

  template <typename K, typename C, typename A, typename RAC>
  inline vector_multiset<K, C, A, RAC>& vector_multiset<K, C, A, RAC>::operator=(const this_type& x) {
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename K, typename C, typename A, typename RAC>
  inline vector_multiset<K, C, A, RAC>& vector_multiset<K, C, A, RAC>::operator=(this_type&& x) {
  ...
  template <typename K, typename C, typename A, typename RAC>
  inline const typename vector_multiset<K, C, A, RAC>::key_compare& vector_multiset<K, C, A, RAC>::key_comp() const {
    return mCompare;
  }

  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::key_compare& vector_multiset<K, C, A, RAC>::key_comp() {
    return mCompare;
  }

  template <typename K, typename C, typename A, typename RAC>
  inline const typename vector_multiset<K, C, A, RAC>::value_compare& vector_multiset<K, C, A, RAC>::value_comp()
      const {
    return mCompare;
  }

  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::value_compare& vector_multiset<K, C, A, RAC>::value_comp() {
    return mCompare;
  }
  ...
  template <typename K, typename C, typename A, typename RAC>
  template <class... Args>
  typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::emplace(Args&&... args) {
  ...
  template <typename K, typename C, typename A, typename RAC>
  template <class... Args>
  typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::emplace_hint(const_iterator position,
                                                                                               Args&&... args) {
#if EASTL_USE_FORWARD_WORKAROUND
    auto value = value_type(eastl::forward<Args>(args)...);  // Workaround for compiler bug in VS2013 which results in a
                                                             // compiler internal crash while compiling this code.
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename K, typename C, typename A, typename RAC>
  typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::emplace(value_type&& value) {
    return insert(eastl::move(value));
  }
  ...
  template <typename K, typename C, typename A, typename RAC>
  typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::emplace(const value_type& value) {
    return insert(value);
  }

  template <typename K, typename C, typename A, typename RAC>
  typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::emplace_hint(
  ...
  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::insert(
  ...
  template <typename K, typename C, typename A, typename RAC>
  template <typename P>
  typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::insert(P&& otherValue) {
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename K, typename C, typename A, typename RAC>
  typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::insert(const_iterator position,
                                                                                         value_type&&   value) {
    if ((position == end()) || !mCompare(*position, value))  // If value is <= the element at position...
    {
      if ((position == begin()) || !mCompare(value, *(position - 1)))  // If value is >= the element before position...
  ...
  inline void vector_multiset<K, C, A, RAC>::insert(InputIterator first, InputIterator last) {
    ...
    for (; first != last; ++first) base_type::insert(lower_bound(*first), *first);
  }

  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::erase(
  ...
  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::erase(const_iterator first,
                                                                                               const_iterator last) {
    return base_type::erase(first, last);
  }

  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::reverse_iterator vector_multiset<K, C, A, RAC>::erase(
  ...
  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::size_type vector_multiset<K, C, A, RAC>::erase(const key_type& k) {
    const eastl::pair<iterator, iterator> pairIts(equal_range(k));

    if (pairIts.first != pairIts.second) base_type::erase(pairIts.first, pairIts.second);

    return (size_type)eastl::distance(pairIts.first,
                                      pairIts.second);  // This can result in any value >= 0.
  }

  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::find(const key_type& k) {
    const eastl::pair<iterator, iterator> pairIts(equal_range(k));

    if (pairIts.first != pairIts.second) return pairIts.first;
    return end();
  }

  template <typename K, typename C, typename A, typename RAC>
  template <typename U, typename BinaryPredicate>
  inline typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::find_as(
  ...
  template <typename K, typename C, typename A, typename RAC>
  template <typename U, typename BinaryPredicate>
  inline typename vector_multiset<K, C, A, RAC>::const_iterator vector_multiset<K, C, A, RAC>::find_as(
      const U& u, BinaryPredicate /*predicate*/) const {
    // To do: Implement this.
    return find(u);
  }

  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::const_iterator vector_multiset<K, C, A, RAC>::find(
  ...
  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::size_type vector_multiset<K, C, A, RAC>::count(const key_type& k) {
    const eastl::pair<const_iterator, const_iterator> pairIts(equal_range(k));
    return (size_type)eastl::distance(pairIts.first, pairIts.second);
  }

  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::lower_bound(
      const key_type& k) {
    return eastl::lower_bound(begin(), end(), k, mCompare);
  }

  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::const_iterator vector_multiset<K, C, A, RAC>::lower_bound(
      const key_type& k) const {
    return eastl::lower_bound(begin(), end(), k, mCompare);
  }

  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::iterator vector_multiset<K, C, A, RAC>::upper_bound(
      const key_type& k) {
    return eastl::upper_bound(begin(), end(), k, mCompare);
  }

  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::const_iterator vector_multiset<K, C, A, RAC>::upper_bound(
      const key_type& k) const {
    return eastl::upper_bound(begin(), end(), k, mCompare);
  }
  ...
  /*
  // VC++ fails to compile this when defined here, saying the function isn't a memgber of
  vector_multimap. template <typename K, typename C, typename A, typename RAC> inline
  eastl::pair<typename vector_multiset<K, C, A, RAC>::iterator, typename vector_multiset<K, C, A,
  RAC>::iterator> vector_multiset<K, C, A, RAC>::equal_range_small(const key_type& k)
  {
          const iterator itLower(lower_bound(k));
          iterator       itUpper(itLower);
