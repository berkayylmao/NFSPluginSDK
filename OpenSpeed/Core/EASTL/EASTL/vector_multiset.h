...
#define EASTL_VECTOR_MULTISET_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " vector_multiset"  // Unless the user overrides something, this is "EASTL vector_multiset".
#endif
...
#define EASTL_VECTOR_MULTISET_DEFAULT_ALLOCATOR allocator_type(EASTL_VECTOR_MULTISET_DEFAULT_NAME)
  ...
  class vector_multiset : public RandomAccessContainer {
   public:
    ...
    typedef typename base_type::iterator
        iterator;  // **Currently typedefing from iterator instead of const_iterator due to const issues **: Note that
                   // we typedef from const_iterator. This is by design, as sets are sorted and values cannot be
                   // modified. To consider: allow values to be modified and thus risk changing their sort values.
   ...
   public:
    ...
    vector_multiset(InputIterator first,
                    InputIterator last);  // allocator arg removed because VC7.1 fails on the default arg. To do: Make a
                                          // second version of this function without a default arg.
    ...
    iterator insert(const value_type& value);  // The signature of this function was change in EASTL v2.05.00 from (the
                                               // mistaken) pair<iterator, bool> to (the correct) iterator.
  ...
  inline void vector_multiset<K, C, A, RAC>::insert(InputIterator first, InputIterator last) {
    ...
    for (; first != last; ++first) base_type::insert(upper_bound(*first), *first);
  }
  ...
  template <typename K, typename C, typename A, typename RAC>
  inline typename vector_multiset<K, C, A, RAC>::size_type vector_multiset<K, C, A, RAC>::count(
      const key_type& k) const {
    const eastl::pair<const_iterator, const_iterator> pairIts(equal_range(k));
