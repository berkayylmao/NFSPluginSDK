...
#include <OpenSpeed/Core/EASTL/EASTL/internal/mem_fn.h>
...
namespace eastl {
  ...
  struct negate<void> {
    typedef int is_transparent;
    template <typename T>
    EA_CPP14_CONSTEXPR auto operator()(T&& t) const -> decltype(-eastl::forward<T>(t)) {
  ...
  struct not_fn_ret {
    ...
    template <class... Args>
    auto operator()(
        Args&&... args) & -> decltype(!eastl::declval<eastl::invoke_result_t<eastl::decay_t<F>&, Args...>>()) {
      return !eastl::invoke(mDecayF, eastl::forward<Args>(args)...);
    }

    template <class... Args>
    auto operator()(Args&&... args)
        const& -> decltype(!eastl::declval<eastl::invoke_result_t<eastl::decay_t<F> const&, Args...>>()) {
      return !eastl::invoke(mDecayF, eastl::forward<Args>(args)...);
    }
  ...
  template <typename T>
  struct hash<T*>  // Note that we use the pointer as-is and don't divide by sizeof(T*). This is because the table is of
                   // a prime size and this division doesn't benefit distribution.
  {
