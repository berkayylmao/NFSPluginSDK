...
#define EASTL_LIST_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " list"  // Unless the user overrides something, this is "EASTL list".
#endif
...
#define EASTL_LIST_DEFAULT_ALLOCATOR allocator_type(EASTL_LIST_DEFAULT_NAME)
  ...
  struct ListNodeBase {
    ...
    void remove() EA_NOEXCEPT;  // Removes this node from the list it's in. Leaves this node's
                                // mpNext/mpPrev invalid.
    void splice(ListNodeBase* pFirst,
                ListNodeBase* pLast) EA_NOEXCEPT;  // Removes [pFirst,pLast) from the list it's in and inserts it before
                                                   // this in this node's list.
    void        reverse() EA_NOEXCEPT;  // Reverses the order of nodes in the circular list this node is a part of.
    static void swap(ListNodeBase& a,
                     ListNodeBase& b) EA_NOEXCEPT;  // Swaps the nodes a and b in the lists to which they belong.
...
#else
    typedef ListNodeBase base_node_type;  // We use ListNodeBase instead of ListNode<T> because we
                                          // don't want to create a T.
#endif
  ...
  ///
  /// -- size() is O(n) --
  /// Note that as of this writing, list::size() is an O(n) operation when EASTL_LIST_SIZE_CACHE is
  ...
  /// EASTL_LIST_SIZE_CACHE option changes this. To consider: Make size caching an optional template
  /// parameter.
  ///
  /// Pool allocation
  ...
  class list : public ListBase<T, Allocator> {
   ...
   public:
    ...
    void assign(InputIterator first,
                InputIterator last);  // version of assign that takes (int size, int value). These
                                      // are not iterators, so we need to do a template compiler
                                      // trick to do the right thing.
    ...
    // splice inserts elements in the range [first,last) before position and removes the elements
    // from x. In the case that the two containers' allocators are unequal, splice copies elements
  ...
  inline ListIterator<T, Pointer, Reference>::ListIterator(const ListNodeBase* pNode) EA_NOEXCEPT
      : mpNode(static_cast<node_type*>(
            (ListNode<T>*)const_cast<ListNodeBase*>(pNode)))  // All this casting is in the name of making runtime
  ...
  template <typename T, typename Pointer, typename Reference>
  inline typename ListIterator<T, Pointer, Reference>::this_type ListIterator<T, Pointer, Reference>::next() const
      EA_NOEXCEPT {
    return ListIterator(mpNode->mpNext);
  }

  template <typename T, typename Pointer, typename Reference>
  inline typename ListIterator<T, Pointer, Reference>::this_type ListIterator<T, Pointer, Reference>::prev() const
      EA_NOEXCEPT {
    return ListIterator(mpNode->mpPrev);
  }

  template <typename T, typename Pointer, typename Reference>
  inline typename ListIterator<T, Pointer, Reference>::reference ListIterator<T, Pointer, Reference>::operator*() const
      EA_NOEXCEPT {
    return mpNode->mValue;
  }

  template <typename T, typename Pointer, typename Reference>
  inline typename ListIterator<T, Pointer, Reference>::pointer ListIterator<T, Pointer, Reference>::operator->() const
      EA_NOEXCEPT {
    return &mpNode->mValue;
  }

  template <typename T, typename Pointer, typename Reference>
  inline typename ListIterator<T, Pointer, Reference>::this_type& ListIterator<T, Pointer, Reference>::operator++()
      EA_NOEXCEPT {
  ...
  template <typename T, typename Pointer, typename Reference>
  inline typename ListIterator<T, Pointer, Reference>::this_type ListIterator<T, Pointer, Reference>::operator++(int)
      EA_NOEXCEPT {
    this_type temp(*this);
    mpNode = static_cast<node_type*>(mpNode->mpNext);
    return temp;
  ...
  template <typename T, typename Allocator>
  const typename ListBase<T, Allocator>::allocator_type& ListBase<T, Allocator>::get_allocator() const EA_NOEXCEPT {
    return mAllocator;
  }

  template <typename T, typename Allocator>
  typename ListBase<T, Allocator>::allocator_type& ListBase<T, Allocator>::get_allocator() EA_NOEXCEPT {
    return mAllocator;
  }
  ...
  template <typename T, typename Allocator>
  inline typename ListBase<T, Allocator>::node_type* ListBase<T, Allocator>::DoAllocateNode() {
  ...
  template <typename T, typename Allocator>
  inline list<T, Allocator>::list() : base_type() {
    // Empty
  }

  template <typename T, typename Allocator>
  inline list<T, Allocator>::list(const allocator_type& allocator) : base_type(allocator) {
    // Empty
  }

  template <typename T, typename Allocator>
  inline list<T, Allocator>::list(size_type n, const allocator_type& allocator) : base_type(allocator) {
  ...
  template <typename T, typename Allocator>
  inline list<T, Allocator>::list(const this_type& x) : base_type(x.mAllocator) {
    DoInsert((ListNodeBase*)&mNode, const_iterator((ListNodeBase*)x.mNode.mpNext),
             const_iterator((ListNodeBase*)&x.mNode), false_type());
  }

  template <typename T, typename Allocator>
  inline list<T, Allocator>::list(const this_type& x, const allocator_type& allocator) : base_type(allocator) {
    DoInsert((ListNodeBase*)&mNode, const_iterator((ListNodeBase*)x.mNode.mpNext),
             const_iterator((ListNodeBase*)&x.mNode), false_type());
  }
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Allocator>
  inline list<T, Allocator>::list(this_type&& x) : base_type(eastl::move(x.mAllocator)) {
    swap(x);
  }

  template <typename T, typename Allocator>
  inline list<T, Allocator>::list(this_type&& x, const allocator_type& allocator) : base_type(allocator) {
  ...
  template <typename T, typename Allocator>
  template <typename InputIterator>
  list<T, Allocator>::list(InputIterator first, InputIterator last) : base_type(EASTL_LIST_DEFAULT_ALLOCATOR) {
    // insert(const_iterator((ListNodeBase*)&mNode), first, last);
    DoInsert((ListNodeBase*)&mNode, first, last, is_integral<InputIterator>());
  }

  template <typename T, typename Allocator>
  typename list<T, Allocator>::iterator inline list<T, Allocator>::begin() EA_NOEXCEPT {
    return iterator((ListNodeBase*)mNode.mpNext);
  }

  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::const_iterator list<T, Allocator>::begin() const EA_NOEXCEPT {
    return const_iterator((ListNodeBase*)mNode.mpNext);
  }

  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::const_iterator list<T, Allocator>::cbegin() const EA_NOEXCEPT {
    return const_iterator((ListNodeBase*)mNode.mpNext);
  }

  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::iterator list<T, Allocator>::end() EA_NOEXCEPT {
    return iterator((ListNodeBase*)&mNode);
  }

  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::const_iterator list<T, Allocator>::end() const EA_NOEXCEPT {
    return const_iterator((ListNodeBase*)&mNode);
  }

  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::const_iterator list<T, Allocator>::cend() const EA_NOEXCEPT {
    return const_iterator((ListNodeBase*)&mNode);
  }

  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::reverse_iterator list<T, Allocator>::rbegin() EA_NOEXCEPT {
    return reverse_iterator((ListNodeBase*)&mNode);
  }

  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::const_reverse_iterator list<T, Allocator>::rbegin() const EA_NOEXCEPT {
    return const_reverse_iterator((ListNodeBase*)&mNode);
  }

  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::const_reverse_iterator list<T, Allocator>::crbegin() const EA_NOEXCEPT {
    return const_reverse_iterator((ListNodeBase*)&mNode);
  }

  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::reverse_iterator list<T, Allocator>::rend() EA_NOEXCEPT {
    return reverse_iterator((ListNodeBase*)mNode.mpNext);
  }

  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::const_reverse_iterator list<T, Allocator>::rend() const EA_NOEXCEPT {
    return const_reverse_iterator((ListNodeBase*)mNode.mpNext);
  }

  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::const_reverse_iterator list<T, Allocator>::crend() const EA_NOEXCEPT {
    return const_reverse_iterator((ListNodeBase*)mNode.mpNext);
  }
  ...
  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::size_type list<T, Allocator>::size() const EA_NOEXCEPT {
#if EASTL_LIST_SIZE_CACHE
    return mSize;
#else
#if EASTL_DEBUG
  ...
  template <typename T, typename Allocator>
  typename list<T, Allocator>::this_type& list<T, Allocator>::operator=(const this_type& x) {
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Allocator>
  typename list<T, Allocator>::this_type& list<T, Allocator>::operator=(this_type&& x) {
    if (this != &x) {
  ...
  template <typename T, typename Allocator>
  typename list<T, Allocator>::this_type& list<T, Allocator>::operator=(std::initializer_list<value_type> ilist) {
  ...
  template <typename T, typename Allocator>
  template <typename... Args>
  inline typename list<T, Allocator>::iterator list<T, Allocator>::emplace(const_iterator position, Args&&... args) {
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::iterator list<T, Allocator>::emplace(const_iterator position,
                                                                           value_type&&   value) {
  ...
  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::iterator list<T, Allocator>::insert(const_iterator position) {
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::iterator list<T, Allocator>::insert(const_iterator position, value_type&& value) {
  ...
  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::iterator list<T, Allocator>::erase(const_iterator position) {
    ++position;
  ...
  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::reverse_iterator list<T, Allocator>::erase(const_reverse_iterator position) {
    return reverse_iterator(erase((++position).base()));
  }

  template <typename T, typename Allocator>
  typename list<T, Allocator>::reverse_iterator list<T, Allocator>::erase(const_reverse_iterator first,
  ...
  void list<T, Allocator>::remove(const value_type& value) {
    ...
    while (current.mpNode != &mNode) {
      if (EASTL_LIKELY(!(*current == value)))
        ++current;  // We have duplicate '++current' statements here and below, but the logic here
                    // forces this.
      else {
        ++current;
  ...
  inline void list<T, Allocator>::remove_if(Predicate predicate) {
    for (iterator first((ListNodeBase*)mNode.mpNext), last((ListNodeBase*)&mNode); first != last;) {
      iterator temp(first);
      ++temp;
      if (predicate(first.mpNode->mValue)) DoErase((ListNodeBase*)first.mpNode);
      first = temp;
    }
  }
...
#else
      if (!x.empty()) ((ListNodeBase*)position.mpNode)->splice((ListNodeBase*)x.mNode.mpNext, (ListNodeBase*)&x.mNode);
#endif
    } else {
  ...
  template <typename T, typename Allocator>
  inline void list<T, Allocator>::splice(const_iterator position, this_type& x, const_iterator first,
                                         const_iterator last) {
    if (mAllocator == x.mAllocator) {
#if EASTL_LIST_SIZE_CACHE
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Allocator>
  inline void list<T, Allocator>::splice(const_iterator position, list<T, Allocator>&& x, const_iterator first,
                                         const_iterator last) {
    return splice(position, x, first, last);  // This will call splice(const_iterator, const
  ...
  void list<T, Allocator>::merge(this_type& x) {
    if (this != &x) {
      ...
      if (firstX != lastX) splice(last, x, firstX, lastX);
    }
  }
  ...
  template <typename T, typename Allocator>
  template <typename Compare>
  typename list<T, Allocator>::iterator list<T, Allocator>::DoSort(iterator i1, iterator end2, size_type n,
                                                                   Compare& compare) {
    ...
    switch (n) {
      ...
      case 3: {
        ...
        for (iterator current = i1.next(); current != end2; ++current) {
          if (compare(*current, *lowest)) lowest = current;
        }
        ...
        if (compare(*--end2, *i1))  // At this point, i1 refers to the second element in this three
                                    // element segment.
        {
          end2.mpNode->remove();
          end2.mpNode->insert(i1.mpNode);
    ...
    iterator i2    = DoSort(end1, end2, n - nMid,
    ...
    if (compare(*i2, *i1)) {
      ...
      iterator ix = i2.next();
      while ((ix != end2) && compare(*ix, *i1)) ++ix;

      // Cut out the initial segment of the second list and move it to be in front of the first
      // list.
    ...
    // Merge the two segments. We do this by merging the second sub-segment into the first, by
    // walking forward in each of the two sub-segments.
    for (++i1; (i1 != end1) && (i2 != end2); ++i1)  // while still working on either segment...
    {
      if (compare(*i2, *i1))  // If i2 is less than i1 and it needs to be merged in front of i1...
      {
        ...
        // Cut this section of the i2 sub-segment out and merge into the appropriate place in the i1
        // list.
        ListNodeBase* i2Cut     = i2.mpNode;
        ListNodeBase* i2CutLast = ix.mpNode->mpPrev;
        if (end1 == i2) end1 = ix;
  ...
  template <typename T, typename Allocator>
  template <typename... Args>
  inline typename list<T, Allocator>::node_type* list<T, Allocator>::DoCreateNode(Args&&... args) {
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::node_type* list<T, Allocator>::DoCreateNode(value_type&& value) {
    node_type* const pNode = DoAllocateNode();
  ...
  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::node_type* list<T, Allocator>::DoCreateNode(const value_type& value) {
    node_type* const pNode = DoAllocateNode();
  ...
  inline void list<T, Allocator>::DoSwap(this_type& x) {
    ListNodeBase::swap(
        (ListNodeBase&)mNode,
        (ListNodeBase&)x.mNode);            // We need to implement a special swap because we can't do a shallow swap.
