...
EA_DISABLE_SN_WARNING(828);  // The EDG SN compiler has a bug in its handling of variadic template arguments and
                             // mistakenly reports "parameter "args" was never referenced"
...
#define EASTL_SLIST_DEFAULT_NAME \
  EASTL_DEFAULT_NAME_PREFIX " slist"  // Unless the user overrides something, this is "EASTL slist".
#endif
...
#define EASTL_SLIST_DEFAULT_ALLOCATOR allocator_type(EASTL_SLIST_DEFAULT_NAME)
...
#else
    typedef SListNodeBase base_node_type;  // We use SListNodeBase instead of SListNode<T> because
                                           // we don't want to create a T.
#endif
  ...
  ///
  /// -- size() is O(n) --
  /// Note that as of this writing, list::size() is an O(n) operation when EASTL_SLIST_SIZE_CACHE is
  /// disabled. That is, getting the size of the list is not a fast operation, as it requires
  /// traversing the list and counting the nodes. We could make list::size() be fast by having a
  /// member mSize variable. There are reasons for having such functionality and reasons for not
  /// having such functionality. We currently choose to not have a member mSize variable as it would
  /// add four bytes to the class, add a tiny amount of processing to functions such as insert and
  /// erase, and would only serve to improve the size function, but no others. The alternative
  /// argument is that the C++ standard states that std::list should be an O(1) operation (i.e. have
  /// a member size variable), most C++ standard library list implementations do so, the size is but
  /// an integer which is quick to update, and many users expect to have a fast size function. The
  /// EASTL_SLIST_SIZE_CACHE option changes this. To consider: Make size caching an optional
  /// template parameter.
  ///
  /// Pool allocation
  ...
  ///     WidgetList myList(&myPool);                            // Create a list that uses the
  ///     pool.
  ///
  ...
  class slist : public SListBase<T, Allocator> {
   ...
   public:
    ...
    // Returns an iterator pointing to the last inserted element, or position if insertion count is
    // zero.
    ...
    // be slow. It's recommended that the splice_after functions be used whenever possible as they
    // are O(1).
    ...
    void splice_after(const_iterator position, const_iterator before_first,
                      const_iterator before_last);  // before_first and before_last come from a source container.
  ...
  inline SListNodeBase* SListNodeGetPrevious(SListNodeBase* pNodeBase, const SListNodeBase* pNode) {
    while (pNodeBase && (pNodeBase->mpNext != pNode)) pNodeBase = pNodeBase->mpNext;
    return pNodeBase;
  }
  ...
  template <typename T, typename Pointer, typename Reference>
  inline SListIterator<T, Pointer, Reference>::SListIterator() : mpNode(NULL) {
    // Empty
  }

  template <typename T, typename Pointer, typename Reference>
  inline SListIterator<T, Pointer, Reference>::SListIterator(const SListNodeBase* pNode) :
      mpNode(static_cast<node_type*>(
          (SListNode<T>*)const_cast<SListNodeBase*>(pNode)))  // All this casting is in the name of making runtime
                                                              // debugging much easier on the user.
  {
    // Empty
  }
  ...
  template <typename T, typename Pointer, typename Reference>
  inline typename SListIterator<T, Pointer, Reference>::reference SListIterator<T, Pointer, Reference>::operator*()
      const {
    return mpNode->mValue;
  }

  template <typename T, typename Pointer, typename Reference>
  inline typename SListIterator<T, Pointer, Reference>::pointer SListIterator<T, Pointer, Reference>::operator->()
      const {
    return &mpNode->mValue;
  }

  template <typename T, typename Pointer, typename Reference>
  inline typename SListIterator<T, Pointer, Reference>::this_type& SListIterator<T, Pointer, Reference>::operator++() {
  ...
  template <typename T, typename Pointer, typename Reference>
  inline bool operator!=(const SListIterator<T, Pointer, Reference>& a, const SListIterator<T, Pointer, Reference>& b) {
    return a.mpNode != b.mpNode;
  }
  ...
  template <typename T, typename Allocator>
  inline const typename SListBase<T, Allocator>::allocator_type& SListBase<T, Allocator>::get_allocator() const
      EA_NOEXCEPT {
    return mAllocator;
  }

  template <typename T, typename Allocator>
  inline typename SListBase<T, Allocator>::allocator_type& SListBase<T, Allocator>::get_allocator() EA_NOEXCEPT {
    return mAllocator;
  }

  template <typename T, typename Allocator>
  void SListBase<T, Allocator>::set_allocator(const allocator_type& allocator) {
    EASTL_ASSERT((mAllocator == allocator) ||
  ...
  template <typename T, typename Allocator>
  inline slist<T, Allocator>::slist() : base_type() {
    // Empty
  }

  template <typename T, typename Allocator>
  inline slist<T, Allocator>::slist(const allocator_type& allocator) : base_type(allocator) {
    // Empty
  }

  template <typename T, typename Allocator>
  inline slist<T, Allocator>::slist(size_type n, const allocator_type& allocator) : base_type(allocator) {
  ...
  template <typename T, typename Allocator>
  inline slist<T, Allocator>::slist(const slist& x) : base_type(x.mAllocator) {
    DoInsertAfter((SListNodeBase*)&mNode, const_iterator((SListNodeBase*)x.mNode.mpNext), const_iterator(NULL),
                  false_type());
  }

#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Allocator>
  slist<T, Allocator>::slist(this_type&& x) : base_type(x.mAllocator) {
    swap(x);
  }

  template <typename T, typename Allocator>
  slist<T, Allocator>::slist(this_type&& x, const allocator_type& allocator) : base_type(allocator) {
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::begin() EA_NOEXCEPT {
    return iterator((SListNodeBase*)mNode.mpNext);
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::begin() const EA_NOEXCEPT {
    return const_iterator((SListNodeBase*)mNode.mpNext);
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::cbegin() const EA_NOEXCEPT {
    return const_iterator((SListNodeBase*)mNode.mpNext);
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::end() EA_NOEXCEPT {
    return iterator(NULL);
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::end() const EA_NOEXCEPT {
    return const_iterator(NULL);
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::cend() const EA_NOEXCEPT {
    return const_iterator(NULL);
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::before_begin() EA_NOEXCEPT {
    return iterator((SListNodeBase*)&mNode);
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::before_begin() const EA_NOEXCEPT {
    return const_iterator((SListNodeBase*)&mNode);
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::cbefore_begin() const EA_NOEXCEPT {
    return const_iterator((SListNodeBase*)&mNode);
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::previous(const_iterator position) {
    return iterator(SListNodeGetPrevious((SListNodeBase*)&mNode, (SListNodeBase*)position.mpNode));
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::previous(const_iterator position) const {
  ...
  inline typename slist<T, Allocator>::reference slist<T, Allocator>::front() {
    ...
    if (EASTL_UNLIKELY(mNode.mpNext == NULL)) EASTL_FAIL_MSG("slist::front -- empty container");
#endif
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::const_reference slist<T, Allocator>::front() const {
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY(mNode.mpNext == NULL)) EASTL_FAIL_MSG("slist::front -- empty container");
#endif
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::size_type slist<T, Allocator>::size() const EA_NOEXCEPT {
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::insert(const_iterator position) {
    return iterator((SListNodeBase*)DoInsertValueAfter(
        SListNodeGetPrevious((SListNodeBase*)&mNode, (SListNodeBase*)position.mpNode), value_type()));
  }
  ...
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::insert(const_iterator    position,
    ...
    return iterator((SListNodeBase*)DoInsertValueAfter(
        SListNodeGetPrevious((SListNodeBase*)&mNode, (SListNodeBase*)position.mpNode), value));
  }
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::insert_after(const_iterator position) {
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::insert_after(const_iterator position, size_type n,
                                                                                  const value_type& value) {
  ...
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::insert_after(
    ...
    return iterator(
        (SListNodeBase*)DoInsertAfter((SListNodeBase*)position.mpNode, ilist.begin(), ilist.end(), false_type()));
  }
  ...
  template <typename T, typename Allocator>
  template <class... Args>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::emplace_after(const_iterator position,
                                                                                   Args&&... args) {
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::erase(const_iterator position) {
    return DoEraseAfter(SListNodeGetPrevious((SListNodeBase*)&mNode, (SListNodeBase*)position.mpNode));
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::erase(const_iterator first, const_iterator last) {
    return DoEraseAfter(SListNodeGetPrevious((SListNodeBase*)&mNode, (SListNodeBase*)first.mpNode),
                        (SListNodeBase*)last.mpNode);
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::erase_after(const_iterator position) {
    return iterator(DoEraseAfter((SListNodeBase*)position.mpNode));
  }

  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::erase_after(const_iterator before_first,
                                                                                 const_iterator last) {
  ...
  inline void slist<T, Allocator>::splice(const_iterator position, this_type& x) {
    ...
    // remove it from the source. Until then it's simply disallowed to splice with unequal
    // allocators. EASTL_ASSERT(mAllocator == x.mAllocator); // Disabled because our member sort
    // function uses splice but with allocators that may be unequal. There isn't a simple workaround
    // aside from disabling this assert.
    ...
    if (x.mNode.mpNext)  // If there is anything to splice...
    {
      if (mAllocator == x.mAllocator) {
        SListNodeSpliceAfter(SListNodeGetPrevious((SListNodeBase*)&mNode, (SListNodeBase*)position.mpNode),
                             (SListNodeBase*)&x.mNode, SListNodeGetPrevious((SListNodeBase*)&x.mNode, NULL));
  ...
  template <typename T, typename Allocator>
  inline void slist<T, Allocator>::splice(const_iterator position, this_type& x, const_iterator first,
                                          const_iterator last) {
  ...
  void slist<T, Allocator>::splice(const_iterator position, this_type&& x, const_iterator first, const_iterator last) {
    return splice(position, x, first,
  ...
  template <typename T, typename Allocator>
  inline void slist<T, Allocator>::splice_after(const_iterator position, this_type& x, const_iterator first,
                                                const_iterator last) {
    if (first != last)  // If there is anything to splice...
    {
      if (mAllocator == x.mAllocator) {
        ...
        SListNodeSpliceAfter((SListNodeBase*)position.mpNode, (SListNodeBase*)first.mpNode,
                             (SListNodeBase*)last.mpNode);
      } else {
  ...
  inline void slist<T, Allocator>::splice_after(const_iterator position, this_type&& x, const_iterator first,
                                                const_iterator last) {
    return splice_after(position, x, first, last);  // This will call splice_after(const_iterator,
                                                    // this_type&, const_iterator, const_iterator)
  }
#endif
  ...
  template <typename T, typename Allocator>
  inline void slist<T, Allocator>::splice_after(const_iterator position, const_iterator before_first,
    ...
    if (before_first != before_last)  // If there is anything to splice...
    {
      ...
      for (; i != iEnd; ++i) {
        if (i == before_first) break;
      }

      if (i == iEnd)  // If the input came from an external range...
        mSize +=
            (size_type)eastl::distance(before_first,
                                       before_last);  // Note that we have no way of knowing how to decrementing the
                                                      // size from the external container, assuming it came from one.
      else {
        EASTL_FAIL_MSG(
            "slist::splice_after: Impossible to decrement source mSize. Use the other splice_after "
            "function instead.");
      }
#endif

      // Insert the range of [before_first + 1, before_last + 1) after position.
      SListNodeSpliceAfter((SListNodeBase*)position.mpNode, (SListNodeBase*)before_first.mpNode,
  ...
  inline void slist<T, Allocator>::splice_after(const_iterator position, const_iterator previous) {
    ...
    if (i == iEnd)  // If the input came from an external range...
      ++mSize;      // Note that we have no way of knowing how to decrementing the size from the
                    // external container, assuming it came from one.
    else {
      EASTL_FAIL_MSG(
          "slist::splice_after: Impossible to decrement source mSize. Use the other splice_after "
          "function instead.");
    ...
    SListNodeSpliceAfter((SListNodeBase*)position.mpNode, (SListNodeBase*)previous.mpNode,
  ...
  template <typename T, typename Allocator>
  template <typename... Args>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoCreateNode(Args&&... args) {
...
#if EASTL_MOVE_SEMANTICS_ENABLED
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoCreateNode(value_type&& value) {
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoCreateNode(const value_type& value) {
    node_type* const pNode = DoAllocateNode();
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoCreateNode() {
    node_type* const pNode = DoAllocateNode();
#if EASTL_EXCEPTIONS_ENABLED
    try {
  ...
  template <typename T, typename Allocator>
  template <typename InputIterator>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoInsertAfter(SListNodeBase* pNode,
                                                                                     InputIterator  first,
                                                                                     InputIterator  last) {
  ...
  template <typename T, typename Allocator>
  template <typename Integer>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoInsertAfter(SListNodeBase* pNode, Integer n,
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoInsertValueAfter(SListNodeBase* pNode) {
#if EASTL_SLIST_SIZE_CACHE
    ...
    return static_cast<node_type*>(
        (base_node_type*)SListNodeInsertAfter((SListNodeBase*)pNode, (SListNodeBase*)DoCreateNode()));
#endif
  }
  ...
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoInsertValueAfter(SListNodeBase* pNode,
    ...
    ++mSize;  // Increment the size after the node creation because we need to assume an exception
              // can occur in the creation.
#endif
    return static_cast<node_type*>((base_node_type*)pNode);
  }
#else
  ...
  template <typename T, typename Allocator>
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoInsertValuesAfter(SListNodeBase*    pNode,
                                                                                           size_type         n,
                                                                                           const value_type& value) {
    for (size_type i = 0; i < n; ++i) {
      pNode = SListNodeInsertAfter((SListNodeBase*)pNode, (SListNodeBase*)DoCreateNode(value));
#if EASTL_SLIST_SIZE_CACHE
      ++mSize;  // We don't do a single mSize += n at the end because an exception may result in
                // only a partial range insertion.
#endif
    }
    return static_cast<node_type*>((base_node_type*)pNode);
  }
  ...
  inline bool slist<T, Allocator>::validate() const {
    ...
    for (const_iterator i(begin()), iEnd(end()); i != iEnd; ++i) ++n;
  ...
  class insert_iterator<slist<T, Allocator> > {
   ...
   public:
    insert_iterator(Container& x, iterator_type i) : container(x) {
      if (i == x.begin())
        it = x.before_begin();
      else
        it = x.previous(i);
