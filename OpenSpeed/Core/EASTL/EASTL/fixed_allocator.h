   ...
   public:
    ...
    void* allocate(size_t n, int /*flags*/ = 0) {
      ...
      if (pLink)  // If we have space...
      {
        ...
        // EASTL_ASSERT(false); To consider: enable this assert. However, we intentionally disable it because this isn't
        // necessarily an assertable error.
        return NULL;
      }
    }
  ...
  class EASTL_API fixed_allocator_with_overflow : public fixed_pool_base {
   public:
    ...
    fixed_allocator_with_overflow(const char* pName = EASTL_FIXED_POOL_DEFAULT_NAME) :
        fixed_pool_base(NULL), mOverflowAllocator(pName), mpPoolBegin(nullptr), mpPoolEnd(nullptr), mnNodeSize(0) {}
    ...
    fixed_allocator_with_overflow(const fixed_allocator_with_overflow&) :
        fixed_pool_base(NULL), mpPoolBegin(nullptr), mpPoolEnd(nullptr), mnNodeSize(0) {}
   ...
   protected:
    EASTLAllocatorType mOverflowAllocator;  // To consider: Allow the user to define the type of this, presumably via a
                                            // template parameter.
    void* mpPoolBegin;  // To consider: We have these member variables and ideally we shouldn't need them. The problem
                        // is that
    void* mpPoolEnd;    //              the information about the pool buffer and object size is stored in the owning
                        //              container
    eastl_size_t mnNodeSize;  //              and we can't have access to it without increasing the amount of code we
                              //              need and by templating more code. It may turn out that simply storing data
                              //              here is smaller in the end.
  };  // fixed_allocator_with_overflow         //              Granted, this class is usually used for debugging
      // purposes, but perhaps there is an elegant solution.
