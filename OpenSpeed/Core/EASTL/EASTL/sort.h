...
//    is_sorted             --
//    sort                  -- Unstable.    The implementation of this is mapped to quick_sort by
//    default. quick_sort            -- Unstable.    This is actually an intro-sort (quick sort with
//    switch to insertion sort). tim_sort              -- Stable. tim_sort_buffer       -- Stable.
//    partial_sort          -- Unstable.
//    insertion_sort        -- Stable.
//    shell_sort            -- Unstable.
//    heap_sort             -- Unstable.
//    stable_sort           -- Stable.      The implementation of this is simply mapped to
//    merge_sort. merge                 -- merge_sort            -- Stable. merge_sort_buffer     --
//    Stable. nth_element           -- Unstable. radix_sort            -- Stable.      Important and
//    useful sort for integral data, and faster than all others for this. comb_sort             --
//    Unstable.    Possibly the best combination of small code size but fast sort. bubble_sort --
//    Stable.      Useful in practice for sorting tiny sets of data (<= 10 elements).
...
//    paritition            This would be like the std STL version. This is not categorized as a
//    sort routine by the language standard. stable_partition      This would be like the std STL
//    version. counting_sort         Maybe we don't want to implement this.
//
//////////////////////////////////////////////////////////////////////////////
...
#include <OpenSpeed/Core/EASTL/EASTL/heap.h>
...
namespace eastl {
  ...
  /// ascending order. The range between first and the iterator returned is sorted. If the entire
  /// range is sorted, the function returns last. The elements are compared using operator< for the
  /// first version, and comp for the second.
  ///
  /// Example usage:
  ///     vector<int> intArray;
  ///     vector<int>::iterator unsorted_element = is_sorted_until(eastl::end(intArray),
  ///     eastl::end(intArray)); vector<int>::iterator unsorted_element_with_user_compare =
  ///     is_sorted_until(eastl::end(intArray), eastl::end(intArray), eastl::less<int>());
  ///
  template <typename ForwardIterator>
  ...
  inline OutputIterator merge(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2,
                              OutputIterator result) {
    typedef eastl::less<typename eastl::iterator_traits<InputIterator1>::value_type> Less;

    return eastl::merge<InputIterator1, InputIterator2, OutputIterator, Less>(first1, last1, first2, last2, result,
                                                                              Less());
  }
  ...
  void insertion_sort(BidirectionalIterator first, BidirectionalIterator last, StrictWeakOrdering compare) {
    ...
    if (first != last)  // if the range is non-empty...
    {
      ...
      for (++iSorted; iSorted != last; ++iSorted) {
        ...
        // but std STL iterators may have a problem with it. Dinkumware STL, for example, will
        // assert. To do: Fix this loop to not decrement like so.
...
{                                                                                                    \
        ...
        {                                                                                            \
                ...
                {                                                                                    \
                        const typename eastl::iterator_traits<RandomAccessIterator>::value_type      \
value(*i);                                                                                           \
                                                                                                   \ \
                        if(compare(value, *first))                                                   \
                        {                                                                            \
                                EASTL_VALIDATE_COMPARE(!compare(*first, value)); // Validate that    \
the compare function is sane. eastl::copy_backward(first, i, i + 1); *first = value;                 \
                        }                                                                            \
                        else                                                                         \
                        {                                                                            \
                                ...
                                {                                                                    \
                                        EASTL_VALIDATE_COMPARE(!compare(*prev, value)); //           \
Validate that the compare function is sane. *end = *prev;                                            \
                                }                                                                    \
                                                                                                   \ \
                                *end = value;                                                        \
                        }                                                                            \
...
value(*i);                                                                                           \
                        ...
                        if(value < *first)                                                           \
                        {                                                                            \
                                EASTL_VALIDATE_COMPARE(!(*first < value)); // Validate that the      \
compare function is sane. eastl::copy_backward(first, i, i + 1); *first = value;                     \
                        }                                                                            \
                        else                                                                         \
                        {                                                                            \
                                RandomAccessIterator end(i), prev(i);                                \
                                ...
                                {                                                                    \
                                        EASTL_VALIDATE_COMPARE(!(*prev < value)); // Validate that   \
the compare function is sane. *end = *prev;                                                          \
                                }                                                                    \
                                                                                                   \ \
                                *end = value;                                                        \
                        }                                                                            \
  ...
  void shell_sort(RandomAccessIterator first, RandomAccessIterator last, StrictWeakOrdering compare) {
    ...
    if (first != last) {
      ...
      while (nSpace < nSize)
        nSpace = (nSpace * 3) + 1;  // This is the Knuth 'h' sequence: 1, 4, 13, 40, 121, 364, 1093, 3280, 9841, 29524,
                                    // 88573, 265720, 797161, 2391484, 7174453, 21523360, 64570081, 193710244,
      ...
      for (nSpace = (nSpace - 1) / 3; nSpace >= 1; nSpace = (nSpace - 1) / 3)  // Integer division is less than ideal.
      {
        for (difference_type i = 0; i < nSpace; i++) {
          ...
          for (iSorted = iInsertFirst + nSpace; iSorted < last; iSorted += nSpace) {
            ...
            for (iBack -= nSpace; (iCurrent != iInsertFirst) && compare(*iCurrent, *iBack);
  ...
  void merge_sort_buffer(RandomAccessIterator first, RandomAccessIterator last, T* pBuffer,
    ...
    if (nCount > 1) {
      ...
      if (nMid > 1) {
        ...
        eastl::merge<RandomAccessIterator, RandomAccessIterator, T*, StrictWeakOrdering>(first, part, part, half,
                                                                                         pBuffer, compare);
      } else
        *pBuffer = *first;

      if ((nCount - nMid) > 1) {
        const difference_type nQ3(
            (difference_type)(((size_t)nMid + (size_t)nCount) >> 1));  // Equivalent to (nQ3 = (nMid + nCount) / 2)
                                                                       // but handles the case of integer rollover.
        ...
        eastl::merge<RandomAccessIterator, RandomAccessIterator, T*, StrictWeakOrdering>(half, part, part, last,
                                                                                         pBuffer + nMid, compare);
      } else
        *(pBuffer + nMid) = *half;

      eastl::merge<T*, T*, RandomAccessIterator, StrictWeakOrdering>(pBuffer, pBuffer + nMid, pBuffer + nMid,
                                                                     pBuffer + nCount, first, compare);
    }
  }
  ...
  void merge_sort(RandomAccessIterator first, RandomAccessIterator last, Allocator& allocator,
    ...
    if (nCount > 1) {
      ...
      value_type* const pBuffer =
          (value_type*)allocate_memory(allocator, nCount * sizeof(value_type), EASTL_ALIGN_OF(value_type), 0);
      eastl::uninitialized_fill(pBuffer, pBuffer + nCount, value_type());

      eastl::merge_sort_buffer<RandomAccessIterator, value_type, StrictWeakOrdering>(first, last, pBuffer, compare);
  ...
  static const int kQuickSortLimit = 28;  // For sorts of random arrays over 100 items, 28 - 32 have
                                          // been found to be good numbers on x86.
#else
  static const int kQuickSortLimit = 16;  // It seems that on other processors lower limits are more
                                          // beneficial, as they result in fewer compares.
#endif
  ...
  namespace Internal {
    ...
    inline void insertion_sort_simple(RandomAccessIterator first, RandomAccessIterator last) {
      for (RandomAccessIterator current = first; current != last; ++current) {
        ...
        for (--prev; value < *prev; --end, --prev)  // We skip checking for (prev >= first) because quick_sort (our
                                                    // caller) makes this unnecessary.
        {
    ...
    inline void insertion_sort_simple(RandomAccessIterator first, RandomAccessIterator last, Compare compare) {
      for (RandomAccessIterator current = first; current != last; ++current) {
        ...
        for (--prev; compare(value, *prev); --end, --prev)  // We skip checking for (prev >= first) because quick_sort
                                                            // (our caller) makes this unnecessary.
        {
  ...
  inline void nth_element(RandomAccessIterator first, RandomAccessIterator nth, RandomAccessIterator last,
    ...
    while ((last - first) > 5) {
      const value_type midValue(
          eastl::median<value_type, Compare>(*first, *(first + (last - first) / 2), *(last - 1), compare));
      const RandomAccessIterator midPos(
          eastl::get_partition<RandomAccessIterator, value_type, Compare>(first, last, midValue, compare));
  ...
  namespace Internal {
    ...
    inline void quick_sort_impl(RandomAccessIterator first, RandomAccessIterator last, Size kRecursionCount) {
      ...
      while (((last - first) > kQuickSortLimit) && (kRecursionCount > 0)) {
        const RandomAccessIterator position(eastl::get_partition<RandomAccessIterator, value_type>(
            first, last, eastl::median<value_type>(*first, *(first + (last - first) / 2), *(last - 1))));

        eastl::Internal::quick_sort_impl<RandomAccessIterator, Size>(position, last, --kRecursionCount);
        last = position;
      }

      if (kRecursionCount == 0) eastl::partial_sort<RandomAccessIterator>(first, last, last);
    }
    ...
    inline void quick_sort_impl(RandomAccessIterator first, RandomAccessIterator last, Size kRecursionCount,
      ...
      while (((last - first) > kQuickSortLimit) && (kRecursionCount > 0)) {
        const RandomAccessIterator position(eastl::get_partition<RandomAccessIterator, value_type, Compare>(
            first, last,
            eastl::median<value_type, Compare>(*first, *(first + (last - first) / 2), *(last - 1), compare), compare));

        eastl::Internal::quick_sort_impl<RandomAccessIterator, Size, Compare>(position, last, --kRecursionCount,
                                                                              compare);
        last = position;
      }

      if (kRecursionCount == 0) eastl::partial_sort<RandomAccessIterator, Compare>(first, last, last, compare);
    }
  }  // namespace Internal
  ...
  void quick_sort(RandomAccessIterator first, RandomAccessIterator last) {
    ...
    if (first != last) {
      eastl::Internal::quick_sort_impl<RandomAccessIterator, difference_type>(first, last,
                                                                              2 * Internal::Log2(last - first));
  ...
  void quick_sort(RandomAccessIterator first, RandomAccessIterator last, Compare compare) {
    ...
    if (first != last) {
      eastl::Internal::quick_sort_impl<RandomAccessIterator, difference_type, Compare>(
...
#define EASTL_COUNT_LEADING_ZEROES eastl_count_leading_zeroes
    ...
    // To consider: Use void eastl::reverse(BidirectionalIterator first, BidirectionalIterator
    // last);
    //
    template <typename RandomAccessIterator>
      ...
      if ((size - start) > 1)  // If there is anything in the set...
      {
        ...
        } else  // Else it is decreasing.
        {
          for (;; ++curr) {
            ...
            if (!compare(*(first + curr),
                         *(first + curr - 1)))  // If this item is not in order... this run is done.
              break;  // Note that we intentionally compare against <= 0 and not just < 0. This is
                      // because
          }           // The reverse_elements call below could reverse two equal elements and break our
    ...
    void tim_sort_merge(RandomAccessIterator first, const tim_sort_run* run_stack, const intptr_t stack_curr,
      ...
      if (A < B)  // If the first run is shorter than the second run... merge left.
      {
        ...
        for (intptr_t i = 0; i < A; i++) *(first + curr + i) = value_type();
#endif
    ...
    static inline bool timsort_check_invariant(tim_sort_run* run_stack, const intptr_t stack_curr) {
      ...
      if (stack_curr > 2) {
        ...
        if ((A <= (B + C)) || (B <= C)) return true;  // Merge the right-most runs.
        ...
        if (A <= B) return true;  // Merge the right-most runs.
      }
    ...
    template <typename RandomAccessIterator, typename T, typename StrictWeakOrdering>
    intptr_t tim_sort_collapse(RandomAccessIterator first, tim_sort_run* run_stack, intptr_t stack_curr, T* pBuffer,
                               const intptr_t size, StrictWeakOrdering compare) {
    ...
    bool tim_sort_add_run(tim_sort_run* run_stack, RandomAccessIterator first, T* pBuffer, const intptr_t size,
                          const intptr_t minrun, intptr_t& len, intptr_t& run, intptr_t& curr, intptr_t& stack_curr,
                          StrictWeakOrdering compare) {
      len = tim_sort_count_run(first, curr, size,
                               compare);  // This will count the length of the run and reverse the
                                          // run if it is backwards.
      run = minrun;

      if (run < minrun)  // Always make runs be of minrun length (we'll sort the additional data as
                         // needed below)
        run = minrun;
      ...
      if (run > len)  // If there is any additional data we want to sort to bring up the run length
                      // to minrun.
      {
  ...
  // This sort is the fastest sort when sort stability (maintaining order of equal values) is
  // required and data sets are non-trivial (size >= 15). It's also the fastest sort (e.g. faster
  // than quick_sort) for the case that at at least half your data is already sorted. Otherwise,
  // eastl::quick_sort is about 10% faster than tim_sort_buffer but is not a stable sort. There are
  // some reports that tim_sort outperforms quick_sort but most of these aren't taking into account
  // that optimal quick_sort implementations use a hybrid approach called "introsort"
  // (http://en.wikipedia.org/wiki/Introsort) which improves quick_sort considerably in practice.
  //
  // Strengths:
  ...
  //
  // Weaknesses:
  //     - User must provide a scratch buffer, otherwise the buffer is dynamically allocated during
  //     runtime.
  //     - Not as fast as quick_sort for the general case of randomized data.
  //     - Requires a RandomAccessIterator; thus must be on an array container type and not a list
  //     container type.
  //     - Uses a lot of code to implement; thus it's not great when there is little room for more
  //     code.
  //
  // The pBuffer parameter must hold at least ((last-first)/2) elements (i.e. half the elements of
  // the container). This minimum size is a worst-case size requirement, but handles all possible
  // cases. pBuffer is just a scratch buffer and is not needed after the return of this function,
  // and doesn't need to be seeded with any particular values upon entering this function.
  //
  // Example usage:
  //     int intArray[64];
  //     int buffer[32];
  ...
  void tim_sort_buffer(RandomAccessIterator first, RandomAccessIterator last, T* pBuffer, StrictWeakOrdering compare) {
    ...
    if (size < 64)
      ...
      if (tim_sort_add_run(run_stack, first, pBuffer, size, minrun, len, run, curr, stack_curr, compare)) return;
      if (tim_sort_add_run(run_stack, first, pBuffer, size, minrun, len, run, curr, stack_curr, compare)) return;
      if (tim_sort_add_run(run_stack, first, pBuffer, size, minrun, len, run, curr, stack_curr, compare)) return;
      ...
      for (;;) {
        if (timsort_check_invariant(run_stack, stack_curr))
          ...
          if (tim_sort_add_run(run_stack, first, pBuffer, size, minrun, len, run, curr, stack_curr, compare)) break;
        }
      }
    }
  }
  ...
  ///     Element buffer[100];
  ///
  ///     radix_sort<Element*, extract_radix_key<Element> >(elementArray, elementArray + 100,
  ///     buffer);
  ///
  ...
  namespace Internal {
    ...
    void radix_sort_impl(RandomAccessIterator first, RandomAccessIterator last, RandomAccessIterator buffer,
                         ExtractKey extractKey, uint8_t) {
      uint32_t EA_PREFIX_ALIGN(EASTL_PLATFORM_PREFERRED_ALIGNMENT)
          bucketSize[256];  // The alignment of this variable isn't required; it merely
      uint32_t EA_PREFIX_ALIGN(EASTL_PLATFORM_PREFERRED_ALIGNMENT)
          bucketPosition[256];  // allows the code below to be faster on some platforms.
      RandomAccessIterator temp;
      uint32_t             i;

      memset(bucketSize, 0, sizeof(bucketSize));

      for (temp = first; temp != last; ++temp) ++bucketSize[extractKey(*temp)];

      for (bucketPosition[0] = 0, i = 0; i < 255; i++) bucketPosition[i + 1] = bucketPosition[i] + bucketSize[i];
    ...
    template <typename RandomAccessIterator, typename ExtractKey>
    void radix_sort_impl(RandomAccessIterator first, RandomAccessIterator last, RandomAccessIterator buffer,
                         ExtractKey extractKey, uint16_t) {
      uint32_t EA_PREFIX_ALIGN(EASTL_PLATFORM_PREFERRED_ALIGNMENT)
          bucketSize[256];  // The alignment of this variable isn't required; it merely
      uint32_t EA_PREFIX_ALIGN(EASTL_PLATFORM_PREFERRED_ALIGNMENT)
          bucketPosition[256];  // allows the code below to be faster on some platforms.
      ...
      for (temp = first; temp != last; ++temp) ++bucketSize[extractKey(*temp) & 0xff];
      ...
      for (temp = buffer, last = buffer + (last - first); temp != last; ++temp) ++bucketSize[extractKey(*temp) >> 8];
    ...
    void radix_sort_impl(RandomAccessIterator first, RandomAccessIterator last, RandomAccessIterator buffer,
                         ExtractKey extractKey, IntegerType) {
      uint32_t EA_PREFIX_ALIGN(EASTL_PLATFORM_PREFERRED_ALIGNMENT)
          bucketSize[256];  // The alignment of this variable isn't required; it merely
      uint32_t EA_PREFIX_ALIGN(EASTL_PLATFORM_PREFERRED_ALIGNMENT)
          bucketPosition[256];  // allows the code below to be faster on some platforms.
      ...
      for (uint32_t j = 0; j < (8 * sizeof(IntegerType)); j += 8) {
        ...
        for (temp = first; temp != last; ++temp) ++bucketSize[(extractKey(*temp) >> j) & 0xff];

        bucketPosition[0] = 0;
        for (i = 0; i < 255; i++) bucketPosition[i + 1] = bucketPosition[i] + bucketSize[i];
  ...
  void radix_sort(RandomAccessIterator first, RandomAccessIterator last, RandomAccessIterator buffer) {
    eastl::Internal::radix_sort_impl<RandomAccessIterator>(first, last, buffer, ExtractKey(),
                                                           typename ExtractKey::radix_type());
  }
  ...
  /// If a default sort function is specified then call it, otherwise use EASTL's default
  /// quick_sort. EASTL_DEFAULT_SORT_FUNCTION must be namespace-qualified and include any necessary
  /// template parameters (e.g. eastl::comb_sort instead of just comb_sort), and it must be visible
  /// to this code. The EASTL_DEFAULT_SORT_FUNCTION must be provided in two versions:
  ///     template <typename RandomAccessIterator>
  ...
  /// If a default sort function is specified then call it, otherwise use EASTL's default
  /// merge_sort. EASTL_DEFAULT_STABLE_SORT_FUNCTION must be namespace-qualified and include any
  /// necessary template parameters (e.g. eastl::tim_sort instead of just tim_sort), and it must be
  /// visible to this code. The EASTL_DEFAULT_STABLE_SORT_FUNCTION must be provided in three
  /// versions, though the third allocation implementation may choose to ignore the allocator
  /// parameter:
  ///     template <typename RandomAccessIterator, typename StrictWeakOrdering>
  ///     void EASTL_DEFAULT_STABLE_SORT_FUNCTION(RandomAccessIterator first, RandomAccessIterator
  ...
  ///     template <typename RandomAccessIterator, typename Allocator, typename StrictWeakOrdering>
  ///     void EASTL_DEFAULT_STABLE_SORT_FUNCTION(RandomAccessIterator first, RandomAccessIterator
  ///     last, Allocator& allocator, StrictWeakOrdering compare);
  ///
  template <typename RandomAccessIterator, typename StrictWeakOrdering>
...
#else
    eastl::merge_sort<RandomAccessIterator, EASTLAllocatorType, StrictWeakOrdering>(first, last,
                                                                                    *get_default_allocator(0), compare);
#endif
  }
...
#else
    eastl::merge_sort<RandomAccessIterator, EASTLAllocatorType>(first, last, *get_default_allocator(0));
#endif
  }
  ...
  // This is not defined because it would cause compiler errors due to conflicts with a version
  // above.
  // template <typename RandomAccessIterator, typename Allocator>
  // void stable_sort(RandomAccessIterator first, RandomAccessIterator last, Allocator& allocator)
  //{
  //    #if defined(EASTL_DEFAULT_STABLE_SORT_FUNCTION)
  //        EASTL_DEFAULT_STABLE_SORT_FUNCTION<RandomAccessIterator, Allocator>(first, last,
  //        allocator);
  //    #else
  ...
  void small_footprint_sort(ContiguousIterator first, ContiguousIterator last)
  {
          ...
          qsort(first, (size_t)eastl::distance(first, last), sizeof(value_type),
  small_footprint_sort_func<value_type>);
  }
  */
