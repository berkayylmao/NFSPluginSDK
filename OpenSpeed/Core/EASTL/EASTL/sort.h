...
//    radix_sort            -- Stable.      Important and useful sort for integral data, and faster than all others for
//    this. comb_sort             -- Unstable.    Possibly the best combination of small code size but fast sort.
...
//    paritition            This would be like the std STL version. This is not categorized as a sort routine by the
//    language standard. stable_partition      This would be like the std STL version. counting_sort         Maybe we
...
namespace eastl {
  ...
  ///     vector<int>::iterator unsorted_element_with_user_compare = is_sorted_until(eastl::end(intArray),
  ///     eastl::end(intArray), eastl::less<int>());
  ///
  template <typename ForwardIterator>
  ...
  void insertion_sort(BidirectionalIterator first, BidirectionalIterator last, StrictWeakOrdering compare) {
    ...
    if (first != last) {
      ...
      for (++i; i != last; ++i) {
        ...
        for (BidirectionalIterator movePosition = i; movePosition != first && compare(insertValue, *(--movePosition));
             --insertPosition) {
  ...
  template <typename RandomAccessIterator, typename T, typename StrictWeakOrdering, typename difference_type,
  ...
  class MergeSorter {
   ...
   private:
    ...
    // sort_impl
    //
    // This sort routine sorts the data in [first, last) and places the result in pBuffer or in the original range of
    // the input.  The actual location of the data is indicated by the enum returned.
    //
    // lastSortedEnd is used to specify a that data in the range [first, first + lastSortedEnd] is already sorted.  This
    // information is used to avoid unnecessary merge sorting of already sorted data.  lastSortedEnd is a hint, and can
    // be an under estimate of the sorted elements (i.e. it is legal to pass 0).
    static ResultLocation sort_impl(RandomAccessIterator first, RandomAccessIterator last, T* pBuffer,
      ...
      if (lastSortedEnd < nCount) {
        ...
        if (nCount <= InsertionSortLimit) {
          eastl::Internal::insertion_sort_already_started<RandomAccessIterator, StrictWeakOrdering>(
    ...
    // The inputs to this method effectively define two large buffers.  The variables 'firstHalfLocation' and
    // 'secondHalfLocation' define where the data to be merged is located within the two buffers.  It is entirely
    // possible that the two areas to be merged could be entirely located in either of the larger buffers. Upon
    // returning the merged results will be in one of the two buffers (indicated by the return result).
    static ResultLocation merge_halves(RandomAccessIterator first, RandomAccessIterator last, difference_type nMid,
                                       T* pBuffer, ResultLocation firstHalfLocation, ResultLocation secondHalfLocation,
    ...
    for (;; ++first) {
      while (eastl::less<PureT>()(*first, pivotValue)) {
        EASTL_VALIDATE_COMPARE(
            !eastl::less<PureT>()(pivotValue, *first));  // Validate that the compare function is sane.
        ++first;
      }
      --last;
      ...
      while (eastl::less<PureT>()(pivotValue, *last)) {
        EASTL_VALIDATE_COMPARE(
            !eastl::less<PureT>()(*last, pivotValue));  // Validate that the compare function is sane.
        --last;
      }
  ...
  inline RandomAccessIterator get_partition(RandomAccessIterator first, RandomAccessIterator last, T&& pivotValue) {
    // Note: unlike the copy-constructible variant of get_partition... we can't create a temporary const
  ...
  namespace Internal {
    ...
    inline void insertion_sort_simple(RandomAccessIterator first, RandomAccessIterator last) {
      for (RandomAccessIterator current = first; current != last; ++current) {
        ...
        for (--prev; eastl::less<value_type>()(value, *prev);
             --end,
             --prev)  // We skip checking for (prev >= first) because quick_sort (our caller) makes this unnecessary.
        {
          EASTL_VALIDATE_COMPARE(
              !eastl::less<value_type>()(*prev, value));  // Validate that the compare function is sane.
  ...
  namespace Internal {
      ...
      while (((last - first) > kQuickSortLimit) && (kRecursionCount > 0)) {
        const RandomAccessIterator position(eastl::get_partition<RandomAccessIterator, value_type>(
            ...
            eastl::forward<PivotValueType>(eastl::median<value_type>(
                eastl::forward<value_type>(*first), eastl::forward<value_type>(*(first + (last - first) / 2)),
                eastl::forward<value_type>(*(last - 1))))));

        eastl::Internal::quick_sort_impl_helper<RandomAccessIterator, Size, PivotValueType>(position, last,
                                                                                            --kRecursionCount);
        last = position;
      }
    ...
    inline void quick_sort_impl_helper(RandomAccessIterator first, RandomAccessIterator last, Size kRecursionCount,
      ...
      while (((last - first) > kQuickSortLimit) && (kRecursionCount > 0)) {
        ...
        eastl::Internal::quick_sort_impl_helper<RandomAccessIterator, Size, Compare, PivotValueType>(
    ...
    inline void quick_sort_impl(RandomAccessIterator first, RandomAccessIterator last, Size kRecursionCount,
                                typename eastl::enable_if<eastl::is_copy_constructible<
                                    typename iterator_traits<RandomAccessIterator>::value_type>::value>::type* = 0) {
      typedef typename iterator_traits<RandomAccessIterator>::value_type value_type;
    ...
    inline void quick_sort_impl(
        ...
        typename eastl::enable_if<
            eastl::is_move_constructible<typename iterator_traits<RandomAccessIterator>::value_type>::value &&
            !eastl::is_copy_constructible<typename iterator_traits<RandomAccessIterator>::value_type>::value>::type* =
            0) {
      typedef typename iterator_traits<RandomAccessIterator>::value_type value_type;
    ...
    inline void quick_sort_impl(RandomAccessIterator first, RandomAccessIterator last, Size kRecursionCount,
      ...
      // copy constructors require const value_type
      quick_sort_impl_helper<RandomAccessIterator, Size, Compare, const value_type>(first, last, kRecursionCount,
                                                                                    compare);
    }
...
#define EASTL_COUNT_LEADING_ZEROES eastl_count_leading_zeroes
    ...
    intptr_t tim_sort_collapse(RandomAccessIterator first, tim_sort_run* run_stack, intptr_t stack_curr, T* pBuffer,
      ...
      while (stack_curr > 1) {
        ...
        {
          if (A < C) {
            tim_sort_merge<RandomAccessIterator, T, StrictWeakOrdering>(first, run_stack, stack_curr - 1, pBuffer,
                                                                        compare);
            ...
            tim_sort_merge<RandomAccessIterator, T, StrictWeakOrdering>(first, run_stack, stack_curr, pBuffer,
                                                                        compare);  // Merge B and C.
    ...
    bool tim_sort_add_run(tim_sort_run* run_stack, RandomAccessIterator first, T* pBuffer, const intptr_t size,
      ...
      len = tim_sort_count_run<RandomAccessIterator, StrictWeakOrdering>(
          first, curr, size, compare);  // This will count the length of the run and reverse the run if it is backwards.
      run = minrun;
      ...
      {
        insertion_sort_already_started<RandomAccessIterator, StrictWeakOrdering>(first + curr, first + curr + run,
                                                                                 first + curr + len, compare);
        len = run;
      }
  ...
  void tim_sort_buffer(RandomAccessIterator first, RandomAccessIterator last, T* pBuffer, StrictWeakOrdering compare) {
    ...
    if (size < 64)
      ...
      if (tim_sort_add_run<RandomAccessIterator, T, StrictWeakOrdering>(run_stack, first, pBuffer, size, minrun, len,
                                                                        run, curr, stack_curr, compare))
        return;
      if (tim_sort_add_run<RandomAccessIterator, T, StrictWeakOrdering>(run_stack, first, pBuffer, size, minrun, len,
                                                                        run, curr, stack_curr, compare))
        return;
      ...
      for (;;) {
        if (timsort_check_invariant(run_stack, stack_curr))
          stack_curr = tim_sort_collapse<RandomAccessIterator, T, StrictWeakOrdering>(first, run_stack, stack_curr,
                                                                                      pBuffer, size, compare);
        else {
          if (tim_sort_add_run<RandomAccessIterator, T, StrictWeakOrdering>(run_stack, first, pBuffer, size, minrun,
                                                                            len, run, curr, stack_curr, compare))
            break;
        }
      }
    }
  ...
  namespace Internal {
    ...
    // 1. Computing a histogram (i.e. finding the number of elements per bucket) for the next pass is done in parallel
    // with the loop that "scatters"
    //    elements in the current pass.  The advantage is that it avoids the memory traffic / cache pressure of reading
    //    keys in a separate operation. Note: It would also be possible to compute all histograms in a single pass.
    //    However, that would increase the amount of stack space used and also increase cache pressure slightly.
    //    However, it could still be faster under some situations.
    // 2. If all elements are mapped to a single bucket, then there is no need to perform a scatter operation.  Instead
    // the elements are left in place
    ...
    void radix_sort_impl(RandomAccessIterator first, RandomAccessIterator last, RandomAccessIterator buffer,
      ...
      for (j = 0; j < (8 * sizeof(IntegerType)); j += DigitBits) {
        if (doSeparateHistogramCalculation) {
          ...
          for (temp = srcFirst; temp != last; ++temp) ++bucketSize[(extractKey(*temp) >> j) & bucketMask];
        }
        ...
        if (bucketSize[((extractKey(*srcFirst) >> j) & bucketMask)] == uint32_t(last - srcFirst)) {
          ...
          // The histogram is either not needed or it will be calculated in parallel with the scatter operation below
      ...
      if (srcFirst != first) {
        ...
        for (temp = srcFirst; temp != last; ++temp) *buffer++ = *temp;
      }
    }
  }  // namespace Internal
  ...
  void radix_sort(RandomAccessIterator first, RandomAccessIterator last, RandomAccessIterator buffer) {
    ...
    eastl::Internal::radix_sort_impl<RandomAccessIterator, ExtractKey, DigitBits>(first, last, buffer, ExtractKey(),
  ...
  ///
  ///     template <typename RandomAccessIterator, typename Allocator, typename StrictWeakOrdering>
  ///     void EASTL_DEFAULT_STABLE_SORT_FUNCTION(RandomAccessIterator first, RandomAccessIterator last, Allocator&
  ///     allocator, StrictWeakOrdering compare);
  ///
  template <typename RandomAccessIterator, typename StrictWeakOrdering>
