...
#include <OpenSpeed/Core/EASTL/EASTL/internal/smart_ptr.h>  // Defines smart_array_deleter
#include <OpenSpeed/Core/EASTL/EASTL/linked_ptr.h>          // Defines linked_ptr_base
...
namespace eastl {
  ...
  class linked_array {
   protected:
    ...
    void link(const linked_array& linkedArray) {  // This code can only be called when we are in a reset state.
   ...
   public:
    ...
    explicit linked_array(T* pArray = NULL) : mpArray(pArray) { mpPrev = mpNext = this; }

    /// linked_array
    /// Shares ownership of a pointer with another instance of linked_array.
    linked_array(const linked_array& linkedArray) : mpArray(linkedArray.mpArray) {
      if (mpArray)
        link(linkedArray);
      else
        mpPrev = mpNext = this;
    ...
    linked_array& operator=(const linked_array& linkedArray) {
      if (linkedArray.mpArray != mpArray) {
        ...
        if (linkedArray.mpArray) link(linkedArray);
      }
      return *this;
    }
    ...
    T& operator*() const { return *mpArray; }
    ...
    int use_count() const {
      ...
      for (const linked_ptr_base* pCurrent = this; pCurrent->mpNext != this; pCurrent = pCurrent->mpNext) ++useCount;
    ...
    bool unique() const { return (mpNext == this); }
    ...
    operator bool_() const {
      if (mpArray) return &this_type::get;
      return NULL;
    }
    ...
    bool operator!() { return (mpArray == NULL); }
