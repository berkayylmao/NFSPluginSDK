...
namespace eastl {
  ...
  class fixed_substring : public basic_string<T> {
   ...
   public:
    fixed_substring() : base_type() {}

    fixed_substring(const base_type& x) : base_type() {
    ...
    fixed_substring(const base_type& x, size_type position, size_type n = base_type::npos) : base_type() {
#if EASTL_NAME_ENABLED
      mAllocator.set_name(x.get_allocator().get_name());
#endif
    ...
    fixed_substring(const value_type* p, size_type n) : base_type() { assign(p, n); }

    fixed_substring(const value_type* p) : base_type() { assign(p); }

    fixed_substring(const value_type* pBegin, const value_type* pEnd) : base_type() { assign(pBegin, pEnd); }
...
#if 0  // !defined(EA_COMPILER_NO_DELETED_FUNCTIONS) We may want to enable these deletions after
       // some investigation of possible user impact.
