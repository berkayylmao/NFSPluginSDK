...
#include <OpenSpeed/Core/EASTL/EABase/config/eacompilertraits.h>
...
#include <OpenSpeed/Core/EASTL/EABase/version.h>
...
//    #error "<inttypes.h> was #included before eabase.h, but without __STDC_FORMAT_MACROS #defined.
//    You must #include eabase.h or an equivalent before #including C99 headers, or you must define
//    __STDC_FORMAT_MACRO before #including system headrs."
//#endif
...
#if !defined(EA_COMPILER_HAS_INTTYPES) && (!defined(_MSC_VER) || (_MSC_VER > 1500)) && \
    (defined(EA_COMPILER_IS_C99) || defined(INT8_MIN) || defined(EA_COMPILER_HAS_C99_TYPES) || defined(_SN_STDINT_H))
...
#pragma warning(pop)
    ...
    (defined(__FLT_EVAL_METHOD__) || defined(_FEVAL))  // GCC 3.x defines __FLT_EVAL_METHOD__ instead
                                                       // of the C99 standard FLT_EVAL_METHOD.
...
// don't define float_t and double_t.
    ...
    (defined(EA_PLATFORM_ANDROID) && !(defined(EA_ANDROID_SDK_LEVEL) && EA_ANDROID_SDK_LEVEL >= 21))
...
typedef signed int   int32_t;     //< 32 bit signed integer. This works for both 32 bit and 64 bit
                                  // platforms, as we assume the LP64 is followed.
#define __int8_t_defined
#endif
typedef unsigned char  uint8_t;   //< 8 bit unsigned integer
typedef unsigned short uint16_t;  //< 16 bit unsigned integer
#if !defined(__uint32_t_defined)
typedef unsigned int   uint32_t;  //< 32 bit unsigned integer. This works for both 32 bit and 64 bit
                                  // platforms, as we assume the LP64 is followed.
...
#ifndef INT16_C
#define INT16_C(x) \
  int16_t(x)  // x##i16 doesn't work satisfactorilly because -32768i8 generates an out of range
              // warning.
#endif
#ifndef UINT16_C
...
#ifndef INT8_C
#define INT8_C(x) \
  int8_t(x)  // For the majority of compilers and platforms, long is 32 bits and long long is 64
             // bits.
#endif
#ifndef UINT8_C
#define UINT8_C(x) uint8_t(x)
...
#ifndef UINT16_C
#define UINT16_C(x) \
  uint16_t(x)  // Possibly we should make this be uint16_t(x##u). Let's see how compilers react
...
#ifndef INT64_C
#define INT64_C(x) \
  x##LL  // The way to deal with this is to compare ULONG_MAX to 0xffffffff and if not equal, then
         // remove the L.
#endif
#ifndef UINT64_C
...
#if defined(EA_COMPILER_MSVC) || defined(EA_COMPILER_BORLAND)  // VC++ 7.1+ understands long long as a data type but
                                                               // doesn't accept %ll as a printf specifier.
...
#endif  // It turns out that some platforms use %q to represent a 64 bit value, but these are not
...
#define PRId8 \
  "c"              // This may not work properly but it at least will not crash. Try using 16 bit versions
                   // instead.
#define PRIi8 "c"  //  "
#define PRIo8 "o"  //  "
#define PRIu8 "u"  //  "
...
#else  // VC++, Borland, etc. which have no way to specify 8 bit values other than %c.
#define SCNd8 \
  "c"              // This will not work properly but it at least will not crash. Try using 16 bit versions
                   // instead.
#define SCNi8 "c"  //  "
#define SCNo8 "c"  //  "
#define SCNu8 "c"  //  "
...
#if defined(_MSC_VER) && (_MSC_VER >= 1600) && defined(_HAS_CHAR16_T_LANGUAGE_SUPPORT) && \
...
#endif
#elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 404) && defined(__CHAR16_TYPE__) && \
    defined(EA_COMPILER_CPP11_ENABLED)  // EDG 4.4+.
#define EA_CHAR16_NATIVE 1
#elif defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4004) && !defined(EA_COMPILER_EDG) && \
    (defined(EA_COMPILER_CPP11_ENABLED) || defined(__STDC_VERSION__))  // g++ (C++ compiler) 4.4+ with -std=c++0x or gcc
                                                                       // (C compiler) 4.4+ with -std=gnu99
#define EA_CHAR16_NATIVE 1
#else
#define EA_CHAR16_NATIVE 0
#endif
...
#if !defined(EA_CHAR32_NATIVE)  // Microsoft currently ties char32_t language support to char16_t
                                // language support. So we use CHAR16_T here.
// To do: Change this to be based on EA_COMPILER_NO_NEW_CHARACTER_TYPES.
#if defined(_MSC_VER) && (_MSC_VER >= 1600) && defined(_HAS_CHAR16_T_LANGUAGE_SUPPORT) && \
    _HAS_CHAR16_T_LANGUAGE_SUPPORT  // VS2010+
...
#endif
#elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 404) && defined(__CHAR32_TYPE__) && \
    defined(EA_COMPILER_CPP11_ENABLED)  // EDG 4.4+.
#define EA_CHAR32_NATIVE 1
#elif defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4004) && !defined(EA_COMPILER_EDG) && \
    (defined(EA_COMPILER_CPP11_ENABLED) || defined(__STDC_VERSION__))  // g++ (C++ compiler) 4.4+ with -std=c++0x or gcc
...
#define char8_t \
  char  // The Apple debugger is too stupid to realize char8_t is typedef'd to char, so we #define
        // it.
#else
...
// In GCC C, __CHAR16_TYPE__ and __CHAR32_TYPE__ are defined instead, and we must define char16_t
// and char32_t from these.
...
#if (defined(_MSC_VER) && (_MSC_VER >= 1600))  // if VS2010+ or using platforms that use Dinkumware under a compiler
                                               // that doesn't natively support C++11 char16_t.
#if !defined(_CHAR16T)
#define _CHAR16T
#endif
...
#if defined(__COUNTER__)  // If this extension is available, which allows multiple statements per
                          // line...
#define static_assert(expression, description)      \
  typedef char EA_STATIC_ASSERT_CONCATENATE_HELPER( \
      compileTimeAssert, __COUNTER__)[((expression) != 0) ? 1 : -1] EA_STATIC_ASSERT_UNUSED_ATTRIBUTE
#else
#define static_assert(expression, description)      \
  typedef char EA_STATIC_ASSERT_CONCATENATE_HELPER( \
      compileTimeAssert, __LINE__)[((expression) != 0) ? 1 : -1] EA_STATIC_ASSERT_UNUSED_ATTRIBUTE
#endif
