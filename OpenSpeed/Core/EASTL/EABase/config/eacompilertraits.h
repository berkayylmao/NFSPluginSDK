...
#include <OpenSpeed/Core/EASTL/EABase/config/eaplatform.h>
...
#define EA_OFFSETOF(struct_, member_) \
  ((size_t)(((uintptr_t) & reinterpret_cast<const volatile char&>((((struct_*)65536)->member_))) - 65536))
#else
...
#define EA_WCHAR_T_NON_NATIVE 1
    ...
    (!defined(_WCHAR_T) &&        \
     (__EDG_VERSION__ < 400))  // EDG prior to v4 uses _WCHAR_T to indicate if wchar_t is native. v4+ may define
...
struct EANonCopyable {
    ...
    defined(__EDG__)   // EDG doesn't appear to behave properly for the case of defaulted constructors;
...
#elif defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION > 4004) && \
    (defined(__i386__) || defined(__x86_64__))  // GCC 4.4+ - Seems to work only on x86/Linux so far. However, GCC 4.4
