  ...
  namespace StdC {
    ...
    class EASTDC_API Stopwatch {
     public:
      ...
      enum Units {
        kUnitsCycles = 0,        /// Stopwatch clock ticks. May or may not match CPU clock ticks 1:1, depending on
                                 /// your hardware and operating system. Some CPUs' low level cycle count instruction
                                 /// counts every 16 cycles instead of every cycle.
     ...
     public:
      ...
      explicit Stopwatch(int nUnits = kUnitsCycles, bool bStartImmediately = false) :
          mnStartTime(0), mnTotalElapsedTime(0), mnUnits(0), mfStopwatchCyclesToUnitsCoefficient(0.f) {}
     ...
     private:
      ...
      float    mfStopwatchCyclesToUnitsCoefficient;  /// Coefficient is defined seconds per cycle
                                                     /// (assuming you want to measure seconds). This is
                                                     /// the inverse of the frequency, done this way for
                                                     /// speed. Time passed = cycle count * coefficient.
...
inline bool EA::StdC::LimitStopwatch::IsTimeUp() const {
  ...
  return (int64_t)(mnEndTime - nCurrentTime) < 0;  // We do this instead of a straight compare to
                                                   // deal with possible integer wraparound issues.
}
...
inline uint64_t EA::StdC::Stopwatch::GetElapsedTime() const {
  ...
  if (mnStartTime)  // We we are currently running, then take into account time passed since last
                    // start.
  {
