...
namespace EA {
  namespace StdC {
    ...
    EASTDC_API inline int Strcmp(const char* pString1, const char* pString2) { return std::strcmp(pString1, pString2); }
    ...
    EASTDC_API inline size_t Strlcpy(T* pDestination, const T* pSource, size_t nDestCapacity) {
      ...
      if (n && --n) {
        do {
          if ((*pDestination++ = *s++) == 0) break;
        } while (--n);
      }

      if (!n) {
        if (nDestCapacity) *pDestination = 0;
        while (*s++) {
        }
      }
