...
#include <Game.MW05/Types/UCrc32.h>
#include <Game.MW05/Types/UMath.h>
#include <Game.MW05/Types/UTL.h>
  ...
  namespace CollisionGeometry {
    ...
    struct _V3c {
      ...
      void Compress(const UMath::Vector3& from) {
        x = static_cast<std::int16_t>(from.x * INT_LEAST16_MAX / 10000.0f);
        y = static_cast<std::int16_t>(from.y * INT_LEAST16_MAX / 10000.0f);
        z = static_cast<std::int16_t>(from.z * INT_LEAST16_MAX / 10000.0f);
      }
      void Decompress(UMath::Vector3& to) {
        to.x = static_cast<float>(x * 10000.0f / INT_LEAST16_MAX);
        to.y = static_cast<float>(y * 10000.0f / INT_LEAST16_MAX);
        to.z = static_cast<float>(z * 10000.0f / INT_LEAST16_MAX);
      }
      void EndianSwap() {
#if defined(__linux__) || defined(_LINUX)
    ...
    struct _Q4c {
      ...
      void Compress(const UMath::Vector4& from) {
        ...
        y = static_cast<std::int16_t>(from.y * INT_LEAST16_MAX / 10000.0f);
        z = static_cast<std::int16_t>(from.z * INT_LEAST16_MAX / 10000.0f);
        w = static_cast<std::int16_t>(from.w * INT_LEAST16_MAX / 10000.0f);
      }
      void Decompress(UMath::Vector4& to) {
        to.x = static_cast<float>(x * 10000.0f / INT_LEAST16_MAX);
        to.y = static_cast<float>(y * 10000.0f / INT_LEAST16_MAX);
        to.z = static_cast<float>(z * 10000.0f / INT_LEAST16_MAX);
        to.w = static_cast<float>(w * 10000.0f / INT_LEAST16_MAX);
      }
      void EndianSwap() {
#if defined(__linux__) || defined(_LINUX)
        x = static_cast<std::int16_t>(bswap16(x));
...
#pragma pack(pop)
    ...
    struct IBoundable : UTL::COM::IUnknown {
      ...
      virtual bool    AddCollisionPrimitive(UCrc32, UMath::Vector3&, float, UMath::Vector3&,
                                            SimSurface*, UMath::Vector4*,
                                            CollisionGeometry::BoundFlags)  = 0;
      virtual bool    AddCollisionMesh(UCrc32, UMath::Vector4&, std::uint32_t, SimSurface*,
                                       CollisionGeometry::BoundFlags, bool) = 0;
    };
  }  // namespace CollisionGeometry
}  // namespace OpenSpeed::MW05
