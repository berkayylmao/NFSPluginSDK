...
#define EA_COMPILER_CPP11_ENABLED 1
    ...
    _MSC_VER >= 1600  // Microsoft unilaterally enables its C++11 support; there is no way to disable it.
#define EA_COMPILER_CPP11_ENABLED 1
...
#define EA_COMPILER_STRING \
  EA_COMPILER_NAME " compiler, version " INTERNAL_STRINGIZE(__GNUC__) "." INTERNAL_STRINGIZE(__GNUC_MINOR__)
...
//#define EA_COMPILER_NO_MEMBER_TEMPLATES 1                       // VC6.x supports member templates properly 95% of the
//time. So do we flag the remaining 5%? #define EA_COMPILER_NO_MEMBER_TEMPLATE_SPECIALIZATION 1         // VC6.x
//supports member templates properly 95% of the time. So do we flag the remaining 5%?
...
//#define EA_COMPILER_NO_MEMBER_TEMPLATES 1                       // VC7.0 supports member templates properly 95% of the
//time. So do we flag the remaining 5%?
...
#define EA_COMPILER_MSVC_2011 \
  1  // Microsoft changed the name to VS2012 before shipping, despite referring to it as VS2011 up to just a few weeks
     // before shipping.
...
#define EA_DISABLE_ALL_VC_WARNINGS()           \
  __pragma(warning(push, 0)) __pragma(warning( \
      disable : 4244 4265 4267 4350 4472 4509 4548 4623 4710 4985 6320 4755 4625 4626 4702))  // Some warnings need to
...
#elif defined(_MSC_VER)
// Disabled warning "nonstandard extension used: 'throw (...)'" as this warning is a W4 warning which is usually off by
// default and doesn't convey any important information but will still complain when building with /Wall (which most
// teams do)
...
#endif
#elif defined(EA_COMPILER_NO_EXCEPTIONS) && !defined(EA_COMPILER_RVCT) && !defined(EA_PLATFORM_LINUX) && \
    !defined(EA_PLATFORM_APPLE) && !defined(CS_UNDEFINED_STRING)
...
// supported. Earlier EDG supported a subset of rvalue references. Implicit move constructors and assignment operators
// aren't supported until EDG 4.5.
...
// supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && defined(__apple_build_version__) && \
...
//
    ...
    (EA_COMPILER_VERSION > 1600)  // VC++ > VS2010, even without C++11 support. VS2010 does support override, however
                                  // will generate warnings due to the keyword being 'non-standard'
                                  // supported.
...
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 401)  // EDG 4.1+.
// supported, but as of EDG 4.3 it isn't supported for defaulted move constructors and move assignment operators until
// EDG 4.5.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
...
// supported.
    ...
    !defined(__apple_build_version__)  // Clang 3.1+, not including Apple's Clang.
// supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && (EA_COMPILER_VERSION >= 4004)  // GCC 4.4+
// supported.
#else
...
// EA_COMPILER_NO_STRONGLY_TYPED_ENUMS
//
// Refers to C++11 strongly typed enums, which includes enum classes and sized enums. Doesn't include forward-declared
// enums.
//
...
// supported, though 4.1 has partial support for variadic templates.
    ...
    !defined(__apple_build_version__)  // Clang 3.0+, not including Apple's Clang.
// supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && (EA_COMPILER_VERSION >= 4007)  // GCC 4.7+
// supported, though GCC 4.3 has partial support for variadic templates.
#else
...
//
                                                       ...
                                                       // supported.
#elif defined(EA_COMPILER_CPP14_ENABLED) && defined(__clang__) && (EA_COMPILER_VERSION >= 304) && \
    !defined(__apple_build_version__)  // Clang 3.4+, not including Apple's Clang.
// supported.
...
// supported via __declspec(noreturn). You need to use that or EA_NORETURN. VC++ up to VS2013 doesn't support any C++11
// attribute types.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 402)  // EDG 4.2+.
// supported.
#elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && (EA_COMPILER_VERSION >= 401) && \
...
#if defined(EA_COMPILER_NO_UNICODE_STRING_LITERALS)  // Some compilers have had support for char16_t prior to support
                                                     // for u"", but it's not useful to have the former without the
                                                     // latter.
