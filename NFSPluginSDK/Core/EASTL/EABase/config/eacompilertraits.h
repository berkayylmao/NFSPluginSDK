...
#include <NFSPluginSDK/Core/EASTL/EABase/config/eaplatform.h>
...
//
#if defined(__GNUC__)  // We can't use GCC 4's __builtin_offsetof because it mistakenly complains about non-PODs that
                       // are really PODs.
...
//    EA_ALIGN_MAX_STATIC       // The max align value that the compiler will respect for EA_ALIGN for static data
//    (global and static variables). Some compilers allow high values, some allow no more than 8. EA_ALIGN_MIN is
//    assumed to be 1. EA_ALIGN_MAX_AUTOMATIC    // The max align value for automatic variables (variables declared as
//    local to a function). EA_ALIGN(n)               // Used as a prefix. n is byte alignment, with being a power of
//    two. Most of the time you can use this and avoid using EA_PREFIX_ALIGN/EA_POSTFIX_ALIGN. EA_ALIGNED(t, v, n) //
//    Type, variable, alignment. Used to align an instance. You should need this only for unusual compilers. EA_PACKED
//    // Specifies that the given structure be packed (and not have its members aligned).
//
// Also we define the following for rare cases that it's needed.
//    EA_PREFIX_ALIGN(n)        // n is byte alignment, with being a power of two. You should need this only for unusual
//    compilers. EA_POSTFIX_ALIGN(n)       // Valid values for n are 1, 2, 4, 8, etc. You should need this only for
//    unusual compilers.
//
// Example usage:
//    size_t x = EA_ALIGN_OF(int);                                  Non-aligned equivalents.        Meaning
//    EA_PREFIX_ALIGN(8) int x = 5;                                 int x = 5;                      Align x on 8 for
//    compilers that require prefix attributes. Can just use EA_ALIGN instead. EA_ALIGN(8) int x; int x; Align x on 8
//    for compilers that allow prefix attributes. int x EA_POSTFIX_ALIGN(8);                                    int x;
//    Align x on 8 for compilers that require postfix attributes. int x EA_POSTFIX_ALIGN(8) = 5; int x = 5; Align x on 8
//    for compilers that require postfix attributes. int x EA_POSTFIX_ALIGN(8)(5);                                 int
//    x(5);                       Align x on 8 for compilers that require postfix attributes. struct EA_PREFIX_ALIGN(8)
//    X { int x; } EA_POSTFIX_ALIGN(8);   struct X { int x; };            Define X as a struct which is aligned on 8
//    when used. EA_ALIGNED(int, x, 8) = 5;                                    int x = 5;                      Align x
//    on 8. EA_ALIGNED(int, x, 16)(5);                                    int x(5);                       Align x on 16.
//    EA_ALIGNED(int, x[3], 16);                                    int x[3];                       Align x array on 16.
//    EA_ALIGNED(int, x[3], 16) = { 1, 2, 3 };                      int x[3] = { 1, 2, 3 };         Align x array on 16.
//    int x[3] EA_PACKED;                                           int x[3];                       Pack the 3 ints of
//    the x array. GCC doesn't seem to support packing of int arrays. struct EA_ALIGN(32) X { int x; int y; }; struct X
//    { int x; };            Define A as a struct which is aligned on 32 when used. EA_ALIGN(32) struct X { int x; int
//    y; } Z;                    struct X { int x; } Z;          Define A as a struct, and align the instance Z on 32.
//    struct X { int x EA_PACKED; int y EA_PACKED; };               struct X { int x; int y; };     Pack the x and y
//    members of struct X. struct X { int x; int y; } EA_PACKED;                         struct X { int x; int y; };
//    Pack the members of struct X. typedef EA_ALIGNED(int, int16, 16); int16 n16;                typedef int int16;
//    int16 n16;   Define int16 as an int which is aligned on 16. typedef EA_ALIGNED(X, X16, 16); X16 x16; typedef X
//    X16; X16 x16;         Define X16 as an X which is aligned on 16.
...
#if defined(_MSC_VER)
#define EA_ENABLE_VC_WARNING_AS_ERROR(w) __pragma(warning(push)) __pragma(warning(error : w))
#else
...
#if defined(EA_COMPILER_GNUC) && \
    (EA_COMPILER_VERSION >= 4006)  // Can't test directly for __GNUC__ because some compilers lie.
#define EA_ENABLE_GCC_WARNING_AS_ERROR(w) _Pragma("GCC diagnostic push") _Pragma(EAGCCWERRORHELP2(w))
#elif defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4004)
#define EA_DISABLE_GCC_WARNING(w) _Pragma(EAGCCWERRORHELP2(w))
#else
#define EA_DISABLE_GCC_WARNING(w)
#endif
#endif
...
#define EA_DISABLE_CLANG_WARNING(w) \
  _Pragma("clang diagnostic push") _Pragma(EACLANGWHELP2(-Wunknown - warning - option)) _Pragma(EACLANGWHELP2(w))
#else
#define EA_DISABLE_CLANG_WARNING(w)
#endif
#endif
...
#define EA_ENABLE_CLANG_WARNING_AS_ERROR(w) _Pragma("clang diagnostic push") _Pragma(EACLANGWERRORHELP2(w))
#else
#define EA_DISABLE_CLANG_WARNING(w)
#endif
#endif
...
// EA_FP128 may be used to determine if __float128 is a supported type for use. This type is enabled by a GCC extension
// (_GLIBCXX_USE_FLOAT128) but has support by some implementations of clang (__FLOAT128__) PS4 does not support
// __float128 as of SDK 5.500 https://ps4.siedev.net/resources/documents/SDK/5.500/CPU_Compiler_ABI-Overview/0003.html
#ifndef EA_FP128
...
// ------------------------------------------------------------------------
// EA_ABM
// EA_ABM may be used to determine if Advanced Bit Manipulation sets are available for the target architecture (POPCNT,
// LZCNT)
//
#ifndef EA_ABM
