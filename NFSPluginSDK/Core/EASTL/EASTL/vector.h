...
#define EASTL_VECTOR_DEFAULT_ALLOCATOR allocator_type(EASTL_VECTOR_DEFAULT_NAME)
  ...
  class vector : public VectorBase<T, Allocator> {
   ...
   public:
    ...
    typedef T* iterator;  // Note: iterator is simply T* right now, but this will likely change in the future, at least
                          // for debug builds.
   ...
   public:
    ...
    iterator erase_first_unsorted(
        const T& value);  // Same as erase, except it doesn't preserve order, but is faster because it simply copies the
                          // last item in the vector over the erased position.
    ...
    iterator erase_unsorted(
        const_iterator position);  // Same as erase, except it doesn't preserve order, but is faster because it simply
                                   // copies the last item in the vector over the erased position.
   ...
   protected:
    ...
    template <typename ForwardIterator>  // Allocates a pointer of array count n and copy-constructs it with
                                         // [first,last).
  ...
  template <typename T, typename Allocator>
  inline VectorBase<T, Allocator>::~VectorBase() {
    if (mpBegin) EASTLFree(internalAllocator(), mpBegin, (internalCapacityPtr() - mpBegin) * sizeof(T));
  }
  ...
  inline void VectorBase<T, Allocator>::DoFree(T* p, size_type n) {
    if (p) EASTLFree(internalAllocator(), p, n * sizeof(T));
  }
  ...
  typename vector<T, Allocator>::this_type& vector<T, Allocator>::operator=(const this_type& x) {
    if (this != &x)  // If not assigning to self...
    {
      ...
      if (bSlowerPathwayRequired) {
        DoClearCapacity();  // Must clear the capacity instead of clear because set_capacity frees our memory, unlike
                            // clear.
  ...
  inline typename vector<T, Allocator>::reference vector<T, Allocator>::emplace_back(Args&&... args) {
    if (mpEnd < internalCapacityPtr()) {
      ::new ((void*)mpEnd)
          value_type(eastl::forward<Args>(args)...);  // If value_type has a move constructor, it will use it and this
                                                      // operation may be faster than otherwise.
      ++mpEnd;  // Increment this after the construction above in case the construction throws an exception.
    } else
  ...
  inline typename vector<T, Allocator>::iterator vector<T, Allocator>::insert(const_iterator    position,
    ...
    // We implment a quick pathway for the case that the insertion position is at the end and we have free capacity for
    // it.
    const ptrdiff_t n = position - mpBegin;  // Save this because we might reallocate.
  ...
  template <typename T, typename Allocator>
  inline typename vector<T, Allocator>::iterator vector<T, Allocator>::insert(const_iterator position, size_type n,
                                                                              const value_type& value) {
  ...
  void vector<T, Allocator>::DoAssignFromIterator(RandomAccessIterator first, RandomAccessIterator last,
    ...
    } else if (n <= size_type(mpEnd - mpBegin))  // If n <= size ...
    {
      pointer const pNewEnd =
          eastl::copy(first, last, mpBegin);  // Since we are copying to mpBegin, we don't have to worry about needing
    ...
    } else  // else size < n <= capacity
    {
      ...
      eastl::copy(first, position, mpBegin);  // Since we are copying to mpBegin, we don't have to worry about needing
                                              // copy_backward or a memmove-like copy (as opposed to memcpy-like copy).
  ...
  void vector<T, Allocator>::DoInsertValue(const_iterator position, Args&&... args) {
    ...
    if (mpEnd != internalCapacityPtr())  // If size < capacity ...
    {
      ...
      // sizeof(value_type) may be too much for the given platform. An alternative solution may be to specialize this
      // function for the case of the argument being const value_type& or value_type&&.
    ...
    } else  // else (size == capacity)
    {
...
#if EASTL_EXCEPTIONS_ENABLED
      pointer pNewEnd = pNewData;
      try {  // To do: We are not handling exceptions properly below.  In particular we don't want to
        ...
        if (pNewEnd)
          ...
          eastl::destruct(pNewData + nPosSize);  // The exception occurred during the first uninitialized move, so
                                                 // destroy only the value at nPosSize.
        DoFree(pNewData, nNewSize);
        throw;
      }
#else
      ...
      pointer pNewEnd = eastl::uninitialized_move_ptr_if_noexcept(
          ...
          pNewData);  // the value first, because it might possibly be a reference to the old data being moved.
      pNewEnd = eastl::uninitialized_move_ptr_if_noexcept(
          destPosition, mpEnd, ++pNewEnd);  // Question: with exceptions disabled, do we assume all operations are
  ...
  inline bool vector<T, Allocator>::validate() const EA_NOEXCEPT {
    ...
    if (internalCapacityPtr() < mpEnd) return false;
    return true;
  }
