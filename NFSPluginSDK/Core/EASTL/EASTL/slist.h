...
// default-initialized 4571 - catch(...) semantics changed since Visual C++ 7.1; structured exceptions (SEH) are no
// longer caught.
...
#define EASTL_SLIST_DEFAULT_ALLOCATOR allocator_type(EASTL_SLIST_DEFAULT_NAME)
  ...
  void SListBase<T, Allocator>::set_allocator(const allocator_type& allocator) {
    EASTL_ASSERT((internalAllocator() == allocator) ||
                 (static_cast<node_type*>(internalNode().mpNext) ==
  ...
  template <typename T, typename Allocator>
  inline slist<T, Allocator>::slist(const slist& x) : base_type(x.internalAllocator()) {
    DoInsertAfter((SListNodeBase*)&internalNode(), const_iterator((SListNodeBase*)x.internalNode().mpNext),
                  const_iterator(NULL), false_type());
  }

  template <typename T, typename Allocator>
  slist<T, Allocator>::slist(this_type&& x) : base_type(x.internalAllocator()) {
    swap(x);
  }
  ...
  inline typename slist<T, Allocator>::reference slist<T, Allocator>::front() {
    ...
    if (EASTL_UNLIKELY(internalNode().mpNext == NULL)) EASTL_FAIL_MSG("slist::front -- empty container");
#endif
  ...
  inline void slist<T, Allocator>::assign(
      InputIterator first, InputIterator last)  // version of assign that takes (int size, int value). These are not
  ...
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::insert(const_iterator position) {
    return iterator((SListNodeBase*)DoInsertValueAfter(
        SListNodeGetPrevious((SListNodeBase*)&internalNode(), (SListNodeBase*)position.mpNode), value_type()));
  }
  ...
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::insert(const_iterator    position,
    ...
    return iterator((SListNodeBase*)DoInsertValueAfter(
        SListNodeGetPrevious((SListNodeBase*)&internalNode(), (SListNodeBase*)position.mpNode), value));
  }

  template <typename T, typename Allocator>
  inline void slist<T, Allocator>::insert(const_iterator position, size_type n, const value_type& value) {
    // To do: get rid of DoAssignValues and put its implementation directly here.
    DoInsertValuesAfter(SListNodeGetPrevious((SListNodeBase*)&internalNode(), (SListNodeBase*)position.mpNode), n,
                        value);
  }
  ...
  inline typename slist<T, Allocator>::iterator slist<T, Allocator>::erase(const_iterator first, const_iterator last) {
    return DoEraseAfter(SListNodeGetPrevious((SListNodeBase*)&internalNode(), (SListNodeBase*)first.mpNode),
  ...
  inline void slist<T, Allocator>::splice(const_iterator position, this_type& x) {
    ...
    // EASTL_ASSERT(internalAllocator() == x.internalAllocator()); // Disabled because our member sort function uses
    // splice but with allocators that may be unequal. There isn't a simple workaround aside from disabling this assert.
    ...
    if (before_first != before_last)  // If there is anything to splice...
    {
      ...
      if (i == iEnd)  // If the input came from an external range...
        mSize += (size_type)eastl::distance(
            before_first, before_last);  // Note that we have no way of knowing how to decrementing the size from the
  ...
  inline void slist<T, Allocator>::splice_after(const_iterator position, const_iterator previous) {
    ...
    if (i == iEnd)  // If the input came from an external range...
      ++mSize;      // Note that we have no way of knowing how to decrementing the size from the external container,
                    // assuming it came from one.
    else {
      EASTL_FAIL_MSG(
          "slist::splice_after: Impossible to decrement source mSize. Use the other splice_after function instead.");
    }
  ...
  inline void slist<T, Allocator>::reverse() EA_NOEXCEPT {
    if (internalNode().mpNext)
      internalNode().mpNext =
          static_cast<node_type*>((base_node_type*)SListNodeReverse((SListNodeBase*)internalNode().mpNext));
  }
  ...
  inline typename slist<T, Allocator>::node_type* slist<T, Allocator>::DoInsertValueAfter(SListNodeBase* pNode,
    ...
    ++mSize;  // Increment the size after the node creation because we need to assume an exception can occur in the
              // creation.
#endif
    return static_cast<node_type*>((base_node_type*)pNode);
  }
...
#if EASTL_SLIST_SIZE_CACHE
      ++mSize;  // We don't do a single mSize += n at the end because an exception may result in only a partial range
                // insertion.
#endif
    }
    return static_cast<node_type*>((base_node_type*)pNode);
