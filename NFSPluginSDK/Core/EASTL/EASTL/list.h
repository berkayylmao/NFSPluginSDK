...
// longer caught. 4623 - default constructor was implicitly defined as deleted
...
#define EASTL_LIST_DEFAULT_ALLOCATOR allocator_type(EASTL_LIST_DEFAULT_NAME)
  ...
  class list : public ListBase<T, Allocator> {
   ...
   public:
    ...
    list(InputIterator first, InputIterator last);  // allocator arg removed because VC7.1 fails on the default arg. To
  ...
  inline void ListBase<T, Allocator>::set_allocator(const allocator_type& allocator) {
    EASTL_ASSERT((internalAllocator() == allocator) ||
                 (static_cast<node_type*>(internalNode().mpNext) ==
                  &internalNode()));  // We can only assign a different allocator if we are empty of elements.
  ...
  inline typename ListBase<T, Allocator>::node_type* ListBase<T, Allocator>::DoAllocateNode() {
    node_type* pNode =
        (node_type*)allocate_memory(internalAllocator(), sizeof(node_type), EASTL_ALIGN_OF(node_type), 0);
  ...
  template <typename T, typename Allocator>
  inline list<T, Allocator>::list(const this_type& x) : base_type(x.internalAllocator()) {
    DoInsert((ListNodeBase*)&internalNode(), const_iterator((ListNodeBase*)x.internalNode().mpNext),
             const_iterator((ListNodeBase*)&x.internalNode()), false_type());
  }
  ...
  template <typename T, typename Allocator>
  inline list<T, Allocator>::list(this_type&& x) : base_type(eastl::move(x.internalAllocator())) {
    swap(x);
  }
  ...
  inline typename list<T, Allocator>::size_type list<T, Allocator>::size() const EA_NOEXCEPT {
    ...
    return (size_type)eastl::distance(const_iterator((ListNodeBase*)internalNode().mpNext),
                                      const_iterator((ListNodeBase*)&internalNode()));
#endif
#endif
  }
  ...
  template <typename T, typename Allocator>
  inline typename list<T, Allocator>::iterator list<T, Allocator>::insert(const_iterator position, size_type n,
                                                                          const value_type& value) {
  ...
  template <typename T, typename Allocator>
  template <typename InputIterator>
  inline typename list<T, Allocator>::iterator list<T, Allocator>::insert(const_iterator position, InputIterator first,
                                                                          InputIterator last) {
    iterator itPrev(position.mpNode);
    --itPrev;
    ...
    if (internalAllocator() == x.internalAllocator()) {
      ...
      if (x.mSize) {
        ((ListNodeBase*)position.mpNode)
            ->splice((ListNodeBase*)x.internalNode().mpNext, (ListNodeBase*)&x.internalNode());
        mSize += x.mSize;
        x.mSize = 0;
      }
#else
  ...
  typename list<T, Allocator>::iterator list<T, Allocator>::DoSort(iterator i1, iterator end2, size_type n,
    ...
    // Merge the two segments. We do this by merging the second sub-segment into the first, by walking forward in each
  ...
  inline void list<T, Allocator>::DoSwap(this_type& x) {
    ListNodeBase::swap(
        (ListNodeBase&)internalNode(),
        (ListNodeBase&)x.internalNode());  // We need to implement a special swap because we can't do a shallow swap.
