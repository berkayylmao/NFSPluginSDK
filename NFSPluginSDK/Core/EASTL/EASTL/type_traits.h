...
//    is_pointer                            T is a pointer type. Includes function pointers, but not pointers to (data
//    or function) members. is_rvalue_reference is_lvalue_reference is_member_object_pointer              T is a pointer
//    to data member. is_member_function_pointer            T is a pointer to member function.
//
...
//
//    alignment_of                          An integer value representing the number of bytes of the alignment of
//    objects of type T; an object of type T may be allocated at an address that is a multiple of its alignment. rank An
//    integer value representing the rank of objects of type T. The term 'rank' here is used to describe the number of
//    dimensions of an array type. extent                                An integer value representing the extent
//    (dimension) of the I'th bound of objects of type T. If the type T is not an array type, has rank of less than I,
//    or if I == 0 and T is of type 'array of unknown bound of U,' then value shall evaluate to zero; otherwise value
//    shall evaluate to the number of elements in the I'th array bound of T. The term 'extent' here is used to describe
...
//    is_convertible                        An imaginary lvalue of type From is implicitly convertible to type To.
//    Special conversions involving string-literals and null-pointer constants are not considered. No function-parameter
//    adjustments are made to type To when determining whether From is convertible to To; this implies that if type To
//    is a function type or an array type, then the condition is false.
//
//    remove_cv
//    remove_const                          The member typedef type shall be the same as T except that any top level
//    const-qualifier has been removed. remove_const<const volatile int>::type evaluates to volatile int, whereas
//    remove_const<const int*> is const int*. remove_volatile add_cv add_const add_volatile
//
//    remove_reference
...
//    is_aligned                            Defined as true if the type has alignment requirements greater than default
//    alignment, which is taken to be 8. is_aligned is not found in Boost nor C++11, though alignment_of is. union_cast
//    Allows for easy-to-read casting between types that are unrelated but have binary equivalence. The classic use case
//    is converting between float and int32_t bit representations. is_array_of_known_bounds is_array_of_unknown_bounds
...
//    has_nothrow_constructor               The default constructor for T has an empty exception specification or can
//    otherwise be deduced never to throw an exception. has_nothrow_copy                      The copy constructor for T
//    has an empty exception specification or can otherwise be deduced never to throw an exception. has_nothrow_assign
//    The assignment operator for T has an empty exception specification or can otherwise be deduced never to throw an
//    exception.
//   *has_trivial_relocate                  T can be moved to a new location via bitwise copy. Note that C++11
//   rvalue/move functionality supercedes this.
//
...
//     GCC 2.96         We used to have a separate set of type traits for this compiler, but removed it due to lack of
//     necessity.
//////////////////////////////////////////////////////////////////////////////
...
#define EASTL_TYPE_TRAIT_is_function_CONFORMANCE 1  // is_function is conforming.
  ...
  struct is_function<ReturnValue __cdecl(
      ArgPack..., ...)>  // The second ellipsis handles the case of a function that takes ellipsis, like printf.
      : public eastl::true_type {};

  // __stdcall specialization
  template <typename ReturnValue, typename... ArgPack>
  struct is_function<ReturnValue __stdcall(ArgPack...)> : public eastl::true_type {};

  // When functions use a variable number of arguments, it is the caller that cleans the stack (cf. cdecl).
  //
  // template <typename ReturnValue, typename... ArgPack>
  // struct is_function<ReturnValue __stdcall (ArgPack..., ...)>    // The second ellipsis handles the case of a
  // function that takes ellipsis, like printf.
  //     : public eastl::true_type {};
#else
  template <typename ReturnValue, typename... ArgPack>
...
#define EASTL_TYPE_TRAIT_remove_cv_CONFORMANCE 1  // remove_cv is conforming.
  ...
  // and T otherwise.
  //
  // Note: due to the reference collapsing rules, if you supply an r-value reference such as T&&, it will collapse to
  // T&.
  //
  ///////////////////////////////////////////////////////////////////////
...
#if defined(_MSC_VER) && \
    (_MSC_VER <= 1600)  // VS2010 and earlier mistakenly report: "cannot add a reference to a zero-sized array."
                        // Actually they are allowed, but there's nothing we can do about it under VS2010 and earlier.
  template <typename T>
...
#define EASTL_TYPE_TRAIT_remove_cvref_CONFORMANCE 1  // remove_cvref is conforming.
  ...
  template <typename T>
  struct remove_cvref {
    typedef typename eastl::remove_volatile<
        typename eastl::remove_const<typename eastl::remove_reference<T>::type>::type>::type type;
  };
...
#define EASTL_TYPE_TRAIT_add_lvalue_reference_CONFORMANCE 1  // add_lvalue_reference is conforming.
  ...
  struct add_lvalue_reference<T&> {
    typedef T& type;
  };  // This shouldn't be required for modern compilers, as they recognize that a reference to a reference is still a
      // reference.
  template <>
...
#define EASTL_TYPE_TRAIT_add_rvalue_reference_CONFORMANCE 1
  ...
  struct add_rvalue_reference {
    typedef T&& type;
  };  // Dinkumware has this as { typedef typename eastl::remove_reference<T>::type&& type; }, but that doesn't seem
      // right to me.
  template <typename T>
  struct add_rvalue_reference<T&> {
    typedef T& type;
  };  // The Standard section 20.7.9.2 specifies that we do this, though it seems like the compiler ought to not require
      // this, as C++11 stipulates that & + && -> &.
  template <>
...
#include <NFSPluginSDK/Core/EASTL/EASTL/internal/type_fundamental.h>
#include <NFSPluginSDK/Core/EASTL/EASTL/internal/type_transformations.h>
#include <NFSPluginSDK/Core/EASTL/EASTL/internal/type_properties.h>
#include <NFSPluginSDK/Core/EASTL/EASTL/internal/type_compound.h>
#include <NFSPluginSDK/Core/EASTL/EASTL/internal/type_pod.h>
