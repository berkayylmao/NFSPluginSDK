...
#define EASTL_DISABLE_BITSET_ARRAYBOUNDS_WARNING 0
  ...
  struct BitsetBase {
   ...
   public:
    ...
    // BitsetBase(uint64_t value); // Disabled because it causes conflicts with the 32 bit version with existing user
    // code. Use from_uint64 to init from a uint64_t instead.
  ...
  class bitset : private BitsetBase<BITSET_WORD_COUNT(N, WordType), WordType> {
   ...
   public:
    ...
    // bitset(uint64_t value); // Disabled because it causes conflicts with the 32 bit version with existing user code.
    // Use from_uint64 instead.
...
#define EASTL_BITSET_COUNT_STRING "\0\1\1\2\1\2\2\3\1\2\2\3\2\3\3\4"
  ...
  inline uint32_t GetFirstBit(
      uint16_t x)  // To do: Update this to use VC++ _BitScanForward, _BitScanForward64; GCC __builtin_ctz,
                   // __builtin_ctzl. VC++ __lzcnt16, __lzcnt, __lzcnt64 requires recent CPUs (2013+) and probably can't
                   // be used. http://en.wikipedia.org/wiki/Haswell_%28microarchitecture%29#New_features
  {
    if (x) {
      uint32_t n = 1;
  ...
  inline uint32_t GetLastBit(eastl_uint128_t x) {
    if (x) {
      ...
      eastl_uint128_t mask(
          UINT64_C(0xFFFFFFFF00000000));  // There doesn't seem to exist compiler support for INT128_C() by any
                                          // compiler. EAStdC's int128_t supports it though.
      mask <<= 64;
  ...
  inline BitsetBase<NW, WordType>::BitsetBase(uint32_t value) {
    ...
    // EASTL_CT_ASSERT(sizeof(value) <= sizeof(word_type)); Disabled because we now have support for uint8_t and
    // uint16_t word types. It would be nice to have a runtime assert that tested this.
  ...
  inline BitsetBase<NW, WordType>::BitsetBase(uint64_t value)
  {
          reset();
                  ...
                  EASTL_CT_ASSERT(NW > 2); // We can assume this because we have specializations of BitsetBase for <1>
  and <2>.
                  //if(NW > 1) // NW is a template constant, but it would be a little messy to take advantage of it's
  const-ness. mWord[1] = static_cast<word_type>(value >> 32); #else mWord[0] = static_cast<word_type>(value); #endif
  }
  */
  ...
  inline unsigned long BitsetBase<NW, WordType>::to_ulong() const {
    ...
    return (unsigned long)
        mWord[0];  // Todo: We need to deal with the case whereby sizeof(word_type) < sizeof(unsigned long)
  }
  ...
  inline typename BitsetBase<NW, WordType>::size_type BitsetBase<NW, WordType>::DoFindPrev(size_type last_find) const {
    if (last_find > 0) {
      ...
      word_type mask = (~static_cast<word_type>(0) >> (kBitsPerWord - 1 - bit_index)) >>
                       1;  // We do two shifts here because many CPUs ignore requests to shift 32 bit integers by 32
                           // bits, which could be the case above.
  ...
  inline BitsetBase<1, WordType>::BitsetBase(uint64_t value)
  {
                  ...
                  mWord[0] = static_cast<word_type>(value);   // This potentially loses data, but that's what the user
  is requesting. #else mWord[0] = static_cast<word_type>(value); #endif
  }
  */
...
#if EASTL_EXCEPTIONS_ENABLED
#if ((EA_PLATFORM_WORD_SIZE > 4) && \
     defined(EA_PLATFORM_MICROSOFT))  // If we are using 64 bit words but ulong is less than 64 bits... Microsoft
                                      // platforms alone use a 32 bit long under 64 bit platforms.
  ...
  */
  ...
  inline bitset<N, WordType>::reference::reference(const bitset& x, size_type i) :
      ...
      mnBitIndex(i & kBitsPerWordMask) {  // We have an issue here because the above is casting away the const-ness of
                                          // the source bitset. Empty
  }
          ...
          : base_type(value)
  {
          if((N & kBitsPerWordMask) || (N == 0)) // If there are any high bits to clear...
                  mWord[kWordCount - 1] &= ~(~static_cast<word_type>(0) << (N & kBitsPerWordMask)); // This clears any
  high unused bits.
  }
  */
