...
#define EASTL_RING_BUFFER_DEFAULT_ALLOCATOR allocator_type(EASTL_RING_BUFFER_DEFAULT_NAME)
  ...
  class ring_buffer {
   ...
   public:  // We declare public so that global comparison operators can be implemented without adding an inline level
    ...
    Container c;  // We follow the naming convention established for stack, queue, priority_queue and name this 'c'.
                  // This variable must always have a size of at least 1, as even an empty ring_buffer has an unused
                  // terminating element.
   ...
   public:
    ...
    ring_buffer(std::initializer_list<value_type> ilist,
                const allocator_type&             allocator =
                    EASTL_RING_BUFFER_DEFAULT_ALLOCATOR);  // This function sets the capacity to be equal to the size of
      ...
      c()  // Default construction with default allocator for the container.
  {
    ...
    c.resize(cap + 1);  // Possibly we could construct 'c' with size, but c may not have such a ctor, though we rely on
                        // it having a resize function.
    mBegin = c.begin();
    mEnd   = mBegin;
    mSize  = 0;
  }
  ...
  ring_buffer<T, Container, Allocator>::ring_buffer(const Container& x) :
      c(x)  // This copies elements from x, but unless the user is doing some tricks, the only thing that matters is
            // that c.size() == x.size().
  {
    // To do: This code needs to be amended to deal with possible exceptions
    // that could occur during the resize call below.
  ...
  ring_buffer<T, Container, Allocator>::ring_buffer(const this_type& x) : c(x.c) {
    ...
    eastl::advance(
        mBegin,
        eastl::distance(const_cast<this_type&>(x).c.begin(),
  ...
  void ring_buffer<T, Container, Allocator>::swap(this_type& x) {
    if (&x != this) {
      const difference_type dBegin = eastl::distance(
          c.begin(), mBegin);  // We can do a simple distance algorithm here, as there will be no wraparound.
  ...
  */
  ...
  ring_buffer<T, Container, Allocator>::DoGetSize(EASTL_ITC_NS::random_access_iterator_tag) const
  {
          ...
          //
          const difference_type dBegin = eastl::distance(const_cast<Container&>(c).begin(), mBegin); // const_cast here
  solves a little compiler const difference_type dEnd   = eastl::distance(const_cast<Container&>(c).begin(), mEnd);   //
  argument matching problem.
  ...
  */
  ...
  void ring_buffer<T, Container, Allocator>::resize(size_type n) {
    ...
    if (n > cap)  // If we need to grow in capacity...
    {
      ...
      mEnd   = mBegin;
      eastl::advance(
          mEnd, n);  // We can do a simple advance algorithm on this because we know that mEnd will not wrap around.
    } else  // We could do a check here for n != size(), but that would be costly and people don't usually resize things
            // to their same size.
    {
      mEnd = mBegin;
  ...
  typename ring_buffer<T, Container, Allocator>::size_type ring_buffer<T, Container, Allocator>::capacity() const
      EA_NOEXCEPT {
    EASTL_ASSERT(c.size() >= 1);  // This is required because even an empty ring_buffer has one unused termination
                                  // element, somewhat like a \0 at the end of a C string.
  ...
  void ring_buffer<T, Container, Allocator>::set_capacity(size_type n) {
    ...
    if (n != capacity)  // If we need to change capacity...
    {
      ...
      eastl::copy(itCopyBegin, end(), cTemp.get().begin());  // The begin-end range may in fact be larger than n, in
      ...
      mEnd   = mBegin;
      eastl::advance(
          mEnd, mSize);  // We can do a simple advance algorithm on this because we know that mEnd will not wrap around.
    }
  }
