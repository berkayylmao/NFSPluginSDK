...
#define EASTL_PRIORITY_QUEUE_DEFAULT_ALLOCATOR allocator_type(EASTL_PRIORITY_QUEUE_DEFAULT_NAME)
  ...
  class priority_queue {
   public:
    ...
    // typedef typename Container::allocator_type           allocator_type;  // We can't currently declare this because
    // the container may be a type that doesn't have an allocator.
   ...
   public:
    ...
    void pop(
        value_type& value);  // Extension to the C++11 Standard that allows popping a move-only type (e.g. unique_ptr).
  ...
  inline priority_queue<T, Container, Compare>::priority_queue() :
      c(),  // To consider: use c(EASTL_PRIORITY_QUEUE_DEFAULT_ALLOCATOR) here, though that would add the requirement
            // that the user supplied container support this.
      comp() {}
